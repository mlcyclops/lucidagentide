...
function secWhitelist(entries: import("./bridge.ts").WhitelistEntryView[], creds: import("./bridge.ts").CredMetaView[] = []): string {
  ...
  const rows = entries.length ? entries.map((e) => {
    ...
    const last4 = e.auth ? (credByRef.get(e.auth.vaultRef)?.last4 ?? "") : "";
    ...
    return `<div class="prov">
      <div class="prov-h"><span class="prov-name">${esc(e.pattern)}
        ...
        <span class="prov-status">
          ...
          ${e.auth ? `<span class="abadge set" data-tip="Auth attached|${esc(e.auth.kind)}${last4 ? ` key ••••${esc(last4)}` : ""} · stored OS-encrypted; the whitelist keeps only a reference, never the secret.">${icon("shield", 11)} ${esc(e.auth.kind)}${mask}${e.auth.username ? " · " + esc(e.auth.username) : ""}</span>` : ""}
        </span></div>
  ...
  const form = `<div class="prov" style="border-style:dashed">
      ...
      <div class="prov-body">
        ...
        <div class="prov-row wl-controls">
          ...
          <select id="wlScope" class="prov-key" data-tip="Trust scope|Always = every session (enforced now). Project / This loop persist but are not yet enforced (P-NETWL.3)."><option value="always">Always</option><option value="project">Project</option><option value="loop">This loop</option></select>
...
function wire(): void {
  ...
  $("#setBody")!.addEventListener("click", async (e) => {
    ...
    if (t.closest("#wlAuthFile")) {
      ...
      const r = await bridge.credStoreFile({ kind, label });
    ...
    if (t.closest("#wlAdd")) {
      ...
      if (authKind) {
        ...
        if (authSecret) {
          const r = await bridge.credStore({ kind: authKind, secret: authSecret, label: (($("#wlAuthLabel", body) as HTMLInputElement)?.value ?? "").trim() || undefined });
      ...
      wlPendingCred = null;
      hydrateWhitelist();
      showToast({ title: "Added to whitelist", desc: `${pattern} will auto-allow${scope === "always" ? "" : " (scope not yet enforced)"}${auth ? " · credential attached" : ""}.`, meta: "under the managed ceiling · fail-closed", timeout: 4200 });
      return;
    }
