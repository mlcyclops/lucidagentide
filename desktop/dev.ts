...
import { renderTurnEvalReport, type ObservedTool, type ObservedTurn } from "../harness/brief/eval_report.ts"; // P-CHAT.C (ADR-0181): settled-turn Model-Evaluation report
...
import { seedTrivia } from "./trivia_seed.ts"; // P-TRIV.4 (ADR-0177): AI re-seed the Trivia Wire (scanned, tool-free)
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    try {
      ...
      // P-CHAT.C (ADR-0181): generate a Model-Evaluation engineering report for a just-settled chat turn.
      ...
      // P-TRIV.4 (ADR-0177): AI re-seed the Trivia Wire for the user's role on their SELECTED model,
      // mined from opt-in context. Context is SCANNED fail-closed before it reaches the model; the model
      // runs on the util seam (throwaway session, permission-gated tools denied). Fail-quiet to the seed
      // bank; a scanner finding/dead scanner returns blocked=true (the renderer keeps its current pack).
