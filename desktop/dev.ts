...
import { injectPreviewBridge } from "./preview_bridge.ts"; // P-PREVIEW.6b (ADR-0148): read-only DOM-inspect bridge
...
// P-PREVIEW.6b (ADR-0148): relay for the agent's read-only DOM inspect of the sandboxed preview.
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    try {
      ...
      // P-PREVIEW.6b (ADR-0148): the agent's `preview_inspect` tool (omp subprocess) reads the live preview
      ...
      // P-PREVIEW.6c (ADR-0148): the agent's preview_click / preview_type tools — a STRUCTURED action (a named
      ...
      if (p === "/api/preview/serve") {
        ...
        if (r.ok) {
          ...
          // P-PREVIEW.6b (ADR-0148): inject the read-only DOM-inspect bridge (inline JS, CSP-allowed; egress
...
// P-PREVIEW.6b (ADR-0148): the agent's preview_inspect tool GETs this (with ?selector=&what=) to read the DOM.
process.env.LUCID_PREVIEW_INSPECT_URL = `http://127.0.0.1:${server.port}/api/preview/inspect?t=${TOKEN}`;
// P-PREVIEW.6c (ADR-0148): preview_click / preview_type GET this (with ?action=&selector=&value=) to act.
