...
//     managed models allowlists / LAI-GOV-PACK packs before anyone can route to them, then re-pin.
	...
	if (added.length || removed.length) {
		throw new Error(
			`omp's builtin provider catalog drifted from the R-07 pin.\n` +
				...
				`Review additions against the sovereignty posture (managed models allowlists, LAI-GOV-PACK ` +
				`classification packs) and removals against any policy that names them, then update ` +
