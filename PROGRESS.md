...
- **next:** relax the ADR-0250 installer clamp (re-enable per-machine installs, its own increment - the gate that justifies it is now green); then drop the DO-NOT-MERGE tip commit, PR this branch, and cut v1.12.2 with the compiled engine. Increment D (bat) rides with the parked wip branch.
