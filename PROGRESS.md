...
**P-TRIV.4 / P-EVAL.1 / P-CHAT.A-C - re-integrated onto master (ADR-0186-0190, renumbered from 0177-0181)**
- **shipped:** the five increments from the stale `feat/skills-kb-desktop` branch, re-based onto master v1.10.5 on `feat/chat-eval-redesign` (the skills/KB commit was a duplicate of the already-merged #242 and was dropped). Pure cores dropped in unchanged; app.ts/dev.ts/bridge.ts/styles.css wiring re-applied against master's current renderer. P-EVAL.1 (evals.ts) pure. P-CHAT.A sections + P-CHAT.B chips + P-CHAT.C run-report CTA wired at the settle points; P-CHAT.C's `/api/eval/report` verified END-TO-END in the preview (saves + lists + reads back). P-TRIV.4 Settings card + `/api/trivia/reseed` + `effectiveTriviaBank` (generated pack ?? seed floor) rerouting both game builders; mini source checkboxes tightened (3px 8px padding, 14px boxes, txt-2 12.5px labels). ADRs renumbered to clear the collision with master's released 0177-0185. All 5 demos + 37 unit tests + full typecheck green.
...
- **next:** open a PR for `feat/chat-eval-redesign`; then the live in-app QA of the streaming settle paths.
