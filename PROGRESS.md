...
**P-AGENT (design): Agent Builder architecture + roadmap (ADR-0129, DESIGN ONLY - no feature code)**
...
**P-AGENT.2 (core) - workflow canvas pure builders + spec↔graph adapters (ADR-0129, BUILT)**
...
**P-AGENT.2b - Agent Builder interactive canvas wired into the app + persistence (ADR-0129, BUILT + verified live)**
...
**P-AGENT.3 - the Agent Builder compiler buildAgent(spec) -> AgentBundle (ADR-0129, BUILT)**
...
**P-AGENT.4a - AgentBundle runner foundation: materialize + gate-first omp launch composition (ADR-0129, BUILT)**
...
**P-AGENT.5 - untrusted-spec quarantine gate (keystone-#2 analogue) (ADR-0129, BUILT)**
...
**P-AGENT.6 - enterprise export: portable, tamper-evident AgentBundle per target (ADR-0129, BUILT)**
...
**P-AGENT.6b - Export wired into the canvas + verified live (ADR-0129, BUILT)**
...
**P-AGENT.4-live - a BUILT agent RUNS on a real Claude model, VERIFIED (ADR-0129)**
...
**P-AGENT.4-live UI - in-app "Run" button, RUN a built agent from the canvas + verified LIVE (ADR-0129, BUILT)**
...
**P-AGENT.8.2 + .8.4 - chat->canvas handoff + the Secrets & connections panel (ADR-0130, BUILT + verified live)**
...
**P-AGENT.8.3 - AGENT_BUILDER_POLICY: the chat agent drives the handoff (ADR-0130, BUILT; frozen-prefix v7->v8)**
...
**P-AGENT.8.2 fix + LIVE END-TO-END DEMO (ADR-0130) — the conversational loop works against a real model**
...
**P-AGENT.8.6 - the `/agent` slash command: one-tap Agent Builder interview (ADR-0130, BUILT + verified live)**
...
**P-AGENT.8.5 - approve connections -> whitelist + doc-assisted credential setup (ADR-0130, BUILT + verified live)**
