...
## CREATOR-3: the video and 3D pipelines, and the bytes deciding their own type (ADR-0295, built by a parallel session)
- **shipped:** four pure cores (`comfy_stream`, `frame_capture`, `blender_cli`, `model_manifest`), the desktop seam `desktop/creator_pipeline.ts`, a fixed-argv Blender runner, a Render tab in Creator Studio, and three routes (`/api/creator/render`, `/blender`, `/manifest`). A render is refused before anything leaves the machine unless a LIVE unexpired probe attested the capability and the governor admitted the work; a workflow with an unfilled placeholder is never submitted; an artifact's type comes from its own MAGIC BYTES, so a server claiming `video/mp4` while sending PNG has its output refused BY NAME and unidentifiable bytes are refused rather than stored under a guessed extension; and every server-supplied string (filename, subfolder, output key, claimed mime) is delimited and scanned FAIL-CLOSED before a single byte is written, with the note admitting the media bytes carry a sha256 and NOT a content scan because the Unicode scanner cannot read a video frame. The `/ws` stream became real (the registry had been declaring `runtime-feedback` as available over websocket with no client anywhere in the tree) and is telemetry BY CONSTRUCTION: `/history` stays the authority, the drain is never awaited, and a silent, noisy, dead, or foreign-addressed socket cannot hang, fail, or corrupt a render. Blender's `ARGV_UNSAFE_CHARS` refuses only NUL, newline, CR, tab, and control codes, so a real `C:\Program Files (x86)\...\blender.exe` is accepted, with the no-shell spawn pinned by a test that walks every argv element (ADR-0296).
- **verified:** every number here was watched print on THIS machine rather than carried over from the authoring session's report. `demo-CREATOR-3` green (13 sections, 101 checks, 0 failures) with four sections driving the real product code against a real server PROCESS over real HTTP and a real websocket, and proven loud (a sabotaged claim exits 1 naming it); 342 new tests across the seven new suites, 0 fail; 774 pass across the 24 Creator-adjacent files; harness tree 1494 pass / 2 fail across 136 files and desktop tree 2626 pass / 9 fail across 215 files, all pre-existing Windows path-separator or TS-resolution cases in `fs_browse`, `symbol_graph`, and `lucid_acp`; `harness/prompt` 11 pass across 2 files (prefix_compaction 4 of them), so the frozen prefix is untouched; root typecheck clean; license headers clean; renderer bundle 127 modules. NOT verified and deliberately not claimed: `make` is absent on BOTH machines, so the `demo-CREATOR-3` target is structurally checked but UNEXECUTED (the script was run directly), and no real ComfyUI was involved anywhere - the end-to-end sections use the repo's own fixture server.
- **corrected after committing:** the first version of this entry published harness 2335 / 3 and desktop 4947 / 15. Both were inflated, because `bun test` matches paths by SUBSTRING: omitting `--path-ignore-patterns='desktop/release/**'` (which `make test` always passes) pulls in the packaged copy of the repo under `desktop/release` and double-counts it. The authoring session omitted it on its harness runs; I then ran the same command with the same omission, got the same number, and reported it as independent confirmation. It was not, and that is the lesson worth keeping: A MATCHING NUMBER IS NOT A SECOND MEASUREMENT WHEN BOTH RUNS SHARE A WRONG FLAG. Re-measured with the flag by hand, both trees now read as above, and the CREATOR-2 and CREATOR-5 entries below carry the same caveat since their desktop counts were taken the same unflagged way.
- **next:** the on-device pass against a real ComfyUI on the remote GPU host (probe, a video render, read the artifact back), which is also the first exercise of the socket against a real queue; then wiring the frame-capture harness into the Preview panel so a three.js scene can actually be stepped through it; then CREATOR-4 (engines) per ADR-0288.

## CREATOR-5: the mixer, and refusing to fix a mix quietly (ADR-0294)
- **shipped:** layering, as a sum rather than a concatenation. `harness/creator/mix.ts` is the graph as DATA plus the one place a sample is added: clips that MAY overlap, tracks with level, pan, mute, solo, and a piecewise-linear envelope, one bus layer for grouping beds, master last. The render is PURE TypeScript over PCM accumulating in a Float64Array in the graph's own order and quantizing exactly once (the ADR-0289 delta: `OfflineAudioContext` cannot promise the plan's own byte-identical keystone and cannot be exercised in the runner at all). The honesty is the feature: `renderMix` reports the TRUE peak measured before the rail, the exact `clipped` sample count, every track that contributed nothing WITH its reason, and `panIgnored` when a mono render could not honor a pan. It never normalizes, limits, or ducks: `headroomGain(peak)` hands the caller the number, applying it is opt-in, and the result states the factor used. `trackSilenceReason` is ONE function returning the sentence or null, and the render uses that same call to skip the track, so the decision and the explanation can never disagree. Equal-power pan (a sweep does not bulge in the middle), linear fades (a straight line sounds like the line you drew), stereo folded by averaging and mono duplicated, and a sample-rate mismatch REFUSED naming both rates because this build has no resampler. `desktop/creator_mixer.ts` picks the mix format as the majority of the library's real WAV headers (never a fabricated 44100/2; a track this build cannot state a format for is listed with nulls and cannot be offered), resolves every source by id, refuses by id, and saves through the library's own `addTrack` as a remix whose provenance line names EVERY input (`mixed from: <id> (Narration), ...`) since a mix has many parents and the ledger has one slot. `trackFromTimeline` lifts a CREATOR-2 document onto a track, which is the seam ADR-0289 promised. The Studio Mixer tab paints the rows, the report, and any refusal verbatim.
- **verified:** `demo-CREATOR-5` green (11 sections, 97 checks) and PROVEN loud (a broken claim exits 1 with the named FAIL); new suites mix 35 / creator_mixer 34 / renderer creator_mixer 50, 183 pass across the five mixer-adjacent files; desktop tree 4855 pass / 15 fail (counted WITHOUT `--path-ignore-patterns='desktop/release/**'`, so it double-counts the packaged repo copy under `desktop/release`; the repo's own gate reads 2626 pass / 9 fail, the same failures without the duplicates); all five Creator demos and verify-creator-comfy green; root and desktop typechecks clean; renderer bundle 126 modules (was 124); license headers clean; zero em dashes. LIVE on this machine: the Creator engine listed both library WAVs with their REAL formats and a 24000/1 majority, then rendered a two-layer mix (narration plus a faded stab under it) to exactly 238844 bytes / 4975ms reporting peak 0.5286 and 0 clipped, saved as a remix whose provenance line names BOTH inputs while each input kept its own bytes. One test that was itself a trap, removed after the commit: the demo asserted that the headroom FACTOR has more than six decimal places, which is a property of the fixture's amplitudes and not of the mixer (a pre-peak of 1.25 gives exactly 0.8), so it would have gone red on a perfectly good change; only the peak half is asserted now and the factor's delta is printed as evidence. Three bugs found and fixed during the increment, none by luck: my own fade expectation had the wrong arithmetic; the renderer's tracks-payload guard was rejecting the route's legitimate formatless answer for an undecodable library (so an honest "no format can be stated" surfaced as a broken route); and `mixerTracks` could let a WAV header declaring sampleRate 0 win the majority-format vote. The keystone is proven byte-wise three ways: a muted track, a track silenced by another's solo, and a track on a muted bus each render BYTE-IDENTICALLY to the same graph with that track removed, each with a non-vacuity check that it is audible otherwise. Clipping honesty is proven with an asymmetric hot source: peak 60000/32768 reported, 768 of 768 samples clipped, +40000 clamped to 32767 and -60000 to -32768 (clamped, never wrapped), then the headroom factor applied EXPLICITLY by the script re-renders to peak 1 with 0 clipped. Interop is proven byte-identical against `renderTimeline`. One test bug of my own caught by the run: my fade expectation had the wrong arithmetic (a 40ms fade-out on a 100ms clip is mid-ramp at offset 80, not silent).
- **also fixed:** CREATOR-2's `/api/creator/editor/save` was answering with a hand-written field-by-field projection of its result, which is a SECOND copy of that route's wire contract living in the one file no headless test can import: a dropped or renamed field would reach the pane as a report its guard refuses on every clean save, with the whole suite still green. The projection is gone and the result is returned whole, matching `/api/creator/mixer/render`, and the fix was verified live (the wire shape is byte-for-byte the same four fields).
- **next:** CREATOR-3 (image, video, and 3D pipelines, ADR-0287) or CREATOR-4 (engines, ADR-0288) are the remaining build items; the still-open on-device work is the remote ComfyUI probe pass through `tools/creator-backend/setup-backend.ts`. Audio follow-ups when they earn it: EQ, compression, and reverb (the mixer deliberately claims none), per-word re-synthesis in the editor, and a scrub-while-dragging preview.

## CREATOR-2: the follow-along audio editor, and an alignment that admits what it is (ADR-0293)
- **shipped:** word-level audio editing where the timing never lies. `harness/creator/timeline.ts` is a PURE edit-decision-list document (contiguous ordered clips, an explicit silence source, no overlap, no time-stretch: a clip's timeline length always equals its source region length) plus every operation on it - `splitItem`, `setItemLock`, `trimClip`, `deleteSpan`, `moveSpan`, `replaceSpan`, `lockToText`, a bounded snapshot `EditHistory`, `renderTimeline`, `waveformPeaks`, `frameEnergy`/`speechRuns`, `alignFromVendor`, `deriveAlignment` - with no node imports, so the renderer runs the ops directly and the server is touched only to OPEN a track and SAVE a render. Alignment provenance is load-bearing, not advisory: engine timings are `vendor` at confidence 1, anything LUCID measures itself is `derived` and capped at 0.7, and `validateDoc` makes a derived item claiming vendor-grade confidence an INVALID document that refuses to render. Derived alignment is a MEASUREMENT (per-frame RMS, speech runs thresholded against that clip's own noise floor, gaps shorter than a stop consonant merged) plus a length-weighted distribution inside each run, and the note it returns states the run count against the word count so the estimate's weakness is on screen. Every re-render is a NEW clip carrying `parentClipId` and the prompt that produced it (ADR-0281 lineage, now inside a track), a re-render with no prompt is refused, and a missing or format-mismatched source REFUSES the render by id rather than substituting silence. `desktop/creator_editor.ts` opens a library track (bytes, peaks, derived alignment, and only those library tracks whose format can actually render as replacement sources; `durationMs` is null when unmeasurable, never 0) and saves through the library's own `addTrack` as a remix, so the edited take keeps every byte and its row. A non-WAV or non-16-bit track is refused with its real mime named; there is no transcoder to pretend with. `desktop/renderer/creator_editor.ts` + the Studio Editor tab paint the chips, the waveform, the transport, and a status line that prints an operation's own refusal verbatim.
- **verified:** `demo-CREATOR-2` green (9 sections, 55 checks) and PROVEN loud - breaking one claim exits 1 with the named FAIL; new suites timeline 41 / creator_editor 18 / renderer creator_editor 37, 364 pass across the 18 Creator-adjacent files; desktop tree 4775 pass / 15 fail and harness tree 2046 pass / 3 fail (both counted WITHOUT `--path-ignore-patterns='desktop/release/**'`, so both double-count the packaged repo copy; the repo's own gate reads 2626 / 9 and 1494 / 2 respectively), every failure a pre-existing Windows path-separator or TS-resolution case in files this increment never touched (`fs_browse`, `symbol_graph`, `lucid_acp`); the four earlier Creator demos and `verify-creator-comfy` still green; renderer bundle builds (124 modules); license headers clean; zero em dashes. Two real bugs caught by the gates rather than by luck: `trimClip` returned an item array where a document belonged (bun test does not typecheck, the test did), and the seam's suite fed `number | undefined` into a `Matchers<number>`. LIVE on this machine: the Creator engine on 5320 opened a real 24kHz WAV with derived alignment at 0.531, saved a 2000ms trim to exactly 96044 bytes (44 + 2000 x 48), recorded it as a remix whose parent kept all 238844 of its bytes, and refused a save naming `trk-ghost`. Invariant 11 checked visually against the REAL stylesheet: 48 chips all exactly 27px at 260px AND 190px column widths, a 45-character token truncated with an ellipsis (scrollWidth 282 vs clientWidth 188, `nowrap`/`hidden`/`ellipsis`), and the provenance note a block paragraph with zero child elements.
- **next:** CREATOR-5's mixer (ADR-0289) is the natural sequel since this document is what it consumes, but the still-open on-device work comes first: run `tools/creator-backend/setup-backend.ts` against the remote GPU host and confirm the attested ComfyUI capabilities match a real install. Editor follow-ups worth doing when they earn it: per-word re-synthesis through a TTS provider (today a span re-render takes audio you already have), and a scrub-while-dragging preview.

## CREATOR-1: probes make `ready` mean something, and every Creator run leaves a row (ADR-0292)
- **shipped:** after CREATOR-0 the Studio could only say `configured` - an endpoint plus a credential, which is not the same as "this works". `desktop/creator_probe.ts` now ASKS: ComfyUI capability is derived from its **installed nodes** (`image` needs a save/preview node, `video` an animation/combine node, `model-3d` a 3D node, `workflow-run` a sampler; nodes-but-no-output is `no-capabilities`, not ready), ElevenLabs from its documented `/v1/models` flags (TTS implies streaming + alignment), a user-run service (dots.tts, a Suno partner endpoint) proves REACHABILITY and says so in the detail line, a desktop app proves it is on disk (Blender's `--version` captured; a tool that refuses it is still installed), and three.js is ready by construction. Secrets ride headers, never a URL, never a detail line. `foldProviderStatus` no longer treats configured-plus-optimism as ready: once a probe has spoken only its ATTESTED capabilities are usable, and `ProbeCache` drops an answer after 15 minutes so a stale probe cannot keep a provider looking healthy after a VPN drop. `desktop/creator_jobs.ts` adds the durable ledger: closed kind/state sets, a `canTransition` table enforced on write AND on fold (a hand-edited or out-of-order ledger cannot resurrect a settled job), bounded history, torn-tail tolerance. Image, sheet, and GIF routes now run through `admitCreatorJob`, which snapshots the governor's measured CPU/memory/GPU/VRAM per job and writes a REFUSAL as a `refused` job carrying the measured reason instead of dropping it into a toast; artifact ids are recorded per job. Cancel is a REQUEST: `cancelRequested` lands immediately, the row reads "(stopping)", and it settles as `cancelled` only when the runner confirms. Surface: a Probe button per provider row plus Probe-everything, a probe line (verdict, age, latency, the server's own escaped words), and a Recent-jobs strip with duration, artifact count, and the governor's numbers.
- **verified:** `make demo-CREATOR-1` green (4 sections, 33 checks) alongside `demo-CREATOR-0` and `demo-CREATOR-IMG`; 259 tests across 16 Creator/imaging files, 0 fail (new: creator_probe 22, creator_jobs 13; creator_studio grew to 29); fleet/acp/system/about suites 95 pass; all three tsc programs clean apart from the documented pre-existing dev.ts `finish(m[1])`; license headers clean; renderer bundle carries the probe + job surface (121 modules).
- **shipped (verification):** `make verify-creator-comfy` + `desktop/fake_comfy_server.ts` (a fixture that implements ONLY the documented ComfyUI routes, serving real PNG bytes from the product's own encoder). The verifier drives the REAL product code end to end - probe, attestation, upload a mixed input, substitute the template, submit, poll `/history`, read the image back, store it with provenance, then re-fold the ledger - plus the failure paths (token-required is UNAUTHORIZED not unreachable, no-output-node proves nothing, a dead endpoint throws nothing). `--url` points the same code at a real server and, without `--workflow`, probes ONLY: LUCID does not invent a graph for someone else's box, and the verifier never provokes the unhappy paths against it. IT FOUND A REAL BUG in my own attestation: a catalog with a sampler but no save node attested `workflow-run` and read as `ready`, so a graph that can never yield a file looked usable. Fixed: ComfyUI is `ready` only when it proves an OUTPUT capability (image, video, or 3D); `workflow-run` and `runtime-feedback` are enablers, never the proof. Two regression tests pin it.
- **shipped (backend scripts):** `tools/creator-backend/provision-comfyui.sh` (runs ON the GPU host: honest preflight that REFUSES without an NVIDIA GPU, clone-or-update, venv, cu130 wheels for aarch64/sm_121, a torch device self-check, unified-memory launch flags, a systemd USER unit + linger, optional DCGM exporter, idempotent, `--dry-run`, no sudo, loopback only) and `tools/creator-backend/setup-backend.ts` (runs on the workstation: preflight -> provision -> tunnel -> verify -> register, `--dry-run` prints every command and executes nothing). Security posture is enforced in code and pinned by tests: every remote call is a FIXED ARGV; a host, user, or REMOTE path with shell metacharacters is REFUSED rather than escaped (values that reach the far-side shell), while a LOCAL argv-only path (`--identity`, `--workflow`) takes an ordinary Windows path as written, since one guard for both threat models produced a FALSE refusal of `C:\Users\me\.ssh\id_ed25519` (found by a peer session hitting the same posture in a Blender path, fixed and pinned in both directions); `BatchMode=yes` means keys only, the tunnel binds the remote LOOPBACK (never 0.0.0.0), the preflight command is read-only (test asserts it contains no rm/mv/dd/curl/apt/pip), the registered declaration is zoned `internal` and carries no credential-shaped field, and the LUCID token for the optional auto-registration comes from the environment, never a command line. No machine name appears anywhere in the scripts. docs/CREATOR-MODE.md carries the phased plan plus a TWO-OPERATOR section: one person provisions (the provisioner ends in `systemctl --user restart`, so re-running it kills the other person's in-flight render), the second runs `--skip-provision`, each adds their own key to the one lingered account, each workstation tunnels its own local port to the same remote loopback, ComfyUI's single prompt queue is the contention point (arrival order, not parallel), and the DCGM exporter must be registered on both sides or the shared GPU reads unknown rather than idle.
- **next:** on-device probe pass against a real ComfyUI - the user's workstation install failed, so the target is their DGX Spark: headless ComfyUI on aarch64/`sm_121` needs CUDA 13 (PyTorch `cu130` aarch64 wheels or NGC \u2265 25.10) and unified-memory flags, kept on loopback behind an SSH tunnel because ComfyUI has no auth; the DGX can also host a DCGM exporter as a remote monitoring target. Steps are in docs/CREATOR-MODE.md. Then CREATOR-2 (the follow-along audio editor) per ADR-0286.

## CREATOR-0 + CREATOR-IMG: the Creator flavor, on its own port and identity (ADR-0279..0284, ADR-0291)
- **shipped:** branch `feature/creator-mode`. ONE branch, TWO products: `desktop/build_flavor.ts` is the whole identity contract (appId com.lucidcreator.desktop, productName LucidCreator, port **5320**, relay 8791, whisper 9112, scheme lucid-creator, LucidCreator-* artifacts, release-creator output), resolved in main.ts BEFORE any userData path so Electron's lock, the Windows os_crypt key, GUI settings, Personal Knowledge, the vault, and the creator data root all separate by construction; the standard build's values are pinned by test and did not move. Packaging is an OVERLAY (`build/electron-builder.creator.cjs` deep-clones package.json's build config + `extraMetadata.lucidBuildFlavor`), so master's payload changes reach Creator for free; `build/build-creator.ts` bakes the flavor into dist/main.js. Creator Mode is a fourth UI MODE next to Agent (not a role, not a frozen AgentMode) whose `uiModePosture` is byte-identical to Agent - a test pins that - and it is absent from the DOM in a standard build, with dev.ts AND acp_backend re-normalizing a forged `creator` to `agent`. Creator adds: an integration registry with HONEST capability labels for ElevenLabs / dots.tts / Suno / ComfyUI / three.js / Blender / Unreal (available vs bring-your-endpoint vs vendor-app-only vs planned, declarations that refuse a pasted secret or a shell string), normalized CPU+GPU telemetry with per-core detail, nvidia-smi and DCGM/JSON remote targets, and evidence-based admission (90% held 30s, or a KNOWN VRAM shortfall, naming both numbers), the right-rail CPU/GPU **odometer chips** (270-degree graduated dials, green-amber-red, hot pulses, stale dims, blind shows "no signal" and NO value arc) with a click-through flyout (per-core strip, per-device VRAM/temp/power, processes, trend whose line breaks across a gap), the local track LIBRARY for the Suno ask (import, listen, rate, review, tag, remix, re-prompt, lineage) which needs no API at all, a Creator prompt-tail block that vanishes on mode exit, `.agents/skills/creator-studio/SKILL.md`, and docs/CREATOR-MODE.md. CREATOR-IMG then added image generation through the USER's own ComfyUI workflow (live `/object_info` model dropdown, `{{prompt}}/{{model}}/{{seed}}/{{image:role}}` substitution that REFUSES on an unresolved placeholder, role-bound image mixing, artifact provenance with sha256) plus native encoders in pure TypeScript - sprite sheets (PNG + frame manifest + steps() CSS), GIF89a (deterministic palette, LZW, loop block), and memes (fit-and-wrap over canvas text) - with markup reusing the existing Preview panel.
- **verified:** `make demo-CREATOR-0` (7 sections) + `make demo-CREATOR-IMG` (5 sections) green; 212 tests across 14 Creator/imaging/preamble/agent-flow files, 0 fail; prior `demo-P-FLEET.L2` still green; all three tsc programs clean apart from the documented pre-existing dev.ts `finish(m[1])`; license headers clean; renderer bundles (121 modules) with the meme math and ZERO node-only code (`deflateSync` absent). LIVE on this machine: `bun run web:creator` answered on **5320** while the standard engine held its own port, `/api/build-info` reported flavor creator + isolated roots, `/api/creator/resources` returned real per-core numbers (i7-6700K, 8 cores, 95%) with an honest `available:false` GPU block naming the missing nvidia-smi, `/api/creator/registry` listed all seven providers, and a real 238 KB WAV round-tripped through import -> review (rating 4) -> remix (parent recorded) -> playback -> honest refusal on an unknown id; a declaration carrying a URL credential AND a pasted secret was refused with both reasons while a clean one saved as `configured`.
- **next:** on-device pass in the packaged Creator app (`bun run dist:win:creator`, install beside the standard app, confirm both run at once and the Resources tab + Studio appear); then CREATOR-1 (capability probes + the durable job spine) per ADR-0285, which turns registry state `configured` into a truthful `ready`.

## Release cut: v1.13.1 - unlimited fleet lanes, lanes from a repo remote, a truthful minimized HUD
- **shipped:** version bump 1.13.0 -> 1.13.1 (patch: P-FLEET.L2 amends the fleet guard + the `FleetStatusData.resources` contract, no new headline surface) across the four release sites (desktop/package.json, version.ts APP_VERSION + the v1.13.0 changelog comment line, about.test.ts pins x2) + README refresh: the Newest paragraph rewritten to v1.13.1 (sustained-pressure admission, repo-remote lanes with host-scoped vault tokens, the real OS folder dialog, the truthful non-flickering pill) with v1.13.0 demoted into the history chain, and a v1.13.1 batch row citing ADR-0273. Payload: the P-FLEET.L2 working tree committed as its own feat commit (ADR-0273; fleet_resources rewritten to window arithmetic, new dependency-free desktop/git_url.ts, clone-then-spawn in /api/fleet/spawn, per-host `git_pat_<slug>` vault refs, mountFleetPill re-adoption + per-state dots, make demo-P-FLEET.L2).
- **verified:** README test counts CORRECTED, not restated - the old "6,200+ (harness 1,876 / desktop 4,326 / sidecar 62)" counted the generated `desktop/release` repo copy and a local `.claude/worktrees` checkout twice; CI run 32838505161 reports harness **1,041 tests across 122 files** (1,037 pass, 4 skip, 0 fail) + packaged-boot 2 pass + sidecar **57 passed**, and `bun test desktop` (release copy excluded) reports **2,036**, so the badge and status block now read 1,041 / 2,036 / 57 (~3,100+). This machine: demo-P-FLEET.L2 7/7 and demo-P-FLEET.L1 5/5 green; fleet_resources + fleet_lanes + git_url + workspace + about suites 92 pass / 0 fail; root and desktop `tsc --noEmit` clean; license headers clean; sidecar pytest 57 passed. Pre-existing Windows-only reds carried (not caused here): harness 2 (lucid_acp asserts POSIX asset paths), desktop 9 (fs_browse resolves a Windows HOME against a POSIX fixture, symbol_graph needs the TS compiler) - all green on the Linux CI runner.
- **next:** watch the v1.13.1 tag build cut installers on the 3 runners (the tag build re-runs the air-gap gate AND the strict Program Files boot gate on the release bytes, then auto-pins the Homebrew cask); user's on-device pass - minimize with lanes in every state through a streaming turn (the flicker), clone a private Azure DevOps repo into a folder created from the Explorer dialog and confirm the token survives a relaunch, then hold a box above 90% for 30s and read the refusal. After that P-FLEET.P1 (profile store + LUCID_INSTANCE_ID + /api/instance) per ADR-0272.

## Release cut: v1.13.0 - the Fleet Manager
- **shipped:** version bump 1.12.2 -> 1.13.0 (minor: Fleet Manager is a headline feature) across the four release sites (desktop/package.json, version.ts APP_VERSION + the v1.12.2 changelog comment line, about.test.ts pins, README Newest paragraph + v1.13.0 batch row). README also refreshed: test counts to 6,200+ (harness 1,876 / desktop 4,326 / sidecar 62) and the release history rows through v1.13.0. Payload: PR #339 squashed to master (`51ce5e7`, the eight integrated fleet/voice sessions, ADR-0264..0272) + the stale quota-toast fix (`737b88a`).
- **verified:** about.test.ts green on the new pins; both suites at the documented baselines pre-tag; the tag build re-runs the air-gap gate AND the strict Program Files boot gate on the release bytes before anything is attached.
- **next:** watch the v1.13.0 tag build cut installers on the 3 runners; on-device passes - fleet dock smoke (real lane, approval glow), dictation, a long fan-out watching the HUD name pending tasks.

## Integration: the parked fleet/voice sessions land on master (Fleet Manager + P-VOICE.7 + ingest/FS/STT fixes)
- **shipped:** `wip/adr-0252-0260-sessions` (eight parked sessions) merged with post-v1.12.2 master on `feat/fleet-voice`. Payload: P-KG-INGEST.5 (bounded ACP requests + die() drain + per-message cancel - SUPERSEDES P-STALL.2's minimal failPending, as ADR-0263 planned), P-FS.2 native OS folder dialogs for the browser build, P-DATA.1 frozen prefix v10 (prefix-hash tests 21/21 green), P-STT.6 Whisper housekeeping UNIONED with master's tiny-default+autostart (tiny is THE default on any hardware; offered set tiny/base/small is the law; gray-out reasons; Remove per model), P-FLEET.1 job handles (all fifteen ADR checks green), P-VOICE.7 spoken thinking snapshots, P-FLEET.L1 local lanes + fleet grid (five demo checks green), Fleet Profiles scope + both bat fixes. ADR renumber at merge: wip 0252..0260 -> 0264..0272 (DECISIONS tail monotonic 0250..0272; PROGRESS entries; every code citation - anchored per-phrase in shared files where master-meaning trainer/WINBOOT citations coexist). native_dialog.ts moved to node:child_process (bridge type-imports it into the Node-typed desktop project); whisper default-tier test re-pinned to the union semantics.
- **verified:** harness 1876 pass / 3 documented pre-existing; desktop 4326 pass / 15 documented pre-existing; merged-area slice (whisper/acp/fleet/mcp/stall) 156 pass; demos P-FLEET.1 (15 checks), P-FLEET.L1 (5), P-KG-INGEST.5, P-STALL.1/.2 all green; root + desktop tsc clean; license clean; prefix v10 byte-stable.
- **next:** PR + squash-merge to master; README refresh (test counts + release history) + v1.13.0 release cut with Fleet Manager; on-device fleet-dock smoke (spawn a real lane, watch working->needs-approval->done glow) is the user's pass - no app window in this environment.
...
- **next:** watch the v1.12.2 tag build cut installers on the 3 runners; on-device pass (Program Files install click-through + a long fan-out watching the HUD name pending tasks); then pick up the FLEET MANAGER + VOICE fixes from `wip/adr-0252-0260-sessions` (renumber its ADRs on top of master's log first).

## P-STALL.2: no turn cutoff - long work runs to completion, and the wait is legible (ADR-0263)
- **shipped:** the 10-minute total-silence kill (P-STALL.1's IDLE_MS raced against session/prompt) was murdering legitimately long turns - subagent fan-outs sit quiet far longer than any fixed clock while the work genuinely runs, and the user reported every long run dying with "the model sent nothing for 10 minutes" and zero visibility into what was pending. USER CALL: the cutoff is REMOVED (a turn ends when the work ends, Stop, or transport death), and the failure the clock actually guarded against is now EVENT-DRIVEN - ACPClient drains every pending request on child exit/spawn error (`failPending`; on this branch a dead omp used to strand requests forever, so removing the timer alone would have traded early kills for infinite hangs). VISIBILITY: new pure `desktop/turn_pending.ts` tracks the turn's open tool calls from the raw ACP stream (spawned subagent tasks labeled `subagent <agent> \u00d7N: <title>`; terminal tool_call_update closes exactly its call), and every `{ type:"slow" }` notice (2-min cadence stays) now carries `pending: {label, elapsedMs}[]` -> HUD phase "Working \u00b7 waiting on N tasks \u00b7 quiet for M min" + a toast listing the longest-running ones; copy never names a cap, always names Stop. Scope: CHAT turns only - util completions (KG extraction / goal checker) keep their deliberate background clocks until the parked P-KG-INGEST.5 rework lands (failPending is its minimal, intentionally-overlapping subset).
- **verified:** `make demo-P-STALL.2` (clock gone from the chat path; a REAL child process exits mid-request and the promise rejects with the code; tracking lifecycle; slow-event contract; honest copy) + the evolved `demo-P-STALL.1` (the visibility half that remains); 20 unit tests (turn_pending 5, acp real-child 2, stall_notice rewrite incl. the inverse lockstep pinning that the chat cutoff can never silently return); root + desktop tsc clean; license clean; full desktop suite at the documented baseline.
- **next:** on-device - run a real long fan-out and watch the HUD name the pending subagent tasks; extend pending visibility to util/ingest jobs when the wip branch's P-KG-INGEST.5 lands; then squash-merge PR #338 and cut v1.12.2.

## P-WINBOOT.3: the installer clamp is relaxed - per-machine installs are legal again (ADR-0262)
- **shipped:** the ADR-0259 mitigation (nsis `allowElevation:false` + `allowToChangeInstallationDirectory:false`) ends, now that its justification is gone: the compiled engine (ADR-0260) boots from protected trees and the ADR-0261 gate PROVES it in CI from the real `C:\Program Files` against the exact packaged bytes. Posture: assisted installer (`oneClick:false`), DEFAULT stays per-user (`perMachine:false`, `%LOCALAPPDATA%\Programs` - writable, no elevation, auto-update friendly), per-machine allowed (`allowElevation:true` + `allowToChangeInstallationDirectory:true`). The flag flips are the exact diff the branch carried as "DO NOT MERGE" since Aug 1 - now intended per this ADR; branch history deliberately kept as-is (no force-push), so the PR must SQUASH-merge to keep the stale title out of master. COUPLING: demo-P-WINBOOT.1 section [5] asserts the posture AND that build-desktop.yml still runs the strict boot gate - removing the gate turns the demo red, so the relax can never outlive its justification silently. engine_boot.ts's protected-location classifier STAYS (stale pre-engine packages must keep failing fast + actionably).
- **stubbed:** the real-installer click-through (pick `C:\Program Files`, elevate, boot the installed app) is the user's on-device pass with the run-32796994917 artifacts; the gate proved the boot half from the identical staged bytes on the runner.
- **next:** PR the branch to master (SQUASH-merge - the DO-NOT-MERGE title remains in branch history by choice) and cut v1.12.2 (compiled engine + boot gate + clamp relax). Increment D (bat) rides with the parked wip branch.

## P-WINBOOT.2C: the Program Files boot gate - the CI smoke that would have caught the brick (ADR-0261)
- **shipped:** the v1.12.0 brick shipped because nothing ever booted the app from an ACL-protected install tree (packaged_boot.test.ts boots from a writable temp dir). Now `desktop/build/pf-boot-smoke.ts` stages the PACKAGED repo (`*-unpacked/resources/repo`; a dev box without a fresh dist builds a source skeleton: compiled engine + prebuilt renderer + the 7 native-addon packages) into a Program Files location, DENIES the current user the specific write/delete rights on the whole tree (`(OI)(CI)(WD,AD,WEA,WA,DE,DC)` - runners are admins, so inherited ACLs alone constrain nothing; and NEVER generic W, which includes SYNCHRONIZE and EPERMs CreateProcess itself, proven by executing the first version), PROVES the denial with a failed probe write, then requires `resolveEngineSpawn` to pick the compiled binary (the exact main.ts decision) and `bin/lucid-engine` to answer `/api/health` + serve the prebuilt `/app.js` from the protected tree. Wired STRICT into build-desktop.yml's Windows runner (strict THROWS rather than downgrade to a temp dir - the silent-downgrade is how the gap survived). Two self-inflicted hangs found by running it: undrained stdout/stderr pipes blocked the chatty engine mid-boot, and awaiting stderr EOF before killing deadlocked the smoke - both fixed (drain from spawn, kill before read). Verified: `make demo-P-WINBOOT.2C` green end to end on this Windows box (write-denial proven, engine booted from the write-denied tree in ~2s, health ok, 7.1MB bundle served, stage un-denied + deleted); 14 unit tests (`engine_pf_smoke.test.ts`); `demo-P-WINBOOT.2` still green; root + desktop tsc clean; license clean; `bun test harness` at the documented baseline (3 pre-existing lucid_acp.test.ts path-separator fails only). Session hygiene: the 8 uncommitted sessions found in the tree (ADR-0252..0260 incl. the fleet epic + bat fixes) were parked on `wip/adr-0252-0260-sessions` (pushed) so this branch carries only the WINBOOT payload; ADR numbering settled at the merge: master's 0250-0258 win (shipped in v1.12.1), the branch's parallel P-WINBOOT 0250/0251 renumbered to 0259/0260, and the wip branch renumbers when it lands.
- **stubbed:** the STRICT path (real `C:\Program Files` root + packaged tree) runs on the next CI installer build, not here - this shell is non-elevated and the local win-unpacked predates the compiled engine (the demo exercises the same code through the hardened-fallback dir + source skeleton). The deny ACE reproduces the standard-user WRITE constraint, not whatever machine-specific loader condition EPERM'd v1.12.0 (never pinned - AV/CIG/mmap); the gate is structural: boot + serve while the install tree refuses every write.
- **DONE (strict gate proven on the real runner):** run **32796994917** = all 3 OSes green, `publish-latest` skipped, releases untouched. The Windows job ran the gate STRICT on the max-fidelity path: mode packaged (`win-unpacked\resources\repo` - the exact installer bytes), staged to `C:\Program Files\Lucid PF Smoke` (real Program Files), write-denial PROVEN, engine booted, `/api/health` ok, 7.1MB `/app.js` served, stage cleaned. One real CI bug found + fixed on the way (`2d2c076`): `/bin/` was fully gitignored so a fresh checkout had NO bin dir, and the runner's floating `bun latest` no longer creates the `--outfile` parent - `dist:win` died ENOENT at compile-lucid BEFORE packaging (the Aug-1 green run predated the Bun change). Fixed with the `desktop/whisper`-style `.gitkeep` pattern (`/bin/*` + `!/bin/.gitkeep`). NOTE: the build toolchain still floats on `bun-version: latest` - pinning it is worth its own decision.
- **next:** relax the ADR-0259 installer clamp (re-enable per-machine installs, its own increment - the gate that justifies it is now green); then drop the DO-NOT-MERGE tip commit, PR this branch, and cut v1.12.2 with the compiled engine. Increment D (bat) rides with the parked wip branch.

## Fix: bat G option self-heals a OneDrive-stripped node_modules\.bin (shim guard)
- **shipped:** option G died with `bun: command not found: electron` because OneDrive sync strips the tiny shim launchers out of `desktop/node_modules/.bin` while leaving the package dirs intact, so the bat's `electron\dist\electron.exe` pre-check passed but `bun run start` could not resolve the shim. `:gui` now detects the stripped state (dist present + `.bin\electron.exe` missing), relinks with a `bun install` in `desktop/`, and the native path additionally requires the shim (`NATIVEOK`) with the browser GUI as the fallback. Verified: block-logic repro green; the guard EXECUTED live during a cancelled E2E run (detected the stripped shims, ran bun install, shims restored on disk); piped control run ("0") quits clean. Baseline `bun test harness` 1773 pass / 3 pre-existing fails (Windows path-separator expectations in lucid_acp.test.ts, one duplicated via a stale desktop/release packaged copy) - untouched by this change; no test references the bat.
- **stubbed:** a fully-piped E2E (g then 0) is limited by cmd's `set /p` requiring CRLF line endings from a pipe (LF-only input slurps both lines into CH and syntax-errors the dispatch - a test-harness artifact; keyboard input is CRLF and unaffected). Interactive click-through is the user's next G launch.
- **next:** P-WINBOOT.2 Increment D items remain (bundled-runtime-aware bat + vault-read key prompt); fix the 3 pre-existing Windows path-separator failures in lucid_acp.test.ts as their own increment, and exclude `desktop/release/**` from `bun test harness` path matching.


## P-WINBOOT.2: the permanent fix - the engine ships as a compiled binary (ADR-0260)
- **shipped:** the root cause behind ADR-0259 - main.ts ran `bun run desktop/dev.ts`, and Bun EPERMs loading a `.ts` from a `Program Files` tree. Now the engine ships as `bin/lucid-engine`, a `bun build --compile dev.ts --external "*.node"` standalone binary: dev.ts + every imported module is EMBEDDED (Bun never module-loads a `.ts` off the install disk), native addons are the ONLY `--external` (DuckDB/onnx load via the OS loader, which works from `Program Files`, unlike Bun's TS-mmap), and the renderer is PREBUILT (`build-renderer` -> `renderer/app.bundle.js`, served statically) so there's no runtime `Bun.build` of `.ts` either. Spike evidence drove the design: a bare `--compile` dies resolving DuckDB's other-OS `.node`; `--external "*.node"` clears it; a compiled probe proved `import.meta.dir` is virtualized (so the engine derives its base from `process.execPath` via `engine_launch.ts:engineDesktopDir`) and that an external DuckDB binding runs from the binary. `resolveEngineSpawn` picks the binary in packaged mode (fallback to `bun run dev.ts`). Verified: `demo-P-WINBOOT.2` BUILDS + BOOTS the real binary -> `/api/health` ok + prebuilt 7 MB `/app.js` served from the execPath root; 23 unit tests (engine_launch + engine_boot); desktop `tsc` clean; `airgap-smoke.ts` now fails the build if the engine binary or renderer bundle didn't ship.
- **stubbed:** the end-to-end boot from an actual `Program Files`-ACL install is NOT verified here (no Windows install harness in this env) - that's Increment C. `--external "*.node"` keeps only native addons out of the binary; all JS shims (incl. DuckDB's) are embedded so nothing `.js`/`.ts` loads off the install disk at runtime.
- **next:** Increment C (CI smoke that installs to a `Program Files`-ACL location + boots `bin/lucid-engine` to `/api/health` - the regression that would have caught the brick), then relax the ADR-0259 installer clamp to re-enable per-machine installs, then Increment D (`LucidAgentIDE.bat` diagnostics).

-----

## P-WINBOOT.1: Windows installed-app startup hardening (ADR-0259) - the Program Files blank-box brick
- **shipped:** v1.12.0 installed under `C:\Program Files` showed a 30s blank window then a generic "could not start" error, because main.ts runs `bun run desktop/dev.ts` from `<resources>/repo` and Bun 1.3.14 EPERMs loading `.ts` out of the ACL-protected tree (portable worked - it runs from a writable dir; `Bun.file().text()` reads the same file, so it is the loader's file-map that Program Files refuses). Three-part mitigation: nsis `allowElevation:false` + `allowToChangeInstallationDirectory:false` so new installs land only in the writable `%LOCALAPPDATA%\Programs` default; `dev.on("exit")` + an early-return `waitForServer()` so a dead engine is diagnosed immediately, not after the full 30s; and a pure, tested `desktop/engine_boot.ts` classifier (`protected-location | engine-exited | timeout`; evidence = protected path / failed write-probe / EPERM|EACCES signal) whose dialog names the exact folder and the two real fixes (reinstall per-user, run portable) and surfaces the EPERM line. Verified: `demo-P-WINBOOT.1` (20 checks) + 16 unit tests + desktop `tsc --noEmit` clean.
- **stubbed:** mitigation only - production still executes raw TypeScript from install resources; a real `Program Files`-install boot was not run here (no Windows install harness in this environment). A bundled `.js` engine would NOT fix the root cause (same Bun file-map load path); only `bun build --compile` or running from a writable location is guaranteed.
- **next:** Increment B (compiled-engine artifact via `bun build --compile`, the permanent fix, its own ADR), Increment C (Windows CI smoke that boots the EXACT Electron spawn from a `Program Files`-ACL location - the gap that let this ship; `packaged_boot.test.ts` only booted from a writable temp dir), Increment D (make `LucidAgentIDE.bat` bundled-runtime aware + stop the spurious API-key prompt on an unreadable vault).
...
- **next:** on-device pass of the role-first boot; a generic deviation-derived Five Whys generator (the drill is still sample-pack-only); P-TRAINER.7 stage-act choreography continues separately.

-----

## docs: ADR-0256 P-FLEET scope - the Chief-of-Staff fleet, one LUCID orchestrating N gated workers
- **shipped:** the fleet feature is now a documented ADR (SCOPE/PLAN, DECISIONS.md ADR-0256). It pins the transport insight (the agent-firewall registry stores an arbitrary command, so `ssh <host> lucid acp` IS a remote gated worker and N same-box workers are N registry entries - the fleet is configuration), the four build increments (P-FLEET.1 async dispatch/status/result job tools on the firewall; P-FLEET.2 worker keepalive + connection ping; P-FLEET.3 fleet visibility in the Settings card + PWA; P-FLEET.4 agent-to-agent threads relayed THROUGH the orchestrator's firewall, never direct), the security invariants (each connection its own trust domain, every inbound byte scanned + UNTRUSTED-wrapped, scan-dead = block, gate in-process, secrets never in the registry, new EventNames quarantined to their increment), and the rejected bespoke fleet-RPC alternative.
- **stubbed:** scope only - no fleet code this session by design; P-FLEET.1 is the first build increment.
- **next:** P-FLEET.1 in its own session (dispatch/status/result MCP tools beside `prompt`, fail-closed law unchanged).

-----

## Fix: Preview panel pinned stale documents - same-URL iframe no-op + openPreview skip-guard
- **shipped:** the panel could show a previously served document FOREVER: re-opening the same file, or re-editing it, never repainted (observed live: an edited trainer.html kept rendering its pre-edit bytes; a panel stuck on an error doc never recovered). Two renderer defects compounded. (1) `bridge.previewServeUrl` was deterministic (path + static launch token), and assigning an iframe src its CURRENT value does not renavigate in Chromium, so `loadPreview` on an already-loaded path was a silent no-op - the server's `no-store` never mattered because the request never happened; the URL now carries a per-call `v` nonce (`Date.now().toString(36)`), making every deliberate load a real navigation + fresh disk read (the serve endpoint ignores `v`; the `t` token gate is untouched). (2) `onPreviewAvailable` with the panel CLOSED routed through `openPreview`, whose unchanged-path guard skipped the reload of the just-written file; it now clears `prevPathByLane.agent` first, so a write always repaints. The guard itself stays: it exists to keep a running previewed app alive across mere panel toggles, and that behavior is preserved.
- **stubbed:** verified by desktop tsc (clean) + the same `bun build` the dev server runs (111 modules, bridge change bundled); the click-through repaint is renderer DOM, QA-gated in-app like the rest of the panel, and the RUNNING app only picks the fix up on its next launch/build.
- **next:** on-device pass - edit a previewed file, confirm the panel repaints; consider an mtime-keyed reload on panel re-open so even a toggle repaints when the file changed underneath.

-----

## Fix: trainer stage WCAG contrast + design-token alignment (audit F1-F17)
- **shipped:** a contrast/design-system audit of `desktop/renderer/trainer.html` and the 17 resulting fixes, all in the one self-contained file. Contrast: every `--txt-4` (#525a70) text usage failed WCAG AA on the dark bgs (2.86-2.99:1) - `.answer::placeholder`, `.gap-next .k`, `.role-lbl span`, `.seq-item .grip` moved to `--txt-3` (~6:1) and `.lesson-tag`, `.game-card .go` to `--txt-2` (~9:1); zero txt-4 usages remain. Token alignment to styles.css: `--bg-0` #08090d -> #0a0b0f (the divergent darker bg was costing ~0.3-0.5 contrast points page-wide), radii 16/11/7 -> 12/9/6, `--t` 200ms -> 180ms, `--ease` -> the canonical curve with `--ease-out` added and entrance/fill animations migrated to it (`--spring` kept for game motion). DRY: the four hand-rolled on-accent hexes (#150018/#00181c/#00180c/#180000) are now :root tokens (`--btn-text-dark`, `--btn-text-cyan`, `--state-correct-text`, `--state-wrong-text`), incl. a 7th hard-coded SVG stroke in the GAMES array; the two JS-built inline `style=""` color sites became classes (`.h3-sub`, `.result-meta`); three literal em dashes in UI strings removed (writing rule).
- **stubbed:** visual re-render in the Preview panel could not be confirmed on this host (the documented preview-harness refresh flake - the panel pins the first served doc); the fixes are CSS token swaps verified on disk, by the trainer suites, and the pre-fix render proved the page loads with zero console errors.
- **next:** on-device pass of the trainer stage to eyeball the brighter labels; fold into P-TRAINER.7 stage-act wiring.

-----

## P-TRAINER model-agnostic pass: the trainer runs on ANY configured, capable model
- **shipped:** the two halves of the any-model guarantee. (1) `desktop/trainer_model.ts` (pure, ADR-0250/0251 resolver pattern): `resolveTrainerModel` keeps a capable current model, else ranks the ACCESSIBLE catalog by distillation tier (flagship > workhorse > fast/small), direct route over the gov gateway, newest last; flash-class and local Ollama tiers stay ELIGIBLE (an air-gapped install still trains), non-chat routes (rag/embed/aux/deprecated/China-gated) never qualify; nothing usable resolves null so the caller shows the provider checklist. (2) the distiller now survives any family's reply style: `extractJsonObject` (fenced block anywhere, else a string-aware balanced-brace scan pulls the first JSON object out of prose) plus ONE corrective retry carrying a reply-with-only-JSON instruction, then fail-safe drop, never a loop.
- **stubbed:** the resolver is pure + tested; wiring it into the stage act's enter flow (set the model like resolveConversationModel does, restore on exit) lands with P-TRAINER.7.
- **verified:** 63 trainer-family tests green (10 new: catalog ranking fixtures incl. gov/local/flash-only installs, prose-wrap + fence-in-prose + braces-in-strings parsing, the retry path incl. the corrective instruction); demo-P-TRAINER.1..5 green; root tsc clean; license clean.
- **next:** P-TRAINER.7 stage act (wire resolveTrainerModel + the interview loop into conversation mode with the coverage HUD).

-----

## P-TRAINER.1-.6: the knowledge-trainer harness core - the flywheel runs end to end
- **shipped:** the ADR-0252..0255 harness core, built on the merged lucid-agent branch. `harness/trainer/`: `coverage.ts` (L0-L3 extraction rubric, weighted scores, gap queue that never re-asks confirmed ground), `planner.ts` (pure interview state machine: scenario-first, ONE question at a time, five-whys on deviation cues, session cap with visible-progress recaps), `store.ts` + kb migration `0012_trainer_tables.sql` (append-only knowledge units: corrections supersede, rejections tombstone), `redact.ts` (PII to typed placeholders BEFORE storage; SSN/account hits force quarantine), `distiller.ts` (answer -> typed unit through the EXISTING fail-closed ingest: dead scanner blocks, poisoned span mints nothing, derived body re-scanned), `teachback.ts` (confirm records the EXISTING promotion_approve approval and promotes through promoteFactGated verbatim; quarantined units get NO one-click path), `quizgen.ts` (trainee quizzes ONLY from confirmed units, source-cited; misses re-open objectives as extraction targets). Plus the 7 `trainer_*` EventNames (contracts.ts, its own commit per the frozen-file rule), the additive signature-bound `coverage_map` lkgpack member, and the 13-objective wealth-management-ops coverage map with the due-diligence checklist seed. Also pinned the stale about.test.ts version asserts to v1.12.0 (pre-existing red from the release cut).
- **stubbed:** P-TRAINER.7/.8 (the immersive stage act, HUD, mascot choreography, desktop wiring + kb_pages projection of confirmed units) are the remaining increments; demos use injected fake scanners/models (the real sidecar path is exercised by the existing ingest suites).
- **verified:** 53 new tests (trainer suites + pack coverage_map) all green; demo-P-TRAINER.1..5 green (the .5 demo runs extract -> distill -> confirm-promote -> trainee quiz -> miss re-opens extraction, plus signed-pack tamper refusal); root tsc clean; license check clean; full suite green minus the flaky pdf timeout.
- **next:** P-TRAINER.7, the stage act: wire the planner/distiller/teach-back loop into the LUCID Agent role's conversation mode with the coverage HUD (invariant 11) and mascot beats.

-----

## P-TRAINER scope: ADR-0252..0255 - the LUCID Agent as knowledge trainer / process extractor (docs-only)
- **shipped:** four SCOPE/PLAN ADRs defining the trainer arc: ADR-0252 (the extract -> distill -> verify -> train flywheel, TacticalGenAI's push mechanics run in reverse, eight P-TRAINER increments), ADR-0253 (coverage maps + append-only knowledge units + the additive lkgpack coverage_map member, migration 0012 planned), ADR-0254 (extraction rides invariant 5 + the P4.3 promotion gate verbatim: teach-back confirmation IS the promotion_approve approval; PII redacts before storage, fail-closed), ADR-0255 (scenario-probe interview craft, L0-L3 coverage rubric, and the wealth-management-operations pack outline for boutique VFO firms). Numbered 0252+ because 0250/0251 live on the unmerged feat/lucid-agent-immersive-mascot branch.
- **stubbed:** docs-only session, no code touched, no demo target yet (targets land with P-TRAINER.1); suite not run in this sandbox (no node_modules; nothing executable changed); grounded instead by a full read of master's seams (contracts, promotion_gate, kb/pack/ingest, loop_preflight, trivia banks), the mascot branch's ADR-0251, and the external TacticalGenAI repo.
- **next:** P-TRAINER.1, the pure interview engine core (coverage rubric + session planner + extract-act state machine) against a WMO fixture map.

-----

## Fix: dictation "heard you, but nothing transcribed" - WebM->WAV transcode + silence not mislabeled as a dead server
- **shipped:** the mic showed Local Whisper "running" yet every utterance failed with "No speech-to-text server answered." Root cause (proven live against the running whisper-server on :9111): the composer POSTed MediaRecorder's `audio/webm` (Opus) straight to `/api/transcribe`, but whisper.cpp's `/inference` decodes WAV/PCM only, so it **400s** the WebM -> `WhisperCppSttBackend` throws "unavailable" -> the OpenAI `/v1` fallback 404s on whisper.cpp -> the note carries "unavailable" -> the failure toast. The server was healthy; the payload was undecodable. `desktop/renderer/dictation.ts` now owns three PURE, unit-tested helpers (`downmixMono`, `resampleLinear`, `encodeWavPcm16` + `WHISPER_SAMPLE_RATE`); `app.ts` decodes each clip with the Web Audio API (Chromium decodes its own WebM/Opus), downmixes to mono, resamples to 16 kHz, and re-encodes 16-bit PCM WAV before upload (raw-blob fallback if decode fails, so the ElevenLabs cloud path still works).

## Settings declutter + the LUCID Agent hero button (live-pass polish)
- **shipped:** user call on the settings panel. (1) EVERY section now starts collapsed (`SET_OPEN` is empty) - deep links still expand their own card because openSettingsToVoice/Asksage/LocalProviders add the id before opening. (2) Workspace became a real collapsible card (was raw always-open markup) with the current folder name as its subtitle; its in-place swap + skeleton updated to the self-wrapping form. (3) Profile is collapsible too, and its role row is now FOUR compact chips plus a full-width, centered, neon-edged HERO button for LUCID Agent (icon + label + "immersive stage \u00b7 hands-free" caption, each a single short element - no raw prose in a flex box, invariant #11). (4) The role note shrank to one line. (5) The AskSage gov-gateway card + its monthly-tokens bar moved BELOW the main Providers card and inherit the collapsed default.
- **stubbed:** two edit-tool calls into the settings-shell region were blocked by a content filter mid-session; applied the same changes via scripted file surgery and verified by reading the region back - no behavior difference, just a different tool path.
- **verified:** renderer tsc clean; demo-P-AVATAR.1 green; the LIVE :5320 server serves a bundle containing the hero button + collapsible workspace and CSS containing the hero styles (fresh fetch, build not in error state); suite 2855 pass / 6 fail (same 6 pre-existing).
- **next:** user reload; the 3-model picker remains the sandbox limitation (agent child cannot spawn in-session) - full lists need a dev server launched from the user's own terminal, or the installed app.

-----

## Fix: retina cut the mini ninja's legs off + rail hover-peek + the 3-model mystery solved
- **shipped:** three live-pass findings. (1) MISSING LEGS (real bug): mascot_runner computed poses with CSS-pixel sprite dims but painted at scale*dpr - on retina the sprite was twice the assumed height, so the ground lane put his lower half past the canvas bottom. Fix: `RunnerLayout.scale` is now the single source for geometry AND paint (mount passes `round(RUNNER_SCALE*dpr)`); new retina regression test sweeps scales 2/3/4 asserting feet never pass the canvas bottom. LESSON (added to the mascot-stage skill next session): pinned QA rendered at dpr 1 and could never catch a dpr-2 bug - QA canvases must sweep scales. (2) Rail hover-peek (user ask): in immersive the left rail slides back in when the mouse comes within 20px of the left edge and tucks past 120px (hysteresis, no flicker) - a width-transition push drawer, still a grid item (the display:none hazard stands). (3) Hero mascot shrunk 55% -> 32% of stage height and anchored low - he no longer drapes over the empty-state logo.
- **stubbed:** THE 3-MODEL MYSTERY is environmental, definitively: two dev servers (eval-spawned :5320, bash-spawned :5321) both run the renderer fine but NEITHER can spawn the omp agent child inside this gated session (no `omp acp` process ever appears; the app's own security sandboxing of nested agent spawns - working as designed). The 3 "older" models are bridge.ts's offline FALLBACK_CONFIG placeholder shown while config warms. Full model lists in dev require launching `PORT=<port> bun desktop/dev.ts` from the user's OWN terminal, outside the sandbox. :5321 killed; :5320 stays for UI testing.
- **verified:** 11 runner tests (incl. the new retina sweep) + demo-P-MASCOT.2 green; renderer tsc clean; suite 2855 pass / 6 fail (same 6); the live :5320 serves the fixed bundle + rail-peek CSS on reload.
- **next:** user reloads :5320 (legs, smaller hero, rail peek all land); full-model dev testing from their terminal; then the release cut.

-----

## Fix: immersive mode shattered the .body grid (first live user pass)
- **shipped:** the user's first real click-through caught it - picking the LUCID Agent role collapsed the center column to an `auto`-width sliver on the left, threw Settings into mid-screen, left the 1fr track empty, and stranded clicks until refresh. ROOT CAUSE: `.body` is a 5-track grid whose children include not just rail/sidebar/center/inspector but also the right-edge panel asides (Settings, Skills, ...); my immersive rule used `display:none`, which REMOVES items from grid placement and re-slots every later sibling leftward. FIX: the rails now collapse IN PLACE (`width:0; min-width:0; opacity:0; pointer-events:none; overflow:hidden`, `!important` where resizers set inline widths) so placement never changes; an open Settings panel keeps working in its own track while immersive. demo-P-AVATAR.1 gained a regex GATE (no immersive rule may ever display:none a .body child) and the mascot-stage skill records the hazard.
- **stubbed:** honest lesson attached: the P-AVATAR.1 visual QA mocked the role card + stage layer but never the assembled grid - a mock that skips the real layout can't catch a layout bug. Live click-through remains the only proof for grid work.
- **verified:** the RUNNING :5320 dev server confirmed serving the fixed CSS (fresh fetch, rule present, display:none absent); demo-P-AVATAR.1 green with the new gate; suite 2854 pass / 6 fail (same 6). Screenshot bonus: the enter flow had correctly picked Claude Sonnet 4.6 (the family fallback - no Terra/Sonnet-5 on this account), the Stage chip, mini runner, and hero mascot all rendered.
- **next:** user reloads :5320 and re-tries the role switch; then release-cut housekeeping.

-----

## P-REMOTE.13: the invisible hourly reconnect - ADR-0251's LAST increment; the arc is COMPLETE
- **shipped:** the user-confirmed design: the Cloud Run 60-minute WS cap STAYS (every reconnect re-presents a fresh identity token - ADR-0227's security feature) and becomes invisible instead. Discovery: the hard half already existed - CollabSocket buffers up to 256 outbound frames across a drop, re-auths on every reconnect, and `reconnectNow()` + visibility/online/pageshow nudges were wired; the user-visible drop was pure PRESENTATION (guest.ts flips to "reconnecting" + amber banner instantly). Fix: `RECONNECT_GRACE_MS` (7s) + pure `presentedStatus(view, flapAt, now)` in pwa_view.ts - a transient flap younger than the grace presents as Live (drive/view wording preserved); a drop that outlives it surfaces honestly; terminal states (ended, kicked, fatal notes) are NEVER masked. app.ts tracks `flapAt` per flap and schedules one repaint at grace expiry so a real outage appears without waiting for a frame.
- **stubbed:** desktop-side Share-dock flap presentation untouched (the desktop user watches a full IDE, not a phone banner - polish if it ever annoys); PWA redeploy rides the release cut with P-REMOTE.12.
- **verified:** 5 grace tests (young-flap masking with wording preserved, post-grace honesty, unknown-flap-start honesty, terminal-never-masked, healthy passthrough) + demo-P-REMOTE.13 green; PWA bundle builds; license + all three tsc clean; suite 2854 pass / 6 fail (same 6 pre-existing).
- **next:** ADR-0251 is fully built (role, stage, ninja + parkour + finale, enter flow, voice approvals, boot cinematic, PWA voice, invisible reconnect). Remaining housekeeping for the release cut: PWA redeploy, stale about.test.ts version asserts, _qa scaffolding deletion after the user's look, on-device passes (packaged app + phone).

-----

## P-REMOTE.12: PWA push-to-talk - hold to talk on the phone, the host transcribes
- **shipped:** the user-confirmed push-to-talk design (never a hot mic on mobile). PROTOCOL: `PromptFrame.audio?: PromptAudio {b64, mime}` - additive, old peers unaffected; `validPromptAudio` (shape + audio-mime allowlist + base64 charset + 4MB hard cap) runs FAIL-CLOSED ON BOTH ENDS: the guest drops bad clips before they leave the phone, `CollabHost` re-validates and never trusts the guest. PWA: a hold-to-record mic button (pointer-capture, 350ms/1200-byte tap-and-silence guards, iOS long-press menu suppressed), on release the clip is transcoded ON the phone to 16k mono WAV via the SAME pure dictation helpers as the desktop mic (whisper.cpp cannot decode WebM host-side - no Web Audio in Bun; raw-blob fallback keeps cloud STT working when decode fails), sent audio-only with a local "[voice message]" echo. HOST: extracted `transcribeClip()` in dev.ts - ONE transcription path for /api/transcribe AND guest clips (elevenlabs / whisper.cpp-then-OpenAI-shape with the sttTransportFailed guard) - transcript folds into the guest text and stages through the EXISTING pendingGuestPrompt path, so the fail-closed scan gate sees voice exactly like typing; fire-and-forget so slow STT never blocks the relay pump.
- **stubbed:** the deployed PWA at lucid-agent.web.app needs a redeploy to pick this up (build verified locally; deploy is the release step); a live phone-to-desktop round-trip is the on-device pass.
- **verified:** 11 new adversarial protocol tests (mime smuggling, script-in-b64, oversize, guest-side drop, audio-only frame straight to host) - collab suites 44 pass / 0 fail; demo-P-REMOTE.12 green; PWA bundle builds; license + all three tsc clean; suite 2849 pass / 6 fail (same 6 pre-existing).
- **next:** P-REMOTE.13 (invisible hourly reconnect) - the LAST open increment of ADR-0251; then the release cut (PWA redeploy + installer fold-in + stale about.test.ts asserts + _qa cleanup).

-----

## P-AVATAR.6d: the keyboard comes alive - dark deck, real keycaps, typing lights that die with the cut
- **shipped:** user call: v2 read as a solid white slab. v3 inverts it - DARK deck ('D') with individual grey keycaps ('M') at every even column on two key rows + a grey spacebar, so the caps read as keys instead of the deck reading as a slab (pinned by a never-a-solid-slab regex test). New pure `keyboardLitKeys(tMs, sliced)`: three "fingers" hop neon-green caps across the key rows at a 150ms typing cadence - deterministic, bounded to real cap positions - and return [] the instant the keyboard is SLICED, forever (the gag: it types until the very last moment, then the lights die). `paintFinaleScene` overlays the lit caps only on the whole keyboard; halves are always dark.
- **stubbed:** the halves keep their caps but never glow - a dying flicker on the falling halves would be a cute later beat.
- **verified:** 22 tests (new: cap-row pattern, spacebar, no-solid-slab, lit keys bounded to real caps + moving + dead-after-slice both at t=0 and forever) + demo green; VISUAL: pinned beats show green caps mid-typing on stop/draw/unsheathe and both halves dark after the cut; license + all three tsc clean; suite 2843 pass / 6 fail (same 6 pre-existing).
- **next:** P-REMOTE.12/.13 close the arc; _qa cleanup after the look.

-----

## P-AVATAR.6c: readable keyboard + a real katana (props, not frame pixels)
- **shipped:** user call: the keyboard didn't read and the sword was tiny. Root cause on the sword: a 20-col character grid physically cannot hold a katana - so the blade became PROPS: `BLADE_FLAT` (18-wide, W edge shine over M body, full horizontal extension for the cut) and `BLADE_DIAG` (12x13, built at module load, 2px thick, sloping tip) painted beside the ninja per beat (unsheathe = over the shoulder, cut = across the keyboard, follow-through = low past the hip); slash frames keep only a grip M in hand. Keyboard v2: 24x8 steel deck, TWO visible keycap rows with per-key separations, a wide SPACEBAR, jagged 12-wide halves that sum to the whole. NEW `paintFinaleScene(ctx, t, scale, nx, footY)` exported from boot_cinematic - the live loop and the QA pins share ONE painter, so the pinned evidence can never drift from the product.
- **stubbed:** nothing this pass; the blade angle set is three poses (up/flat/down) - in-between sweep frames would be a P-MASCOT.2 nicety.
- **verified:** 21 tests (new: blade props carry the edge shine + slope, keyboard grid uniformity + keycap rows + spacebar + halves-sum-to-whole) + demo-P-AVATAR.6 green; VISUAL: all five pinned beats captured in one frame - the keyboard instantly reads as a keyboard, the katana is a real sword, the cut lands over two separating halves; license + all three tsc clean; suite 2842 pass / 6 fail (same 6 pre-existing).
- **next:** P-REMOTE.12/.13 to close ADR-0251; _qa cleanup after the user's look.

-----

## P-AVATAR.6b: the boot finale - blade out, keyboard in half (the fist-pump is gone)
- **shipped:** user call: the end-of-boot fist-pump read weird - replaced with the full bit. Four new BASE-diff frames in mascot.ts (`draw` reach-across with the hilt showing, `slashUp` blade diagonal over the shoulder, `slash` full horizontal extension to the grid edge, `slashEnd` follow-through low) + a 16x5 KEYBOARD prop with jagged-cut LEFT/RIGHT halves. `bootFinale(t)` pure timeline in boot_cinematic.ts (FINALE_MS 2050): stop -> draw -> unsheathe -> CUT (the split lands mid-swing at 1090ms) -> follow-through, halves separating linearly and dropping quadratically (gravity). The ready branch paints ninja + keyboard at hand height; `finish` waits for the full finale instead of the old 900ms victory hold. Victory frames stay for the stage mascot's turn-landed pose (unchanged there).
- **stubbed:** the live full-screen capture crops the foot of the frame in the preview panel, so the slice was verified via PINNED pure-timeline panels (same technique as the runner QA) - stop/draw/unsheathe/cut/fallout all render exactly as intended, keyboard visibly in two pieces.
- **verified:** 20 tests across boot+mascot suites (new: beat order, mid-swing split timing, monotonic separation + quadratic drop, steel in every blade frame, full extension on the cut frame) + demo-P-AVATAR.6 extended + green; license + all three tsc clean; suite 2841 pass / 6 fail (same 6 pre-existing).
- **next:** P-REMOTE.12 (PWA push-to-talk) + P-REMOTE.13 (invisible hourly reconnect) close the ADR-0251 arc; _qa scaffolding cleanup after the user's look.

-----

## P-AVATAR.6: the boot cinematic - a hero opening over the real session warm
- **shipped:** `renderer/boot_cinematic.ts`: full-screen opening for the lucid-agent role covering the ~6-9s config warm. PURE half (7 tests): `bootLines` binds four staged rows to REAL signals (gate answered / session config / live model-route count / voice) - never a fake bar; `bootDone` = 2.2s minimum beat + config-gated + 12s HARD CAP (voice/models are reported, never awaited - an unconfigured voice cannot hold boot hostage); `bootRunnerX` sprint choreography (pass + rest + four-beat gait). Shell: classic 2D katakana rain (fillText columns + translucent wash trails, DPR-capped, hidden-pause), the glowing LUCID wordmark, the pixel ninja sprinting the foot of the screen and holding a victory beat at readiness, Esc (capture-phase, so it never also parks the stage) + a Skip button, reduced-motion = static card with live lines and zero animation frames, fade-out handoff. app.ts mounts it once per load inside the settings .then, gated to the role + a bootCine id check; signals read live state (config options / model count / voice loaded).
- **stubbed:** the preview screenshot cache again hid the live frame until a cache-bust; the captured frame (wordmark + all-done lines + rain + the ninja at the foot) is the money shot. Electron's native splash stays untouched (it ends before the renderer exists).
- **verified:** 7 boot tests + demo-P-AVATAR.6 green; VISUAL: `desktop/scripts/_qa_boot.html` loops the real cinematic with staged fake signals - captured mid-run with rain trails, glowing wordmark, four green lines; DOM inspect confirmed lines flip with signals and zero console errors; license + all three tsc clean; suite 2838 pass / 6 fail (same 6 pre-existing). The `_qa_mascot* / _qa_runner* / _qa_boot*` scaffolding awaits one user look, then deletion.
- **next:** that completes the DESKTOP arc of ADR-0251 (role, stage, mascot, enter flow, voice approvals, boot). Remaining: P-REMOTE.12 (PWA push-to-talk) + P-REMOTE.13 (invisible hourly reconnect); then the release-cut fold-in incl. the stale about.test.ts version asserts.

-----

## P-AVATAR.5b: spoken prompts summarize IMPACT, never dump the command; popup grants confirmed intact
- **shipped:** two user calls. (1) CONFIRMED + pinned: the approval POPUP renders every option omp offers - allow once / this turn / session / project / always all stay clickable; only the VOICE path narrows (pickOption is read-only; new test + demo check prove it never filters the card's options). (2) The spoken prompt no longer reads raw commands aloud: new `commandImpact()` heuristic speaks what the command DOES and its realistic blast radius ("force-deletes files or folders, then 2 more steps", "with administrator rights, installs software onto this machine", "downloads a script from the internet and executes it"), first-pipeline-segment classification over rm/curl-pipe-sh/git/installs/make/chmod/kill/docker with an honest generic fallback; trailing steps are COUNTED, not read. `speakableHost()` speaks only the hostname for egress (paths/query are noise). Danger prompts keep the literal-approve demand and now say "the exact command is on screen" - the card (with its TLDR button) remains the full-detail surface.
- **stubbed:** commandImpact is a curated heuristic, not a parser - unknown programs get "runs <prog>"; the TLDR model explanation stays a card-side button (async model calls do not belong in the blocking spoken prompt).
- **verified:** 17 approval tests (impact table incl. sudo-make-install honesty fix caught by its own test, host-only egress, raw-command-never-spoken, options-untouched) + demo-P-AVATAR.5 updated + green; license + both tsc clean; suite 2831 pass / 6 fail (same 6 pre-existing).
- **next:** live hands-free pass; then P-AVATAR.6 (cinematic boot) or P-REMOTE.12/.13.

-----

## P-AVATAR.5: voice tool approval - keyword-strict, fail-closed, the card stays boss
- **shipped:** `renderer/voice_approval.ts` (pure, 13 adversarial tests): `matchApprovalUtterance` accepts only WHOLE short utterances from an allowlist (sentences mentioning "approve" are dictation, \u22644 words, normalized); DANGER class fires only on the literal approve family - "yes" returns `vague-yes` which re-prompts (max 2) and never runs anything; deny phrases work everywhere. `pickOption` maps voice to the NARROWEST grant (once > this-turn > plain allow) and returns null when only widening always/session options exist - a voice approval can never widen standing permissions, and no voice path touches permissionMode. `approvalPrompt` reads the command/host BACK before demanding the word (the user-confirmed repeat-back), `speakableTarget` word-safe truncation. app.ts: `createPermissionCard` now exposes `respond` (the same internal choose path as the buttons, so cleanup/toasts happen once) + an `onAnswered` hook that clears a moot pending voice approval; permission events in hands-free lucid-agent sessions arm `voiceApproval`, speak the prompt, open the mic if conversation isn't listening (streaming blocks maybeListen), and the dictation transcript handler routes ONE matching utterance to the verdict (`consumeApprovalUtterance`) - everything else lands in the composer as before. Silence approves nothing; the backend 300s fail-closed timeout is untouched; the visual card renders always.
- **stubbed:** live mic round-trip (speak "approve", watch the tool run) is the on-device pass; multiple simultaneous permissions keep only the newest voice-armed (cards cover the rest).
- **verified:** 13 voice_approval tests + demo-P-AVATAR.5 green (sentence immunity, danger literalism, widening-grant unreachability, repeat-back prompt); one tsc miss (return object without `respond`) caught by the gate and fixed; license + all three tsc clean; suite 2827 pass / 6 fail (same 6 pre-existing).
- **next:** live in-app pass of the full hands-free loop (enter role -> gap card -> approve by voice); then P-AVATAR.6 (cinematic boot) or the P-REMOTE.12/.13 PWA pair.

-----

## P-AVATAR.4: the enter flow - picking the role lands you in a working hands-free session
- **shipped:** `renderer/agent_flow.ts` (pure, 11 tests): `CONVERSATION_MODEL_PREFS` in the user's stated order (GPT-5.6 Terra > Claude Sonnet 5 incl. the gov id > Gemini 3.5/3.6 Flash > family fallbacks) with `resolveConversationModel` that never switches pointlessly; `readinessChecklist`/`nextGap` surfacing ONE gap at a time in fix order (provider -> voice out -> voice in) with the Knowledge Graph as a NON-required offer that fires at most once per session (`nextGap(_, vaultAskedAlready)`); `restoreModel` for exit. app.ts: entering the lucid-agent role captures `AgentPrior` (model/uiMode/autoSpeak/conversation), flips `applyConfig("mode","agent")`, switches to the fast model, then a 4s self-healing readiness tick (bridge.auth + voices + whisperStatus + personal) that renders the one-gap card (`.agent-setup-card`, invariant-#11-safe block prose), SPEAKS a new gap when TTS already works, and arms conversation mode (`applyVoicePatch({ttsAutoSpeak,ttsConversation})` + "You're in" toast) the moment the required set is green. Gap actions park the stage first (the fix surfaces live in hidden rails), then deep-link: openProviderHub / openSettingsToVoice / openKnowledge. Exit restores model (only if still accessible), uiMode, and unwinds conversation only if the flow turned it on.
- **stubbed:** the enter flow is proven at the pure layer + seam-wired; a live in-app pass (pick role -> hear "You're in" -> talk) needs the running desktop with a configured voice, which is the user's next look. Voice tool APPROVAL (speak to approve) is P-AVATAR.5, untouched. Whisper autostart in installed builds already covers the common STT gap (P-STT.6).
- **verified:** 11 agent_flow tests + demo-P-AVATAR.4 green (preference order incl. gov Sonnet-5 id, no pointless switch, gap ordering, one-time vault offer, exit restoration); license + all three tsc clean; suite 2814 pass / 6 fail (same 6 pre-existing). NOTE: fumbled a comment-header edit into transient syntax garbage mid-session - caught by re-read, fixed to a plain ASCII header before any gate ran; box-glyph headers via edit-tool JSON are a repeat hazard, plain ASCII from now on.
- **next:** live in-app pass of the enter flow; then P-AVATAR.5 (voice tool approval, keyword-strict + repeat-back) - the last piece before the boot cinematic.

-----

## P-MASCOT.3: the parkour goes smooth (more frames + rAF + easing)
- **shipped:** the user asked for smoother motion, frames permitting - both delivered. FIVE new BASE-diff frames: runC/runD complete the classic FOUR-BEAT gait (contact-pass-contact-pass @95ms, ~10.5 steps/s), hangB gives the climb an alternating-grip wiggle, `mantle` sells the pull-over at the top edge, `land` adds an impact squash. Timeline upgrades: smoothstep ease-in-out on the climb (pulls, not an elevator), new `mantle` and `land` phases (route is now run -> climb -> mantle -> sneak -> pause -> drop -> land -> rest), sneak got a 4-beat creep. Painter swapped from a 50ms interval (~5px hops) to requestAnimationFrame at device-pixel positions with integer sprite scale - glide without losing pixel-art crispness; pose-key skip keeps rest cheap. `RUNNER_TIMINGS` exported so tests/demos/QA never hardcode phase offsets again.
- **stubbed:** the stage mascot's beat loops (kata etc.) still swap at their original cadence - locomotion was the ask; richer kata in-betweens are a future nicety.
- **verified:** 20 tests (new: four distinct gait frames in-cycle, climb easing curve, mantle on-edge geometry, visible landing squash) + demo-P-MASCOT.1/.2 green; QA page rebuilt with mantle/land pins - the LIVE capture caught the head-peek over the bar edge mid-climb, exactly the intended charm beat; license + all three tsc clean; suite 2803 pass / 6 fail (same 6 pre-existing).
- **next:** user look; then P-AVATAR.4 (enter flow + guided setup). The `_qa_mascot*` + `_qa_runner*` scaffolding still awaits deletion after the look.

-----

## P-MASCOT.2: three.js confirmed fully out + the prompt-bar parkour mini ninja
- **shipped:** (1) TRIM CONFIRMED: zero `three` traces anywhere (deps, lockfile, imports) - removal completed last increment, verified this one; dev.ts splitting comment updated to reflect the retained-capability status. (2) The mini ninja: `renderer/mascot_runner.ts` - a PURE parkour timeline (`runnerAt`/`runnerCycle`) over the composer box: he runs the FOOT of the prompt bar, slips up BEHIND it (clip contract hides him crossing the bar band - cartoon-logic "behind" with zero z-order fragility), pops over the top edge, sneaks along it, pauses, and SILENTLY drops back down, rests, then runs the route mirrored the other way (`mirrorFrame` involution). Six new BASE-diff frames (runA/B stride, hang, sneakA/B, fall); `paintRows` split out of `paintFrame` for mirrored painting; mounted whenever the lucid-agent role is active (immersive OR parked; `pointer-events:none`, under the z-30 slash autocomplete; `.composer-wrap` was already position:relative).
- **stubbed:** the climb shows only a brief head-peek sliver before he vanishes (more drama = polish); the live-bar screenshot timing is luck (pinned pure-timeline strip is the deterministic visual evidence); P-MASCOT.3 candidates: peek timing, occasional wave at the user, working-state tie-in (he only parkours when otherwise idle?).
- **verified:** 7 new runner tests (route order, lane geometry, clip contract, gravity easing, direction alternation, mirror involution) + 10 mascot tests; demo-P-MASCOT.1 + new demo-P-MASCOT.2 green; QA page `desktop/scripts/_qa_runner.html` (live fake bar + 5 pinned phase canvases; run/sneak read charming, climb pin proves the vanish); license + all three tsc clean; suite 2800 pass / 6 fail (same 6 pre-existing).
- **next:** user look; then P-AVATAR.4 (enter flow + guided setup) or P-MASCOT.3 polish. Delete the `_qa_mascot*` + `_qa_runner*` scaffolding after the look.

-----

## P-MASCOT.1: direction pivot - the talking head + particles are OUT, LUCID the ninja is IN
- **shipped:** user killed the face direction outright ("still very bad") and re-briefed: a fun, cool, original fighting-game NINJA mascot that plays while the agent works and follows real session state. Built `renderer/mascot.ts`: 15 hand-authored 20x26 pixel frames as code (chibi ninja, navy gi, neon #41ff8b headband/belt; all variants are row-diffs off one BASE so he stays on-model), pure `stepMascot` machine (victory pose when work LANDS, uninterruptible + finishing; speaking > listening > working > idle), `workActivity` rotation (kata combo -> shuriken practice -> meditation every ~6.5s), beat-timeline `mascotFrame`, DPR-crisp nearest-neighbor `mountMascot` (bottom-center hero scale, hidden-pause). app.ts cut over (static import - 5KB, no chunk needed); tour step re-themed "The dojo". CLEAN CUTOVER: three.js + @types removed from root deps, avatar_stage/avatar_math/tests/demo/QA files deleted, Makefile target replaced (demo-P-MASCOT.1), dev.ts splitting KEPT as a documented generic capability, `threejs-stage` skill retired and replaced by first-party `mascot-stage` (art direction incl. the no-faces rule, frame conventions, preview QA workflow); SOURCES.md updated. ADR-0251 carries the pivot section (P-MASCOT.1 replaces .2a/.2b/.3; lip sync dead - speaking is a talk-gesture loop).
- **stubbed:** victory pose is not visually captured in QA (timing-dependent; proven by tests); P-MASCOT.2 gets more activities, an optional audio-reactive bounce, and polish; the working panel screenshot caught a between-strikes beat (punch/kick cycling proven by tests).
- **verified:** VISUAL QA in the preview - 4-panel page (`desktop/scripts/_qa_mascot.html`, delete _qa trio after a look): the ninja reads instantly as a cute retro game character in all four states, zero uncanny anything. 10 mascot tests + demo-P-MASCOT.1 + demo-P-AVATAR.1 green; license clean; all three tsc clean; suite 2793 pass / 6 fail (the same 6 pre-existing; 16 avatar tests out, 10 mascot tests in).
- **next:** user verdict on the ninja; then P-AVATAR.4 (enter flow + guided setup) with the mascot as the stage's resident, or P-MASCOT.2 polish.

-----

## P-AVATAR.2a-fix: the face goes SERENE (user: v1 was scary) + the threejs-stage skill
- **shipped:** art-direction revision driven by user feedback and RESEARCH, not another blind sculpt. Consulted skills.sh (cloudai-x threejs-shaders + threejs-postprocessing SKILL.md, emalorenzo three-best-practices) and particle-portrait prior art (Codrops interactive-particles; ektogamat/ParticleProjector GLTF-to-particles). Rebuilt `headPoints`: CLOSED eyes as downward lash arcs (no eye contact - THE uncanny trigger; iris rings deleted), calm mouth with lifted corners via a curved-band helper, soft brows + faint cheek warmth, quadratic jaw taper (no egg point), smooth glow fields over speckle. Stage: per-point sphere normals + inverted-fresnel VIEW FADE dissolve the hard silhouette rim (the threejs-shaders fresnel pattern), mint-white lift on feature glow. New FIRST-PARTY `.agents/skills/threejs-stage` skill encodes the architecture contract, additive-points pitfalls, the settled art rules, and the preview QA workflow; SOURCES.md row added - the strict pull rule honored, nothing third-party vendored.
- **stubbed:** slight arc asymmetry from sampling noise (2b polish can mirror-sample); selective bloom still parked for 2b with the recipe now in the skill.
- **verified:** three visual QA rounds in the preview (ghostly -> balanced): settled face reads calm - closed lashes, brows, nose hint, soft smile, dissolving edge, glyph rain intact. 16 avatar tests, demo-P-AVATAR.2a (incl. lazy-chunk proof), license, both tsc clean; suite 2799 pass / 6 fail (the same 6 pre-existing).
- **next:** user look at the QA page; then P-AVATAR.3 (lip sync off the voice_eq analyser tap) with 2b polish trailing.

-----

## P-AVATAR.2a: the hero-scale three.js face stage (ADR-0251, increment 2 of 9)
- **shipped:** the neon LUCID face is real. `renderer/avatar_math.ts` (PURE, 16 tests): mode machine (materialize -> speaking > listening > thinking > idle), fps degrade ladder with hysteresis + TERMINAL minimal, per-rung particle budgets, deterministic mulberry32 head cloud (parametric skull + brow/nose/lips/sockets as radial displacements, ~2% of points RE-TARGETED onto two glowing iris rings - random sampling alone leaves the eyes invisible, verified visually), glyph-rain field seeds, MODE_LOOK table. `renderer/avatar_stage.ts` (the ONLY three.js importer): procedural katakana+digit CanvasTexture atlas, deep rain field + face-front curtain (GPU-side fall via uTime, zero per-frame CPU), 24k-point additive face with scatter->target materialize (feature-staggered), per-mode choreography, DPR cap 1.75, hidden-pause, drawRange degrade, dispose. `three@0.182` at repo ROOT (the katex packaged-build precedent). dev.ts `Bun.build splitting:true` + in-memory `/chunk-*.js` route; app.ts mounts via dynamic import on entering immersive (justified split boundary) + a 250ms poll mapping #ctSpeak/dictation/state.streaming onto the stage. `mountAvatarStage` takes `materializeMs` (0 = settled; the P-AVATAR.6 handoff + QA use it).
- **stubbed:** post stack (bloom/grain/aberration), audio-reactive coupling, and choreography tuning are P-AVATAR.2b by design; uLevel breathes on time until P-AVATAR.3 wires the analyser; the 60fps gate is proven by mechanism (ladder tests + budgets), not on a live integrated-GPU pass.
- **verified:** demo-P-AVATAR.2a green incl. THE LAZY-CHUNK PROOF (real split build: three.js REVISION in a chunk, absent from /app.js entry); 16 avatar tests; harness suite 2799 pass / 6 fail (same 6 pre-existing); all three tsc clean; license clean. VISUAL: iterated live in the preview (blowout -> egg -> featureless -> FACE): settled stage shows head silhouette, brow, nose column, lips band, two glowing iris rings, full-stage glyph rain (QA page `desktop/scripts/_qa_avatar2a.html`, delete the _qa trio after a look). Screenshot caveat learned: preview captures are cached per page-load - reload-then-inspect, and use materializeMs:0 for frame-one captures.
- **next:** P-AVATAR.2b (cinematic polish: post stack, idle drift, audio-reactive rain) or jump to .3 (lip sync off the voice_eq analyser tap) - .2b can trail.

-----

## P-AVATAR.1: the "LUCID Agent" role + the immersive stage (ADR-0251, increment 1 of 9)
- **shipped:** `lucid-agent` joins the closed role set in both mirrors (settings_store + tour list) with fail-safe normalization intact; full-width neon hero card in the role picker (real `.role-card` idiom, `data-role` styled); animated `ri-agt` glyph (head + 3 glyph-rain streams, reduced-motion safe); `#app-inner.immersive` hides BOTH rails + the inspector while `#agentStage` (neon ambient glow layer, non-interactive, behind the thread) lights up; Esc parks the stage (overlay-aware: modals/tour/palette own Esc first, `defaultPrevented` respected), the titlebar Stage chip re-enters; role select, Settings role switch, and boot all sync via `syncImmersiveWithRole`; bespoke 4-step tour (composer, model, stage, closer) that never targets a hidden rail. User locked the remaining ADR-0251 calls: repeat-back voice approvals, PWA push-to-talk, invisible hourly reconnect.
- **stubbed:** the stage is ambience only until P-AVATAR.2a mounts the three.js face; entering the role does not yet flip conversation/agent mode or the fast model (that is .4 by design). Visual QA mock at `desktop/scripts/_qa_avatar1_mock.html` (+ `_qa_avatar1.css`, product CSS copied verbatim) - VERIFIED rendered in the preview panel: 2x2 grid of standard cards (234px, no wrapping) + the full-width (481px) neon lucid-agent hero card + the stage glow layer. Note for future mocks: the preview only auto-opens on a DIRECT write-tool call, and a ~300KB single-file mock did not render - link a sibling .css (the preview folds relative assets inline, P-PREVIEW.4c). Delete both _qa files after the user's look.
- **verified:** new `demo-P-AVATAR.1` green (role set mirrors, no-rail tour, glyph, CSS contract incl. reduced-motion); demo-P-ROLE.1/.1b updated + green; tour/role_icons contract tests updated to the 5-role set; harness suite (release-copies excluded, as `make test-harness` runs it) 2783 pass / 6 fail - the same 6 pre-existing (2 about.test.ts version, 4 symbol_graph); renderer + server tsc show no NEW diagnostics; license check clean.
- **next:** P-AVATAR.2a - the hero-scale three.js face stage (lazy chunk, procedural head, rain field, entry convergence, perf gate).

-----

## ADR-0251: SCOPE/PLAN for P-AVATAR - the "LUCID Agent" immersive role (three.js talking face, voice approvals, cinematic boot, PWA voice)
- **shipped:** the plan only (roadmap ADR, no feature code): 8 increments - P-AVATAR.1 role+immersive layout, .2 procedural neon face stage (three.js, lazy chunk, CSP-verified), .3 amplitude lip sync off the EXISTING voice_eq AnalyserNode (tap API; one MediaElementSource per element is the keystone risk), .4 enter flow + guided setup/KG-unlock using the existing readiness signals, .5 fail-closed voice tool approval (keyword-strict, repeat-back for exec/egress, silence never approves), .6 cinematic boot over the real 6-9s config warm, P-REMOTE.12 PWA push-to-talk via a PromptFrame.audio field + host /api/transcribe, P-REMOTE.13 invisible hourly relay reconnect (the 60-min cap is Cloud Run's and stays - it backs the hourly re-verify).
- **stubbed:** everything - this session produced the verified seam map (5 parallel scouts over roles/rails, voice arc, approvals, PWA, boot) and the ADR; zero increments built.
- **next:** P-AVATAR.1 (role + immersive layout) after the user settles the four open calls in the ADR summary (art direction, voice-approval scope for danger class, PWA push-to-talk vs VAD, accept invisible-reconnect).

-----

## P-MODEL.1: the model picker defaults to the user's model, not Claude 4.8 Opus (ADR-0250)
- **shipped:** fresh sessions no longer open on omp's hardcoded Opus 4.8 default. New pure resolver `desktop/startup_model.ts`: (1) the LAST-USED model when still offered + its provider still credentialed, (2) else the BEST model among CONFIGURED providers (flagship tier first, direct over gov gateway, newest version; never auxiliary/RAG/deprecated/China-gated), (3) else omp's default stands. `ensureSession` captures `lastModel()` BEFORE session/new (syncModelEnv was stamping the default over it) and applies the pick fire-and-forget via set_config_option (ADR-0217 pattern); AskSage lockdown wins unchanged. `budget_gate.ts` went bridge-free (structural `ProviderAuthLike`, generic-preserving) so the server can import providerForModel without DOM types.
- **stubbed:** applied once per fresh session with no retry if omp reports an empty option list at init (worst case = today's behavior for that session); a live packaged-app launch pass rides the next release cut.
- **verified:** 13 new resolver tests + budget_gate/provider_hub/checker_model/model_families suites 141 pass / 0 fail; all three tsconfigs show no NEW diagnostics; full `bun test desktop` 4541 pass / 6 fail, the same 6 pre-existing (4 symbol_graph + 2 stale about.test.ts version asserts).
- **next:** fold into the pending release cut; consider a status-poll retry if omp ever reports options late.

-----

## P-STT.6: tiny is the default Whisper model + the installed app autostarts it
- **shipped:** two defaults that make packaged-app dictation zero-touch. (1) `DEFAULT_WHISPER_TIER = "tiny"` (`whisper_install.ts`): every no-pick path (`planWhisperInstall`, `installWhisper`/`startWhisper` with no tier, the mic quick-start, the Voice-card picker preselect via a new `defaultTier` on `WhisperStatusView`) now lands on tiny (~78MB, fastest load) instead of the hardware recommendation; the recommendation stays informational in the summary and is still honored when picked explicitly. (2) autostart in the INSTALLED app: dev.ts fires `startWhisper` at boot when `LUCID_RESOURCES` is set (main.ts threads it only when `app.isPackaged`, and that build bundles whisper-server), gated by the new pure `shouldAutostartWhisper` - never in a dev run, never over a non-whisper STT engine, never clobbering a user's REMOTE sttUrl (loopback/unset only), and never when incapable, binary-less, running, or mid-install; fire-and-forget so the HTTP server never waits on the first-launch model download.
- **stubbed:** the autostart is proven through the unit-tested gate + the existing injected-deps `startWhisper` suite, not a packaged-installer relaunch on this machine; the on-device pass (install a cut build, launch, mic works with zero Settings visits) rides the next release cut. No user-facing off switch for autostart beyond switching the STT engine or pointing sttUrl at a remote server.
- **verified:** whisper suites **110 pass / 0 fail** (new: default-tier plan/install, gate truth table incl. remote-url no-clobber + already-running skip); `demo-P-STT.2` extended to 19 checks, all green; desktop + server tsc show no NEW diagnostics (only the pre-existing `symbol_graph` TS7 set and the pre-existing `dev.ts` `finish(m[1])` TS2345); full `bun test desktop` 4528 pass / 6 fail, all 6 pre-existing (4 symbol_graph + 2 `about.test.ts` still asserting v1.11.12 against the already-bumped 1.12.0).
- **next:** fix the stale `about.test.ts` version assertions during the pending release cut; on-device packaged-app pass (fresh install -> mic dictates with no setup).

-----

## Fix: dictation "heard you, but nothing transcribed" - WebM->WAV transcode + silence not mislabeled as a dead server
- **shipped:** the mic showed Local Whisper "running" yet every utterance failed with "No speech-to-text server answered." Root cause (proven live against the running whisper-server on :9111): the composer POSTed MediaRecorder's `audio/webm` (Opus) straight to `/api/transcribe`, but whisper.cpp's `/inference` decodes WAV/PCM only, so it **400s** the WebM -> `WhisperCppSttBackend` throws "unavailable" -> the OpenAI `/v1` fallback 404s on whisper.cpp -> the note carries "unavailable" -> the failure toast. The server was healthy; the payload was undecodable. `desktop/renderer/dictation.ts` now owns three PURE, unit-tested helpers (`downmixMono`, `resampleLinear`, `encodeWavPcm16` + `WHISPER_SAMPLE_RATE`); `app.ts` decodes each clip with the Web Audio API (Chromium decodes its own WebM/Opus), downmixes to mono, resamples to 16 kHz, and re-encodes 16-bit PCM WAV before upload (raw-blob fallback if decode fails, so the ElevenLabs cloud path still works).
- **shipped (#2):** a SECOND latent cause of the same misleading toast: `/api/transcribe` fell back to the OpenAI `/v1` shape whenever `!r.text`, so a HEALTHY whisper.cpp that simply heard silence got probed on a path it 404s and was mislabeled "no STT server answered." Added `sttTransportFailed(r)` to `harness/voice/transcription.ts` (empty text AND an "unavailable" note = the server was unreachable, vs. an empty-but-successful silence response) and gated the fallback on it, so silence from a working server stays silent.
- **shipped (#3):** on-device dictation surfaced a THIRD issue: whisper transcribes a non-speech segment to a placeholder token (a silent tail clip came back as the literal text `[BLANK_AUDIO]` and merged into the composer). Added `stripNonSpeech()` (curated whisper non-speech set: `[BLANK_AUDIO]`, `[SILENCE]`, `(soft music)`, music-note glyphs; a real bracketed token like `a[0]` survives) applied in all three STT backends (whisper.cpp / OpenAI-compatible / ElevenLabs). A pure-silence clip now yields empty text -> merges nothing, no toast. Verified LIVE against the running server: a 1s silent WAV returns `" [BLANK_AUDIO]\n"` from whisper -> `/api/transcribe` returns `text: ""`; speech still returns the transcript.
- **verified LIVE on this Mac (:9111, small model):** raw non-WAV container -> **HTTP 400**; my `encodeWavPcm16` bytes -> **HTTP 200** "The quick brown fox jumps over the lazy dog."; the full `resampleLinear` 16k->48k->16k + `downmixMono` round-trip -> **HTTP 200** same transcript. Unit tests: dictation + transcription suites **60 pass / 0 fail** (WAV header+clamp+LE samples, resample identity/decimation/interpolation, downmix averaging, `sttTransportFailed` truth table). Renderer + root tsc clean (no new errors); server tsc shows only the pre-existing `symbol_graph` TS7 + the pre-existing `dev.ts` URL-scan `finish(m[1])` TS2345.
- **stubbed:** the `OfflineAudioContext.decodeAudioData` step is standard Chromium and runs only in the renderer (no Web Audio in Bun), so it's covered by the everything-downstream-of-decode live proof, not a unit test; an on-device mic click-through is the final confirmation. WAV is ~32 KB/s vs Opus, fine for short localhost utterances.
- **next:** on-device mic pass (speak, confirm words land, no false "nothing transcribed" on silence); fold into the pending version bump / release cut.

-----

## P-STT.2d: download progress stream (#2) + CI matrix wiring for the bundled installers (#1)
- **shipped (#2):** the no-code **Install & start** button now shows a REAL progress bar, not a blind spinner. `whisper_runtime.ts` tracks module-scoped `installState {active, tier, fraction, phase: idle|downloading|starting|done|error}`, surfaced in `whisperStatus()`; `startWhisper`/`installWhisper` drive it (download `onProgress` -> fraction; then `starting`; then `done`/`error`). The renderer polls `/api/whisper/status` every 500ms while the (long) start request is in flight and paints the bar ("Downloading NN%" -> "Starting the server...").
- **shipped (#1):** installers are cut by the EXISTING `build-desktop.yml` matrix (macOS/Windows/Ubuntu runners run `dist:{mac,win,linux}`), which already runs `bun run whisper` (wired in P-STT.2c). Added a mac **cmake-ensure** step (the from-source build) and a **fail-closed whisper check** in `airgap-smoke.ts` so a build that didn't bundle `whisper-server` + its libs fails BEFORE shipping.
- **verified LIVE on this Mac:** drove the real runtime through a fresh **74 MB** tiny-model download and watched `installState` advance `downloading 0->7->21->35->52->67->82->96%` -> `starting:100%` -> `done`, then the bundled binary transcribed `/tmp/spk.wav` correctly. Plus **48 whisper tests** (4 new progress-state cases); renderer + server tsc clean (no new errors); full desktop **1778 pass / 4 fail** (pre-existing `symbol_graph` TS7); license clean.
- **stubbed:** progress is a 500ms status-poll (fine for a bar), not an SSE push; cancel-mid-download isn't wired (Stop after). The CI matrix change is authored + the whisper gate typechecks, but the actual 3-OS installer cut runs on the runners - I can't spawn a Windows/Linux runner here (mac bundling was proven in P-STT.2c).
- **DONE (test build cut):** branch `test/bundled-installers` (`dcee0b3`+), run **30206895668** = **all 3 OSes green**, `publish-latest` **skipped**, releases untouched (v1.11.11 still Latest). Air-gap gate proved bundling on every runner: macOS `whisper-server + 18 libs` (from-source, universal, Metal), Windows `+ 12 libs`, Linux `+ 23 libs` (both pinned + SHA-verified). Two CI bugs caught before they shipped: (1) a plain `workflow_dispatch` used to publish the rolling `latest` + the auto-update feed, i.e. it pushed at EXISTING users, now double-gated on an opt-in toggle AND `refs/heads/master`; (2) the test version stamp was computed off `package.json` (1.11.10) while v1.11.11 was live, so an installed test build would auto-update itself back to the release mid-test - now `max(package.json, newest published) + patch` -> `1.11.12-test.<run#>` (semver-verified above the release, below a future real 1.11.12). Also fixed an argv bug found by EXECUTING the extracted stamp script (`node -e` eats a positional -> `42.undefined.1-test.undefined`, which electron-builder rejects; values now pass via env).
- **next:** on-device pass of each installer (Voice card should read "whisper-server (bundled)"); Linux is the untested-by-me one. Then version bump + release cut folding P-PROV.2 / P-LOCAL.4 / Opus 5 readiness / TLDR+exec / dictation / bundled Whisper.

-----

## P-STT.2c: bundle the whisper.cpp binary in the installer (zero-prereq) - all 3 OSes
- **shipped:** the packaging step that makes offline Whisper truly one-click, no prereqs. `whisper_binaries.ts` (pure, tested) PINS which server binary per OS/arch: **Windows + Linux** use the official ggml-org **v1.9.1** release archives, each committed **SHA-256 cross-checked against the GitHub release API digest**; **macOS** has no prebuilt server (xcframework only) so it **builds from source** (universal arm64+x86_64, Metal). `build/fetch-whisper.ts` stages the CURRENT platform (mirrors `fetch-runtimes.ts`: fail-closed size+SHA verify, a keep-filter that extracts ONLY `whisper-server` + the whisper/ggml libs - dropping parakeet/tests/SDL2/other CLIs -> ~4-10 MB), preserving symlinks + the exec bit; on macOS it forces `@loader_path` and STRIPS the absolute build rpath so the co-located dylibs load on the user's machine. `extraResources` maps `whisper/ -> <resources>/whisper/`; `dist:{mac,win,linux}` run `bun run whisper` after `runtimes:*`; `main.ts` threads `LUCID_RESOURCES` so the dev child resolves the bundle (`resolveWhisperBin` already prefers env -> bundled -> PATH). Staged binaries are gitignored (like `runtimes/`).
- **verified LIVE on this Mac:** ran the staging script (reused checkout) -> it stages `whisper-server` + the 6 dylib triplets, `resolveWhisperBin` returns `source:"bundled"`, the relocated binary carries **only `@loader_path`** (dead abs rpath stripped), and it transcribed `/tmp/spk.wav` -> "The quick brown fox jumps over the lazy dog" loading its co-located dylibs. Separately proved the **Windows + Linux archives by REAL download**: SHA-256 == the pinned hashes, and each contains `whisper-server[.exe]` + the whisper/ggml libs (member layout confirmed). Renderer + server tsc clean (no new errors); **44 whisper tests** (11 new); full desktop **1774 pass / 4 fail** (pre-existing `symbol_graph` TS7); license clean.
- **stubbed:** I cannot RUN the Win/Linux binaries on this Mac (no cross-exec) - their download+verify+layout is proven, but end-to-end transcription on those OSes is the per-platform CI / on-device pass. macOS from-source needs `cmake` on the build host (fail-closed with guidance if absent; `LUCID_WHISPER_SRC` reuses a checkout). Installer delta ~4 MB (mac/linux) to ~8 MB (win); the model is still downloaded on demand.
- **next:** run `dist:{mac,win,linux}` on their runners (or a CI matrix) to cut the bundled installers; version bump + release.

-----

## P-STT.2b: no-code managed offline Whisper - LIVE-VERIFIED on real hardware
- **shipped:** the one-click offline-STT lifecycle on top of P-STT.2a. `whisper_runtime.ts` (install -> start -> wire STT -> stop, ALL IO injected, unit-tested); dev.ts supplies the real deps (os specs, model dir `~/.omp/whisper`, the streamed downloader + integrity gate, `Bun.spawn`, health-poll, `setVoiceSettings`) behind `/api/whisper/{status,install,start,stop}`; bridge methods; and a no-code **Local Whisper** block in the Voice card (hardware readout + capable-tier picker + one **Install & start** / **Stop** button). **Found + fixed a real integration bug by executing live**: whisper.cpp's `whisper-server` serves its native `/inference` route, NOT the OpenAI `/v1/audio/transcriptions` the existing seam posted to (that 404s) -> added `WhisperCppSttBackend` (posts to `/inference`) and made `/api/transcribe` try `/inference` first, then fall back to OpenAI-shape (so whisper.cpp AND faster-whisper both work). Also widened the health-wait to ~30s after a real cold boot took **11.3s** (the old 5s would have failed).
- **verified LIVE on this Mac (not just unit tests):** fetched a prebuilt cmake, **built whisper.cpp from source** (whisper-cli + whisper-server, ~40s), downloaded the ggml model from Hugging Face (74MB, integrity-checked), synthesized speech with macOS `say`, and transcribed it end-to-end -> "The quick brown fox jumps over the lazy dog" via whisper-cli AND via the shipped `WhisperCppSttBackend` AND via the full `whisper_runtime` (real spawn -> health -> STT wired -> transcript -> clean stop). Plus: 41 whisper/transcription unit tests + `demo-P-STT.2`; renderer tsc clean; server tsc back to baseline (only the pre-existing symbol_graph TS7 + dev.ts OAuth-scan); full desktop 1763 pass / 4 fail (pre-existing).
- **stubbed:** BUNDLING the whisper.cpp binary in the installer (electron-builder extraResources) is the remaining packaging step for a truly zero-prereq install; until then the manager resolves `LUCID_WHISPER_BIN` / a binary on PATH (a built one is at `/tmp/whisper.cpp/build/bin/whisper-server` on this machine right now). Install/start is a single long-lived request (no progress stream yet - a 1.5GB tier shows a spinner). The HTTP round-trip through the running dev server + the in-app button click is the last on-device app pass; the runtime beneath it is live-verified.
- **next:** bundle the binary in the installer; a download progress stream for large tiers; in-app click-through.

-----

## P-STT.3: fluid, native-feeling live dictation (replaces the batch mic)
- **shipped:** the mic was batch (record the whole clip -> one transcribe -> dump the text at the end). It's now LIVE, hands-free dictation: you speak, pause, and each utterance lands in the composer while you keep going, and it auto-stops after a longer silence. Pure `desktop/renderer/dictation.ts` owns the timing brain - voice-activity thresholds, when to flush an utterance, when to auto-stop, and the transcript merge - fully unit-tested (a fresh session waits indefinitely for your first word; a stray blip below min-speech never flushes; a long monologue still streams in via a mid-segment flush). `app.ts` drives it: an AudioContext AnalyserNode RMS feeds `dictationTick` every 100ms; on `flush` the per-utterance MediaRecorder stops -> transcribes -> `mergeTranscript` appends -> a fresh recorder starts immediately (no dropped words); on `stop` the session ends. Reuses the existing `/api/transcribe` (offline Whisper or ElevenLabs, per Settings), so it rides straight on P-STT.2a's local Whisper. Mic tooltip updated to describe hands-free dictation.
- **stubbed:** the mic CAPTURE is renderer DOM and needs an on-device mic pass (no microphone in this sandbox); the pure timing machine (8 tests) + the wiring typecheck are the verification here. True mid-utterance streaming partials aren't attempted (per-utterance is the fluid unit, and webm chunks aren't independently decodable); push-to-talk (hold-the-hotkey) not added - the flow is click-to-start + auto-stop.
- **next:** on-device mic pass (confirm live utterances land + auto-stop feels right, tune `DICTATION_DEFAULTS`); optional push-to-talk; P-STT.2b (managed Whisper install/UI/binary-bundle) so the offline engine is one click.

-----

## P-STT.2a: guided on-device Whisper - the verified core (capability gate + install/serve plan + download)
- **shipped:** the decision + download engine for no-code local speech-to-text, all pure/injected + tested (no new Python - whisper.cpp is C++, invariant #2). `whisper_capability.ts`: the HARDWARE GATE - install/run only where RAM fits, recommending the largest COMFORTABLE tier (8GB laptop -> `small`; M3 Ultra 512GB -> `large-turbo`; 1GB -> refused fail-closed). `whisper_install.ts`: the ggml model catalog (tiny / **base = the "mini" default** / small / medium / large-v3-turbo, official Hugging Face weights) + `planWhisperInstall` + `whisperServeUrl`/`whisperServerArgs` + a fail-closed integrity gate (rejects an HTML error page saved as `.bin` or a truncated file). `whisper_manager.ts`: `resolveWhisperBin` (env -> bundled -> PATH) + `downloadWhisperModel` (streamed progress + integrity + atomic `.part` rename), all with INJECTED fetch/fs so the logic is unit-tested. It targets the EXISTING `OpenAiCompatibleSttBackend`: the managed server's loopback URL becomes `sttUrl`, and `/api/transcribe` already routes to it - so `sttProvider="whisper"` transcribes against the local server with zero new trust surface. 27 tests across 4 modules + `demo-P-STT.2` (14 checks).
- **stubbed:** P-STT.2b (the live glue) is NOT built and needs a real machine + a packaging decision: (a) `/api/whisper/{status,install,start,stop}` endpoints + the no-code Voice-card UI (hardware readout, capable-tier picker, Install/Start/Stop, progress); (b) the real spawn + health-check of `whisper-server`; (c) BUNDLING the whisper.cpp binary in the installer (electron-builder extraResources) for the zero-prereq experience - until then the manager uses a binary on PATH / `LUCID_WHISPER_BIN`. Also: confirm whisper.cpp's server exposes the OpenAI `/v1/audio/transcriptions` route (vs its native `/inference`); if `/inference`, add a whisper.cpp-native STT backend variant. None of these can be exercised in this sandbox (no network/binary), so they are honestly deferred rather than shipped unverified.
- **next:** P-STT.2b on a real machine (endpoints + UI + spawn + binary bundle); or the version bump + release cut folding P-PROV.2 / P-LOCAL.4 / Opus 5 readiness / TLDR+exec fixes / P-STT.2a.

-----

## Fix: TLDR works for OAuth users + clearer exec-approval labels
- **shipped:** the exec card's **TLDR** button dead-ended OAuth-only users - `explain_command.ts` only read direct API keys (`ANTHROPIC/OPENAI/GEMINI_API_KEY`) and, finding none, told them to "add a key" even with Anthropic/OpenAI/Gemini OAuth connected (ADR-0119 deliberately keyed-only). Now `/api/explain` (dev.ts) FALLS BACK to `backend.complete()` - the omp util completion that uses whatever auth is configured (OAuth or key) - with a cheap accessible model (`checkerModelInfo().recommended`), reusing the EXACT delimited inert-DATA prompt (now exported as `EXPLAIN_SYSTEM` + `explainUserPrompt`). Direct-key path stays first (cheapest, no session). Also clarified the exec approval labels the user found ambiguous: **"Allow once (this command)"** and **"Allow for the rest of this turn"** (was "Allow once" / "Allow for this turn").
- **stubbed:** the OAuth TLDR path is type-clean + routes through the proven util connection but needs an on-device confirm (no live OAuth session here). Per-PROJECT exec scope still not built - the persistent option remains the GLOBAL "Always allow this program" (shown only for a SIMPLE, single-program command; a compound/redirecting command like `a; b | c` offers only once / this-turn / block, since there's no single program to pin - by design, DECISIONS.md).
- **verified:** server tsc adds ZERO new diagnostics (baseline symbol_graph TS7 + dev.ts:620 only); exec/egress policy 192 tests green; new `explain_command.test.ts` 3 tests pin the TLDR trust-boundary (command stays inside `<command>` delimiters; system frames it as inert data).

-----

## P-LOCAL.4: local-model presets + the unified secure NGINX endpoint
- **shipped:** self-hosting several open-weight models on your own hardware (Mac Studio M3 Ultra, NVIDIA DGX Spark) behind ONE secured NGINX port is now one-click. New PURE `desktop/renderer/local_presets.ts`: a curated catalog (Laguna 2.1 Poolside, Gemma 4, Qwen 3.8, gpt-oss 120B, DeepSeek V3.2, Qwen3 Coder 30B, Llama 3.3 70B, each tagged which box it fits) + `unifiedEndpointForm` that turns several presets into ONE Local Provider (one base URL = the NGINX endpoint, many models, bearer auth) flowing through the EXISTING P-LOCAL.3 `draftFromForm -> validate -> vault -> upsert` path (zero new secret surface). Surfaced two ways: one-click quick-fill CHIPS in the Settings Local Providers add form (`localPresetChipsHtml`, wired via `applyLocalPreset`), and a new **Local & self-hosted** section in the Provider Hub with preset tiles that pre-fill that form (`pendingLocalPreset` applied after the card hydrates). New reference `docs/LOCAL-MODELS-UNIFIED-ENDPOINT.md` scopes the secure unified pattern (TLS + bearer, loopback backends, LAN/VPN bind, model-routed NGINX). Also (same session, per user feedback): REMOVED provider logos from the hub (names only) and deleted the dead `provider_logos.ts`/test; dropped "MoE" from Gemma; widened the hub grid + non-wrapping badges; fixed the mockup's `.set-note` flex-columns bug; added AGENTS.md **invariant #11** (UI text never cramped into wrapping columns; a flex row holds a single text child; mockups reuse real CSS).
- **stubbed:** the hub's preset tile routes to the Settings add form pre-filled (base URL + token entered there), rather than a full add form inside the popover - deliberate (the form + vault flow live in one place). Model ids/specs are editorial defaults the user edits to match their served ids; no live probe of the endpoint's model list yet.
- **next:** P-STT.2 (guided no-code Whisper); on-device pass of the hub + presets once the M3 Ultra / DGX Spark arrive. NOTE: still un-versioned; no release bump this session.

-----

## P-PROV.2: the Provider Hub - a dedicated, discoverable provider popup + onboarding step + open-weight providers
- **shipped:** providers are no longer buried in a collapsed Settings card. New dedicated Provider Hub popup (`openProviderHub` in app.ts) lists EVERY provider omp offers as same-size, native-logo'd tiles grouped into sections (Government gateway / Frontier U.S. / Open-weight & regional); clicking a tile drops its OAuth/key config inline (reuses the exact `provCard` body + a shared `startProviderOauth` helper factored out of the Settings handler, so OAuth device-code + GitHub-Copilot two-step + key save all behave identically). Reachable from: the first-run onboarding chain (new `promptForProviderIfNeeded` step, after gov/CUI, skipped under gov lockdown or when a provider is already configured), the command palette ("Connect a provider..."), a prominent button in the Settings Providers card, and an always-visible footer in the model picker. New NATIVE open-weight providers registered in `auth_status.ts`: Alibaba **Qwen** (`qwen-portal`, real OAuth broker + `QWEN_PORTAL_API_KEY`), **GLM** (`zai`, `ZAI_API_KEY`, key-only), **MiniMax** (`MINIMAX_API_KEY`, key-only) - alongside the existing Kimi/Moonshot; models are pre-bundled in omp's catalog so a saved key/OAuth surfaces them in the picker. The open-weight / non-U.S. section stays HIDDEN behind the typed ACKNOWLEDGE (pure `buildHubSections` emits ZERO providers until `thirdPartyAck`), and China-origin MODELS remain gated by the existing `isChinaModel` + `chinaAck` picker gate. Two pure modules: `provider_hub.ts` (grouping + the data-sovereignty gate) and `provider_logos.ts` (uniform, brand-tinted inline-SVG marks, one fixed box size for the whole grid; monogram-chip fallback).
- **verified:** `provider_hub.test.ts` (6) + `provider_logos.test.ts` (4, coverage checked against the REAL registry so a new provider without a logo fails) + `demo-P-PROV.2` (12 checks: new providers' auth methods, the locked/empty->revealed gate, elevenlabs exclusion, all 17 logos same-size+branded) all green; renderer `typecheck:desktop` clean; full `bun test desktop` 1715 pass / 4 fail (the 4 are the pre-existing `symbol_graph` typescript-7.0.2 API mismatch, none of the changed files). Live in-app pass (open the hub, connect via OAuth + key, confirm models land) is the remaining QA step (renderer DOM).
- **next:** P-LOCAL.4 - one-click LOCAL-model presets (Laguna 2.1 Poolside + top-10) on the existing P-LOCAL ollama/vllm/llama.cpp system; P-STT.2 - a MANAGED local Whisper (bundle/download whisper.cpp, hardware-capability gate, spawn + wire to the existing `OpenAiCompatibleSttBackend`). NOTE: "Kimi K3" is NOT in the installed omp 16.1.20 catalog (only K2 variants); the Kimi provider is wired and surfaces whatever Kimi models omp bundles.

-----

## Fix: model picker empty on fresh macOS install + after OAuth (per-cycle warm budget)
- **shipped:** the model-picker cold-start warm-poll budget is now PER WARM CYCLE, not per process. New PURE `desktop/renderer/config_warm.ts` (`warmStep`): a DELIBERATE load (boot, OAuth connect, provider-key change, opening a still-warming picker, manual refresh) RE-ARMS the retry budget; only the internal re-poll continuation spends it. Root cause of "picker empty the first few times after install and after OAuth": `configWarmTries` was a monotonic module counter reset ONLY on a successful live load, so a slow FIRST omp spawn (worst on a fresh macOS install: Gatekeeper first-run scan of the notarized bundle + embedded binaries, then session/new) exhausted the 6 tries, and the post-OAuth refresh (against a freshly-respawned, cold omp) found the budget spent and scheduled no retry, so the just-connected models never appeared until a manual Refresh. `loadConfig(newCycle=true)` threads the flag; one re-poll chain via `configWarmTimer` (cleared on re-arm). Also: `openConfigPopover` now renders the model section + spinner while pending (was gated on a cached list, so a fresh install with no cache opened BLANK), kicks a re-armed `loadConfig()` on open so opening a warming picker drives the refresh, and shows a "Loading models..." affordance until the live list lands (`pickerRedraw` swaps it in). New `state.configWarming` flag drives the spinner.
- **verified:** `config_warm.test.ts` 5 tests green (the load-bearing one: budget re-arms on a new cycle AFTER a prior give-up = the post-OAuth path); renderer `typecheck:desktop` clean (exit 0); picker-area suites 51/51 (config_warm + model_families + model_favorites). Pre-existing `symbol_graph.ts` 4 failures are environmental only (installed typescript 7.0.2, the Go port, lacks the classic `createSourceFile` API `loadTs()` probes) and touch none of the changed files. Live in-app pass on a real fresh macOS install + OAuth connect is the remaining QA step (renderer DOM path).
- **next:** on-device fresh-install + OAuth pass to confirm the picker fills without a manual Refresh; if a session still wedges past the budget, capture dev-mode Logs to pin the omp session/new stall itself (this fix makes the UI self-heal, not the underlying warm-up faster).

-----

## P-GOVCUI.1: first-run Government / GovCon + CUI onboarding step -> AskSage lockdown
- **shipped:** onboarding now ASKS (after role + email, before the tour) whether the user is a Government/GovCon user handling CUI, and walks a novice (manager/exec/new dev) into the CUI-safe posture. A "yes" opens a guided setup: it PREFILLS the AskSage CIV (government) routing endpoint (`https://api.civ.asksage.ai/server`), shows step-by-step "how to get your AskSage API token" instructions (Settings -> Account -> Manage your API Keys, verified against AskSage docs) with an inline paste field, and on save turns ON AskSage-only LOCKDOWN (saves the key, switches to a gov model, shows the DoD banner) then opens Settings on the AskSage card (scrolled + flash-highlighted) so they SEE where the control lives. New PURE `desktop/renderer/gov_onboarding.ts` (`decideGovOnboarding` ask/skip/auto-enable - asks exactly once, org-forced routing auto-enables; `planGovSetup` - with a key => CIV persisted + lockdown ON, without a key ("I'll add it later") => endpoint prefilled but lockdown NEVER flipped, since a keyless lockdown leaves no gov model and the backend fail-closes). Cosmetic onboarding state `govconCui` in `settings_store` (mirrors userRole/tourSeen) threaded through `/api/settings` + `bridge.setGovconCui`. The gate, prompt prefix, and trust labels are untouched.
- **verified:** gov_onboarding.test.ts (8) + demo-P-GOVCUI.1 (`demo_pgovcui1.ts`, 5 checks: ask-once/auto-enable, key=>lockdown, keyless-never-flips, CIV-not-commercial, novice steps) green; renderer tsc clean (exit 0); onboarding-adjacent suites 74/74. The first-run modals + the openSettingsToAsksage scroll/flash are renderer DOM (QA-gated in-app like the rest of onboarding); the load-bearing decision + setup plan are pure + covered. Server tsc unchanged from baseline (pre-existing symbol_graph.ts API-version + dev.ts:620 OAuth-scan diagnostics only; nothing in the touched files).
- **next:** on-device pass of the full flow (pick Yes -> paste a real AskSage key -> confirm lockdown routes gov + the DoD banner + the Settings deep-link); optional: a Settings "Government / CUI mode" restart affordance for someone who answered No but later needs it, and persisting the DoD acknowledgment timestamp for audit.

-----

## P-SHARE.3: mobile-safe invite links in the Session Share dock
- **shipped:** the Share dock no longer lets you text a phone the wrong link. A live room mints an https PHONE link (secret in the fragment) AND a wss LUCID-to-LUCID link (secret in the path); the new PURE `classifyInviteLink` (`desktop/renderer/share_dock.ts`) is the single source of truth the UI renders from: it FEATURES the https link (accent "Copy phone link" button + the QR), keeps the wss link in a demoted "Desktop only, don't open on a phone" row (muted dashed field + "Copy desktop link"), NEVER offers or QR-codes a non-http(s) value to a phone (closing the footgun where the old `qrOf(st.browserLink || st.fullLink)` fallback QR-coded the wss link, which fails to join AND leaks roomId.secret into whatever HTTP server answers the host), and flags an https link on a private/LAN/loopback host as same-network-only. Copy toasts now carry a per-button `data-copy-desc` so each says where the link belongs.
- **stubbed:** the live Electron pointer/paint (accent button, dashed field, caution banner) is QA-gated in-app like the rest of the dock DOM; the pure classification + wiring are covered by unit tests + the demo. The LAN-vs-public read is a conservative host heuristic (a bare single-label or `.local` host is treated as LAN), not a reachability probe.
- **next:** on-device pass confirming the phone link is the one copied/scanned across hosted-relay, self-hosted-LAN, and direct-P2P shares; remaining roadmap is new asks.

-----

## Release note - 2026-07-10 - KG Packs + marketplace (ADR-0205 / ADR-0206)

**KG Packs (ADR-0205, P-KGPACK.1-.6).** The combined "Compiled KB" became a set of named, swappable knowledge graphs (file-per-KG + a JSON registry; frozen `0011` reused, no schema change) with a filter-as-you-type picker. Each KG is seedable from a ChatGPT/Claude/Gemini export or an Obsidian vault (a background job, no doc cap, gated fail-closed) and exportable/importable as a signed `.lkgpack` (Ed25519 verify + re-scan every page fail-closed, installed read-only). Surfaced in an in-app "Role KG Packs" storefront.

**Marketplace gating (ADR-0206, P-KGMARKET.1/.3).** A fail-closed client entitlement gate (`decidePackAction` - three independent gates: payment=access, signature=origin, scanner=safety) plus a Firebase-backed provider (SDK-free callable client, injected auth) that registers into the market seam only when configured. The private Stripe/Firebase backend lives in the add-on repo.

**Status.** SCOPE/PLAN + BUILT, all verified (unit tests + real-scanner demos + tsc + license). Landed **local-only** (held from push). Per-increment detail is in the entries below and on the `feat/p-kgpack-*` / `feat/p-kgmarket-*` branches. Remaining: the Firebase sign-in surface + the download -> unzip -> gated-import bridge (P-KGMARKET.4).

-----

## P-REMOTE follow-ups: comp-grant recheck + invite-link leak hardening (ADR-0227)
- **shipped:** the Remote paid-tier live-test session's three fixes, all deployed + verified live. (1) `tools/remote-pwa/app.ts`: the Subscribe screen now does a ONE-SHOT per-sign-in forced token refresh on mount, so an admin comp grant (`setRemotePremium` sets the `premium` claim with no checkout moment) connects instead of paywalling on a stale cached token; loop-guarded so a relay that refuses a fresh entitled token can never cycle connect->4403. (2) `desktop/renderer/app.ts` Share panel: the primary copyable is now the browser-safe fragment link (`https://lucid-agent.web.app/remote/#...`) whenever the hosted rendezvous provisions one; the `wss://.../r/roomId.secret` form (secret IN THE PATH) is demoted to a labeled LUCID-to-LUCID row - texting it to a phone was observed live leaking the room secret into the apex site's request logs. (3) a `/r/**` hosting rewrite + `r.html` forwarder on lucid-agent.web.app moves a path-borne bare link into the PWA fragment client-side. Also: sherbert105@gmail.com comped premium (claim + entitlement doc written + verified server-side), the deployed-only addon source committed/pushed as `feat/remote-admin-plane`, and the em-dash cleanup in the PWA shell.
- **stubbed:** the APEX `aiworkshopapps.com/r/*` still swallows old relay links (the COVERT site's own hosting, separate repo - needs the same forwarder); the mis-opened room's secret already transited those logs (stop + reshare rotates it). The comped phone's live end-to-end join is pending the user's rescan. Stripe stays in TEST mode - the LIVE flip needs the user's sk_live + webhook secret (runbook staged).
- **next:** add the `/r/*` forwarder to the COVERT site; flip Stripe LIVE (user sets secrets, then redeploy functions + re-verify fail-closed); confirm sherbert105's presence row over the gated relay.

## Headless KG-pack builder — `tools/build_kg_pack.ts` (ADR-0207)
- **shipped:** a CLI that runs the exact in-app pack-authoring pipeline so the whole role-pack product line builds repeatably (`make kg-pack ROLE=bd`, or `--all`). It composes the already-shipped, already-tested pieces with ZERO new trust path: `readKbSources` (the app's export loader) → `createKg` → `ingestSourcesIntoKg` (fail-closed scan + compile; the model injected as `backend.complete` — the SAME omp-spawning backend the LUCID session uses, resolved to the app's most-used model exactly like `/api/kb/ingest-batch`) → `exportKgPack` (Ed25519-sign when `LUCID_KG_PACK_SIGNING_KEY` is set) → `importKgPack` (verify + re-scan round-trip, proving the pack installs). Builds into an ISOLATED temp KB workspace (never the user's `~/.omp` working graphs) and writes `<slug>.lkgpack(.zip)` next to its source dataset. A build catalog maps each role folder under `LucidAgentDesigns/KG Packs/` to its pack name/role/description/licensing + stable product `id` (the five existing storefront ids match, so `packs/<id>.lkgpack.zip` lines up); every field is `--`-overridable. Cost controls: `--limit N` (cap model calls), `--dry-run` (load + count, no model calls), `--all` (sequential). `make kg-pack` / `kg-pack-all` targets.
- **verified:** 10 pure-helper unit tests (arg parsing incl. fail-loud on bad flags/values, catalog key/folder/path resolution + basename synthesis, slug prediction matching `kb_pack`, id alignment with the storefront, metadata overrides). `--dry-run` on the BD dataset loads 365 conversations (chat/openai). A live `--limit 2` smoke against the AI TECH BD export compiled 2 → 28 KG pages (0 quarantined, 0 errored) with the app's model (`claude-fable-5`), exported the `.lkgpack.zip`, and the verify-import round-trip returned OK (28 pages). Root `tsc` clean, license clean.
- **stubbed:** the smoke pack was UNSIGNED — `LUCID_KG_PACK_SIGNING_KEY` (the TechLead 187 authoring key, private add-on IP) isn't set on this machine; real sellable builds must set it or the marketplace treats the origin as unsigned. No signing key was committed. The BD Capture Manager (`AI TECH BD` / `dow-dod-business-development`) is a NEW pack not yet in the storefront catalog (`desktop/renderer/kg_packs.ts`) or the marketplace catalog. Compile is ~real model time (~50s/doc here incl. omp startup), so a full 365-doc pack is a long run — a background/`--limit`-staged job, not interactive.
- **next:** set the signing key, then `make kg-pack ROLE=bd` (full) and work the rest of the folder sequentially; add storefront + marketplace-catalog rows for the roles beyond the current five (BD, SBIR, ML, Frontend, Backend, STE); optionally upload each `<id>.lkgpack.zip` to the entitlement backend's Storage bucket.

## P-KGMARKET.4 part 2: the marketplace SIGN-IN flow, against stubs (ADR-0206)
- **shipped:** the sign-in half of the pack paywall, built so the whole flow runs with NO deployed backend. `desktop/renderer/market_auth.ts` — a fail-closed token store (`MarketAuth`: injected storage + clock; `parseAuthCallback` for the `lucid://auth?token=&email=&exp=` deep link; `getUser`/`getIdToken` that return signed-out/null on an expired or absent token; a dev `signInStub`). `desktop/renderer/market_stub_provider.ts` — a DEV `EntitlementProvider` (same seam as the Firebase one) backed by `MarketAuth` + a local owned-packs ledger; signed out ⇒ nothing entitled/granted, signed in ⇒ checkout grants instantly so `decidePackAction` walks signin→checkout→pull offline. `desktop/renderer/market_boot.ts` — the orchestration singleton + PURE helpers (`chooseMarketMode`, `buildSignInUrl`): `initMarket` registers firebase / stub / off; `beginSignIn` opens the hosted page (firebase) or mints a local token (stub); `handleAuthCallback` applies the forwarded deep link. Wired: `getPackFlow`'s signin action now calls `beginSignIn` (stub re-runs into checkout/pull; firebase shows a "finish in browser" toast); app boot calls `initMarket(readMarketBootConfig())` and subscribes to `window.lucid.onAuthCallback`. Electron `main.ts` claims the `lucid://` scheme (`setAsDefaultProtocolClient`), enforces a single instance, and forwards the deep link from second-instance argv / macOS `open-url` (queued + flushed on `did-finish-load` for a cold launch); `preload.ts` exposes `onAuthCallback`. A plain public build injects no config ⇒ mode "off" ⇒ the fail-closed nullProvider, so nothing changes there.
- **verified:** 20 new unit tests (market_auth 8: deep-link parse valid/invalid/expiry-default, signed-in/out/expired fail-closed, persistence, stub sign-in; market_stub_provider 5: signed-out fail-closed, signin→checkout(grant)→pull, seeded-owned, revoke; market_boot 7: mode/URL helpers, stub sync sign-in, firebase open+callback, off no-op, bad deep link) — **37 market tests total** green. `demo-P-KGMARKET.4b` green: STUB mode drives signin→checkout→pull→the SAME P-KGPACK.4 gated install on a REAL `.lkgpack.zip` offline (installed read-only), FIREBASE mode opens the hosted URL then the `lucid://auth` deep link signs in, OFF leaves the nullProvider; sign-out fail-closes (no download). Renderer + server `tsc` exit 0, license clean; `demo-P-KGMARKET.4` (part 1) still green.
- **stubbed:** the real Firebase token exchange + Stripe checkout are exercised only through the stub / injected `openExternal` here (the live provider is P-KGMARKET.3 in the add-on, deployed separately by the user). In-app STUB checkout opens the product page after its instant grant (dev-only path); the shipping firebase path opens real Stripe. No sign-out button in the storefront UI yet (the API exists: `marketSignOut`).
- **next:** deploy the add-on Cloud Functions + upload the `.lkgpack.zip` objects, inject `__LUCID_MARKET__` (functionsBaseUrl + signInUrl) for provisioned builds, and add a storefront account chip (signed-in email + sign-out).
...
**P-COLLAB.15 - the renderer-side WebRTC session (keystone, ADR-0200)**
- **shipped:** proof that the renderer-side WebRTC session stack works end-to-end with the REAL classes. `desktop/collab/webrtc_session.ts` (renderer-only): `webrtcHost`/`webrtcGuest` drive the unchanged CollabHost/CollabGuest over a `WebRtcTransport` (host=offerer, guest=answerer); signaling via a SignalingChannel (prod: RelaySignaling; test: loopback); frames stay E2E-sealed over the DataChannel. `webrtcLoopbackSelfTest` (exposed as window.__lucidWebrtcSelfTest, no side effects unless called) stands up a host+guest over a real DTLS DataChannel + asserts the E2E welcome + a broadcast ChatEvent arrive P2P. Also FIXED latent TS-6 errors in webrtc_transport.ts (a dep bump tightened DataChannel.send / ICE types - now copies to a plain ArrayBuffer + constructs ICE explicitly; was silently failing desktop tsc). VERIFIED LIVE: the diagnostic returned ok:true (welcome round-tripped, participants=1, events=[token,done]) deterministically 4/4. 67 collab tests + root/desktop tsc + license green.
- **stubbed:** RTCPeerConnection is renderer-only so preview-verified, not bun-tested. The production swap-in (renderer coordinator: relay CollabSocket for signaling + demux, a WebRtcTransport per guest, WebSocket fallback when no DataChannel, STUN/TURN, UI) is the next slice.
- **next:** the production swap-in above.

**P-COLLAB.16 - the production WebRTC swap-in (ADR-0201)**
- **shipped:** the engine that flips a live share to peer-to-peer, as two unit-tested building blocks + a coordinator. `prefer_p2p.ts` `PreferP2PTransport`: starts on the relay + transparently UPGRADES to the DataChannel when it opens, downgrades back if it drops, each frame sent once (this IS the WebSocket fallback - relay is the reliable baseline, P2P a transparent optimization). `fanout_host.ts` `FanoutHostTransport`: one unchanged CollabHost fanned out to one PreferP2PTransport PER guest (broadcast→all, unicast→one, lazy create, peer-left teardown). `webrtc_coordinator.ts` `webrtcHostCoordinator`/`webrtcGuestCoordinator`: connect a relay CollabSocket + demux it into signaling / control / relay-fallback, wiring real WebRtcTransports under the fan-out. Both blocks DOM-free + unit-tested (10 tests). VERIFIED LIVE: `window.__lucidWebrtcRelaySelfTest()` runs BOTH coordinators over an in-memory relay faithful to relay_server routing → ok:true, welcome round-tripped, guests=1, events=[token,done], UPGRADED to a direct DataChannel (path=p2p) deterministically 4/4. 77 collab tests + root/desktop tsc + license green.
- **stubbed:** RTCPeerConnection is renderer-only so the coordinator is preview-verified (the pure blocks ARE bun-tested). Live-flow integration behind a "prefer direct P2P" UI toggle + STUN/TURN Settings is the final slice.
- **next:** wire the coordinator into the live share flow behind the UI toggle (replace the backend host/guest path when P2P is enabled) + STUN/TURN config.

**P-COLLAB.17 - the "prefer direct P2P" toggle + live-flow swap-in (ADR-0202)**
- **shipped:** the user-facing WebRTC toggle. `desktop/renderer/collab_p2p.ts` owns the renderer P2P lifecycle (RTCPeerConnection is renderer-only, so a P2P share moves hosting into the renderer): mints the room client-side, drives webrtcHostCoordinator/webrtcGuestCoordinator (ADR-0201), exposes teeEvent/teeUserTurn (feed the live ChatEvent stream in) + startP2PGuest + teardown. app.ts onEvent calls teeEvent(e) + send() calls teeUserTurn(text) (zero new plumbing); a direct EDIT guest's prompts run through the host's OWN composer (shared runGuestPromptLocally → gate+approvals fire). Share panel: "Prefer a direct connection (WebRTC)" checkbox + STUN/TURN disclosure + "direct P2P" live badge; Join panel uses the renderer guest coordinator when preferred; currentShareStatus() unifies the status source. settings_store collabP2PConfig/setCollabP2P + routes /api/collab/p2p (GET/POST, POST refused under org lock) + /api/collab/authorize-connect (managed allowedRelays gate before a P2P join). VERIFIED: window.__lucidP2PModuleSelfTest drives the ACTUAL toggle path (startP2PHost mint → startP2PGuest parse minted link → tee → deliver) over an in-memory relay = ok:true, 3/3; live UI renders the toggle+ICE (persisted), starts a share badged "direct P2P" with an embedded-relay link, settings round-trip persists. 77 collab tests + tsc + license green.
- **stubbed:** the raw WS to the embedded relay is blocked in the sandboxed preview (only the dev-server port is proxied), so the cross-socket relay hop isn't exercisable there - hence the in-memory-relay self-test; works in a real Electron build. TURN credential sits in the user-local settings file (a future slice could move it to the OS vault).
- **next:** audit events for P2P share/join start/stop; optionally move TURN creds to the vault; richer guest rendering (tools/thinking) over P2P.

**P-COPY.1 - copy affordances for chat + code blocks (ADR-0203)**
- **shipped:** a Copy button on every code block + a right-click Copy menu for chat text/code (Electron has no default menu, so right-click did nothing before). `enhanceCodeBlocks` now injects a `.code-copy-btn` (data-code-copy, top-right) next to View-in-IDE; a document-level delegated click copies the block's exact text + flashes a check (works in chat, intro, skill bodies, reports). `ctxmenu.ts` extended: factored a shared `spawnMenu`, and the contextmenu listener now offers "Copy" (on a selection) + "Copy code block" (inside a <pre>) for non-editable content, [] otherwise (pure `readonlyMenuItemsFor`). VERIFIED LIVE: real rendered block gets the copy button (top-right, 26px); right-click on code → "Copy code block"; right-click on selection → "Copy". Clipboard read/write blocked in headless preview (no user-gesture) but it's the same navigator.clipboard path the existing message-copy button uses in Electron. 12 ctxmenu tests + tsc + license green.
- **stubbed:** clipboard result not assertable in the sandboxed preview; no syntax highlighting on code blocks (separate concern).
- **next:** (unrelated) resume the P-COLLAB arc follow-ups.

**P-COLLAB.18 - the live-collaboration audit trail (ADR-0204)**
- **shipped:** share + join audit events for BOTH the relay + direct-P2P paths. 4 new EventNames (collab_share_started/_stopped/_guest_joined/_guest_left) in contracts.ts, emitted via the canonical Telemetry (name-validated, metadata-only → ~/.omp/lucid-events.ndjson): transport/access/relaySource/opaque roomId/guest name — NEVER key/link/content. `desktop/collab/collab_audit.ts` = record fns + a fail-closed dispatcher `recordCollabAudit` (closed action set + metadata whitelist) for the renderer P2P path. CollabHost gained an onParticipant("join"|"leave") hook (fires on hello + peer-left) → relay wires it via CollabManager→dev.ts, P2P via startP2PHost→app.ts→bridge. Share start/stop: relay at /api/collab/start|stop, P2P from app.ts. New POST /api/collab/audit route (the renderer can't write the log) maps a closed action set to a validated EventName. VERIFIED LIVE: the P2P route wrote collab_share_started (direct-p2p) with an injected key/link STRIPPED; relay start/stop wrote paired share_started/stopped (relay, real roomId + relaySource); a bogus action wrote nothing (fail-closed). collab_audit.test (7) + host.test onParticipant (+1); 85 collab + 20 telemetry tests + tsc + license green.
- **stubbed:** guest-authoritative "I joined a remote share" event + relay serve start/stop events + DuckDB ingestion of collab events are deferred.
- **next:** those deferred items, or resume other collab follow-ups.

**P-KGPACK.1 - named, swappable KGs: file-per-KG + registry (ADR-0205)**
- **shipped:** the combined "Compiled KB" is now a SET of named KGs. `harness/kb/registry.ts` = a typed JSON registry (`~/.omp/kg_registry.json`, atomic tmp+rename write, no `existsSync`-then-read) mapping a stable Snowflake `kg_id` + a renamable `name` to that KG's OWN `kb_graph.duckdb` (frozen `0011` reused verbatim - NO schema change, invariant #10). `desktop/kb_store.ts` resolver: `kbStore(kgId?)` returns a per-KG cached store; `listKgs/activeKgId/createKg/renameKg/setActiveKg` added; a default "My Knowledge" KG auto-adopts the pre-existing `kb_graph.duckdb` (zero data loss) so no-arg `kbStore()` stays backward-compatible for `dev.ts`. A corrupt registry throws (never wipes); a missing one opens empty. 16 tests (registry 10 + resolver 6, incl. adoption + isolation + active-switch) + `demo-P-KGPACK.1` green; tsc/license clean (only the pre-existing untracked `output_*_proto.ts` error).
- **stubbed:** no UI yet (filter-as-you-type dropdown = P-KGPACK.2); ingest-into-a-named-KG = P-KGPACK.3; `.lkgpack` pack format + gated import = P-KGPACK.4; Marketplace "Role KG Packs" SKU hint = P-KGPACK.5; the 5 `kg_*` EventNames are named in ADR-0205, deferred to a `contracts.ts` increment (#8).
- **next:** P-KGPACK.2 - swap the single "Compiled KB" row in `kg_header.ts` for the filter-as-you-type KG picker (rename inline, "New KG", active check) + `app.ts`/bridge wiring.

**P-KGPACK.2 - the named-KG picker: filter-as-you-type dropdown (ADR-0205)**
- **shipped:** the "Compiled KB" views-menu row now opens a filter-as-you-type KG picker (pure builders in `kg_header.ts`: `filterKgList` + `kgPickerHtml/kgPickerRowsHtml`, marketplace-search pattern) - one row per KG with the active one checked, an origin badge (Chat/Obsidian/Pack) + read-only lock, inline **Rename** (shared `promptText` modal) and a **New KG** action; the views button label now reads the active KG's NAME in kb mode (`kgViewLabel` + `kbName`). Bridge `kbList/kbCreate/kbRename/kbActivate` over new `dev.ts` routes `/api/kb/{list,create,rename,activate}` returning the refreshed list (validation error rides on `error`, never nulls the list). Picking a KG activates it server-side then draws its page graph (`/api/kb/graph` reads the ACTIVE KG); re-picking the on-canvas KG returns to Personal (retired the old single-graph `toggleKbGraph`). `.kgp-*` CSS. User KG names are `esc`'d (invariant #5).
- **verified:** 15 `kg_header` tests (label+kbName, filter, rows escaping/active-check/badges/empty, picker shell) + `demo-P-KGPACK.2` green; renderer + server `tsc` 0 errors, license clean; LIVE server smoke (real `dev.ts` on 5319, token+origin gated): `/api/kb/list` → default KG active; create "Backend Engineer" → 2 KGs, active unchanged; blank name → list intact + `error` set.
- **stubbed:** ingest-into-a-named-KG (rename-at-ingest for chat/Obsidian) = P-KGPACK.3; `.lkgpack` pack format + gated import = P-KGPACK.4; Marketplace "Role KG Packs" SKU hint = P-KGPACK.5; the picker popover/rename/new wiring is typechecked + smoke-verified server-side, final in-app QA of the popover (app chrome, not the preview iframe) pending.
- **next:** P-KGPACK.3 - route chat-history + Obsidian-markdown import into a chosen/created KG (rename-at-ingest), gated + fail-closed, `untrusted`.

**P-KGPACK.3 - seed a named KG from a folder (chat export or Obsidian markdown), gated (ADR-0205)**
- **shipped:** a KG is now SEEDABLE, not just selectable. `harness/kb/batch_ingest.ts` (`ingestSourcesIntoKg`) is a thin loop over `ingestDocument` - every security guarantee inherited (source scanned fail-closed BEFORE compile, every page re-scanned, flagged source/page quarantined + never stored, dead scanner blocks); it adds only batch concerns: a cap (`skipped` reported, never silent), per-doc progress, cancel-at-boundary, and an honest `errored` count (a compile/backend throw is caught + counted, NOT admitted and NOT a security quarantine, so one outage can't abort the run). `desktop/kb_sources.ts` (`readKbSources`) normalises a folder to `KbSourceDoc[]`: a ChatGPT/Claude/Gemini export (reusing the TOCTOU-safe `loadExportData` + `parseExport`, one doc per conversation, BOTH sides) or an Obsidian vault (bounded recursive `.md` walk, hidden/system dirs skipped). Route `POST /api/kb/ingest-batch` ({path, name?/kgId?}) - `name` creates + names the KG at ingest ("rename at ingest") with `source_kind` chat/obsidian; blocks audited via `recordBlock`; capped at 50 docs. Bridge `kbIngestBatch`; picker footer gains an **Import files** action → folder → name → gated compile → the seeded KG is activated + drawn (summary toast counts pages/quarantined/errored/skipped).
- **verified:** 13 new tests (batch_ingest 7: clean/poison-doesn't-stop/dead-scanner/cap/cancel/errored/progress; kb_sources 6: chat→docs, obsidian walk+skips, chat-wins, friendly errors) + `demo-P-KGPACK.3` green (REAL scanner: a Trojan-Source note quarantined while clean notes compile, two named KGs isolated, default untouched); renderer + server + harness `tsc` clean, license clean; **LIVE** server smoke (real `dev.ts`, token+origin gated): a markdown vault → new "Obsidian Notes" KG with **7 pages / 9 links** compiled by the real model, 0 quarantined; a garbage folder → friendly error.
- **stubbed:** synchronous + 50-doc cap (background-job chunking + progress polling is a follow-up, the batch core already supports `onProgress`/`signal`); ingest into an EXISTING KG is wired server-side (`kgId`) but the UI only offers "new KG at ingest"; `.lkgpack` pack format + gated import = P-KGPACK.4; Marketplace "Role KG Packs" SKU hint = P-KGPACK.5.
- **next:** P-KGPACK.4 - the `.lkgpack` pack format (`kb_graph.duckdb` + signed `manifest.json`) + author/export tool + gated import (verify sha256/signature for ORIGIN, then re-scan pages fail-closed, store read-only, trust stays untrusted).

**P-KGPACK.4 - the .lkgpack KG Pack: author (export) + gated import (ADR-0205)**
- **shipped:** a KG is now a portable, sellable artifact. `harness/kb/pack.ts` (pure): the manifest shape + `canonicalManifestBytes` (a stable signed payload that mirrors JSON.stringify's undefined handling - a divergence there silently breaks every signature), `buildManifest` (Ed25519-signed when a signer is given, else unsigned), and `verifyPackManifest` - the TRUST MODEL: integrity (db sha256 must match), signature-as-ORIGIN (PRESENT-but-invalid is REFUSED; ABSENT = unsigned community pack, allowed because the SCANNER is the safety gate), self-contained crypto (harness must not import desktop). `desktop/kb_pack.ts`: `exportKgPack` (flush the KG's db via new `closeKg`, hash, sign-if-configured, write `<slug>.lkgpack/` = db + manifest) and `importKgPack` (mirrors `installRegistrySkill`: integrity → origin → re-scan EVERY page fail-closed, any finding/dead-scanner blocks the whole import → register a NEW read-only KG + copy the clean db in; never auto-trusted, keystone #2). Keys/signer from env (`LUCID_KG_PACK_KEYS`/`_SIGNING_KEY`), fail-soft to unsigned - the public repo verifies + builds unsigned packs; the real TechLead 187 signing key is private-repo IP (same public-seam split as P-SKILLREG.1). Routes `POST /api/kb/pack/{export,import}`, bridge `kbPackExport/kbPackImport`; picker gains a per-row **export** (download) icon + an **Import pack** footer action (activate + draw on success; danger toast with the reject stage on block). `createKg` gained `readOnly`; `kgEntry`/`closeKg` added.
- **verified:** 14 new tests (pack 9: canonical-ignores-sig, signed/unsigned build, integrity mismatch, present-but-invalid refused, no-keys refused, tampered-payload, bad-format; kb_pack 5: read-only round-trip, tamper→integrity, poison→scan-blocks-whole-import, dead-scanner, signed-verifies-only-vs-trusted-key) + `demo-P-KGPACK.4` green (REAL scanner + REAL Ed25519: signed round-trip installs read-only, tampered db refused at integrity, Trojan-Source page blocks import, untrusted-key signature refused); renderer+server+harness `tsc` clean, license clean; **LIVE** server smoke: seed a KG (5 pages, real model) → `POST /pack/export` → `contracts-officer.lkgpack` → `POST /pack/import` → a new **read-only `source_kind:pack`** KG (5 pages, 0 findings), all three KGs in the list.
- **stubbed:** `.lkgpack` is a DIRECTORY bundle (a single-file zip archive is a follow-up; the private repo can zip the dir for distribution); import-into-existing / pack versioning-upgrade deferred; the picker export/import popover wiring is typechecked + server-smoke-verified, final in-app QA (app chrome, not the preview iframe) pending.
- **next:** P-KGPACK.5 - the Marketplace "Role KG Packs" section: a public SKU surface whose rows point at the private add-on repo, install is this gated import path; the actual PD-based packs are authored + hosted private.

**P-KGPACK.5 - the Role KG Packs storefront (public SKU surface) (ADR-0205)**
- **shipped:** the SKU shopfront that closes the arc. `desktop/renderer/kg_packs.ts` (pure, PUBLIC-SEAM): a curated `KG_PACKS` catalog of 5 role/Position-Description packs (GovCon Contracts Officer, CMMC & RMF Security Lead, Capture & Proposal Manager, Program Manager (EVM), Cleared Software Engineer) authored by TechLead 187 LLC - each a HINT that links out to the product page (`KG_PACKS_URL`), the files themselves live in the private add-on repo (same public-seam / private-IP split as P-SKILLREG.1). `filterKgPacks` + `kgPackRowsHtml`/`kgPacksHtml` (marketplace-search pattern, reusing the `.mkt-*` styles for one look). app.ts `openKgPacks()` mirrors `openMarketplace` (scrim + Escape/backdrop, live search, "Get pack" → `window.open` the product page); the modal's **Import a pack you own** action routes straight into the P-KGPACK.4 gated `importPackFlow` (integrity + origin verified, every page re-scanned fail-closed, installed read-only). Reached from the KG picker footer (**KG Packs** button, replacing the raw "Import pack") + a command-palette entry ("Browse Role KG Packs"). Names/copy `esc`'d.
- **verified:** 5 `kg_packs` tests (registry well-formed + unique ids, filter, rows carry Get-pack link + role chip + empty state + escaping, modal has search/list/close/gated-import) + `demo-P-KGPACK.5` green; renderer + server `tsc` 0 errors, license clean; full P-KGPACK arc regression **87 tests pass across 13 files**. The catalog modal is app chrome (not the preview iframe), so its scrim wiring is typechecked + QA-in-app; the underlying import gate was already verified LIVE in P-KGPACK.4.
- **stubbed:** the catalog is a curated static registry (a live fetch from the add-on repo / a real payment+entitlement flow is future); `community` tier (the open LUCID KG Marketplace) is roadmap only - all shipped packs are `first-party`; `KG_PACKS_URL` points at the product site (a dedicated /packs page + per-pack deep links are a follow-up).
- **next:** P-KGPACK arc is FEATURE-COMPLETE (.1 registry → .2 picker → .3 seed-from-folder → .4 .lkgpack author+import → .5 storefront). Remaining: author the real PD packs in the private add-on repo; optional single-file `.lkgpack` zip; background-job chunking for large ingests.

**P-KGPACK.6 - background KG-seed job: lift the 50-doc cap for real authoring (ADR-0205)**
- **shipped:** authoring a real role pack means compiling HUNDREDS of conversations (one model call each) - the old `/api/kb/ingest-batch` capped at 50 to avoid hanging the request. `desktop/kb_ingest_job.ts` (mirrors `import_job.ts`): a single-active background job with a KB-shaped progress/result (`documents/totalDocuments/pagesCompiled/quarantined/errored`, state running→done/failed/cancelled), `startKbIngest`/`kbIngestJobStatus`/`cancelKbIngest`, fail-safe (a cancelled/failed run keeps the pages already compiled). The route now STARTS a job (no cap - the whole dataset) and returns a jobId; `GET /status` + `POST /cancel` added. Bridge `kbIngestBatch` returns a start (`KbIngestStartView`) + `kbIngestStatus`/`kbIngestCancel`; `importKgFlow` reuses the shared import PILL (progress bar + Cancel, retitled "Compiling knowledge graph") polling to completion, then activates + draws the seeded KG. `KB_BATCH_CAP` removed.
- **verified:** 4 `kb_ingest_job` tests (start→running with live ticks, second-start refused, cancel→cancelled keeping partial counts, thrown→failed, jobId-scoped status) + `demo-P-KGPACK.6` (120-doc seed - past the old cap - all 120 compile, 0 skipped, 240 pages) green; renderer+server tsc clean, license clean; arc regression **91 tests / 14 files**. **LIVE on the user's REAL data**: seeded "Senior Proposal Manager" from a 6-conversation slice of a 365-convo ChatGPT export → **75 pages** compiled by the real model (progress polled 0→75, ~4.5 min), then `POST /pack/export` → a valid `senior-proposal-manager.lkgpack` (75 pages, role + author in the manifest). Extrapolates to ~4-5 h for the full 365 - which is exactly why the background job + cancel exist.
- **stubbed:** still ONE active job at a time (a queue is a follow-up); no resumable/checkpointed restart of a killed job; the full 365-conversation authoring run is left for the user to kick off in-app (or a monitored server run). Commerce gating (Stripe + Firebase entitlement, chosen: Both licensing + card/GPC + PO/invoice) is the next arc = ADR-0206 (P-KGMARKET).
- **next:** write ADR-0206 (P-KGMARKET): Firebase Auth + Stripe Checkout + Firestore entitlement ledger fronting the pack pull (three-gate model: payment=access, signature=origin, scanner=safety); public seam / private Cloud Functions.

**P-KGMARKET.1 - the fail-closed entitlement gate (public seam) (ADR-0206)**
- **shipped:** the PAYMENT gate's client core (one of three independent gates - payment=access, signature=origin, scanner=safety; never a substitute for the P-KGPACK.4 import gate). `harness/market/entitlement.ts` (PURE): `decidePackAction(auth, entitlement, nowIso)` → `signin | checkout | pull` + `isEntitled` - FAIL-CLOSED (not signed in → signin; ONLY an active, unexpired entitlement → pull; a lapsed subscription `expiresAt<=now` or a stale "active" record or anything missing → checkout; absence is never ownership) + `PackLicensing`/`PurchaseMethod` types. `desktop/renderer/market_gate.ts` (PUBLIC seam): an `EntitlementProvider` interface + a fail-closed `nullProvider` (no identity/entitlement/URLs) + `registerMarketProvider`/`getMarketProvider` (the private Firebase provider injects at boot, mirroring the skills-publisher `setPublishers` pattern). Storefront wired: `KgPack` gained a `licensing` field (+ a One-time/Subscription chip); "Get pack" now routes through `getPackFlow` - an UNCONFIGURED public build (nullProvider) opens the product page (storefront-as-hint, no regression), a CONFIGURED build enforces signin/checkout/pull.
- **verified:** 16 tests (entitlement 11: every fail-closed branch incl. lapsed-sub + expiry-at-now; market_gate 3: null default, unconfigured→signin, register/clear; kg_packs +2 licensing/handle) + `demo-P-KGMARKET.1` green; renderer + server + harness `tsc` clean, license clean. The `getPackFlow` popover wiring is typechecked + QA-in-app; the real signed-download + install is P-KGMARKET.2 (private Firebase provider).
- **stubbed:** the `pull` path only recognises ownership today - the signed-download + gated install + Firebase Auth sign-in UI land with the private provider (P-KGMARKET.2); no Stripe/Firebase keys in the public repo (fail-closed until configured). Separately, the FULL 365-conversation "Senior Proposal Manager" authoring is RUNNING detached (isolated KB at `~/lucid-authoring/proposal-mgr`, jobId in `jobid.txt`; ~14/365 · 166 pages at last check, ETA ~3.5-4 h) - export the `.lkgpack` once it reports done.
- **next:** P-KGMARKET.2 (PRIVATE add-on): the Firebase Cloud Functions (`createCheckout`/`stripeWebhook`/`getPackDownload`/`grantEntitlement`) + Firestore entitlement schema + private Storage; register the real provider. Host in a walled-off section of the existing `lucid-agent` GCP project (WIF).

**P-KGMARKET.3 - the Firebase client provider (marketplace entitlement transport) (ADR-0206)**
- **shipped:** `desktop/renderer/firebase_market_provider.ts` - the `EntitlementProvider` (P-KGMARKET.1 seam) implemented as a thin, **SDK-free** fetch client over the private Firebase callables (ADR-A023: `getEntitlement`/`createCheckout`/`getPackDownload`). No secrets + no Firebase SDK dep: a Firebase web config is public and security is server-side (Firestore/Storage rules + the callables); calls use the documented callable protocol (`POST {data}` + `Authorization: Bearer <idToken>`). **Auth is INJECTED** (`getUser`/`getIdToken`) so the sign-in mechanism is a separate, swappable concern. `registerFirebaseMarketProvider(cfg)` registers it into `market_gate` ONLY when configured (from managed config) - unconfigured stays on the fail-closed `nullProvider` (storefront = product-page hint, no regression). Every call needs a signed-in token; a missing token short-circuits to null WITHOUT a network call; a callable error resolves to null (the `decidePackAction` core re-checks fail-closed). Add-on: a matching `getEntitlement` callable + README/ADR-A023 notes. Reconciled Cleared Software Engineer -> subscription across app catalog to match Stripe.
- **verified:** 6 provider tests (mocked fetch + injected auth: right endpoint+token+packId, signed-out no-network short-circuit, error→null, register-only-when-configured) - **22 market tests total** (entitlement 11 + market_gate 3 + provider 6 + kg_packs 2) + entitlement-core 9 (add-on) green; renderer `tsc` 0 errors, license clean. `getPackFlow` (P-KGMARKET.1) already routes through the seam, so no app.ts change needed - it activates the moment the provider is registered.
- **stubbed:** the SIGN-IN flow (Firebase Auth → an ID token in Electron: a browser OAuth / deep-link callback) is the injected `auth` dependency, NOT built here - the provider is inert until it + a managed Firebase config land; the `pull` install path (download the signed `.lkgpack.zip` → unzip → gated import) needs a bridge route (renderer can't write files) + single-file pack zipping (core follow-up); the website storefront still opens the product page (its own Firebase-Web-SDK buy flow is a website increment).
- **next:** P-KGMARKET.4 - the sign-in surface (Firebase Auth in Electron) + wire the download→unzip→gated-import bridge so "Get pack" is a real end-to-end entitlement-recorded purchase + install.

**P-KGMARKET.4 (part 1) - single-file .lkgpack.zip + the download→unzip→gated-install bridge (ADR-0206)**
- **shipped:** the two auth-agnostic pieces of P-KGMARKET.4. `harness/kb/zip.ts` - a minimal, first-party zip WRITER (pure `node:zlib`, DEFLATE with a STORED fallback + CRC-32), the counterpart to the existing reader `harness/personal/unzip.ts`; no zip dependency (airgap-clean). `exportKgPack` now also emits `<slug>.lkgpack.zip` (flat manifest + db) beside the dir (returns `zipPath`) - the single object the entitlement backend signs. `installPackFromUrl(url)` in `desktop/kb_pack.ts` - fetch (injected for tests) → unzip manifest + db (basename-matched, robust to a folder prefix) into a temp `.lkgpack` → run the SAME `importKgPack` gate (integrity + Ed25519 origin + re-scan every page fail-closed → read-only install), temp cleaned up. Route `POST /api/kb/pack/install-from-url`, bridge `kbPackInstallFromUrl`; `getPackFlow`'s `pull` action now really downloads + installs (provider.downloadUrl → bridge → activate + render). A purchase grants ACCESS; signature + scanner still prove origin + safety.
- **verified:** 7 new tests (zip 3: round-trips through the real reader for DEFLATE + STORED + folder-prefix + empty; install 4: clean read-only install, download-error + non-zip → manifest stage, poisoned page → blocked at scan with nothing registered) + `demo-P-KGMARKET.4` green (REAL scanner: export→zip→download→install read-only; a Trojan-Source pack blocked at re-scan); renderer + server + harness tsc clean, license clean; 18 pack/market tests total.
- **stubbed:** SIGN-IN (part 2) - decided: **hosted sign-in on `lucid-agent.web.app` + a `lucid://` deep-link callback** handing the app a Firebase ID token (the injected `auth.getUser`/`getIdToken` the provider needs); needs a website `/signin` page + an Electron custom-protocol handler + a token store. End-to-end live test still needs the functions deployed to the sandbox.
- **next:** P-KGMARKET.4 (part 2) - the hosted sign-in page + `lucid://` protocol handler + token store, registered into `market_gate` so "Get pack" is a real signed-in, entitlement-recorded purchase → gated install.


**P-PROV.1 - first-party enterprise providers: Azure OpenAI + GitHub Copilot OAuth + Gemini Enterprise / Vertex (ADR-0210)**
- **shipped:** three omp-native providers the Settings UI didn't surface, plus the env that made Gemini Enterprise OAuth fail. The provider descriptor (`desktop/auth_status.ts`) gained an optional `fields: ProviderField[]` of EXTRA config env vars (and allows an empty primary `env` for OAuth-only providers) - each field is just another omp-read env riding the SAME `setKey`->`process.env`->omp seam as the primary key, so NO new storage/save path. New/updated majors: `azure` (Azure OpenAI: `AZURE_OPENAI_API_KEY` + resource-name/base-URL/api-version/deployment-map fields), `github-copilot` (OAuth-only device-flow; `startOauthBroker` now writes the broker's leading GitHub-Enterprise-domain prompt to stdin - default blank=github.com - so the device URL surfaces, and the renderer drives it in a dedicated two-step branch), `google-vertex` (Gemini Enterprise: `GOOGLE_CLOUD_API_KEY` OR ADC via project/location/credentials fields), and `google` gains `GOOGLE_CLOUD_PROJECT` (the missing env without which omp aborts Workspace/Enterprise Gemini OAuth). `providerAuth()` reports each field's status - secret->last4, non-secret config echoed to pre-fill. Corrected the user guide's wrong `VERTEX_API_KEY` to omp's real `GOOGLE_CLOUD_API_KEY`/ADC.
- **verified:** `desktop/auth_status.test.ts` (6 tests: descriptors + secret-masked/non-secret-echoed reporting) + `demo-P-PROV.1` green; server tsc + renderer tsc (modulo the sandbox's pre-existing missing-`electron`-types gap) + license clean; existing auth tests unaffected.
- **stubbed:** the live OAuth device dance (Copilot's GHE-domain prompt + one-time code) and real Azure/Vertex model surfacing in the picker need a provider account + the packaged app (omp owns the catalog) - QA-gated in-app, not headlessly testable here.
- **next:** P-IMG.1 - render generated/tool images inline in the chat reply.

**P-IMG.1 - generated / tool-produced images inline in chat: render + download + push-to-preview (ADR-0208)**
- **shipped:** images now flow tool/agent->chat (previously dead-ended in `payload.nonTextParts`). Extracted the strict image-data-URL primitives into a shared, server-importable `desktop/renderer/image_data_url.ts` (so `dev.ts`/`acp_backend.ts` reuse them without dragging DOM into the server typecheck; `composer_attachments.ts` re-exports them - existing importers unchanged). New pure `desktop/renderer/chat_images.ts`: `extractToolImages` lifts images from a tool result (BOTH ACP `{type:"content",content:{image}}` and bare omp `{type:"image"}`), validating each through the single gate (png/jpeg/webp/gif + base64 only; SVG refused as script risk), capping count/bytes fail-closed - a tool result is UNTRUSTED. `acp_backend.ts` emits a new `tool-image` ChatEvent from `tool_call_update`; the renderer draws each inline via the EXISTING safe `img.src`-property idiom with a Download (safe filename, no traversal) + "Send to preview". "Send to preview" POSTs to `/api/preview/image`, which writes a self-contained wrapper HTML (image as a `data:` URI - allowed by the preview frame CSP `img-src data:`) so the UNCHANGED `onPreviewAvailable` pipeline renders it under the markup canvas -> annotate -> Screenshot->chat to iterate. The frozen result_adapter is untouched (additive path).
...
**Settings clone parity + AskSage GPT-5.6 (ADR-0214 / ADR-0215) — 2026-07-14**
- **shipped:** Settings "Clone a git repo" now clones PRIVATE repos headlessly like the agent does — `cloneRepo` injects a host token (`GITHUB_TOKEN`/`GH_TOKEN`/`GITLAB_TOKEN`, https only) via a per-command `http.extraHeader` (never persisted to `.git/config`, redacted from errors), sets `GIT_TERMINAL_PROMPT=0`, trims the trailing-dot repo name (the Windows folder-desync behind `l.e.a.p.s.`), clears leftover partial clones, and surfaces an actionable auth hint instead of a generic toast. New AskSage CIV **GPT-5.6** added to `OPENAI_MODELS` + `MODEL_CTX` so it populates the picker.
- **stubbed:** vault-backed git PAT (a GitHub token entered once in Settings, decrypted main-process-only at clone time via `cred_vault.ts`) — env-var tokens ship now; ADR-0214 names the vault path as the follow-up. AskSage LIVE `/get-models` fetch scoped in ADR-0215 (fail-SAFE merge over the static specs) — not built; GPT-5.6 was added by hand.
- **next:** build ADR-0215 — `desktop/asksage.ts` `listModels()` + merge into `asksage_extension.ts` so future gov models appear without a code change; then the vault-backed git PAT for the clone path.

**Vault-backed git PAT for private clones (ADR-0216) — 2026-07-14**
- **shipped:** the ADR-0214 follow-up. Settings → Workspace has a "Git access token" field that stores a PAT in the OS-encrypted vault (ref `git_pat`, via the existing `credStore` IPC) with a saved indicator + Remove. `main.ts` `prepareGitToken()` injects it as `LUCID_GIT_PAT` into the dev child at spawn (the Figma/Local-Providers vault→env pattern); `hostTokenForUrl` uses it as the host-agnostic fallback after a workflow's own `GITHUB_TOKEN`. A token saved THIS session works immediately with no relaunch — the clone request carries an optional inline `pat` (`sessionGitPat`), and `resolveCloneToken` prefers it over env. Token stays leak-safe: `-c http.extraHeader` (never in `.git/config`), redacted from errors, never sent to the agent.
- **verified:** 4 new pure tests (LUCID_GIT_PAT fallback ordering, inline-override-wins, env fallback, ssh-ignored) — 17 `desktop/workspace.test.ts` total green; tsc clean on all touched files (main/dev/bridge/app/workspace); license clean. Live end-to-end (real private clone with a vaulted PAT) needs a desktop run with credentials.
- **next:** optional — a "Test token" affordance on the git PAT field (a cheap authenticated GET) so a bad/expired PAT is caught before a clone; and PAT rotation reminders via the existing `cred_vault` rotation metadata.

**AskSage lockdown enforced server-side, fail-closed (ADR-0217) — 2026-07-14**
- **shipped:** fixed the data-sovereignty hole where AskSage-only "lockdown" was enforced ONLY in the renderer (one-shot switch on toggle + picker hiding) and so silently failed on a fresh launch with lockdown persisted ON, and after every omp respawn (workspace change / clone / Refresh models / provider-key change) — the maker turn just used omp's default, a direct model. Added `enforceAsksageLock()` in `acp_backend.ts`: a FAIL-CLOSED clamp in `prompt()` that forces the active model to an `asksage`-routed one before every send and REFUSES the turn (`[blocked: …]`) if lockdown is on but no gov model exists (never routes direct), plus a best-effort clamp on fresh session init so the picker/status reflect gov immediately. Also fixed the checker's broken `/gov/i` predicate (real ids like `asksage-openai/gpt-5.6` have no "gov" substring → it matched nothing and fell through to all models) → now the shared `isAsksageRouted = /asksage/i`. Renderer guard: can't enable lockdown without a configured AskSage key.
- **verified:** 15 `desktop/checker_model.test.ts` (5 new: isAsksageRouted matches real gov ids, direct excluded; resolveLockdownModel keeps gov / switches a direct model to gov / FAIL-CLOSED blocks when no gov available). tsc clean on acp_backend/checker_model/app; the fail-closed "no gov ⇒ block" case is now unit-covered (the sovereignty analogue of the scanner kill-test). Live end-to-end (respawn under lockdown routes gov) needs a desktop run with an AskSage key.
- **next:** clamp scheduled BUILT-AGENT runs (`startAgentRun` model default `"haiku"`) under lockdown — the one remaining un-clamped routing path; add an in-app QA check that a respawn under lockdown keeps a gov model active.

**Lockdown covers egress + agent runs; RAG on GPT-5.6 Luna; real 5.6 ids (ADR-0218) — 2026-07-14**
- **shipped:** (1) Under AskSage lockdown the in-process gate now FAIL-CLOSED blocks ALL public egress (web_search / browser / web / fetch / navigate / external http(s)) with an audited high-severity event — no CUI backflow to a public search/site; local-file previews stay allowed; omp owns web_search and takes no endpoint, so the control is deny + AskSage RAG as the sanctioned alternative. (2) Built-agent runs honor lockdown via a shared `Backend.resolveAgentRunModel()` — clamps to a gov model or REFUSES (never the default direct "haiku"), wired into both the scheduled-automation path and the Builder "Run" route (the last gap ADR-0217 named). (3) Corrected the GPT-5.6 ids against the LIVE gateway: the family is `gpt-5.6-luna`(mid)/`-sol`/`-terra` — the earlier bare `gpt-5.6` was invalid and is removed; RAG `/query` default moved from `gpt-5.2` → `gpt-5.6-luna` (validated live: /openai/v1/models + a real /query each 200).
- **verified:** discovered + validated the model ids by testing through LUCID's backend against the live AskSage gateway (/openai/v1/models listed luna/sol/terra; a /query smoke test returned 200 "OK" for all three). 15 checker_model tests green; tsc clean on acp_backend/dev/app/asksage/settings_store/checker_model; license clean. Egress-block + agent-run-clamp are wiring over the unit-tested pure core.
- **next:** implement ADR-0215 (live `/get-models` fetch) so the model list + RAG default stop needing per-release hand-maintenance; consider a lockdown UI note that public search is disabled + points to the RAG model.

**Per-session CUI/Search mode + CUI banner + DoD consent + titlebar Datasets picker (ADR-0219) — 2026-07-14**
- **shipped:** refined ADR-0218's global egress block into a PER-SESSION control. New `sessionModes` store (settings_store, fail-closed default "cui", bounded) + `/api/session-mode` route (defaults to the active omp session) + bridge; the lockdown egress gate now blocks only when `asksageLocked() && sessionMode(active)==="cui"` — a CUI session blocks web egress, a Search session (no CUI datasets) may search while still lockdown-gov-routed. Renderer: a titlebar CUI/Search toggle + a violet CUI banner (both lockdown-only, per current session, reloaded on session switch; new sessions default CUI); switching to Search routes through a spillage warning that steers CUI work to a separate Search session. DoD/STIG Notice & Consent banner (standard USG/DoD text in one editable constant) shown once per launch when the gov gateway is configured. Restored the AskSage **Datasets** picker to the titlebar next to Skills (short name shown, full name on premium hover, multi-select, count on chip), reusing the Skills/`popover` pattern.
- **verified:** 5 new `desktop/session_mode.test.ts` (fail-closed default, persist roundtrip, back-to-CUI, bad-value coercion, bounded pruning) — 37 tests green across session_mode/checker_model/workspace; tsc clean on app/bridge/dev/settings_store/acp_backend; id↔handler consistency checked; license clean. Visual E2E (banner, toggle, blocked search) needs the desktop app reloaded with an AskSage key — not run headlessly to avoid disrupting the user's live session.
- **next:** optional — persist the DoD acknowledgment with a per-logon timestamp for audit; a small "web search disabled (CUI)" inline hint in the composer when a blocked search is attempted; surface the active session mode in the status bar.

**knowledge_search: RAG grounding for non-AskSage users (ADR-0220, increment 1) — 2026-07-14**
- **shipped:** the missing wire that lets ANY model (Claude/GPT/local) ground on the user's OWN knowledge - not just AskSage users. New `harness/omp/knowledge_extension.ts` registers a read-only `knowledge_search` tool (mirrors codegraph_extension) that, from omp's subprocess, POSTs the query to the EXISTING `/api/kb/retrieve` via a token'd `LUCID_KB_RETRIEVE_URL` (preview-tool env-URL pattern) and returns the delimited, cited UNTRUSTED hits; empty → "add an Obsidian vault or folder" guidance, never a loop. dev.ts sets the URL + adds `/api/kb/retrieve` to the `?t=` token allowlist (additive; renderer's header call unchanged); acp_backend registers the `-e` extension in ompArgv (always-on, non-AskSage path). The titlebar Datasets chip is dual-purposed: non-AskSage → "Knowledge" menu (Add Obsidian vault/folder via the existing importKgFlow + Open Knowledge panel). Reuses the already-wired ingest/compile/retrieve/KG-registry unchanged.
- **verified:** 9 `harness/omp/knowledge_extension.test.ts` (result shaping: hits pass the delimited block through with a cite reminder, empty→guidance, no-URL→graceful, query truncation; registration: read-only tool, no-op on older omp, empty-query + no-URL execute branches) — 68 tests green across the KB + knowledge suites; tsc clean on knowledge_extension/dev/acp_backend/app; license clean. Live agent-calls-knowledge_search E2E needs the desktop app reloaded with a compiled KG (can't drive headlessly without conflicting with the user's running instance).
- **next:** increment 2 (semantic) — wire the built-but-unshipped vector store + an `ApiEmbedder` calling the user's own OpenAI key or local Ollama/vLLM (`/embeddings`) for true semantic search (`mode:"hybrid"`), no WASM bundling; addon-side: live Notion/SharePoint/Confluence/Drive connectors + a shared server-side vector DB.

**Bring-your-own-embeddings ApiEmbedder (ADR-0221, increment 2 part 1) — 2026-07-14**
- **shipped:** `harness/knowledge/api_embedder.ts` - the "bring your own embeddings" primitive behind the existing `Embedder` seam. Calls an OpenAI-compatible `/embeddings` endpoint (the user's OpenAI/Azure key, or a local Ollama/vLLM via Local Providers), sidestepping the unshipped WASM bundle entirely. Pure (`fetchImpl`-injected), holds no secret at rest, FAIL-LOUD on any non-2xx / count / dim / non-finite so a broken endpoint can't poison cosine retrieval. Dormant until wired.
- **verified:** 9 `harness/knowledge/api_embedder.test.ts` (URL/auth-header helpers, response parse incl. out-of-order proxy + count/dim/non-finite guards, embed() posts+parses, empty→no-call, non-2xx→throws, bearer token sent). tsc clean; license clean.
- **next (blocked on a config decision + endpoint):** wire it in - ingest→embed into the vector store per KG, retrieve→hybrid, and a Settings surface to configure the embeddings endpoint. The current profile has only XAI/ElevenLabs keys (no `/embeddings`), so the semantic path can't be live-verified until an OpenAI/Azure/local-Ollama endpoint is configured; not guessing the config UX blind.

**Semantic RAG wired end-to-end: BYO-embeddings (ADR-0221 part 2) — 2026-07-14**
- **shipped:** the full non-AskSage SEMANTIC search path on top of ADR-0220's lexical `knowledge_search`. `harness/knowledge/embed_config.ts` resolves the stored config + vault secret → an `ApiEmbedder` (fail-safe null → lexical). settings_store `embeddings` config + a dedicated "Semantic search" Settings card (local Ollama for air-gap, or OpenAI/Azure; key vaulted, injected as `LUCID_EMBEDDINGS_KEY` by main). `kb_store.ts` gains a per-KG vector store (sibling `_vec.duckdb`) + `vectorDatasetFor` (model+dim-scoped so vector spaces never mix). `/api/kb/ingest-batch` now ALSO embeds each source into the KG's vector store (scan-gated `ingestText`, best-effort, never fails the compile job); `/api/kb/retrieve` auto-upgrades to `mode:"hybrid"` when an embedder + embedded chunks exist — so `knowledge_search` transparently gains semantic recall, else stays lexical. No WASM bundling.
- **verified:** 15 tests across the new units — ApiEmbedder (9: URL/auth/parse/embed/fail-loud/bearer), embed_config resolver (3: fail-safe null, bearer-needs-secret), desktop vector-store glue (3, against REAL DuckDB: per-KG store, find-or-create/reuse dataset, different-model→new-dataset, store→retrieve round-trip). 48 KB/knowledge tests green; tsc clean across embed_config/api_embedder/kb_store/dev/main/settings_store/bridge/app; license clean. LIVE gap: this profile has only XAI/ElevenLabs keys (no `/embeddings`), so the end-to-end path (real endpoint → embed at ingest → hybrid retrieve in the app) needs an OpenAI/Azure/local-Ollama endpoint configured to exercise.
- **next:** a "Test endpoint" button on the card (a 1-vector probe) for instant config feedback; a "re-index existing KGs" action after enabling; addon-side shared server vector DB (pgvector/Qdrant) for team-scale corpora.

**Semantic search: Test-endpoint + Re-index (ADR-0221 finishers) — 2026-07-14**
- **shipped:** two UX finishers on the Semantic search card. TEST ENDPOINT: `probeEmbeddings` + `/api/embeddings/test` do a one-vector probe against the ENTERED values (incl. an inline key, so it works before save/relaunch) and DISCOVER the model's dimension - the button auto-fills Dim so a mismatch can't silently break retrieval. RE-INDEX: `/api/embeddings/reindex` rebuilds every KG's semantic index from its compiled PAGES via a new shared, idempotent `syncVectorIndex` (clear-then-embed) - so a KG compiled before semantic search was enabled gets indexed without re-ingesting. Ingest now uses the SAME routine (embeds compiled pages, not raw source), keeping ingest + re-index consistent; added `KnowledgeStore.clearChunks` for idempotency.
- **verified:** +2 probe tests (discovers dim; throws on non-2xx / empty vector) → 14 green across api_embedder + the DuckDB vector-store glue; tsc clean across api_embedder/store/dev/bridge/app; license clean. Live gap unchanged (no `/embeddings` endpoint on this profile) - Test + Re-index exercise the same path and need a real/local endpoint configured to run end-to-end.
- **next:** RAG arc is feature-complete for the public repo (lexical + semantic + BYO-embeddings + test/re-index); addon-side shared server vector DB (pgvector/Qdrant) + live API connectors remain.

**Shared-session viewer: thinking + tools, full-window layout (ADR-0222) — 2026-07-14**
- **shipped:** two fixes to the P-COLLAB guest "Watching" panel. (1) The viewer now sees the host's THINKING (collapsible dim block) + TOOL calls (compact name+path chips) + subagent delegations + blocks - not just the final answer; the guest event handler used to drop those. It's still a pure read-render of frames the host ALREADY relays (the full ChatEvent stream is teed at app.ts:1459) - no protocol change. (2) When watching, the modal fills the window (`calc(100vw/vh - 56px)`, ~28px border), drops `max-width:520px`, hides the redundant icon/title/desc (the join-head labels the session), and cuts padding (22px→~12px, transcript 340px→flex:1) - maximizing real estate to follow/drive the session. The connect (paste-link) step stays the small centered modal; X / Leave / Escape still close.
- **verified:** tsc clean on app.ts + styles.css; the change is additive read-rendering + CSS. Live 2-party visual (host + guest + relay) can't be driven headlessly, so the fullscreen layout + activity rendering want a real shared session to eyeball; logic reviewed (accumulate thinking/tools on pending → fold into the turn on done; `.watching` class toggled on connect→live).
- **next:** optional - a per-turn "N tools · thought Ns" collapse for long histories; mirror the host's richer tool chips (diffstat) if watchers want more detail.

**Turn ON the KG-pack marketplace in the client (ADR-0223) — 2026-07-14**
- **shipped:** the one missing wire that makes "Get pack" reach STRIPE instead of opening the product web page. The whole marketplace was built but dark: the client (entitlement gate + Firebase provider + sign-in + gated install) AND the backend (addon `entitlements/functions`: createCheckout/stripeWebhook/getEntitlement/getPackDownload, project lucid-agent) are done and their pack-id/callable contracts match — but `market_boot.ts` reads `globalThis.__LUCID_MARKET__` and nothing set it, so it booted fail-closed "off" and packs opened `lucid-agent.web.app` / `aiworkshopapps.com`. New `desktop/market_config.ts` (pure) + a preload `contextBridge.exposeInMainWorld("__LUCID_MARKET__", …)` inject the config; default = firebase at the official PUBLIC endpoints (no secrets ship), with `LUCID_MARKET_MODE=off` to revert and URL overrides. Graceful before deploy (unreachable checkout → storefront fallback).
- **verified:** 4 `desktop/market_config.test.ts` (default firebase endpoints; off/stub modes; URL overrides + trailing-slash trim); tsc + license clean. Live checkout needs the addon backend deployed (functions + Stripe prices keyed by the catalog lookup_keys + pack upload + the /signin page) — an ops step; the client is now ready.
- **next (ops, addon repo):** `firebase deploy` functions + hosting, set Stripe secrets, create the Product+Price per pack (lookup_key = KGP-*; product metadata pack_id = storefront id) + webhook, upload the `.lkgpack.zip` packs to `gs://lucid-agent-packs/packs/`.

**Fix AskSage model-picker hang + CUI/DoD banner behavior (ADR-0224) — 2026-07-15**
- **shipped:** (1) model picker no longer hangs on "updating…" - the ADR-0217 lockdown model-clamp in `ensureSession` was AWAITED and, with lockdown on, a slow model switch blocked the config load the picker waits on; now fire-and-forget (`void`), the `prompt()` clamp stays the fail-closed guarantee. (2) CUI banner centered (`justify-content:center`; it's in the chat column so it re-centers when panels open). (3) DoD/STIG banner shows ONLY in lockdown (was: whenever a gov key was configured, which made it feel like lockdown defaulted on) - at launch if persisted on + when lockdown is toggled on; lockdown itself stays opt-in. (4) client pack licensing synced to subscription for EVM + Capture (matches deployed Stripe, ADR-0223).
- **verified:** tsc clean on acp_backend/app/styles/kg_packs; 20 checker_model + session_mode tests green. Live picker behavior needs a rebuilt app to confirm (ships next build); WORKAROUND for the current install: Settings → AskSage → turn Lockdown OFF, then Refresh models.
- **next:** if the picker still hangs with lockdown OFF after rebuild, capture the dev-mode Logs to pin any remaining cause.

**"Lucid Agent" rename + air-gap-capable installer (ADR-0225) — 2026-07-14**
- **shipped:** (1) display name `LucidAgentIDE` → **Lucid Agent** (productName + nsis.shortcutName + artifactNames `LucidAgent-*` + window/`<title>`; `appId` UNCHANGED so auto-update + userData survive). (2) Vendored omp offline: re-include `node_modules/.bin/omp*` + `runtime.ts findOmp()` resolves the bundled shim first (relative path to the shipped package, needs only bundled bun) → no `bun add -g` at first run. (3) Bundled a relocatable CPython 3.12.13 (`fetch-runtimes.ts`, SHA-pinned/fail-closed, cross-checked vs the release's published SHA256SUMS) into `runtimes/python-<plat>-<arch>/`; `runtime.ts bundledPython()` resolves it first so `uv venv` never runs offline (scanner has zero pip deps). `license_headers.ts` excludes the vendored `runtimes/` tree.
- **verified (Windows, live):** fetch downloaded+SHA-verified+extracted CPython (fixed a GNU-tar `C:\` drive-as-host path quirk); `python.exe`→3.12.13; scanner runs offline under it (clean→`[]` no false positives, dirty→flags U+200B + U+0430); bundled `.bin/omp.exe`→`omp/16.1.20`; desktop tsc + license-header check green. (Pre-existing unrelated `lucid_acp` "theme extension" test fail confirmed on the clean tree.)
- **next:** packaged `electron-builder` OFFLINE smoke test per OS (assert zero egress + scanner gate green cold) - especially electron-builder's handling of the POSIX `.bin/omp` symlink shim (Windows ships a real omp.exe); then decide on cutting an air-gap installer.

**Fix v1.11.5 rename regressions → v1.11.6 (ADR-0225 correction) — 2026-07-14**
- **shipped:** v1.11.5's `productName: "Lucid Agent"` broke the tag build + was PULLED (marked prerelease; "latest" fell back to v1.11.4). Two bugs from that one field: (1) Linux **rpm** failed - `/opt/Lucid Agent` has a space, `rpmbuild` %files can't; (2) worse, productName IS `app.getName()` on packaged Electron, so userData would move → every user's settings/vault orphaned on upgrade (appId does NOT govern userData - my earlier claim was wrong). FIX: productName stays `LucidAgentIDE` (userData + rpm path unchanged); "Lucid Agent" is display-only per OS - nsis.shortcutName (Win), extendInfo.CFBundleDisplayName not CFBundleName (mac), linux.desktop.Name (Linux), window/`<title>`.
- **verified:** package.json valid + fields correct; `about.test.ts` (version single-source, now 1.11.6) green; desktop tsc + license-header check green. The air-gap gate itself PASSED on Windows in the v1.11.5 run (omp shim + bundled-Python scanner ran from `resources/`); the Linux failure was the rpm step, BEFORE the gate (gate skipped).
- **next:** tag v1.11.6 → confirm all 3 OSes build + pass the air-gap gate (esp. Linux rpm + the POSIX omp symlink); then it becomes "latest" and supersedes the pulled v1.11.5.

**Fix OAuth "Disconnect" not clearing (branch fix/oauth-disconnect) — 2026-07-15**
- **context:** OAuth logins wouldn't clear from Settings→Providers, "especially after a reinstall." Root cause: `~/.omp/agent/agent.db` (auth_credentials) lives in HOME, so reinstalling LUCID doesn't touch it; and Disconnect only ran `omp auth-broker logout` (soft-disable, sets `disabled_cause`) via a 4s spawn whose exit status was IGNORED — on a cold/reinstalled box that spawn times out, nothing changes, yet the route reports success. Plus orphan oauth rows (a broker id with no descriptor e.g. `google-antigravity`, or a `canOauth:false` provider e.g. `perplexity`) had NO Disconnect button. The token blob is in the row's `data` column, so a real clear = deleting the row.
- **shipped:** `auth_vault.ts` gains `disconnectCredential(provider)` (DELETEs the provider's `credential_type='oauth'` row — authoritative, not soft-disable) + `clearAllOauthCredentials()` (deletes every oauth row incl. orphans; API keys untouched). `/api/auth/logout` now runs omp logout best-effort THEN the authoritative delete; new `/api/auth/logout-all` route + `bridge.oauthLogoutAll()`. Settings→Providers gains an always-available "Sign out of all providers" footer (confirmed, danger) — the reliable post-reinstall reset that also clears orphans.
- **verified:** 9/9 auth_vault tests (delete scoped to oauth, API keys survive, clear-all incl. orphan, fail-closed on missing vault/provider); desktop tsc + license-header check green. Live dev-server smoke on this box: per-provider Disconnect shows for the 3 active logins, the orphans (perplexity/google-antigravity) correctly have none, the "Sign out of all" button renders + its confirm dialog appears — and I did NOT press it, so the real credentials (6 rows) are all still intact.
- **next:** none blocking; fold into the next release once the picker root-cause + air-gap are ready. (User can already use it on the branch to clear their stuck logins.)

**Fix "Connect via OAuth" opens no browser (branch fix/oauth-disconnect) — 2026-07-15**
- **context:** after clearing logins, clicking "Connect via OAuth" showed a toast but no browser opened. Confirmed live: `omp auth-broker login <id>` DOES print the sign-in URL to stdout ("Open this URL in your browser: https://…") and does NOT open a browser itself — opening it is entirely the desktop's job. The desktop did that ONLY via `window.open(url)` → `setWindowOpenHandler` → `shell.openExternal`, which can silently no-op (popup-timing in a web/dev context, browser-association issues, handler not firing) — and the URL was never shown, so the user was stuck with no way to reach it.
- **shipped:** a real IPC path — `lucid:openExternal` (main, http/https-only) → preload → `bridge.openExternal()` → `shell.openExternal`. New `openAuthUrl()` tries the IPC first, falls back to `window.open`. And the safety net: the OAuth toast now SURFACES the URL with "Open sign-in page" (a real click → never popup-blocked) + "Copy URL" actions, and persists — so even if auto-open fails the user can always open/copy it. Applied to redirect-flow, device-flow, and the Copilot two-step.
- **verified:** desktop tsc + license-header check green; dev-server load smoke clean (no console errors); omp URL-to-stdout confirmed by running the broker directly. The Electron `shell.openExternal` path mirrors the proven `revealPath` IPC; final confirm is on the user's packaged app.
- **next:** user tests on the running app; if `shell.openExternal` still doesn't surface a browser there, the visible URL is the guaranteed fallback.

**ROOT CAUSE: OAuth broker used the wrong omp — dev.ts ompBin ignored LUCID_OMP_BIN (branch fix/oauth-disconnect) — 2026-07-15**
- **context:** live-tested in the running Electron app — clicking Connect showed the NO-URL toast ("Follow omp's prompt in the GUI server window", OK-only), i.e. `startOauthBroker` got `r.url` empty AND no stdout. So openExternal was a red herring (nothing to open). Root cause: `dev.ts ompBin()` resolved omp ONLY from `~/.bun/bin/omp.exe` → bare `omp`, IGNORING `LUCID_OMP_BIN` — unlike `acp_backend.ts` which honors it. So the model list (acp_backend → bundled omp 16.1.20) worked, but OAuth used a stale `~/.bun` omp (16.0.8) that produced no URL in the app's env. Both omp versions print the URL to stdout in isolation, so it's the resolver/env mismatch — the two paths must use the SAME omp.
- **shipped:** `dev.ts ompBin()` now checks `LUCID_OMP_BIN` FIRST (mirrors acp_backend) → OAuth uses the same known-good omp as the models. Hardened `startOauthBroker`: scan BOTH streams for the URL, require a COMPLETE url (whitespace-terminated → no chunk-boundary truncation), and a dev-mode diagnostic log (`[oauth] no URL from broker … via <omp> stdout=… stderr=…`) so any recurrence is pinpointable from the terminal.
- **verified:** desktop tsc + license green; confirmed both omp builds print the URL to stdout; the new complete-URL regex extracts the full 444-char URL (identical to greedy, no truncation). End-to-end click needs the rebuilt app (user), but OAuth now resolves the same omp that already works for models.
- **next:** user runs `bun run start` again → Connect should open the browser; if not, the terminal's `[oauth]` diagnostic line tells us exactly which omp + what it emitted.

**Silent overloaded-model failures now surface + recommend a fallback (P-NORESP.1) — 2026-07-15**
- **context:** user report — when AskSage is overloaded, new gov models (GPT-5.6, even 5.5, likely a shared GPU pool) fail SILENTLY: no answer, empty bubble, no signal. Wanted a clear "no response from [Provider], may be overloaded/oversubscribed" message + a recommended fallback (a lower model in the same family or an equivalent from another provider) they can switch to.
- **shipped:** DETECT in `acp_backend` — a turn that ends `!errored` but emitted NO content (no token/thinking/tool) → new `no-response` ChatEvent (+ omp stopReason). RECOMMEND in `model_families.ts` `recommendFallbacks()` (pure, tested) — a LOWER version in the same family (GPT-5.6→5.5) AND a cross-provider equivalent (Claude first = different pool), both matching the failed model's gov-ness so a lockdown session stays gov-routed. RENDER in `app.ts` — the empty bubble becomes a `.noresp-card`: "No response from AskSage (gov gateway) — <model> may be overloaded…", with one-click "<fallback> & retry" buttons (switch model + re-send the last prompt) + "Pick another model…". `providerLabelOf()` names the provider.
- **verified:** 60/60 model_families tests (incl. lower-version pick, cross-provider Claude, gov-only under lockdown, sibling fallback, null when nothing suitable); desktop tsc + license green; dev-server load clean; injected the card markup live — amber accent + 2 recs + picker render correctly. Live overload path is deterministic in the backend condition; the user will see it on a real overloaded model.
- **next:** optional polish — dedupe if the same model no-responds twice in a row; a settings toggle to auto-retry once on a fallback instead of asking.

**P-NORESP.1 follow-up: also catch ERRORED-with-no-output (Claude Fable 5 / HTTP errors) — 2026-07-15**
- **context:** user report — Claude Fable 5 (gov) also "not working," wants to drop to 4.8 Opus. Two failure modes on the AskSage Claude route: a 200-with-empty-body (already caught) and an HTTP error (429/5xx/4xx) that THROWS → `errored=true`, which the original `!errored` condition skipped, so Fable 5 showed "[agent unavailable]" with NO fallback card.
- **shipped:** detection now fires on `!sawOutput && !lockBlocked` (covers BOTH silent-empty AND errored-with-no-output); the catch no longer prints a bare "[agent unavailable]" when there's no output (it would clobber the card) — it defers to the notice, which carries the error as `reason`. The card shows the actual reason for an errored failure vs "overloaded" for a silent one. Confirmed: a failed Fable 5 → recommends **Claude 4.8 Opus gov** (the "48" id outranks the "5" siblings in the version heuristic) + a cross-provider GPT.
- **verified:** 61/61 model_families tests (added the Fable-5 → 4.8 Opus case); desktop tsc + license green; dev load clean. Lockdown block + partial-failure (output then error) behavior unchanged.

**CRITICAL air-gap bug: bundled bun name-suffixed → omp shim can't find it (v1.11.5 regression) — 2026-07-15**
- **context:** user installed a local build on a CLEAN Windows system (no global bun); got "bun is not installed in %PATH%" on OAuth AND no AskSage models. Root cause: omp's `.bin/omp` shim (`.bunx`) shells out to a PLAIN `bun`/`bun.exe` on PATH, but fetch-runtimes bundles it name-suffixed (`bun-win32-x64.exe`). So the air-gap omp-vendoring (ADR-0225 findOmp→bundled shim) can't start omp without a GLOBAL bun — defeating the whole air-gap point. Dev boxes + the CI runner have global bun, which masked it (my smoke test kept system PATH → found the global bun).
- **shipped:** `fetch-runtimes.ts` now also emits a plain `bun[.exe]` alongside the suffixed binary (runs even when the suffixed one is cached, so a rebuild self-heals). `airgap-smoke.ts` hardened: asserts the plain `bun` alias exists AND runs the omp shim with global bun SCRUBBED from PATH — so it fails if the bundle can't stand alone.
- **verified:** plain `runtimes/bun.exe` created + valid (`--version` 1.3.14); desktop tsc clean. Rebuilding the installer with the fix; the real proof is the corrected installer on the clean system (no global bun) — omp should start, AskSage models load, OAuth open.
- **workaround (current broken build):** install global bun (`powershell -c "irm bun.sh/install.ps1|iex"`) → restart LUCID → omp resolves it. Logs: `%APPDATA%\LucidAgentIDE\engine.log`.

**P-REMOTE remote mobile access: plan + ADR (ADR-0226) — 2026-07-15**
- **shipped:** ADR-0226 (SCOPE/PLAN) — drive the running desktop LUCID from a phone browser (iPhone Chrome) as a P-COLLAB GUEST: content plane reused wholesale (E2E AES-256-GCM room, key in the link fragment only, guest prompts run the host's own fail-closed gate per ADR-0198, prefer-P2P WebRTC upgrade per ADR-0200–0202). New pieces phased: P-REMOTE.1 relay Firebase ID-token gate (Google OAuth aud/iss `lucid-agent` + email allowlist, zero-dep JWKS verify), .2 Cloud Run deploy in `lucid-agent` (max-instances=1, 60-min WS cap → heartbeat/reconnect/room re-claim; manifests in the add-on repo), .3 guest PWA on Firebase Hosting bundling the SAME desktop/collab modules, .4 persistent vault-stored pairing + QR + opt-in auto-share (the quarantined contracts increment for any new EventName), .5 remote approvals deferred opt-in. Four independent gates: OAuth rendezvous → room key → write token → host gate; relay/GCP compromise yields ciphertext only.
- **stubbed:** implementation deferred to the phased increments — no code yet. Named open questions: desktop Firebase ID-token refresh custody, host re-claim vs 4009, QR pairing UX. TURN server explicitly optional (relay is the guaranteed path).
- **next:** P-REMOTE.1 — `RELAY_AUTH=firebase` mode on tools/relay/ with offline fake-JWKS tests (expired/wrong-aud/unlisted-email/tampered refused; valid admits; anonymous mode byte-identical).

**P-REMOTE goes multi-tenant: paid tier + admin dashboard (ADR-0227, amends ADR-0226) — 2026-07-15**
- **shipped:** ADR-0227 (SCOPE/PLAN amendment). Remote Access becomes a Stripe subscription (~$9.99/mo) on the ALREADY-DEPLOYED marketplace entitlement rails (`createCheckout`/`stripeWebhook`/`getEntitlement`; `grantEntitlement` = the admin comp path) — entitlement delivered as Firebase custom claims (`premium`/`admin`, admin rides free + implies premium) so relay admission is pure token verification (zero DB reads, zero new deps); the Cloud Run 60-min cap doubles as the revocation channel (hourly re-verify ⇒ lapsed/revoked dies ≤1h). AUTH CORRECTION to ADR-0226: ID token in a first `auth` control frame, NEVER the WS query param (bearer tokens in request logs). Google-provider-only enforced in the verified token (`sign_in_provider==="google.com"` + `email_verified`). New increments: P-REMOTE.6 (paid tier + PWA subscribe path), P-REMOTE.7 (metadata-ONLY admin plane — relay writes session docs to Firestore via zero-dep REST/ADC fail-SOFT, GeoLite2-in-container coarse location, 30-day TTL retention, `lucid-agent.web.app/admin/` dashboard gated server-side on the `admin` claim: active sessions w/ email + location, usage counts, comp/revoke). Payment ≠ trust: premium buys rendezvous admission only — room key/write token/host gates unchanged.
- **stubbed:** implementation deferred to the increments — no code yet. Open: GeoLite2 EULA vs DB-IP lite, retention default + admin purge, post-checkout claim-refresh UX, Stripe customer-portal link in the PWA.
- **next:** P-REMOTE.1 (as amended): first-frame auth + provider/claims checks + per-uid quotas, offline fake-JWKS test matrix (expired/wrong-aud/unverified/non-Google/no-claim refused; premium/admin/allowlisted admitted; unauthenticated socket reaped).

**P-REMOTE.1: the relay identity gate (ADR-0226/0227) - 2026-07-15**
- **shipped:** the hosted-rendezvous auth gate, zero new deps. `desktop/collab/relay_auth.ts` - a WebCrypto RS256 Firebase ID-token verifier (JWKS fetched + cached per Cache-Control, injectable fetch/clock; checks alg/kid/signature, exp/iat with skew, aud/iss = the project, `email_verified`, `sign_in_provider==="google.com"` - the Google-OAuth-only rule enforced in the token) + pure `admissionDecision` (`admin` implies premium, rides free; `premium` claim = the paid tier; else the `RELAY_ALLOWED_EMAILS` self-host allowlist; else 4403) + `authFromEnv` (fail-LOUD on a half-configured gate). `relay_server.ts` gains the injected `auth` option (mirrors `authorizeBind`): admission moved out of open() into `admit()`; in gated mode the FIRST frame must be `{"t":"auth","token"}` (never a URL param - request logs), silent sockets reaped at a deadline (4401), verified-but-unentitled 4403, per-uid quotas 4429 (rooms/uid + connects/min, pruned); auth-ok confirms admission. FIXED a pre-existing bug the gate exposed: close() tore down a room on ANY host-role socket close - a REJECTED duplicate host (4009) nuked the legit room + kicked its guests; now only the owning socket (`room.host === ws`) tears down. `tools/relay/serve.ts` wires `RELAY_AUTH=firebase` + project/allowlist/JWKS/deadline env (exit 2 on bad config); README documents the mode + close codes.
- **verified:** +30 tests green (relay_auth.test.ts 22: the offline fake-JWKS matrix incl. forged/payload-swapped signature, alg-confusion, unknown kid, dead/500 JWKS FAIL-CLOSED, cache single-fetch, truthy-but-not-true claim refused, env seam; relay_auth_gate.test.ts 8: real-socket first-frame flow + opaque forwarding, binary/junk-before-auth 4401, deadline reap, 4401/4403/throwing-verifier fail-closed, room + rate quotas 4429, anonymous mode unchanged + the 4009-room-survival regression). `make demo-P-REMOTE.1` green (7 checks: real verifier against a REAL local JWKS endpoint gating a REAL relay). Full first-party suite 2353 pass / 7 fail - the SAME 3 pre-existing/environmental files as the session baseline (auth_status env leak, fs_browse linux-fixture-on-Windows x5, lucid_acp theme extension per ADR-0225); collab suite 200/200; root tsc clean on my files (output_*_proto.ts pre-existing); license check green.
- **next:** P-REMOTE.2 - Cloud Run deploy (add-on manifests) + desktop host wiring: CollabSocket sends the auth frame + treats 4401/4403/4429 as fatal, market_auth ID-token custody/refresh, heartbeat + reconnect + 4009 re-claim for the 60-min cap; managed `collab.allowedRelays` entry for the hosted URL.
**P-REMOTE.2: hosted-rendezvous transport hardening (ADR-0226/0227) - 2026-07-15**
- **shipped:** the client + server pieces that make the Cloud-Run relay usable from a phone. `relay_client.ts` (CollabSocket): an injected `authToken` provider called on EVERY (re)connect - sends `{t:'auth',token}` as the FIRST frame and HOLDS all traffic (new `#ready` flag) until the relay's `auth-ok`, so a frame whose async seal() resolves after the socket opens but before auth can no longer leak unauthenticated (bug the client test caught); a null token is TERMINAL (sign-in), and 4401/4403/4429 join the fatal-close set (no retry storm against the gate); a 45s string keepalive rides under the relay's 120s idle ceiling (ignored post-auth, works gated + anonymous). `relay_server.ts`: identity-based host RE-CLAIM - the SAME uid replaces its own lingering socket (adopt-first, so the stale close can't nuke the room) or resumes a room held in a GRACE window (`reclaimGraceMs`, default 30s) after a drop; guests stay connected across the hourly 60-min-cap reconnect, roster is resent to the new host socket; a different uid still gets 4009; grace expiry tears down (4001); anonymous mode unchanged (immediate teardown, no grace). Resolves the ADR-0226 open question 'host re-claim vs 4009'. Cloud Run deploy bundle in the add-on mirror (`lucidaddon_audit/iac/remote-relay/`: service.yaml maxScale=1/timeout=3600/concurrency=250/sessionAffinity, deploy.sh Cloud-Build + replace, README) - no secrets (public Firebase config + non-secret allowlist).
- **verified:** +11 tests (relay_client_auth.test.ts 6: auth-frame-first + hold-until-auth-ok, fresh token per reconnect, null-token terminal, 4401/4403/4429 fatal no-retry, keepalive cadence + stop-on-close, anonymous unchanged; relay_auth_gate.test.ts +5: same-uid live re-claim with roster resend + seamless guest delivery, different-uid 4009, grace HOLD + owner resume, grace EXPIRY kicks guest 4001, real-CollabSocket authToken E2E) -> collab suite 211/211. `make demo-P-REMOTE.2` green (4 checks: real CollabSocket host+guest first-frame auth, host drop held in grace, same-account re-claim resumes delivery invisibly, null-token terminal). Full first-party 2364 pass / 7 fail - the SAME pre-existing/environmental set (auth_status env leak, fs_browse linux-fixture-on-Windows x5, lucid_acp theme ext); tsc clean on collab files; license green.
- **next:** P-REMOTE.2 finisher (2b): market_auth ID-token refresh custody on the desktop (freshIdToken via securetoken exchange; the /signin deep link handing over a refresh token) + threading CollabSocket.authToken through dev.ts's makeTransport + a managed collab.allowedRelays entry for the hosted URL. Then P-REMOTE.3 (the phone guest PWA on Firebase Hosting) and P-REMOTE.6 (the Stripe paid tier + claims).

**P-REMOTE.4a: the invite link as a scannable QR (ADR-0226/0227) - 2026-07-15**
- **shipped:** a first-party, DEPENDENCY-FREE byte-mode QR encoder (`desktop/collab/qr.ts`) - chosen over an npm lib because the repo is air-gap-conscious (ADR-0225) + Unicode-security-obsessed: GF(256) + Reed-Solomon, spec module placement (finders/timing/alignment/format/version), penalty-scored masking, EC level M, versions 1..10 (up to 213 bytes - any invite link fits). Emits a module matrix + a self-contained crisp SVG (`qrSvg`, no <script>/external refs; the link is encoded as geometry, never interpolated as text). Wired into the Share panel (`app.ts shareBodyHtml`): the active share now renders a QR of the same view/edit invite link under the copy row (white quiet-zone card + 'Scan with your phone camera'), guarded so an over-capacity/empty link degrades to no-QR (Copy still works). New `phone` icon; `.share-qr` styles. Reusable by the phone PWA (P-REMOTE.3) later - same file, no DOM dep. The room key rides IN the link fragment, so scanning carries the E2E secret exactly like copying - nothing extra touches a server.
- **verified:** 12 `qr.test.ts` - the correctness anchor is the Reed-Solomon SYNDROME being zero at a^0..a^(ecLen-1) for every encoded block (a valid codeword by construction, independent of placement); the hardcoded format(15,5)+version(18,6) tables cross-checked against an independent BCH computation; exact finder/timing/dark-module structure; version selection + over-capacity throw; safe-to-inline SVG + transparent-bg (caught + fixed a real bug: `opts.light ?? '#fff'` clobbered null); a deterministic matrix fingerprint locks the full pipeline. `make demo-P-REMOTE.4a` green (5 checks + prints a genuinely scannable ANSI-colored QR to the terminal and writes an SVG/HTML). Collab suite 223/223; full first-party 2376 pass / 7 fail (same pre-existing set); root tsc clean on qr/app/icons; license green. NOTE: final scan acceptance is the user's phone (no reference decoder bundled); the encoder follows the ISO/IEC 18004 reference algorithm and the preview HTML is open for scanning.
- **hygiene:** an over-eager escape-repair sweep converted 16 unrelated `\u00b7`/`\u201c`/`\u2026` STRING-LITERAL escapes in app.ts to literal chars; caught it via git diff and RESTORED app.ts to baseline, re-applying only the +8 QR-wiring lines (ASCII-only, matching app.ts's escaped-string convention). Confirmed literal em-dashes in .ts COMMENTS are the established codebase convention (relay_client.ts et al.), so the relay/qr/demo comments are consistent.
- **next:** P-REMOTE.3 (the phone guest PWA on Firebase Hosting, reusing desktop/collab + this QR), P-REMOTE.2b (market_auth token refresh + dev.ts makeTransport threading + managed collab.allowedRelays), P-REMOTE.6 (Stripe paid tier + claims), P-REMOTE.7 (admin dashboard).

**P-REMOTE.3: the phone guest PWA on Firebase Hosting (ADR-0226/0227) - 2026-07-15**
- **shipped:** a standalone mobile web app (`tools/remote-pwa/`) that sees + drives a running desktop LUCID from an iPhone, built from the SAME `desktop/collab` modules (CollabSocket + CollabGuest + crypto + link) so the E2E protocol is single-sourced, never reimplemented (confirmed all are browser-safe: WebCrypto, btoa/atob, WebSocket - no node/Buffer). New PURE `desktop/collab/pwa_view.ts` - the compact mobile viewer: a `foldEvent` reducer (token deltas coalesce into a streaming answer, thinking/tool/subagent/block as their own items, `done` reconciles the lossy stream) + HTML renderers that ESCAPE every host-authored string (the phone never turns host/echoed content into markup). The app: Google sign-in via `firebase_auth.js` (CDN-loaded, NEVER bundled) -> a Firebase ID token; invite read from the URL FRAGMENT (`#roomId.secret`, key never hits a server); connect to the hosted Cloud Run relay (config.js `relayWsBase`) presenting the token as the first frame (P-REMOTE.1); render the live turn + a prompt bar for full-link guests (prompts run on the desktop through its own gate). Mobile-first `index.html` (self-contained CSS, sign-in/session/fatal views), `manifest.webmanifest` + `sw.js` (installable; caches ONLY the shell, never tokens/session) + `icon.svg`; `build.ts` bun-bundles to a 12 KiB `app.js` + shell; dedicated DOM `tsconfig.json` (the browser entry is excluded from the root Bun-scope config). `config.example.js` carries PUBLIC values only (relay wss + Firebase web config). Add-on deploy bundle `lucidaddon_audit/iac/remote-pwa/` (firebase.hosting.json with /remote security headers + sw no-cache, deploy.sh, README). Made `relay_client.ts` #keepalive type cross-env (`ReturnType<typeof setInterval>`, was Bun-only `Timer`) so it typechecks in the browser.
- **verified:** 10 `pwa_view.test.ts` (reducer folds token/thinking/tool/subagent/block/done + no-response, done reconciliation, a tool interrupts the answer stream; rendering ESCAPES hostile answer/thinking/tool/subagent/block/note/header/prior-turn; statusLabel phase+readOnly+note) -> collab suite 233/233. `make demo-P-REMOTE.3` green (4 checks): a real CollabSocket+CollabGuest 'phone' (the exact modules the PWA bundles) authenticates to a REAL gated relay, goes live against a real CollabHost, and pwa_view folds/renders the host's thinking + tool chip + reconciled streamed answer with hostile content escaped, then reflects host-stop. PWA bundles clean (12 KiB, browser target). tsc clean on all 3 configs for my files (root 6 pre-existing proto errors only; pwa 0; desktop 0); full first-party 2386 pass / 7 fail (same pre-existing set); license green. LIVE gap (QA, not headless): the real Firebase sign-in popup + a live phone against the deployed Cloud Run relay - needs the deployed backend + a real device.
- **next:** P-REMOTE.2b (desktop side: market_auth ID-token refresh custody + thread CollabSocket.authToken through dev.ts makeTransport + mint the browser invite link as https://lucid-agent.web.app/remote/#roomId.secret + managed collab.allowedRelays entry), then P-REMOTE.6 (Stripe paid tier + premium/admin claims) and P-REMOTE.7 (metadata-only admin dashboard).

**P-REMOTE.2b: token refresh custody + PWA-pointing invite links (ADR-0226/0227) - 2026-07-15**
- **shipped:** the desktop wiring that makes the phone chain actually usable. (A) `market_auth.ts` gains refresh custody: `parseAuthCallback` reads an optional `refresh` param, `TokenRecord.refreshToken`, and a new `freshIdToken({apiKey,fetchImpl,skewMs})` that returns the stored ID token while >skew from expiry, else EXCHANGES the refresh token via Google's securetoken endpoint (`securetoken.googleapis.com/v1/token`, grant_type=refresh_token), re-persists, and FAILS CLOSED to null (signed out / no refresh token / non-2xx / throw) - so the desktop relay reconnect always presents a live token past the 1h cap. (B) PWA-pointing browser links: new pure `link.ts formatPwaLink(pwaBase,roomId,key,writeToken?)` (secret in the fragment); `RelayTarget.pwaBase?` + `StartP2PHostOpts.pwaBase?`; `manager.start()` + `collab_p2p.startP2PHost()` mint the browser link at the PWA when pwaBase is set, CARRYING the write token on an EDIT share (phone can drive) - else the legacy relay-host link (unchanged). `dev.ts effectiveRelay()` sources pwaBase from `LUCID_REMOTE_PWA_BASE` (MAIN->dev-child env, the provisioning channel), so only a provisioned hosted build points at the PWA; flows to both the backend share + the renderer P2P path via `/api/collab/status`.relay (bridge `CollabRelay.pwaBase?`). The Share panel QR now encodes the PHONE `browserLink` (the openable PWA/browser form) not the relay-path link (that stays the copy field for a desktop LUCID guest). (C) governance: `authorizeRelayConnect` returns ok when `allowedRelays` is unset, so the hosted relay connects by default (unmanaged); a managed org adds it via `CollabAllowedRelays` - documentation-only, no code change.
- **verified:** +6 tests - market_auth (+4: parse refresh, freshIdToken returns-stored-no-fetch / near-expiry-exchange+persist / fail-closed matrix), link (+1: formatPwaLink view+full fragment round-trip), manager (+1: pwaBase browser link is PWA form + write token when editing, legacy without). `make demo-P-REMOTE.2b` green (5 checks: securetoken refresh persist + fail-closed; EDIT/VIEW PWA links + write-token carry; no-pwaBase legacy). collab suite 235/235; full first-party 2392 pass / 7 fail (same pre-existing set: auth_status env leak, fs_browse linux-fixture-on-Windows x5, lucid_acp theme ext); tsc clean (root 6 pre-existing proto only; desktop 0; pwa 0); license green. LIVE gap: the real securetoken exchange + a provisioned build with LUCID_REMOTE_PWA_BASE set = QA (needs the deployed backend + the /signin page handing over a refresh token).
- **stubbed / deferred to P-REMOTE.2c:** delivering the refreshed token to the BACKEND host CollabSocket + threading `authToken` through dev.ts `makeTransport` (renderer freshIdToken -> token'd POST /api/collab/token -> a pure expiry-aware RelayTokenCache -> makeTransport authToken; null -> anonymous, unchanged). The /signin page must start returning a `refresh` param for freshIdToken to renew (addon).
- **next:** P-REMOTE.2c (backend token delivery + authToken threading), then P-REMOTE.6 (Stripe paid tier + premium/admin claims) and P-REMOTE.7 (metadata-only admin dashboard).

**P-REMOTE.2c: backend token delivery to the host relay socket (ADR-0226/0227) - 2026-07-15**
- **shipped:** the last link of the auth loop - a provisioned desktop now authenticates to the GATED hosted relay. New PURE `desktop/collab/relay_token_cache.ts RelayTokenCache` (set/get/clear + `present`, injectable clock, skew-aware): the renderer PUSHes a fresh Firebase ID token to a new token-gated `POST /api/collab/token` (dev.ts), which caches it; the host `CollabSocket` (dev.ts `makeTransport`) reads it via `authToken` - but ONLY when `effectiveRelay().gated` (RELAY_AUTH), so the anonymous embedded/self-hosted/public path is UNCHANGED (wiring authToken unconditionally would hang a socket waiting for an `auth-ok` an un-gated relay never sends - caught + gated). `gated` derives from `LUCID_RELAY_GATED` (explicit) else `!!pwaBase` (the hosted rendezvous). Renderer glue: `market_config.firebaseApiKey` (public web key, `LUCID_FIREBASE_API_KEY`) + `market_boot.marketFreshCredential()` (silent securetoken refresh via 2b's `freshIdToken` + `MarketAuth.currentExpiresAt()`) + `bridge.collabPushToken` + app.ts `startRelayTokenPush`/`stopRelayTokenPush` (seed before the host connects, refresh every 10 min while a RELAY share is active, clear on stop). Empty/stale cache -> null -> the relay refuses (4401), never a silent unauthenticated session.
- **verified:** 5 `relay_token_cache.test.ts` (serves fresh, null within skew, empty-by-default/after-clear, set() rejects empty/past/NaN, newer push replaces). `make demo-P-REMOTE.2c` green (4 checks over a REAL gated relay: cached token -> host auth-ok + room open; empty cache -> terminal fail-closed no room; refused token -> 4401 fatal; a plain no-authToken socket still connects anonymously = un-gated default unchanged). collab+market suites 260/260; full first-party 2397 pass / 7 fail (same pre-existing set: auth_status env leak, fs_browse linux-fixture-on-Windows x5, lucid_acp theme ext); tsc clean (root 6 pre-existing proto; desktop 0; pwa 0); license green. LIVE gap: a provisioned build (LUCID_REMOTE_PWA_BASE + LUCID_FIREBASE_API_KEY + RELAY_AUTH relay) end-to-end from a phone = QA; the /signin page must return a `refresh` param for silent renewal past 1h.
- **stubbed/deferred:** the desktop WATCHING another gated session as a guest stays anonymous (out of scope; the phone is the guest that matters). The P2P-preferred share (renderer WebRTC coordinators) doesn't present a token to a gated relay yet - its sockets are renderer-side, not the backend cache; a follow-up if gated-relay + prefer-P2P is needed together.
- **next:** P-REMOTE.6 (the Stripe paid tier + premium/admin custom claims - grantEntitlement is the admin comp path) and P-REMOTE.7 (the metadata-only admin dashboard). The transport chain (.1/.2/.2b/.2c) + phone PWA (.3) + QR (.4a) are now complete end to end (pending deploy + the /signin refresh param).

**P-REMOTE.6: the paid Remote Access tier - the phone's unentitled -> Subscribe path (ADR-0227) - 2026-07-16**
- **shipped:** the OSS client seam of the $9.99/mo Remote Access subscription (premium/admin Firebase custom claim; prices/functions are private add-on, deployed per ADR-0223). New PURE, browser-safe `desktop/collab/remote_entitlement.ts`: `decodeClaims` (fail-closed JWT-payload read, NO verify - the relay verifies cryptographically), `hasRemoteAccess`/`entitlementActive` (mirror the relay's admin-implies-premium admission), `isEntitlementDenied` (the AUTHORITATIVE 4403 signal - single-sourced off a new exported `RELAY_NOT_ENTITLED_REASON` in `relay_client.ts`), `remoteGate` (signin|connect|subscribe), and `createRemoteCheckout` (opens a Stripe session over the SAME Firebase-callable protocol the marketplace provider uses, fail-closed to null). DESIGN: admission stays on the RELAY, never a client claim pre-check - the phone always ATTEMPTS to connect (so an allowlisted self-host user is never stranded) and only a real 4403 close routes it to Subscribe. Wired into `tools/remote-pwa/`: a `#subscribe-view` (price + Subscribe + "I've subscribed - continue") in index.html; `app.ts` routes a 4403-ended guest to Subscribe -> `createRemoteCheckout` -> opens Stripe -> Continue force-refreshes the ID token (`getIdToken(true)`, added to firebase_auth.js) and reconnects once `entitlementActive`; `functionsBaseUrl` added to config (fail-closed "can't purchase in this build" when absent).
- **verified:** 21 new `remote_entitlement.test.ts` (decode valid/malformed/non-object/wrong-seg-count; hasRemoteAccess strict-true vs truthy-not-true/null; entitlementActive pre/post-webhook; isEntitlementDenied only for an ENDED view with the exact 4403 note; remoteGate matrix; createRemoteCheckout success + fail-closed on error/non-2xx/blank-url/no-token + Bearer/packId shape). `demo-P-REMOTE.6` green (6 checks over a REAL gated relay: remoteGate matrix; a no-claim token refused 4403 -> guest ended -> Subscribe, no room; a premium phone goes LIVE against a real host, never shown Subscribe; createRemoteCheckout returns the Stripe URL + fail-closed; a post-webhook refreshed token gains `premium` -> reconnect). collab+market 256/0 baseline + 21 new = 277/0; PWA tsc 0 + bundles clean (13.9 KiB); root tsc = 6 pre-existing proto only, desktop renderer 0, desktop-server = 1 pre-existing (dev.ts oauth-fix, uncommitted, not mine); license green. Full first-party 2627 pass / 25 fail - ALL in files I never touched (auth_status env leak, fs_browse linux-fixture-on-Windows x5, lucid_acp theme ext, + the private `lucidaddon_audit/**` PKI/network suite); zero collab/market/remote failures.
- **shipped (backend half, same increment - private add-on `lucidaddon_audit/entitlements/`):** the marketplace backend shipped with the 6 KG packs only, NOT `remote-access` - so the paid tier had no price + never set a claim. Added: a `remote-access` catalog entry (`catalog.ts`+`catalog.json`, `licensing:"subscription"`, `lookupKey:"LUCID-REMOTE-MONTHLY"`, no download `object` - `object` made optional, `getPackDownload` now fail-closes a service entitlement); the PURE `claimForPack`/`nextClaims` core (Remote Access grants `premium`; sets it when active, REMOVES it when a sub lapses, PRESERVES `admin`); `syncEntitlementClaim` in the webhook + `grantEntitlement` (Admin-SDK `setCustomUserClaims`, `auth` added to admin.ts) so a paid/comped user's token gains `premium` and the relay admits within the hour; `remote-access` added to `pricing.example.json` ($9.99/mo = 999).
- **verified (backend):** entitlement core 15/15 (+6: claimForPack remote-access\->premium / others null; nextClaims set/clear preserving admin, null for download packs, idempotent). catalog.json valid + `remote-access` present. index.ts I/O (firebase-admin/stripe) typechecks at `firebase deploy` per the add-on convention (only the pure core is bun-tested).
- **stubbed:** NOT deployed/live-tested here - needs the user's ops: `firebase deploy` the functions, set `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET`, `stripe_setup.ts` to create the Product+Price, register the webhook, provision the PWA (`functionsBaseUrl` + Firebase web config + `relayWsBase`) + deploy the relay with `RELAY_AUTH=firebase`. The Subscribe screen has no Stripe customer-portal / "manage subscription" link yet (ADR-0227 open question); post-checkout retry is manual ("Continue"), not an auto-poll; `admin` claim is seeded manually (README).
- **next:** P-REMOTE.7 (the metadata-only admin dashboard: the relay session-metadata writer via Firestore REST fail-SOFT + GeoLite2 + heartbeat/lastSeen + TTL; the admin-claim-gated dashboard page + rules are private add-on). Then a provisioned build to live-verify the whole phone chain (sign-in -> 4403 -> checkout -> claim -> drive).
- **shipped:** TEST Stripe `pricing.json` now gives both missing subscription packs (`capture-proposal-manager`, `program-manager-evm`) a monthly interval; all seven catalog/pricing interval guards pass.
- **stubbed:** Stripe API dry-run/apply could not run inside the agent process because it does not inherit the user's `STRIPE_SECRET_KEY`; no secret was requested or exposed.
- **next:** from `lucidaddon_audit/entitlements/scripts`, rerun `bun run stripe_setup.ts --dry-run`, then `bun run stripe_setup.ts`; the setup is idempotent and will reuse the three prices already applied.
- **shipped:** Firebase Storage now targets the existing private `gs://lucid-agent-packs` bucket explicitly; deny-all client rules deployed, uniform access + public-access prevention enforced, project-viewer reads removed, and `getPackDownload` has object-read + self-`signBlob` IAM. Full functions/firestore/storage deploy green.
- **stubbed:** no sellable `.lkgpack.zip` objects are uploaded yet; a temporary object proved the real five-minute V4 signed-URL path and was removed. Node.js 20/firebase-functions lifecycle warnings remain a separate upgrade increment.
- **next:** build and upload the six downloadable pack archives with `bun run upload_packs.ts --src "<pack-output-dir>" --dry-run`, then apply and exercise an entitled download in Stripe TEST.
- **shipped:** P-REMOTE-LIVE.1 provisioned the Firebase Web app + Google provider, deployed the authenticated single-instance Cloud Run relay and `/remote` PWA, persisted desktop relay/PWA/Firebase config, and fixed three live-only defects: Cloud Build custom-Dockerfile invocation + missing `relay_auth.ts`, `/remote` relative assets after canonicalization, and the top-level-await auth bridge race. Stripe TEST webhook registration is correct; all six P-REMOTE demos, PWA tsc, shell syntax, and license checks pass.
- **stubbed:** the final user-owned browser chain is not claimed: no Google Firebase identity/Stripe TEST checkout event exists yet, so live 4403 -> Checkout -> webhook -> `premium` refresh -> relay admission remains unobserved. The Cloud SDK token cannot substitute (wrong OAuth audience), and no token/secret was requested or exposed.
- **next:** open the supplied `/remote#e2e-probe...` invite, Google-sign in, complete the TEST-card subscription, then verify the entitlement document/custom claim and post-claim relay reconnect before starting P-REMOTE.4/.5/.7.
- **shipped:** P-REMOTE-UX.1 rebuilt login/paywall/session as a professional 320–430px mobile UI (cyan/teal mark, no pink treatment, 44px+ controls, safe areas, one visible view via `[hidden]`); fixed Subscribe at all three live roots (`prompt-form`/`composer` init abort, async popup blocking, stale cache-first shell); aligned canonical `/remote` service-worker scope. User completed real Stripe SANDBOX checkout: webhook 200, `remote-access` entitlement updated, `premium:true` claim synced. Follow-up deployed: service checkout returns to a dedicated secret-free `/remote?checkout=success|cancel` confirmation (never sends room fragment to Stripe), and teal `icon.svg?v=3` is now the desktop favicon, Apple touch icon, installed-PWA icon, and v3 offline shell asset. Live success/cancel states are single-view, error-free; worker controls `/remote`. P-REMOTE.3 4/4, P-REMOTE.6 6/6, remote units 31/31, backend 15/15, PWA tsc + license green.
- **stubbed:** Stripe LIVE mode was intentionally not exercised; the active SANDBOX subscription is retained so Remote keeps working. Browser/OS icon repaint may require one tab reload or installed-PWA relaunch, but versioned URLs + network-first v3 worker prevent the old pink asset from persisting afterward.
- **next:** refresh/relaunch Remote once on any tablet/laptop to pick up the v3 mark; future sandbox checkouts now land on the Remote confirmation page. Then continue with the still-separate P-REMOTE.4/.5/.7 increments.
- **shipped:** P-REMOTE.7 admin plane — metadata-only relay presence + a mobile-first admin console. New zero-dep `desktop/collab/relay_presence.ts` (SHA-256 room-hash doc ids, in-memory identity/state, ADC metadata-token Firestore REST PATCH, bounded coalescing fail-SOFT queue; NEVER stores roomId/IP/key/link/token/prompt/content/ciphertext) wired into `relay_server.ts` lifecycle + `serve.ts`/Dockerfile/service.yaml (`RELAY_TELEMETRY=firestore`, 30-day TTL). Strict-admin callables `adminMe`/`adminListUsers`/`adminRelayOverview`/`setRemotePremium` (each `requireAdmin` first, premium is not admin, `cors: CORS`) + `bootstrap_admin_claim.ts`. `/admin/` console (same Google auth, no direct Firestore, callable-only) deployed to Hosting; operator `admin` claim seeded (premium preserved); Firestore `expireAt` TTL enabled; deploy scripts hardened to co-stage `/remote`+`/admin`. Verified: 70 relay + 23 admin/entitlement unit tests, all four callables 403 fail-closed unauthenticated, admin console renders sign-in on mobile with a real Google OAuth popup, license+tsc green.
- **stubbed:** GeoIP is a real privacy-preserving disabled mode (`geo:{status:'unavailable'}` -> "Location not collected") pending an approved offline DB/license — no third-party IP lookup. Live presence rows + admin-console-driven grant/revoke are unverified live because (a) the hosted Cloud Run relay's public `.run.app` URL returns a GFE 404 for every newly-created gcloud service in this project (container healthy + bound on 0.0.0.0:8080, sibling `lucid-site` + Firebase functions reachable, owner-auth also 404s, survived clean recreate/new name/both ports/`--default-url`/propagation) — a project/platform routing condition, not this code, pre-existing (the relay never carried live traffic); and (b) admin mutations need the operator's interactive Google admin token (no headless mint). Grant/revoke LOGIC is unit-verified and the webhook already proved the same claim path live.
- **next:** resolve relay public reachability (GCP console/support: why new Cloud Run services 404 at the edge — check billing/default-URL/networking, or front the relay through a working surface); then sign into `/admin/` as the seeded admin to confirm the user list + comp/revoke live, and drive a real host+phone session to confirm presence rows populate. Provision an offline GeoIP DB to light up location.
- **shipped:** ROOT-CAUSED the relay edge 404 as ACCOUNT-WIDE (a brand-new separate project's Cloud Run service also 404s; only pre-~2026-07-10 services + Firebase-managed cloudfunctions.net route) — a Google account-level block on new public `*.run.app` URLs, not code/config. Confirmed Firebase Hosting CANNOT bridge it (serves the relay's HTTP but rejects the WS upgrade: `1002 Expected 101`). Built the permanent GCP-native fix (user-approved): an **External HTTPS Load Balancer** in `lucid-agent` (reserved IP `8.232.53.156`, serverless NEG → `lucid-collab-relay`, EXTERNAL_MANAGED backend, url-map, managed cert for `relay.aiworkshopapps.com`, HTTPS proxy, :443 forwarding rule) — a different front-end than the blocked default URL, WebSocket-capable, connection duration governed by the Cloud Run 3600s timeout. Codified in `lucidaddon_audit/iac/remote-relay/loadbalancer.sh`. Repointed PWA `config.js` + desktop host relay to `wss://relay.aiworkshopapps.com`; relay `fetch` accepts an optional `/relay` path prefix (harmless, backward-compatible). Deleted the diagnostic `lucid-relay-hub` project. 39 relay tests + license green.
- **stubbed:** LIVE cutover is gated on ONE user DNS action + Google cert provisioning: add `relay.aiworkshopapps.com  A  8.232.53.156` at the registrar; the managed cert then goes ACTIVE (~15-60 min once DNS resolves), after which `wss://relay.aiworkshopapps.com` carries live sessions and admin-console presence populates. GeoIP still disabled (no third-party IP lookup) pending an approved offline DB.
- **next:** after DNS+cert ACTIVE, verify `https://relay.aiworkshopapps.com/healthz` = 200 and a WSS invalid-auth probe = 4401, drive a real host+phone session, then confirm presence rows + comp/revoke in `/admin/`. Optionally add a Cloud Run min-instances=1 if cold-start on the relay is noticeable.
- **shipped:** relay is LIVE. User added DNS `relay.aiworkshopapps.com A 8.232.53.156` (Google Cloud DNS, no CAA); the managed cert went ACTIVE and `https://relay.aiworkshopapps.com/healthz` returns 200 from the container. The WebSocket UPGRADE succeeds through the LB (`opened:true`) \u2014 the capability Firebase Hosting lacked \u2014 so the account-wide `run.app` edge-404 block is permanently sidestepped on a stable custom domain. PWA + desktop host both target `wss://relay.aiworkshopapps.com`.
- **stubbed:** through the LB, an INVALID/expired-token close surfaces as WS `1006` instead of the relay's `4401/4403` (Google LB doesn't reliably forward 4xxx close codes on an immediate post-handshake close). Impact is limited to the unentitled\u2192Subscribe redirect (a non-premium phone may see a generic error rather than the Subscribe screen); the premium drive path (auth-ok via normal data frames) is unaffected. Live premium-phone drive + admin presence rows still need a real on-device session (no headless Firebase token).
- **next:** harden the deny path by sending a `{t:'denied',code}` DATA frame before `ws.close()` so the client detects entitlement denial regardless of close-code stripping; then live-drive from the phone and confirm `/admin/` presence + comp/revoke.

## P-REMOTE.2b/2c completion: the hosted desktop `/signin` surface (unblocks host relay auth)
- **shipped:** the MISSING keystone behind the Share "Loading…"/host-can't-connect saga. The desktop already defaults `market_config.marketBootConfig` to firebase mode pointing at `https://lucid-agent.web.app/signin`, and all protocol wiring ships (main.ts `lucid://` registration + `open-url`/`second-instance` forward, preload `onAuthCallback`, app.ts `initMarket`→`handleAuthCallback`, `pushRelayTokenOnce`→`collabPushToken`) — but that sign-in PAGE was never deployed (only `/remote` + `/admin` were), so the desktop opened a 404, got no Firebase token, and the gated relay refused the host. Built `lucidaddon_audit/website/firebase-public/signin/`: a teal mobile-first `index.html` ("Authorize this desktop" + Google button), `config.js` (public Firebase ids), `app.js` (Google popup → force-refresh ID token → `lucid://auth?token&email&exp&refresh` deep link), and a PURE, DOM-free `authlink.js` (redirect-scheme validation + deep-link builder). `firebase.json`: `/signin` rewrite, no-cache/security headers, and `**/*.test.ts` ignore. Deployed to Hosting (18 files).
- **verified:** 11/11 `authlink.test.ts` incl. a ROUND-TRIP through the desktop's real `market_auth.parseAuthCallback` (idToken/email/`exp`×1000/refresh recovered exactly; an https `redirect_uri` is refused → the bearer token is only ever handed to `lucid://auth`, no web-origin exfiltration). Clean `bun build` of `app.js`; live probe: `/signin` + `app.js`/`authlink.js`/`config.js`/`icon.svg` = 200, `/signin/authlink.test.ts` = 404 (excluded), `/remote` + `/admin` still 200 (no surface dropped); deployed-content spot-check matches source. Preview render confirms the card layout.
- **stubbed:** the actual Google-popup → `lucid://auth` → desktop round trip needs an interactive on-device sign-in (no headless Firebase token). NO desktop rebuild required. `LUCID_FIREBASE_API_KEY` is unset on the installed build, so silent >1h ID-token refresh is off (hourly re-sign-in) until a provisioned build sets it. The "Loading…" pixel itself couldn't be reproduced headlessly (`getData` swallows errors→null, `effectiveRelay` is pure/no-IO) — the certain, load-bearing fix is enabling host sign-in, which this ships.
- **next:** user signs into Google on the desktop (Marketplace rail → click any pack → opens `/signin` → deep link stores the token in the shared `MarketAuth`), then Start Share → host authenticates to the gated relay → drive from the phone; confirm `/admin` presence rows. Optional polish (desktop rebuild): a dedicated "Sign in for Remote" button inside the Share panel, and set `LUCID_FIREBASE_API_KEY` for silent refresh.
- **follow-up 3 (same session):** live drive worked (room created, 2 participants) and surfaced 4 more issues, all addressed. (1) EDIT share showed the phone as VIEW: the relay-host fallback `browserLink` carries NO write token, so `effectiveRelay()` now derives `pwaBase` (not just `gated`) from the hosted relay host -> the host mints a PWA invite link that CARRIES the write token for edit shares (needs a HOST restart to take effect; the PWA already honored `writeToken`). (2) iOS screen-lock dropped the socket with no prompt recovery: added `CollabSocket.reconnectNow()` + `isClosed`, and the PWA now reconnects on `visibilitychange`/`online`/`pageshow` (rebuilds a fresh socket if it died fatally while away). (3)+(4) logged-out-after-refresh + can't-re-login: switched the PWA from `signInWithPopup` (unreliable on iOS Safari) to `signInWithRedirect`+`getRedirectResult` with same-domain `authDomain=lucid-agent.web.app` (both `/__/auth/handler` return 200) and sessionStorage fragment preservation (OAuth drops the room hash; restored via history.replaceState before app.ts reads it). SW bumped v3->v4. VERIFIED: PWA tsc clean, 55/55 collab tests, server tsc clean (only the pre-existing dev.ts regex error), rebuilt+redeployed the PWA, live assets confirmed (authDomain/redirect/reconnectNow/v4). NEEDS on-device check (the real Google redirect can't run headlessly) + a HOST restart for the edit link.
- **follow-up 4 (same session):** the redirect switch FAILED on-device with Google `Error 400: redirect_uri_mismatch` - only the `firebaseapp.com/__/auth/handler` redirect URI is registered on the OAuth client, not the `web.app` one. REVERTED `authDomain` to `lucid-agent.firebaseapp.com` and the PWA sign-in back to `signInWithPopup` (the proven-working flow) so sign-in is unblocked immediately; KEPT the reconnect-on-resume fix (`reconnectNow` + visibility/online/pageshow) and the host-side edit-link fix. Redeployed (SW v5); live confirms firebaseapp.com + popup + reconnectNow. Durable mobile-auth upgrade (same-domain redirect) stays available but is GATED on one console step: add `https://lucid-agent.web.app/__/auth/handler` to the OAuth 2.0 Web client's Authorized redirect URIs, then flip authDomain back to web.app + redirect.
- **follow-up 5 (same session):** user registered the `web.app/__/auth/handler` redirect URI, so RE-ENABLED the same-domain flow: `authDomain=lucid-agent.web.app` + `signInWithRedirect`/`getRedirectResult` + sessionStorage fragment preservation; SW v6; rebuilt+redeployed. Cache-busted live confirms web.app authDomain + signInWithRedirect (a non-busted edge read briefly served a stale edge copy; no-cache revalidates shortly). Needs on-device confirmation that Google no longer 400s and the phone signs in via redirect + joins.

- **follow-up 2 (same session):** live phone test surfaced two more real defects, both fixed. (1) The stale relay-host QR showed `not a relay room`: added an OPT-IN fragment-forwarding page at the relay's `/` (`relay_server.ts` `pwaRedirectBase` + `pwaRedirectHtml`, `tools/relay/serve.ts` reads `RELAY_PWA_REDIRECT`, `service.yaml` sets it to `https://lucid-agent.web.app/remote`; 3 new relay tests, 11/11 relay green; rebuilt+redeployed the Cloud Run relay; live `/` now 200 HTML forwarding `location.hash` to the PWA, `/healthz`+`/r/` intact). (2) After that, the phone reached the PWA + authenticated but got `no such room` / 1006 -> relay logs proved the HOST connects (GET 101) but is NEVER admitted (`rooms:0`, zero `auth refused`) because it sent NO token: `gated` was derived from a per-machine `LUCID_RELAY_GATED`/`LUCID_REMOTE_PWA_BASE` env var that the running host process didn't have (lost on in-app relaunch / other laptop). Fix: `effectiveRelay()` now recognizes the first-party hosted relay BY HOST (`wss://relay.aiworkshopapps.com`) so ANY install authenticates without an env var; a WS probe through the LB confirmed frames+auth flow (the LB was never the problem). Server tsc clean (only the pre-existing `dev.ts` regex-match error). NOTE: the dev.ts change reaches the DEV instance on a full restart (respawns dev.ts) and packaged builds only on rebuild.
- **follow-up (same session):** built the "Sign in with Google" control DIRECTLY into the Share panel (`app.ts` `shareBodyHtml`/`openSharePanel`, `data-share-signin`/`data-share-signout`) so a gated-relay host authorizes without the Compiled-KB-marketplace detour: signed out shows the sign-in step (Start hidden), signed in shows the account + Sign out; a window-focus + short poll flips the panel to ready the moment the `lucid://auth` deep link returns. Added `gated?` to `CollabRelay`; renderer tsc clean; authlink 11/11 still green. Removed every em dash from the `/signin` UI + its files and redeployed (live `/signin` em-dash count = 0). ALSO root-caused the phone QR `not a relay room`: the invite `browserLink` fell back to the relay-host form (`manager.ts:118`) because the running host process predated `LUCID_REMOTE_PWA_BASE` (already correct in the registry) — a full OS relaunch (NOT the in-app relaunch, which carries the stale env) + a fresh EDIT share mints the PWA link. The Share-panel change reaches a dev instance on a plain reload (dev.ts bundles `app.ts` per request) but needs a rebuild for packaged v1.11.9.

## P-COLLAB.14: edit-guest model + already-used-folder selection (ADR-0228)
- **shipped:** a remote EDIT guest (phone PWA / desktop watch-guest) can now pick the MODEL and an already-used FOLDER, not just prompt/abort. Additive collab frames (`frames.ts`, COLLAB_PROTOCOL_VERSION unchanged at 1): host->guest `options{CollabOptions: models[{value,name}], workspaces[{id,name,isGit}], active*}`, guest->host `set-model{value}` + `set-workspace{id}`. `CollabHost` offers `options` to EDIT guests only (view guests get nothing), edit-gates + RE-VALIDATES every pick against the offered allowlist fail-closed (unknown model/id -> `error` frame, never the callback), and `setOptions()` rebroadcasts the live selection; `CollabGuest` stores options + guards `setModel`/`setWorkspace`. Folders travel as an OPAQUE id the host resolves to a path LOCALLY - no filesystem path or arbitrary model id ever crosses the wire. Wired both transports: RELAY host (`dev.ts` `buildCollabOptions`/`collabWsById` + `guest-inbox` model/workspace + `/api/collab/guest-model`|`guest-workspace` + `refreshOptions` after `/api/setConfig`(model)/`/api/workspace`) and DIRECT-P2P host (`collab_p2p.ts` `setP2PHostOptions` + renderer `buildRendererCollabOptions`). A guest pick is applied through the host's OWN `applyConfig`/`applyWorkspace` path (its UI + omp reconcile; a folder switch restarts the agent in the new cwd), attributed via a toast. PWA pickers: pure `pwa_view.renderControls` (edit-only, escaped) + `#controls` row wired to `guest.setModel/setWorkspace`; bundle rebuilt 17.4 KiB.
- **verified:** 10 new `model_folder_select.test.ts` (host: options to edit-only, set-model/set-workspace allowlist-validated + off-allowlist refused with error, view-guest refused read-only, setOptions state+options rebroadcast + model follows; guest: options stored + onOptions, setModel/setWorkspace send-when-offered / refuse-otherwise / refuse-when-readonly; renderControls empty for view/ended/no-options, active pre-selected, hostile names escaped) -> collab suite 278/278. `make demo-P-COLLAB.14` green (6 checks over a REAL relay: edit guest offered pickers, drives model+folder switch, off-allowlist guarded client + refused host-side via raw frame, host.setOptions rebroadcast, view guest offered nothing + refused). renderer + remote-pwa tsc clean; server tsc clean except the pre-existing `dev.ts` OAuth-broker `m[1]` regex-null error (untouched). PWA bundles clean.
- **stubbed / next:** the desktop watch-guest has the bridge methods (`collabGuestSetModel/SetWorkspace`) + endpoints but no join-panel picker UI yet (the phone PWA is the primary remote editor; a desktop-guest picker row is a small follow-up). Folder allowlist = workspace history (current + recent); no remote "open a brand-new folder" (by design - allowlist-only). A live phone->desktop model/folder switch wasn't exercised on-device here (token-gated relay + real omp restart); covered by the unit tests + real-relay demo + tsc.

## P-REMOTE.8: PWA guest composer - image attachments + autosize + live reconnect status (ADR-0229)
- **shipped:** three phone-PWA composer fixes from a live session. (1) RECONNECT BUG: `CollabGuest` conflated the transient "connection lost - retrying (code 1006)" note with terminal error/bye notes and never cleared it on recovery, and `pwa_view.statusLabel` showed ANY note as red "ended" - so the banner stuck through streaming. Fixed with a `#reconnecting` flag cleared by `#clearReconnectNote()` on the next live host frame (welcome/event/state; a genuine error/bye note is untouched); `statusLabel` now renders reconnecting as amber WAIT and flips back to Live the instant traffic resumes. (2) COMPOSER: `#prompt-input` autosizes with content up to ~6 lines then scrolls (max-height 160px), a roomier review area vs the fixed one-liner. (3) IMAGE ATTACHMENTS (additive protocol): `PromptFrame` gains optional `images?: string[]` (validated image data URLs); `CollabGuest.sendPrompt(text, images?)` (image-only messages allowed) -> host `onGuestPrompt(text, guest, images?)` -> the host renderer stages them into its OWN `state.attachments` and sends them to the model through the EXISTING gated `send()`, reusing `acceptAttachment`/`promptImageBlocks` (single-sourced with the desktop P-VISION.1 paste path, which re-validate type/size/count fail-closed). Wired both transports (relay inbox + direct-P2P) + the desktop watch-guest bridge. PWA gets an attach (+) button, paste, drag-drop, and a thumbnail strip (img.src set as a DOM property, never interpolated; SVG excluded). No COLLAB_PROTOCOL_VERSION bump (additive/optional).
- **verified:** 12 new `pwa_composer.test.ts` (reconnect: note clears on welcome AND on a resumed event, terminal error note preserved; statusLabel reconnecting=wait / live-after-recover / ended; guest sendPrompt carries images + image-only + refuses empty + refuses read-only; host forwards images to onGuestPrompt for edit + refuses a view guest) -> collab suite 290/290. `make demo-P-REMOTE.8` green (6 checks: images E2E over a real relay to onGuestPrompt, image-only, view-guest refused, reconnect note shows then CLEARS on recovery). renderer + PWA tsc clean; server tsc clean except the pre-existing `dev.ts` OAuth-broker `m[1]` error; PWA bundle rebuilt 20.4 KiB; first-party suite 2447 pass / 7 fail (the 7 = same pre-existing Windows-path/env, none in touched files).
- **security / stubbed:** an edit-guest's image BYTES reach the host model as vision input UNSCANNED (scanner gates text, not pixels) - same posture as P-VISION.1 (a pasted image is trusted input) and the edit guest is already trusted to drive; prompt TEXT still passes the fail-closed gate and every tool call stays gated. Next: a live on-device phone check of paste/drop -> model vision reply (needs a provisioned relay + real model; covered here by unit + real-relay demo + tsc).
- **follow-up (same session, on-device feedback):** fixed three phone composer issues + redeployed. (1) The `+` image button did nothing on iOS - `input.click()` on a `hidden` (display:none) file input is BLOCKED by iOS Safari; changed `+` to a native `<label for="file-input">` and made the input `.sr-only` (rendered off-screen, NEVER display:none) so a tap opens the picker natively (removed the JS `.click()`). (2) Composer is now a TWO-ROW layout - a full-width autosizing textarea on top, a compact action row (`+` left, Stop/Send right at 44px) below - so the input spans the whole width and visibly grows (the old single flex row cramped it). (3) The on-screen keyboard clipped the composer: iOS treats the keyboard as an overlay and doesn't shrink 100dvh, so added a VisualViewport handler (`resize`/`scroll`/focus -> size the app to `visualViewport.height`, pin scroll top, keep transcript at bottom) + `interactive-widget=resizes-content` in the viewport meta. Verified LIVE at lucid-agent.web.app/remote on a 390x844 viewport: `+` label click OPENS the file chooser (the exact iOS bug) and accepting a file renders a thumb; composer flex-direction=column with input at 94% width and buttons below; autosize 48->129px capped 160px; VisualViewport handler wired + error-free. PWA tsc clean, bundle 20.6 KiB, hosting redeployed.

## P-REMOTE.9: PWA transcript - own-message echo + per-edit diffstats + end-of-run mobile report (ADR-0230)
- **shipped:** three guest-side transcript improvements (NO protocol change - the data is already in the stream), all in `desktop/collab/pwa_view.ts` + the PWA. (1) OWN-MESSAGE ECHO: the host doesn't broadcast user turns live, so the PWA optimistically appends a `user` ViewItem on a successful `sendPrompt` (a reconnect resets items + the welcome replay re-supplies past user turns via the host's `tapUserTurn`, so no dup). (2) PER-EDIT +/- DIFFSTAT: the `tool` ChatEvent already carries `code{path,oldText,newText,content,patch}` and the host already broadcasts it, so the guest sizes the diffstat itself by REUSING the desktop's pure `answer_chips.toolChip`+`linediff` (one convention, nothing new on the wire); the tool item gained `path`/`add`/`del` and `renderItem` shows the file path + a `+N -M` badge. (3) END-OF-RUN REPORT: pure `buildTurnReport` folds ONE turn's items (segmented by `done`) into {model,contextPct,task,answer,files[] (diffstats merged per path),tools[] (counts),totals}; `renderReportHtml` = screenshot-friendly mobile cards (summary/task/files/tools/summary), `reportMarkdown` = copyable Markdown; a "Report" button appears after a run and opens a full-screen modal (Copy->clipboard, Close, screenshot-any-card hint). Every host string + path is HTML-escaped.
- **verified:** 9 new `pwa_report.test.ts` (foldEvent edit/write diffstat + read=none + hostile-path escape; user-item render + escape; buildTurnReport merges per-file + counts tools + task/answer/totals + read-only=0 files; renderReportHtml escapes hostile task/answer/path; reportMarkdown structure) -> collab suite 299/299. `make demo-P-REMOTE.9` green (5 checks over a REAL relay: own-message echo, edit diffstat computed from E2E-delivered code +2/-1, end-of-run report 2 files +5/-1 w/ model+context, copyable Markdown). LIVE-verified on a 390x844 viewport (screenshots): user bubble + `edit src/auth.ts +6 -1` chip + answer render; report modal cards (Model/Files/Lines/Context, per-file diffstats, tool counts) + Copy/Close wired; no console errors. PWA tsc clean, bundle 26.1 KiB, hosting redeployed.
- **stubbed/next:** the report covers the last completed turn (segmented by `done`); no cross-turn/session rollup. User turns are echoed to the SENDING guest only - a true multi-guest live mirror (host broadcasts every user turn) is a possible follow-up. Diffstats depend on the host populating the tool event's `code` (it does for edit/write/patch); a tool without authored code shows no badge (correct).

## P-COLLAB.15: live user-turn mirroring across all participants (ADR-0231)
- **shipped:** every user turn is now broadcast LIVE to all guests, attributed by author - so in a multi-guest share several phones watching one host see who typed what, in order (previously only the sender's own echo showed; the host's typed turns + other guests' turns appeared only on a reconnect replay). Additive `user-turn {text,from}` host->guest frame (no COLLAB_PROTOCOL_VERSION bump). ONE broadcast point: `CollabHost.pushUserTurn(text, from?)` - already called for every turn via the relay's `tapUserTurn` (dev.ts /api/chat) AND the direct-P2P `teeUserTurn` (renderer) - now also broadcasts the frame (default author = host name). Author threading: `runGuestPromptLocally` stashes `nextTurnFrom=guest.name`, `send()` consumes it once into `p2pTeeUserTurn` + `bridge.sendPrompt(...,from)` -> `/api/chat` -> `tapUserTurn(text,from)`; a host-typed turn stays the host. `CollabGuest` gained `onUserTurn(text,from)`; the PWA renders a `user` item with an author label (`from`), and the sender keeps its INSTANT optimistic echo (avoids the ~2s inbox-poll latency) while deduping the host's returning broadcast of that same text. View guests see turns too (watching); late joiners still get prior turns via the welcome replay.
- **verified:** 5 new `user_turn.test.ts` (host broadcasts host-authored + guest-attributed turns to all, records in the replay for late joiners, no broadcast once stopped; guest fires onUserTurn) -> collab suite 304/304. `make demo-P-COLLAB.15` green (5 checks over a REAL relay: 2 guests both see the host's turn (attributed alice) + a bob-attributed turn (view-only carol included), a late joiner sees prior turns in the replay). LIVE-verified on a 390x844 viewport (screenshot): own message as a plain bubble + another participant's turn labelled `bob` + the answer; bundle carries user-turn/onUserTurn/msg-from, no console errors. server/pwa/renderer tsc clean (except pre-existing dev.ts:586), bundle 26.4 KiB, hosting redeployed.
- **stubbed/next:** the DESKTOP watch-guest (backend-mediated `CollabGuestFrame` join path) doesn't surface `user-turn` yet - the phone PWA (the primary multi-guest watcher) does; wiring the desktop Join panel is a small follow-up. Sender dedup is by matching text (FIFO) within the session; a peerId-based dedup would be exact but the guest doesn't track its own peerId today.

## P-SHARE.1: the Session Share panel becomes a floating, minimizable dock (ADR-0232)
- **shipped:** rebuilt the desktop Share panel from a busy CENTERED MODAL that blocks the app into a non-distracting FLOATING DOCK (item A of a 3-part ask; B=Drive reconnect, C=preview-snapshot). New PURE `desktop/renderer/share_dock.ts` (DOM-free, injected storage): DockShape/DockState, defaultShape/clampToViewport, snapDecision (drag-drop snaps flush-right or beside the LEFT rails via railWidth() so it never sits under them, else floats), loadDockState/saveDockState (localStorage, fail-soft), participantSummary, isCollapsed. `app.ts openSharePanel` now builds `#shareModal.share-dock` (kept the id + all delegated change/click handlers UNCHANGED) - a position:fixed popover with NO backdrop, draggable by its header, resizable via e/s/se handles, persists shape/side/minimized/collapsed. `shareBodyHtml` regrouped into chevron `<details class="dock-sec">` sections (Invite+QR, Participants; Relay, Sharing options, Host-the-relay) - collapsed by default except the primary, each persisted (capture-phase toggle listener; native details = accessible expand-on-click). Header shows a live connection COUNT badge (green dot when active) opening a by-EMAIL participant dropdown; MINIMIZE-to-pill (custom up-arrow SVG) hides the dock to a position:fixed `#shareDockPill` bottom-right near the trivia wire (own element, so `renderStatus`'s 2s `#statusbar` rebuild never clobbers it), click restores. `pollRoster` updates badge+dropdown+pill+roster together; viewport resize re-clamps on-screen. styles.css: `.share-dock`/head/count/people-dropdown/`.dock-sec` chevron/resize handles/`.share-dock-pill`, reusing the app design tokens.
- **verified:** 13 new `share_dock.test.ts` (defaultShape bottom-right + on-screen + min-size; clamp oversized/offscreen; snap right/left-beside-rails/float; state round-trip + corrupt-fallback + smaller-viewport re-clamp + broken-storage no-throw; participantSummary count/access/blank; isCollapsed default-vs-stored) -> renderer suite 717/717, renderer tsc clean, first-party 2474 pass / 7 fail (same pre-existing Windows/env, none in touched files). VISUAL: a static `mockups/share_dock_preview.html` harness (real tokens + the dock CSS) screenshotted via the Preview panel (the Electron renderer can't be browser-loaded) - confirms the floating dock, chevron-collapsed sections, count badge + open participant-by-email dropdown, footer actions, corner resize grip, and the minimized pill all render correctly. license clean. NO firebase deploy (desktop app; reaches a dev instance on reload, packaged on rebuild).
- **stubbed/next:** interactive drag/resize/snap/minimize are wired + typed but exercised via the pure geometry tests + the visual mockup, not a live Electron pointer test (no headless Electron here). Item B (Drive relay-codes reconnect: `drive.file`, optional PIN-encryption, view-link-default-else-edit) + item C (preview snapshot -> PWA: save/tap-fullscreen/hand-to-agent-for-markup + PWA-awareness) are the next two increments; their toggles/controls dock into these new sections.

## P-REMOTE.10: out-of-band reconnect via a Google Drive relay-codes file (ADR-0233) - secure core
- **shipped:** the hardened, fully-tested CORE of item B - reconnect codes in the user's OWN Google Drive so a dropped Session Share (app closed / room torn down past the 30s grace) can rejoin. New PURE `desktop/collab/drive_relay_codes.ts` (WebCrypto, DOM-free): `chooseReconnectLink` (view by default; EDIT link when the share is/was edit - decision 3), buildCode/appendCode(cap 20)/latestValidCode(expiry), and the load-bearing OPTIONAL PIN encryption at rest - AES-256-GCM with a PBKDF2-SHA256(210k)-derived key, envelope [version][16B salt][12B iv][ct]; wrong-PIN/tamper/missing-PIN decrypt to null (fail-closed, never throws). New `desktop/collab/drive_file.ts` - a tiny `drive.file`-scoped Drive REST client (INJECTED fetch + access token): find/create/read(alt=media)/update(PATCH media)/share(per-file writer permission)/ensure. `drive.file` = LUCID touches ONLY files it created, never the rest of the Drive (Google-enforced least privilege). All untrusted JSON (codes file + Drive responses) parsed with unknown + in/typeof narrowing (no any).
- **verified:** 23 new tests (`drive_relay_codes.test.ts` 15: link-selection rules, append/cap, expiry, parse validation, plaintext round-trip, PIN round-trip + link-absent-at-rest + wrong-PIN/missing-PIN/tamper fail-closed + fresh-salt-per-encrypt; `drive_file.test.ts` 8: request shapes + Bearer token + multipart create + media update + per-file permission + find-or-create + fail-closed non-2xx over a mock fetch) -> collab suite 327/327. `make demo-P-REMOTE.10` green (6 checks, REAL WebCrypto + in-memory mock Drive: write PIN-encrypted edit code -> ciphertext at rest (link absent) -> reader decrypts with PIN -> wrong PIN null -> reconnect appends fresh (newest wins) -> per-file share). server tsc clean (bar pre-existing dev.ts:586); no `any`; license clean. NO deploy (desktop app).
- **stubbed/next (P-REMOTE.10b, deploy-gated):** the drive.file OAuth CONSENT + UI - the injected access token IS the seam; producing it (PWA incremental Firebase `addScope('drive.file')` re-consent; desktop `/signin` returning a drive.file token) needs a provisioned Google OAuth client + a LIVE consent screen that can't be verified headlessly, so it ships once provisioned rather than as an unverified path. Then the Share-dock "Reconnect (Google Drive)" section (enable toggle + optional PIN + share-with-teammate) writing on share start/reconnect, and the PWA "get a reconnect code" reader. Then item C (preview snapshot -> PWA).

## P-REMOTE.10b: drive.file consent + desktop WRITER + Share-dock Reconnect section (ADR-0233)
- **shipped:** the OAuth consent path + the desktop write UI on top of the P-REMOTE.10 core. NO new OAuth client id - the drive.file token rides Firebase's existing web client via the hosted `/signin` page: `?drive=1` adds `provider.addScope('.../auth/drive.file')`, and `GoogleAuthProvider.credentialFromResult(res).accessToken` (a ~1h Drive-capable token) is appended to the `lucid://auth` deep link as `&drive_token=&drive_exp=` (same lucid:-only channel as the idToken; `safeRedirectBase` still guarantees lucid://). `market_auth.ts`: `TokenRecord` += driveToken/driveExpiresAt, `parseAuthCallback` reads them, PRESERVED across an idToken refresh, `freshDriveToken(skew)` serves-while-live-else-null (re-consent, fail-closed). `market_boot.ts`: `beginDriveSignIn()` + `freshDriveToken()`. Renderer: a Share-dock `sec-reconnect` "Reconnect (Google Drive)" section (enable toggle persisted; Authorize Google Drive; optional PIN; Save-a-code-now; Share-with-a-teammate) + WRITE-ON-SHARE (after a share starts + enabled + drive token: chooseReconnectLink -> buildCode -> find-or-create/append lucid_relay_codes via the tested core, PIN-encrypted when set; PIN entered per action, never stored). CSP `connect-src` += googleapis.com + securetoken.googleapis.com. /signin `app.js`+`authlink.js` updated + deployed.
- **verified:** market_auth drive-token tests (+5: parse drive_token/drive_exp, default-exp, freshDriveToken live/expired/absent, preserved across idToken refresh) -> market_auth 24/24; authlink.test.ts (+2: buildDeepLink drive fields, drive round-trip to parseAuthCallback) -> 13/13. renderer + server tsc clean (bar pre-existing dev.ts:586); no `any`. `/signin` DEPLOYED + live-probed (auth/drive.file + credentialFromResult in app.js, drive_token in authlink.js). first-party suite unchanged pass+delta / same 7 pre-existing fails.
- **assist / next:** the LIVE Google consent needs the user to (1) enable the Google Drive API and (2) add the `https://www.googleapis.com/auth/drive.file` scope to the OAuth consent screen in the lucid-agent Google Cloud project - then the Authorize-Google-Drive button + a real Drive write can be exercised on-device (unverified-app warning screen is expected until Google app verification). P-REMOTE.10c = the PWA READER ("get a reconnect code" -> decrypt -> rejoin).

## P-SHARE.2: Session Share dock UI polish - routable-first bind list, checkbox persistence, cold-boot cache (ADR-0234)
- **shipped:** three reported UI bugs on the Share dock (ADR-0232) + Reconnect section (ADR-0233). (1) The "Reachable at" bind picker defaulted to loopback (127.0.0.1) - unreachable by a guest - because it rendered `serve.addresses` in backend order and the browser selects the first option; new PURE `orderBindAddresses` (share_dock.ts) sorts guest-routable (lan/vpn) before loopback and IPv4 before IPv6 (stable, non-mutating), `shareRelayServeHtml` marks the first `selected`, loopback DEMOTED (kept for same-machine/tunnel-VPN, never default). (2) "Let a guest drive this session" silently unchecked itself - it had no persisted checked state so every draw() re-render (focus / be-the-relay toggle / Drive-auth poll) reset it; the two ephemeral toggles are now held in openSharePanel closure vars (allowEditPref/preferP2PPref), recorded on change, fed back through a new `ui` param of shareBodyHtml; and `.dock-foot` gained `margin-bottom:8px` so the action row no longer touches the next panel (consistent inter-panel gap). (3) The dock was a blank "Loading..." on cold boot; it now paints a SECRET-FREE cached snapshot instantly via new swr_cache helpers + a single `composeBody` path + an `updating...` header chip (Settings SWR pattern, ADR-0084), then revalidates. New PURE `redactShareSnapshot` WHITELISTS only the non-secret relay descriptor + serve status + a redacted P2P config - NEVER an invite link / browser link / room id (E2E key rides the fragment) nor a TURN credential, and always active:false (no stale "Live").
- **verified:** +7 unit tests (share_dock.test.ts: orderBindAddresses default/order/stable-non-mutating; redactShareSnapshot drops TURN cred + only {relay,serve,p2pCfg} + null relay; swr_cache.test.ts snapshot round-trip) -> touched suites 34/34; `make demo-P-SHARE.2` green (5 checks incl. no link/room-id/TURN-cred/Live in the cache); renderer tsc clean, no `any`; visual confirm via mockups/share_dock_preview.html (updating chip, LAN-default dropdown, button-row->panel gap). first-party suite: same 7 pre-existing Windows/env fails, none in touched files.
- **next:** P-REMOTE.10c (PWA reconnect-code reader -> decrypt -> rejoin) or item C (preview snapshot -> PWA), a fresh increment/session per the AGENTS.md ritual.

## P-REMOTE.10c: the phone PWA reconnect-code READER (ADR-0235)
- **shipped:** the sibling of the P-REMOTE.10b desktop writer - a dropped phone recovers a live invite link out of band. New PURE `resolveReconnect(text,pin,now)` in `drive_relay_codes.ts` composes fileIsEncrypted+readFileContent+latestValidCode into the reader UX state machine: ok{link,edit} | locked (encrypted,no PIN) | bad-pin (wrong PIN/tamper) | expired | empty; fail-closed (locked/bad-pin never return a link). PWA: `firebase_auth.js` gains `signInForDrive()` (separate incremental drive.file consent - a viewer is never asked) + `getDriveToken()` (accessToken captured from credentialFromResult on the redirect, gated by a reconnect flag so a scope-less sign-in token is never mistaken for a Drive token; ~55m localStorage stash, cleared on sign-out); `index.html` gains a `#reconnect-view` (Get-a-reconnect-code btn + PIN input + status); `app.ts` no-fragment path (was a dead-end fatal) now runs `reconnectFlow` -> findRelayFile/readRelayFile (browser-safe drive_file.ts) -> resolveReconnect -> on ok, parseShareLink(stored relay-path link) + formatShareLink -> `location.hash` + reload (the whole existing sign-in/connect boot path runs unchanged - no duplicated connect logic). NO new OAuth client id (reuses Firebase's web client, like 10b). drive.file scope only.
- **verified:** +6 `resolveReconnect` unit tests (`drive_relay_codes.test.ts` -> 19/19); `make demo-P-REMOTE.10c` green (`demo_premote10c.ts`, 7 checks over a mock Drive: PIN-encrypted write, locked/bad-pin fail-closed, ok recovers the edit link, LOSSLESS link->fragment normalization round-trip [room+32B key+16B write token], expired + empty refused); PWA bundles (`bun run tools/remote-pwa/build.ts` -> app.js 30.6 KiB) + PWA tsc clean + renderer tsc clean; no `any`.
- **next / assist:** DEPLOY the rebuilt `/remote` PWA (`firebase deploy --only hosting`, preserving the real config.js) + the SAME Google Cloud console steps 10b awaits (enable Drive API + add the `drive.file` scope to the OAuth consent screen) to exercise the live Drive read on-device. Remaining roadmap: item C (preview snapshot -> PWA).

## P-REMOTE.11: phone PWA mobile-UX redesign + reconnect catch-up + Session Share polish (ADR-0236)
- **shipped:** (1) PWA screen real estate - transcript now fills the screen; the session header + account + model/folder controls collapse into ONE condensed, auto-collapsed DOUBLE-DECKER strip UNDER the composer (thin status bar `Live - you can drive` -> tap to expand header/pickers/email/Sign out); the two-row composer became single-row `[+] [input] [Stop] [Send]` (wasted spacer + second row reclaimed). (2) Reconnect CATCH-UP - on screen-lock the PWA snapshots the seen host-turn count; on the reconnect welcome-replay it shows an auto-collapsed "N updates while you were away" card (tap to expand missed turns), bandwidth-minimal (summarizes the replay the guest already gets - no extra wire data; textContent-safe), dismissed on send. (3) Session Share polish: swept em dashes from the bind-address labels (`net_addrs.ts` U+2014 -> hyphen, +guard test) + app.ts fallback; moved the minimize PILL from floating above the status bar to INSIDE it (right of the trivia ticker), re-adopted after each renderStatus swap like the ticker (`sharePillEl`/`mountSharePill`).
- **verified:** net_addrs em-dash guard + collab suite 333/0; PWA bundles (app.js ~32 KiB) + PWA tsc + renderer tsc clean; visual confirm via `mockups/pwa_session_preview.html` (maximized transcript, single-row composer, auto-collapsed catch-up + double-decker strip). DEPLOYED to lucid-agent.web.app/remote + live-probed (botstrip/catchup/reconnect-view in index.html, signInForDrive in app.js, getDriveToken in firebase_auth.js). 10c is now LIVE too (shipped in the same deploy). Desktop em-dash + pill reach a dev instance on reload.
- **next:** item C (preview snapshot -> PWA: scaled-down broadcast, save-to-device, tap-fullscreen, hand-to-agent-for-markup, send-back-with-markups, agent PWA-awareness) - a dedicated increment. Still gated for the live Drive read: the Google Cloud console steps (enable Drive API + add the drive.file scope).

## P-PREVIEW-PWA.1: preview snapshot to the phone PWA, slice 1 of item C (ADR-0237)
- **shipped:** the desktop's Preview panel can be sent to connected phone guests. New ChatEvent `{type:"preview-snapshot",image,label?}` (chat_events.ts) rides the existing host->guest tap for BOTH transports: direct-P2P via `p2pTeeEvent`, relay via new `bridge.collabBroadcastPreview` -> `POST /api/collab/preview` -> `collabManager.tapEvent` (fail-closed: no active share / non-image sends nothing; broadcast to guests only, never the local transcript). A "To phone" button in the Preview toolbar reuses `screenshotPreviewToChat`'s capture (`laneFrame` rect + Electron `capturePreview`), downscaled to a 1280px longest-edge cap (pure `fitWithin`, preview_snapshot.ts) before sealing. PWA: a new `preview` ViewItem (pwa_view.ts) folds into a tappable thumbnail whose data URL is hydrated as an `<img>` property (never inlined), tap opens a fullscreen viewer, Save writes the PNG to the device; the label is HTML-escaped.
- **verified:** fitWithin + fold/render unit tests -> collab suite 340/0; `make demo-P-PREVIEW-PWA.1` green (6 checks); renderer + PWA tsc clean (bumped desktop/tsconfig lib to ES2024 for Promise.withResolvers), server tsc at baseline (pre-existing dev.ts:586 only), license clean. DEPLOYED to lucid-agent.web.app/remote + live-probed (cu-shot/sv-img in index.html; cu-shot-btn/shot-viewer/preview-snapshot in app.js). Live capture->broadcast->phone is the on-device step (Electron capturePage + a live share).
- **next:** slice 2 (PWA markup on the fullscreen snapshot + send-back via PromptFrame images), then slice 3 (agent PWA-awareness / autodetect).

## P-REMOTE.11b: PWA composer double-decker + favorites-filtered guest model picker (ADR-0238)
- **shipped:** (1) the composer is its own double-decker - the full-width prompt bar owns its row and the action row ([+] ... [Stop] [Send]) sits UNDER it, AUTO-COLLAPSED: shown while composing (focus / non-whitespace text / staged attachments) or while a turn streams (Stop stays reachable), hidden otherwise, with the blur-path hide delayed 200ms so a Send tap lands before the row collapses (iOS fires blur before click). With the catch-up card (top) and the status/account strip (bottom) already auto-collapsed, a live session now shows ONLY the transcript + prompt bar + two thin bars. (2) The guest MODEL picker offers just the host's FAVORITES + the CURRENT model (pure `offeredModels` in model_favorites.ts; no favorites -> full list), applied at BOTH hosts so the phone never receives the full catalog: direct-P2P reads renderer localStorage (FAVS_KEY); relay rides `favModels` on /api/collab/start (validated, MAX_FAVS cap) into dev.ts buildCollabOptions. FOLDER options unchanged (already-used allowlist).
- **verified:** offeredModels tests (favorites-only / current-always / no-favs-full-list / stale-hidden) -> favorites 20/0 + collab 360/0; PWA + renderer tsc clean, server tsc baseline; DEPLOYED to lucid-agent.web.app/remote + live-probed (composer-actions in index.html + app.js); mockup mirrors the final layout.
- **next:** item C slice 2 (PWA markup on the fullscreen snapshot + send-back via PromptFrame images), slice 3 (agent PWA-awareness). Live Drive reconnect is now console-unblocked (user completed the Google Cloud steps) - exercise on-device: desktop Share -> Reconnect -> Authorize + save a code; phone /remote with no link -> Get a reconnect code.

## P-PREVIEW-PWA.2: phone markup on preview snapshots + send-back, SVG buttons, install path (ADR-0239)
- **shipped:** item C slice 2. The fullscreen snapshot viewer gains finger MARKUP: an overlay canvas armed by a Draw toggle (disarmed = pinch/scroll untouched), red ink, per-stroke Undo. Strokes live in NORMALIZED image space (pure `toNormPoint`: clamped, never NaN; survives rotation/resize) and scale losslessly onto the natural-size composite; one pen-width formula (`penWidthFor`) at both scales (ink parity). "Send to agent" composites image+ink and STAGES it through the SAME fail-closed P-REMOTE.8 attachment path as a pasted image (`acceptAttachment`; SVG refused), the user adds words + taps Send -> arrives as a guest image prompt through the host's gate. Hidden for view-only guests. Save-to-device composites too. ALL composer/viewer controls are now flex-centered inline SVGs (plus/stop/send/pen/undo/download/X) - the text "+" glyph sat off-baseline. Full screen: the manifest is already standalone, so the sign-in view now surfaces the install path (Android: a REAL one-tap Install via beforeinstallprompt; iOS: the Add-to-Home-Screen tip; standalone: no hint).
- **verified:** toNormPoint/penWidthFor tests -> collab 364/0; `make demo-P-PREVIEW-PWA.2` green (5 checks incl. PNG-accepted/SVG-refused through the real P-REMOTE.8 gate); PWA tsc clean, 36.1 KiB; DEPLOYED to lucid-agent.web.app/remote + live-probed. Finger ink + composite = the on-device step.
- **next:** item C slice 3 (agent PWA-awareness / autodetect: tell the agent a phone guest is watching so it offers previews). Live Drive reconnect ready to exercise on-device (console steps done).

## P-PREVIEW-PWA.3: agent PWA-awareness via a trusted per-turn preamble (ADR-0240) - item C COMPLETE
- **shipped:** while guests watch a Session Share, the agent's prompt now carries a `<session-share>` awareness block: audience size + access mix ("3 remote guests are watching... 1 can drive, 2 view-only") + the actionable suggestion (broadcast the Preview via "To phone"; marked-up snapshots come back as image prompts). Design: rides the USER-TURN preamble channel (like active-skill/recall - never the frozen prefix, invariant #6); rebuilt from the LIVE roster every /api/chat turn (autodetect: appears with the first guest, vanishes with the last); COUNTS ONLY - guest names are untrusted and withheld (invariant #5; pure `buildShareAwareness` builds from clamped integers + fixed strings, `accessCounts` reduces the roster); the P-COLLAB.15 mirror taps the CLEAN prompt BEFORE augmentation so guests + the transcript see exactly what was typed. Both transports: relay reads the backend CollabManager roster; direct-P2P sends validated counts with the turn (new `share` param on sendPrompt).
- **verified:** share_awareness tests (autodetect / wording / hostile-name-never-survives / clamping) -> collab 350/0; `make demo-P-PREVIEW-PWA.3` green (5 checks incl. the composition rule); renderer tsc clean, server tsc baseline, license clean. Desktop-only (dev instance on reload); the live mid-share suggestion is the on-device step.
- **next:** item C (preview snapshot -> PWA) is COMPLETE: slice 1 broadcast+view+save (ADR-0237), slice 2 markup+send-back+SVG buttons+install path (ADR-0239), slice 3 agent awareness (ADR-0240). On-device: exercise the live Drive reconnect + preview-to-phone + markup round-trip; remaining roadmap is new asks.

## P-COLLAB.19: dual invite links - EDIT and VIEW-ONLY guests in one share (ADR-0241)
- **shipped:** an edit share now hands out BOTH capabilities of the one room. New pure `mintRoomLinks` (link.ts) mints fullLink (write token: drives) + viewLink (key only: watches) + their phone twins browserLink (edit-capable when allowed) + NEW `browserViewLink` (always watch-only); BOTH hosts (relay manager.ts + direct-P2P collab_p2p.ts) call it, killing the duplicated minting blocks. The Share dock shows TWO invite sections on an edit share - "Edit invite link (guest can drive)" and "View-only invite link (watch only)" - each with its own copy row + phone QR; a view-only share keeps the single section. Preserved exactly: the legacy (no-PWA) browser form stays token-free, and allowEdit still gates writes host-side (fail-closed).
- **verified:** link.test.ts mintRoomLinks tests (capability split / same room + key / view + legacy token-free) + manager.test.ts dual-capability status test -> collab suite 354/0; `make demo-P-COLLAB.19` green (5 checks, real crypto); renderer tsc clean, server baseline, license clean. Host-side write refusal for view guests already pinned by demo-P-COLLAB.12.
- **next:** on-device pass over the whole remote stack (Drive reconnect round-trip, preview -> phone -> markup -> agent, dual links to two different guests); remaining roadmap is new asks.

## P-COLLAB.20: the Join panel becomes a floating, non-blocking dock (ADR-0242)
- **shipped:** watching another LUCID is no longer a centered BLOCKING modal - the Join panel now rides the P-SHARE.1 dock chassis: movable (header drag), resizable (e/s/se handles), snappable (flush-right / beside-the-rails), minimizable to a status-bar pill (eye + live dot, beside the share pill), NON-BLOCKING (no backdrop - the local LUCID stays fully usable while watching or driving the remote one with an edit link). `loadDockState`/`saveDockState` gained a storage KEY (+ first-open fallbackShape): the join dock persists under `lucid.joinDock.v1` and first-opens bottom-LEFT beside the rails so the two docks never stack. Escape MINIMIZES (non-destructive, only when focus is inside the dock); the X closes + leaves. ADR-0222's fill-the-app watching mode retired; dead `.join-modal` CSS removed; all join logic (connect / live transcript / drive composer / P2P-vs-relay) untouched.
- **verified:** keyed-persistence tests (independent share/join state, fallback placement, stored-beats-fallback, viewport re-clamp) -> renderer suite 734/0, renderer tsc clean, license clean; `make demo-P-COLLAB.20` green (5 checks incl. one-chassis snapping). Drag/resize/pill = the exact P-SHARE.1 pointer wiring; dev instance on reload.
- **next:** P-COLLAB.21 multi-join ORCHESTRATOR - N watched sessions in N docks. Seams named in ADR-0242: dev.ts single `collabGuest` slot -> Map keyed by joinId; bridge.ts single stream-abort -> per-join handles; collab_p2p.ts single guest slot -> multi; joinId routing on guest-prompt/abort/model/workspace; cascaded dock placement + per-dock pills.

## P-SHARE.3: the Session Share rail glyph always reveals the dock (bug fix)
- **shipped:** clicking the left-rail Share glyph (#railShare) now ALWAYS reveals the Session Share dock. Two dead paths fixed in openSharePanel (app.ts): (1) after the user minimized the dock to a status-bar pill, #shareModal stayed in the DOM (just hidden), so the single-instance guard `if ($("#shareModal")) return` made every subsequent rail click a no-op; (2) a persisted `minimized:true` (from a prior session) made an explicit open run straight to a hidden pill via `if (dockState.minimized) minimize()`, so the module never popped up. Now a module-level `restoreShareDock` handle points at the live dock's `restore`; the guard calls it to un-minimize + remove the pill instead of no-op'ing, `close` nulls it, and an explicit open clears a stale persisted `minimized` flag (persisting the correction) rather than opening minimized.
- **verified:** renderer tsc clean; share_dock.test.ts 21/0 (pure module untouched); QA-gated in-app against the live-bundled dev renderer (browser) - plain open visible, minimize->rail-click reveals + drops the pill, restart-with-persisted-minimized reveals + clears the flag, close->reopen still works. Minimize-to-pill unchanged.
- **next:** none for this fix; remaining Share roadmap is new asks.

## P-SKILL.6: the Skills panel loads instantly (SWR cache) + never shows blank while loading (ADR-0243)
- **shipped:** the Skills directory used to `await bridge.skills()` (the slow /api/skills disk scan) with `#skillsBody` left BLANK the whole time - no feedback, and slow on every open. Now stale-while-revalidate, mirroring the session-list / Share-dock caches: the discovered skill list (`SkillView[]`, metadata already plaintext on disk) is cached in swr_cache (`lucid.skills`, new `cachedSkills`/`setCachedSkills`) and painted INSTANTLY on open (bundled INSTALLED_SKILLS are inline, always instant); a cold first open with no cache shows a spinner (`.skdir-loading`), never a blank panel; then /api/skills revalidates in the background behind a subtle `.skdir-updating` chip and repaints only if the directory actually changed. `renderSkills` split into pure-ish `skillRowsFrom` + `paintSkills`; new pure `skillsDirSig` (skills_dir.ts) gates the repaint (captures key/name/desc/trust/enabled/enableable/removable/findings) so a toggle / re-scan verdict / add / remove still repaints but an unchanged revalidation just drops the chip - no flicker. Side benefit: an enable toggle now reflects instantly from cache instead of awaiting the slow fetch.
- **verified:** renderer tsc clean; swr_cache.test.ts (+cachedSkills round-trip) + skills_dir.test.ts (+skillsDirSig: identical-match / toggle / trust+findings / add-remove) -> 42/0 across the touched suites; QA-gated in-app against the live-bundled dev renderer (browser): cold open shows the spinner (not blank), a seeded cache paints all 15 rows SYNCHRONOUSLY on open with the updating chip + no spinner, layout screenshot clean. The slow /api/skills is the pre-existing call (unchanged), now wrapped by the cache + spinner.
- **next:** none for this fix; skills roadmap is new asks.
**Model-picker freeze safety-net: getConfig() never blocks (branch fix/model-picker-freeze) — 2026-07-15**
- **context:** releases rolled back to v1.11.3 (v1.11.4/.5/.6 pulled to prerelease) because the picker stays frozen on "updating…" whenever AskSage is configured (user repro: frozen with lockdown OFF too, permanent). Traced to `getConfig()` awaiting `ensureSession()`, whose `session/new` hangs under AskSage → `bridge.config()` never resolves → renderer `loadConfig()` await hangs → spinner spins forever on the stale cached (commercial) list. The AskSage-extension changes since v1.11.3 are benign (data-only), so the hang is omp-session-level — root cause pending the user's live omp stderr log.
- **shipped (safety net, NOT the root fix):** `getConfig()` now `Promise.race`s `ensureSession()` against a 6s cap and returns the current config either way (never blocks); a `sessioning` dedupe guard shares one `session/new` across racing callers. Renderer `loadConfig()` RE-POLLS (6×1.5s) until the live list lands, then STOPS and clears the spinner rather than freezing forever. Healthy path unchanged.
- **verified:** desktop tsc clean; live dev-server smoke (this box, non-AskSage): model badge resolved to the real `claude-opus-4-8` (labeled "Claude Opus 4.8"), not busy, zero console errors → live config still adopts, no polling loop. The AskSage-hang path awaits the user's repro + log.
- **next:** read the omp stderr from a frozen AskSage session to pin WHY `session/new` hangs (initialize vs session/new vs a provider call), fix the actual cause, THEN re-enable releases.

**Fix Linux air-gap Python bundling → v1.11.7 (ADR-0225) — 2026-07-15**
- **shipped:** v1.11.6 fixed the rpm build but its CI air-gap gate then FAILED on Linux (Win+mac passed): python-build-standalone ships `bin/python3`/`bin/python` as SYMLINKS to the real `bin/python3.12`, and electron-builder DROPPED them copying `runtimes/` into the Linux package → packaged app had no interpreter (would fall back to `uv venv`/network on air-gapped Linux). The gate caught it before any Linux installer shipped. FIX: `fetch-runtimes.ts` copies the Python tree with symlinks DEREFERENCED (all real files); `runtime.ts bundledPython()` + `airgap-smoke.ts` also fall back to `bin/python3.12`. v1.11.6 pulled (prerelease).
...
## P-NVIM.6: view the knowledge graph from Neovim — `lucid kb` + `:LucidKb`
- **shipped:** the GUI's knowledge-graph browser, now in Neovim via a keybinding. A new read-only `lucid kb` CLI (`tools/kb_cli.ts`, wired into the launcher's `main()` like `lucid stats`) exposes `list | pages | show <id|slug> | search <query>` over the SAME `~/.omp` KG registry + per-KG DuckDB the desktop app uses (`desktop/kb_store.ts`; defaults shared, no env override) — `--json` for editors, `--kg <id>` to target a KG, human-readable otherwise. Pure read: no agent, no gate spawn, no mutation. The plugin (`extensions/neovim/lua/lucid/init.lua`) adds `M.kb()` + `:LucidKb` + `<leader>lk`: pick a KG (active first; skipped when there's only one) → pick a page → read it in a read-only markdown float. Uses `vim.ui.select`, so telescope/fzf-lua/snacks drive the picker with ZERO extra dependency.
- **verified:** `make test` 2313 pass / 0 fail + sidecar green — new `tools/kb_cli.test.ts` (20: kbList active-flag/counts, kbPages, kbShow by-slug/by-id/missing + a JSON-safe/no-BigInt guard, kbSearch title-first + snippets, snippetAround, runKb dispatch + exit codes + --json/text + --kg) and `lucid_acp.test.ts` `main kb` routing (kb doesn't fall through to the TUI). Neovim pure helpers (`_kb_kg_label`/`_kb_page_label`/`_kb_body_lines`) added to the headless spec (`helpers_spec.lua`, `luac -p` clean; run by CI + `make demo-P-NVIM.1` where nvim exists). `make demo-P-NVIM.6` green against a seeded temp KG; root tsc clean; the live resolver returns the seeded pages end-to-end.
- **stubbed:** the GUI force-graph (node/edge canvas) has no terminal equivalent — the viewer is a page browser + body reader (the terminal-native form of "look at the KG"); page links aren't surfaced yet. `kbSearch` is lexical (title+body substring), not semantic — the in-chat `knowledge_search` tool does RAG. The headless nvim spec + interactive `:LucidKb` flow weren't run locally (nvim not installed on this box); the pure helpers are `luac`-checked + covered by the Bun/CI headless run, and the `vim.ui.select`/float glue mirrors the proven `:LucidStats` path.
- **next:** optional — surface a page's links (related pages), wire `lucid kb search` into `:LucidKb` via `vim.ui.input`, and a `:LucidKb <slug>` direct-open.


## P-BRAND.1: the LUCID TUI welcome — omp's welcome box suppressed + a branded banner (issue #314)
- **shipped:** `lucid tui` no longer looks like omp. omp's welcome (the block-Π logo + the "omp v…" title) is a frozen module const with no injection seam, so — extending, never forking (invariant #1) — the launcher suppresses it per-session with a `--config` overlay (`harness/omp/lucid_tui.config.yml` → `startup.quiet: true`, deep-merged so bare omp is untouched) and a new optional `-e` (`harness/omp/lucid_welcome_extension.ts`) paints the LUCID brand: on `session_start` it sets an above-editor widget with the LUCID block wordmark (accent-coloured by the P-THEME.1 skin) + `LUCID v<version>` + model, then clears it on the first `turn_start` so it greets without lingering. Wired into `LaunchAssets`/`buildTuiArgs`/`runTui` AFTER the gate `-e` and the theme `-e`; TUI-only (ACP/desktop have their own chrome). FAIL-OPEN like the theme: no UI / throwing setWidget / unreadable version all degrade to "not applied", never throwing, never weakening the fail-closed gate.
- **shipped (cont.):** the banner now reproduces omp's welcome CONTENT too, so suppressing omp's box loses no info — only the branding: static **Tips**, the workspace's **LSP servers** (omp's own `discoverStartupLspServers(cwd,"available")`), and **recent sessions** (`getRecentSessions`), each fail-open. Refactored into a PURE, omp-free core (`harness/omp/lucid_welcome.ts`, unit-tested) + a thin wrapper (`lucid_welcome_extension.ts`) that gathers data via omp's supported subpath exports.
- **verified:** `make test` 2327 pass / 0 fail + sidecar green (30 `lucid_welcome.test.ts`: welcomeEnabled matrix, renderWelcomeLines LUCID-not-omp via word-boundary + version/model/Tips/LSP/recent + LSP_MAX/SESSION_MAX caps + paint, lucidVersion parse+fallbacks, applyWelcome 4 fail-open paths; +5 `lucid_acp.test.ts`: argv gate→theme→welcome + `--config`, welcome on/off). `make demo-P-BRAND.1` green (renders the full LUCID welcome preview); `make demo-P-THEME.1` still green; `make demo-02` prefix-hash unchanged (invariant #6); `make test-failclosed` 15/15; root tsc clean; `lucid check` OK; live omp resolver returns `clangd [.c .cpp .cc .cxx]` for this repo (matches the screenshot).
- **stubbed:** LSP shows the **"available"** set (exactly what omp's welcome shows in lazy mode, the default); live "ready" status is emitted by omp only when its own welcome is NOT suppressed (the emit is gated on `!startup.quiet`), so it's unavailable while we suppress the box — a deliberate omp coupling, not closable without a fork. Interactive PAINT proven via the DI `applyWelcome`/`setWidget` seam + the demo preview, not a PTY screenshot (same limit as P-THEME.1). Version from `desktop/package.json` (mirrors APP_VERSION). Escape hatch `LUCID_WELCOME=off` restores omp's own welcome.
- **next:** optional — a compact narrow-terminal fallback for the wordmark; a light-mode variant; upstream a `welcome.logo`/`welcome.appName` hook to omp (would let the box be rebranded in place AND recover live LSP status).


## P-REMOTE.9b: readable Thinking on the phone PWA (open-state + gist)
- **shipped:** the guest PWA's Thinking blocks are now actually usable while the agent works. Root cause of "guests just watch tool calls": (1) the per-event `innerHTML` repaint reset every `<details>` to collapsed, so an opened Thinking block snapped shut on the next streamed token (functionally un-openable mid-turn), and (2) the collapsed row was a bare dim "Thinking" with no gist. Fix, kept in the PURE layer: `pwa_view.ts` `thinkingGist` (last non-empty line, whitespace-collapsed, 64-char clip) rendered ESCAPED into the summary (hidden while open), `renderItem(item, i, activeThinking)` stamps `data-think="<i>"` and renders the TRAILING thinking block OPEN (desktop parity: live reasoning streams visible, collapses when the answer starts); `app.ts` records the user's tap intent per `data-think` index and re-applies it after every repaint (cleared on reconnect).
- **verified:** pwa_view 15/15 (+4: gist last-line/clip/blank, trailing-open-then-closed, data-think keying, escaped-gist incl. hostile text); PWA tsc clean; rebuilt 36.9 KiB; DOM-geometry check in the browser preview (closed row 43px single-line ellipsis with gist, open 106px full text with gist hidden, toggle round-trip). The live repaint-persistence path needs an on-device relay session (QA) - the mechanism is the tested intent map + a 3-line render hook.
- **next:** deploy the rebuilt PWA to lucid-agent.web.app/remote via the addon repo (CI, runbook-safe) and an on-device pass; consider the same gist treatment for the desktop guest's thinking blocks.

## P-SHARE.4: Session Share Start/Stop feels instant (progress readout + optimistic teardown)
- **shipped:** the Share dock no longer looks frozen during the relay handshake. Start now DISABLES its button and shows a two-line progress readout right under it that narrates each phase ("Preparing to share…" -> gated: "Authorizing this device…" -> "Opening the room…"; P2P: "Finding a relay…" -> "Connecting…"), with the relay label; a `startFail` helper re-enables + toasts on any error, and a double-click guard blocks re-entry mid-handshake. Stop is now OPTIMISTIC: it tears the room down locally (synchronous), paints the idle body immediately from cached serve/p2p/relay config (`drawIdleOptimistic`, no status round-trip), toasts, and runs the backend `collabStop()` + a reconciling `draw()` in the background - so the button never hangs on a slow socket close. New pure-ish helpers `setShareProg`/`drawIdleOptimistic` + `lastServe`/`lastP2pCfg`/`lastRelay` caches seeded each draw.
- **verified:** renderer tsc clean (exit 0); collab + share_dock suites 384/384; the `.share-prog` readout rendered against the REAL styles.css in the browser preview (accent card + pulsing dot, bold status line + muted detail; hidden when idle; Start dims while disabled). The live Electron start/stop timing is QA-gated in-app like the rest of the dock DOM.
- **next:** on-device pass across hosted-relay / self-hosted-LAN / direct-P2P (confirm each phase label fires + Stop is instant); consider a phase line for the guest PWA connect, too.

## P-PREVIEW.9: reliable preview screenshots/inspection for tool-conservative models (Fable 5)
- **shipped:** Claude Fable 5 (and the 4.8 family) reach for CUSTOM tools less than Opus/Sonnet and sometimes narrate intent ("I'll screenshot it") without emitting the call, so they "just watched tool calls" instead of verifying UI in the preview. Two fixes, no new mechanism. (1) `harness/omp/preview_extension.ts`: rewrote the `preview_open` / `preview_screenshot` / `preview_inspect` descriptions to be TRIGGER-prescriptive ("call this right after you write/edit a UI file … a build/design task is not done until you've looked at it") — the Anthropic-recommended lever for this model (prescriptive at the tool-description level beats piling scaffolding into the system prompt). The screenshot tool's empty-cache replies are now SELF-CORRECTING: they route the model to preview_open (re-surface) or preview_inspect (reads the rendered DOM via the postMessage bridge, works even when the panel isn't front) instead of dead-ending. (2) `desktop/renderer/app.ts` `kickPreviewShotSoon()`: fires an immediate shot capture (250/700ms) the moment the agent preview lane surfaces (onPreviewAvailable + switch-to-agent), so a preview_open→preview_screenshot sequence finds a FRESH shot without waiting a full 1.5s freshness tick. NOT changed: the capturePage on-screen-pixels constraint (a hidden agent lane still can't be screenshotted — that's what preview_inspect is for) and Fable 5's vision (it can see the returned PNG fine).
- **verified:** preview_extension 24/24 + demo-P-PREVIEW.3a and demo-P-PREVIEW.3a-shot green; desktop + root tsc clean. The live model-behavior change (does Fable 5 now call the tools) and the renderer capture timing are QA-gated in-app.
- **next:** on-device pass with Fable 5 selected (write an .html, confirm it screenshots/inspects the preview and self-corrects); optional held-request shot relay so a hidden agent lane can be surfaced-and-captured on demand.

## P-STT.4: visible dictation - failure surfacing + live mic waveform in the composer
- **shipped:** the mic no longer fails silently: the STT backends fail-safe (empty text + note) and the driver DROPPED the note, so a never-started Whisper server looked like "mic on, nothing in the prompt bar". New pure `sttFailureMessage` (dictation.ts) classifies engine failure vs genuine silence; the driver toasts once per session with one-click "Start local Whisper" (managed runtime, when capable + bin present) and a jump to Settings -> Voice. Plus the live waveform chip next to the mic (#ctWave): DPR-crisp canvas, 90ms peak buckets scrolling right-to-left, whole-second gridlines, VAD-threshold coloring (red = counts as speech), and an elapsed m:ss clock - pure bits (`pushWave`, `waveHeight`, `waveClock`) in dictation.ts, canvas in app.ts.
- **verified:** renderer/dictation.test.ts 16/16 (failure classifier, ring buffer cap, level-curve clamp/monotonicity through the VAD threshold, clock); desktop tsc clean; full desktop suite green except pre-existing symbol_graph/egress_audit env failures (fail identically on stashed HEAD).
- **next:** on-device pass - dictate with the managed server stopped (expect the warn toast + one-click start, then words landing), and eyeball the wave chip at 1x/2x DPR; consider a preflight probe of sttUrl at mic-start.

## P-STT.5: never two whisper-servers - orphan reap/adopt + shutdown hooks
- **shipped:** live QA on the packaged app found TWO whisper-server pids co-bound to 127.0.0.1:9111 (httplib binds SO_REUSEPORT, so a relaunch double-spawned and the kernel split requests across two model loads; main.ts stops the dev child with SIGTERM, which skips "exit" handlers, so nothing killed the old child). Fixes: whisper_runtime.startWhisper now health-probes the port pre-spawn, reaps a squatter via injected deps.reapPort (dev.ts: lsof/SIGTERM, netstat/taskkill on win32), and ADOPTS an unkillable survivor rather than double-spawn (status running, tier honestly null); stopWhisper(deps) reaps an adopted server; dev.ts registers exit + SIGTERM/SIGINT handlers that stop the child.
- **verified:** whisper_runtime.test.ts extended (reap-then-spawn ordering + accurate tier, adopt with no duplicate spawn + null tier, stop-reaps-adopted; start-path health mocks made stateful) - 62/62 across the whisper suites + dictation, desktop tsc clean; on-device: killed the stale orphan, single LISTEN pid remains, /inference round-trip still transcribes.
- **next:** repackage (bun run dist:mac) so the running lucid-test build picks up P-STT.4 + P-STT.5; confirm quit-and-reopen leaves zero whisper-server pids.

## P-VOICE.2: live read-aloud from the composer - voice picker + auto-speak (ADR-0247)
- **shipped:** LUCID can now talk while it writes, and the controls are where the mic is. New `#ctVoice` chip in the composer opens a picker (auto-speak switch + engine + that engine's voices + Test), and a `#ctSpeak` indicator with a one-click stop appears while a reply is being read. Auto-speak pumps the STREAMING answer buffer through a new pure chunker (`harness/voice/speech_stream.ts`): each complete sentence is synthesized on its own and played in order, so audio starts after sentence one instead of after the whole reply - and text after an unterminated ``` fence is withheld, so mid-stream source is never narrated. Ordering/cancellation live in an injected, DOM-free `SpeechQueue` (`desktop/renderer/speech_queue.ts`): serial synthesis, one clip audible, prefetch ceiling 2, generation counter so a cancelled turn's in-flight synth is discarded. Fixed two real bugs behind the old picker: `/api/tts/speak` PINNED OpenAI/Kokoro to `alloy`/`af_heart` (the selection was read then thrown away), and `ttsVoice` was ONE field for three engines (an ElevenLabs id could reach Kokoro). Now `harness/voice/catalog.ts` is the canonical voice set for every engine, `resolveVoice()` blocks cross-engine ids server-side, and the store remembers a voice PER engine (the legacy scalar is read as the ElevenLabs fallback, then retired - no migration step). `GET /api/voices?provider=` answers for all three engines, not just ElevenLabs. Auto-speak defaults OFF (cloud egress + per-character cost).
- **shipped (cont.):** popover placement extracted to a tested pure `placePopover()` (`desktop/renderer/popover_place.ts`) and `popover()` now returns `reposition()`. The composer pickers anchor to the BOTTOM of the window, so "flip above" is their normal case - and the voice card GROWS after opening (skeleton -> fetched list), which would have extended it straight off the bottom edge from a `top` computed for the shorter card. Placement also caps height to the room available (`.popover` gained `overflow-y:auto`), so a tall card in a short window scrolls instead of clipping; previously nothing capped it at all.
- **verified:** `make demo-P-VOICE.2` green - 18 checks: per-engine voice memory + legacy migration + auto-speak default-off; chunker over a fenced reply (first span at char 35/75, open fence withheld, every sentence spoken exactly once, cursor round-trips losslessly); SpeechQueue order/ceiling/cancellation/handle accounting (5/5 handles released, stop() drops in-flight work); placement (520px card flips above the chip, re-place after load moves it UP 340px, 620px window caps to 548px + scrolls, right-edge anchor pulled back inside). +13 unit tests (`harness/voice/speech_stream.test.ts`, `catalog.test.ts`). `bun test harness` 1745 pass / 3 fail - the 3 are pre-existing `lucid_acp.test.ts` launcher-asset failures that fail identically on a stashed HEAD. Renderer tsc clean; root + server tsc at baseline (31 pre-existing diagnostics, unchanged). CSS eyeballed against the real stylesheet in the browser preview (chip states, upward flip, height cap).
- **stubbed:** TTS keys are still plaintext-at-0600 in `~/.omp/lucid-gui.json` - TTS has no key of its own (it reads the providers' `ELEVENLABS_API_KEY`/`OPENAI_API_KEY` via `applyEnv()`), so vaulting it is a provider-key-store ADR, not a voice change: `cred_vault.ts` is main-process-only while `dev.ts` is a child, and `setKey()` currently takes effect immediately by writing that child's env. Live audio (does it actually sound continuous on a real engine) is QA-gated in-app like the rest of the composer DOM.
- **next:** on-device pass - auto-speak a long reply on each engine, confirm mid-stream start, instant stop, and no code narration; optional `/api/tts/stream` for true chunked transport; dots.tts drops in as `local-tts` behind an ~80-line FastAPI `/v1/audio/speech` shim with zero client change.

## Side-by-side LUCID instances: the control panel's free-port pick was dead effort (P-VOICE.2 follow-up)
- **shipped:** `LucidAgentIDE.bat` -> `G` rolls a free high port when 5319 is held ("another LUCID app?") so a dev build can run BESIDE an installed one - but pressing G just re-focused the already-running window. The port was never the problem: Electron's single-instance lock (`main.ts`, ADR-0206/P-KGMARKET.4) is keyed on the **userData directory**, which knows nothing about `LUCID_PORT`, so the second launch hit the guard, called `app.quit()`, and fired `second-instance` -> `win.focus()` on the first. Fix: `main.ts` now keys instance identity on the port - a non-default `LUCID_PORT` gets its own userData dir (`<userData>-<port>`), set BEFORE `requestSingleInstanceLock()`. Two instances can never share a port, so they can never share a lock. The default-port instance keeps the canonical identity and is the ONLY one that registers the `lucid://` scheme, so the OAuth deep-link still re-focuses the primary app instead of being hijacked by a test build.
- **verified:** desktop tsc clean; `bun run build` (main+preload) green. Shared state is unaffected - settings, chat sessions and the credential vault live under `~/.omp` (homedir), not userData; the only userData consumer in main.ts is `engine.log`, which correctly splits per instance. The live two-window run is the on-device check.
- **next:** on-device - launch via `G` while a LUCID is already open and confirm a SECOND window appears on the rolled port, with the same providers/keys and its own engine.log.

## P-VOICE.3: engine readiness in the picker + hands-free conversation mode (extends ADR-0247)
- **shipped (readiness):** the picker offered every engine unconditionally, so a user signed in to ChatGPT via OAuth could select ChatGPT/OpenAI, see thirteen voices, pick Cedar - and get "Couldn't read it aloud" on every single reply. Root cause is EXTERNAL and unfixable in LUCID: `/v1/audio/speech` is an OpenAI PLATFORM endpoint that takes a platform key (`sk-...`) only; a ChatGPT subscription login is a different credential for a different backend and the platform API rejects it, and there is no OAuth flow that mints a platform key. So the fix is honesty at the point of choice: new pure `ttsEngineStatus()` (`harness/voice/catalog.ts`) computes per-engine readiness + the SPECIFIC reason, `dev.ts` `ttsEngines()` resolves it live (key presence, the omp OAuth row via `providerAuth()`, and a cached 700ms probe of `LUCID_TTS_URL` for Kokoro), and `/api/voices` ships it. Unavailable engines render greyed + non-selectable with a "needs setup" tag (reusing the existing `.cfg-opt.unavail` treatment), clicking one explains itself with a jump to Settings, the selected-but-blocked engine shows its reason inline, and Test voice disables. `/api/tts/speak` now returns the SAME string, so a failure can never contradict the menu. An OAuth-only OpenAI user is told their sign-in cannot reach the speech API and pointed at both fixes (add a key, or use ElevenLabs/Kokoro) instead of the old "Add your OpenAI API key", which reads like a bug when you are visibly signed in.
- **shipped (conversation):** its own checkbox, nested under auto-speak in the composer picker and mirrored in Settings -> Voice. The loop: reply finishes being SPOKEN -> `SpeechQueue.onIdle` -> mic opens by itself -> the tested dictation VAD ends the turn on a longer silence -> the transcript sends. New `ttsConversation` setting reads false whenever auto-speak is off (the preference survives, but nothing opens the mic while the agent is silent). Guards on every side of `maybeListen()`: both toggles, `!state.streaming`, `!speech.busy`, no session already running, and window not hidden - so it can never listen to the agent's own voice. Auto-send only fires when the session ended on SILENCE (`autoSend`), never on a manual mic click, and only once every in-flight transcription has landed (`pending`) - a click-stop still means "let me read it first". `getUserMedia` now requests `echoCancellation`/`noiseSuppression`/`autoGainControl` explicitly rather than trusting the UA default, since the mic is open while the speakers settle. `onIdle` fires on natural drain ONLY, never from `stop()`, so silencing the agent cannot re-open the microphone.
- **verified:** `make demo-P-VOICE.2` 25 checks green (+3: conversation gated-but-remembered across an auto-speak toggle; onIdle fires exactly once on drain; `stop()` never fires onIdle). +3 catalog unit tests on the readiness reasons - the OAuth-only OpenAI message must name the ChatGPT sign-in, the platform key, AND the ElevenLabs/Kokoro escape hatch; an OAuth row must never flip readiness; Kokoro's reason must contain the URL it probed. `bun test harness/voice` 47/47. Renderer tsc clean, root/server tsc at the 31-diagnostic baseline, electron main rebuilt. The live loop (does it actually take turns without hearing itself) is on-device QA.
- **next:** on-device - ElevenLabs or Kokoro + conversation mode, confirm the mic opens only after the audio stops, that a pause mid-sentence does NOT send, and that a 3s silence does; tune `autoStopSilenceMs` if 3s feels short in practice. OpenAI TTS stays unavailable on OAuth-only by design - the escape hatch is a platform key or another engine.

## P-VOICE.3b: the voice picker clips - ONE scroll region, not two (follow-up)
- **shipped:** with conversation mode + a blocked-engine reason note, the picker grew to ~620px of content. `placePopover()` correctly capped it to the room above the composer, but the card itself was the scroll container (`.popover{overflow-y:auto}`), so the cap STRETCHED the card to the full available height and everything past the fold - the rest of the voice list and the Test voice / Voice settings footer - fell below the visible edge, with the last row sliced mid-height. Worse, the voice list had its OWN `overflow`, so a wheel over it fought the outer scroller. Fix: the card is now a flex column in which every block is `flex:none` and only the voice list flexes (`flex:0 1 auto`, cap 236px, floor 64px). It caps at a sane height on a tall window and SHRINKS on a short one, so the header, both toggles, all engine rows, the reason note and the footer are always on screen and there is exactly one scroll region. The popover keeps `overflow-y:auto` purely as a floor case (a scrollbar beats a cut-off card). Also trimmed the OAuth reason and the conversation blurb, which is what pushed the card over the edge in the first place.
- **verified:** `make demo-P-VOICE.2` 25/25 still green; `bun test harness/voice` 47/47; renderer tsc clean. Eyeballed against the REAL stylesheet in the browser preview at the reported state (both toggles on, OpenAI blocked, Kokoro offline, 13 voices) with a live readout asserting `footer VISIBLE` - held while dragging the window from full height down to ~500px.
- **next:** on-device confirm in the Electron window at a few sizes; if the card still feels tall, the engine rows and the reason note are the next things to compress.

## P-VOICE.4: the speech equalizer + the draggable "LUCID Agent [Voice]" panel (ADR-0248)
- **shipped:** a glowing hi-fi spectrum analyser that dances to the agent's ACTUAL voice - segmented LEDs, teal to amber to red, with the hanging peak caps that are the signature of the era. It taps real audio, not a decorative animation: one persistent `<audio>` element is routed through a Web Audio `AnalyserNode`. That forced a real change to the queue - `createMediaElementSource()` may be called ONCE per element, so the old fresh-`Audio()`-per-sentence play path could never have been analysed (and churned an element per sentence besides); the SpeechQueue now swaps `.src` on a single element. Two surfaces share ONE analyser and ONE rAF loop that retires itself once the bars settle: a mini strip inside the composer's Speaking chip, and a floating **LUCID Agent [Voice]** panel the user pops out and anchors anywhere - drag the header (snaps to a frame edge), drag the corner to resize, minimize to a header pill, close to send it back to the prompt bar. Geometry/clamping/snapping/persistence are the already-tested share-dock primitives (`share_dock.ts`, ADR-0232/0242) under their own `lucid.voiceDock.v1` key, so the Voice panel and a live Share dock coexist without moving each other; the popped-out state persists, so an anchored panel survives a restart.
- **shipped (cont.):** the display maths is pure + tested (`harness/voice/eq_bands.ts`). Two properties decide whether it reads as an instrument or as noise, and both are pinned: LOG-spaced bands (an FFT is linear, so linear buckets give three busy bars and eleven dead ones - ears are logarithmic and so was every stereo that had this display), and asymmetric ballistics (40ms attack / 220ms release + a 380ms peak hold falling at 1.15/s) which produce the characteristic snap-up-glide-down. FAIL-SAFE by construction: the audio graph is built lazily and ONLY after `AudioContext.resume()` actually succeeds, because routing an element into a suspended context mutes it irreversibly - if Web Audio cannot run, playback stays native and the display simply idles. A visual is never traded for the agent's voice.
- **verified:** 7 new unit tests (`eq_bands.test.ts`, 54/54 across the voice suites): band edges strictly increasing / in range / provably non-linear (top band > 4x the width of the bottom) and well-formed with more bands than bins; a narrow formant survives folding (peak, not mean); release measurably slower than attack; a cap holds through 300ms then falls at exactly `peakFallPerSec`, never below its own bar; the display reaches rest so the loop can stop; a 100-second stalled frame glides instead of teleporting and out-of-range targets are clamped. `make demo-P-VOICE.2` 25/25 still green; renderer tsc clean; root/server tsc at the 31-diagnostic baseline; electron main rebuilt. The full thing was driven end-to-end in the browser preview against the REAL stylesheet with a synthesized speech-like signal through a real AnalyserNode - both surfaces live, drag/snap/resize/minimize exercised.
- **next:** on-device with a real engine (confirm the graph builds and audio still reaches the speakers on the first click - the autoplay-policy path is the one thing a synthetic harness cannot prove); consider a VU-style needle variant and a per-channel stereo split; the panel is a general shell, so a future "now playing" line or a scrubber drops straight in.

## P-VOICE.5: in conversation mode the agent answers for the EAR, automatically (ADR-0248)
- **shipped:** live use showed the gap immediately - the user had to TYPE "keep the response short and neat so I can understand you in conversation mode". That is the app's job, not theirs. Hands-free, a normal reply (headings, numbered lists, tables, code fences, file paths) is unusable: speakable() strips the markup but what is left is a written answer read at dictation speed, and a four-item list is forgotten by item three. New pure `spokenReplyGuidance()` (`harness/voice/spoken_reply.ts`) emits a trusted `<spoken-reply>` block on the user-turn preamble - the same channel as the active skill and the DESIGN.md invariants, never the frozen prefix - rebuilt from live voice settings every turn by `acp_backend`, so it appears the turn after conversation mode goes on and vanishes the turn after it goes off. It constrains SHAPE only (lead with the answer, two-three short sentences, spoken prose, no paths/code unless asked, at most one follow-up) and states explicitly that the work behind the answer is unchanged - a naive "be brief" makes agents investigate less, trading correctness for brevity. NOT applied to plain auto-speak: there the user reads along while listening, and a rich answer is still right.
- **fixed:** `stripInjectedPreamble()` listed only four of the blocks that ride the user-turn tail, so `<design-invariants>` (ADR-0154) and `<session-share>` (ADR-0240) had been LEAKING into the displayed transcript as if the user had typed them. Both, plus the new `<spoken-reply>`, are now stripped.
- **verified:** 6 new spoken_reply tests (gating - conversation only, never narrated/screen; a stale conversation flag with auto-speak off cannot force eyes-off mode; the block is delimited + stable byte-for-byte; it bans exactly the unlistenable shapes; it explicitly protects the effort behind the answer) + 2 preamble tests (standing across turns, vanishes with the toggle, ordered last of the standing blocks) + 2 sessions tests (each newly-stripped block, and all seven stacked). 60 voice + 29 preamble/sessions pass; typecheck at baseline.
- **next:** on-device - flip conversation mode on and confirm replies come back short and spoken without being asked, and that turning it off restores full written answers on the very next turn.

## P-VOICE.6: spoken progress cues + a conversation-mode hotkey (ADR-0249)
- **shipped:** eyes-off, a long turn was DEAD AIR - thirty seconds of silence after you speak is indistinguishable from a crash, and the natural reaction is to talk again, which in conversation mode starts a second turn on top of the first. New pure `nextThinkingCue()` (`harness/voice/thinking_cues.ts`) speaks short acknowledgements while the agent works: "Got it. Thinking this through." then "Still working on it. Give me a few seconds." then "Almost there." - with a tool-aware bank ("Let me look into that." / "Still digging through this.") when a call is in flight. The design is all restraint: gaps ESCALATE (2.6s / 11s / 22s), a hard cap of three per turn, nothing once the answer has started speaking, nothing queued behind audio already playing, and the clock measures silence since the last thing SAID - so a long tool run mid-reply is covered the same way the opening think is. Conversation mode only: with your eyes on the screen the thinking block and tool chips already say "working".
- **shipped (hotkey):** `Ctrl/Cmd+G` toggles conversation mode. G is the nearest key to the mic's `Ctrl/Cmd+D` that is free on all three platforms - Ctrl+E/F/R/S/W/A/Q are browser or OS bindings, and LUCID has no find bar for Cmd+G to collide with on macOS. Turning it ON implies auto-speak (the loop is meaningless without the speaking half), so one press from a silent session goes fully hands-free and starts listening immediately if the agent is already idle; turning it OFF closes an open mic and stops cues but LEAVES auto-speak on, which is the state most people want back. The chip tooltip and the picker row both show the combo.
- **verified:** 6 new thinking_cues tests (nothing before the first gap; the answer always wins; gaps provably escalate so cue 2 is not due at cue 1's threshold; hard cap; three distinct lines per turn + a different tool-flavoured bank; every line is short, markdown-free and ends a sentence so the engine lands the intonation). 66 voice tests pass; renderer tsc clean.
- **next:** on-device - confirm the first cue lands about two and a half seconds after a long turn starts and that it never clips the answer's first sentence; consider making the gaps configurable if 2.6s reads as impatient on fast models.
**P-SANDBOX.7b - the mediated --loopback-only posture for the AppContainer helper (ADR-0174)**
- **shipped:** tools/appcontainer/lucid_appcontainer.ts --loopback-only now runs the SAME empty-caps AppContainer as --deny-network (no internetClient ⇒ NO direct internet - guarantee identical + unconditional); the child inherits HTTP(S)_PROXY (seam-set) and relies on a one-time ADMIN loopback exemption to reach ONLY the proxy. Added --register-loopback/--unregister-loopback (shell to `CheckNetIsolation LoopbackExempt -a/-d -n=<our AppContainer>`; pure checkNetIsolationArgs unit-tested) - AppContainers block loopback by default + the exemption needs admin (verified: Access Denied w/o elevation), so it's an install-time step. Fail-safe: no exemption ⇒ no network at all (never direct internet). VERIFIED live on Windows: --loopback-only curl → http_code=000 (blocked), benign runs (exit 0), --register-loopback reaches CheckNetIsolation (exit 5 needs-admin). Tests + demo-P-SANDBOX.7b green; demo-P-SANDBOX.7 updated to post-.7b truth + still green.
- **stubbed:** the "contained child reaches the loopback proxy WITH the exemption registered" completion is NOT verifiable without admin - exercised at install-time + packaging QA; the no-direct-internet security guarantee IS fully verified. Still not bundled/activated.
- **next:** P-SANDBOX.8 (activation/packaging) - build + code-sign + bundle lucid-appcontainer.exe into the Electron app, run `--register-loopback` elevated at install, put it on PATH so resolveBackend selects it (then trusted-local Windows sessions are mediated). Also still open: Linux slirp4netns raw-socket funnel + privileged in-namespace :53.
## P-GPUFIX.1: zombie-SID GPU-sandbox self-heal + the silent engine-spawn failure (ADR-0246)
- **shipped:** the v1.11.9 boot brick (electron/electron#51761: a zombie AppContainer SID in the install dir's DACL kills every sandboxed GPU child 0xC0000022; FATAL after 9 retries, no window) now self-heals: a pure watchdog core (`desktop/gpu_watchdog.ts`) counts fatal GPU deaths from `app.on("child-process-gone")` and on death #2 BEFORE first render main.ts persists `gpu-sandbox-off.flag` in userData (survives the NSIS reinstall that re-inherits the SID), tees a self-diagnosing 0xC0000022 line into engine.log, and relaunches with `--disable-gpu-sandbox` (argv carries the switch even if the flag write fails; applied at module load via `appendSwitch` before the GPU child spawns). ONLY the GPU sandbox drops - renderer sandbox untouched; a sandbox-off instance never relaunches again (loop guard) and post-render GPU crashes / clean exits / non-GPU children are ignored. Plus `dev.on("error")` in startDevServer tees a spawn failure into engine.log instead of swallowing it.
- **stubbed:** verdicts proven on the pure core + source-level wiring checks (`make demo-P-GPUFIX.1`); not reproduced against a live zombie-SID machine this session (the brick is host-state; the 2026-07-18 icacls fix already cleared this host). Flag-file removal UI deferred - delete the file by hand to re-enable the GPU sandbox.
- **next:** after the next NSIS upgrade on the affected host, confirm engine.log shows the watchdog line + exactly one relaunch; consider surfacing "GPU sandbox disabled" in the About/diagnostics panel.

## P-RELEASE.3: Homebrew cask pinned per release + CI re-pin (ADR-0258)
- **shipped:** root-caused "updated to v1.12.1 but About says v1.12.0": the cask tracked the rolling `latest` release (stale since 2026-07-05, refreshed only by manual dispatch, and 404-doomed anyway by the LucidAgent-* artifact rename), while mac in-app auto-update is inert on the unsigned build (ADR-0246 confirmed live). Cask now pins `version` + per-arch `sha256` to the tagged release; new `update-cask` job re-pins it on master on every tag build (fail-closed); README states brew is the macOS update channel.
- **verified:** `brew fetch --cask` on the pinned cask pulled the real v1.12.1 arm64 pkg and the sha256 matched the bytes; workflow YAML parses; `brew style` at the file's pre-existing cosmetic baseline. Not exercised: the CI job end-to-end (first exercised by the next tag cut after merge).
- **next:** cut the next release tag from master and confirm the job pushes the cask bump; then P-RELEASE.2a (signing) so mac in-app update works and brew becomes convenience instead of the only channel.

## P-KG-INGEST.5: the chat-history ingest cannot hang, and Stop always stops (ADR-0264)
- **shipped:** an AI import sat at `0/500 messages - 0 facts` forever and Stop stuck on `Stopping...`, with no retry until the app restarted. `ACPClient.request()` had no timeout and never rejected when the omp child died, so the util connection's `initialize` (awaited BEFORE message 1) stayed pending forever and `utilLock` queued everything behind it. Requests now take `{ timeoutMs, signal }`, a `die()` path drains every pending request on child exit AND on spawn error, and the handshake / session calls carry real bounds (20s / 30s / 180s whole-attempt) while prompts keep their idle clock. Cancel is threaded end to end: the importer checks the signal per MESSAGE (not per conversation), `distillTurn` and `modelExtractor` short-circuit and forward it, and a cancelled message is reported as cancelled, never as a gate block. Stop is now terminal: first press unwinds, a wedged run is force-cancelled after 15s, a second press forces it at once, single-flight releases, and the pill goes amber with "No response from the model for Ns" instead of faking a healthy bar. Also: every folder picker (chat-history import, KG pack import/export, save-to) now opens the NATIVE Explorer dialog through one `pickFolderDialog()` helper, exactly like the workspace/git picker; the in-app dark browser is the plain-browser fallback only.
- **stubbed:** the abort signal reaches `backend.complete` from KB batch-ingest but not through `batch_ingest` -> `ingest` -> `compiler`, so a KB compile still cancels only between documents; the KB pill has no stalled state yet.
- **next:** watch a real import on the reporting machine - confirm the counter now climbs (or that the pill says the model went quiet within 90s, which is the honest answer), and if the util omp is the thing dying, chase WHY it exits from the drained-exit reason now surfacing in the logs.

## 2026-08-03 - P-FS.2 + P-DATA.1
- **shipped:** every folder pick in the BROWSER build now opens the real OS dialog - the loopback backend shows the modern Explorer IFileOpenDialog (FOS_PICKFOLDERS, never the legacy tree) / Finder / zenity via POST /api/fs/pickfolder, pickFolderDialog() tries Electron preload -> backend native -> in-app browser, a native cancel never re-prompts, and titles ride env vars / argv (no shell splicing). Plus the frozen data-integration steer (prefix v9 -> v10, ADR-0266): prompt-stuffed datasets get the context-rot explanation and are routed to native ingest -> MCP -> a real RAG pipeline -> secure GraphQL/cloud vendor connections, with nicholas.chadwick.ctr@gmail.com as the declared custom-contract fallback; wired into both FROZEN_PREFIX and the live omp --append-system-prompt chain.
- **stubbed:** the win32 dialog C# was compile-verified live (Add-Type + type load) but Show() itself is interactive and unexercised headlessly; defaultPath is not yet threaded through the backend picker path.
- **next:** click a real picker on the reporting machine (import-KG flow) to smoke Show(), then consider a /connect-data command that ends in a drafted MCP-backed Agent Builder workflow.

## 2026-08-03 - P-STT.6
- **shipped:** Whisper model housekeeping (ADR-0267) - the Voice card picker now offers ONLY tiny/base/small (medium/large proved slow + buggy through the local whisper-server; install AND start refuse them at the single plan choke point), grays out tiers the hardware cannot run with the capability reason instead of hiding them, clamps the recommendation/summary to the offered set (a 512GB box defaults to small, not large-turbo), and lists every downloaded model with its real on-disk size + a Remove button - legacy medium/large installs show a "not offered" badge and delete cleanly, the running tier is refused until stopped. TTS engine select mirrors the composer menu: an engine that cannot speak right now is grayed with the reason (current selection stays enabled; Kokoro is a self-hosted server, so TTS has no installed weights to remove).
- **stubbed:** no purge-all control (one Remove per row); the DOM click paths are typechecked + rendered from the tested view model but not headlessly clicked.
- **next:** exercise a real Remove of a legacy medium install on the reporting machine, then consider a "remove all unused models" affordance and surfacing the model directory path in the card.

## 2026-08-14 - P-FLEET.1 (ADR-0268, SCOPE/PLAN)
- **shipped:** ADR-0268 - the design for async job handles through the Agent Firewall, which is the one thing blocking the Chief-of-Staff fan-out. Grounded in the code: `tools()` exposes exactly one tool and its handler awaits the whole remote turn (agent_firewall.ts:61-64, 80-112), so a second worker needs a second sequential tool call; `promptTimeoutMs` is never passed at agent_firewall.ts:156-159 so every worker turn dies at the acp_client.ts:99 default of 120s; and overlapping prompts on one connection would CROSS, because one `AcpAgentClient` holds one `#sessionId` (acp_client.ts:87) and `prompt()` resets the per-turn collectors on entry (acp_client.ts:106-108). Decision: four tools over ONE execution path (`dispatch` -> `job-<8 hex>` handle with an idempotency `key`, `job_status` for one/all, `cancel`, and `prompt` kept as dispatch plus a bounded inline wait), a closed state set where an unknown job id is an ERROR and never "running", progress as counts-only metadata (chunk-wise scanning is an evasion surface, so text still arrives once after the whole-turn scan, and the `onProgress` type has no text field to leak through), serialization inside a connection with fan-out ACROSS connections, and a per-entry `jobTimeoutMs` defaulting to P-STALL.1's ten minutes. Job records hold post-gate bytes only, in memory, never DuckDB and never on disk.
- **stubbed:** no code this session (design only). contracts.ts is deliberately untouched: `fleet_job_dispatched` / `fleet_job_completed` / `fleet_job_blocked` are NAMED and deferred to the P-FLEET.3 contracts increment, as P-SANDBOX and P-KB.1 deferred theirs. Unmeasured: omp's own `tools/call` ceiling, which sets the default inline `wait_ms` (first task of the build session). A worker reached over `ssh` can still be orphaned on the far side - that is P-FLEET.2's supervisor, not a job table's problem.
- **next:** build P-FLEET.1 - the pure `harness/mcp/jobs.ts` table + state machine first, then the four tools over `#runJob`, the `onProgress` counts hook, the two additive registry fields, and `make demo-P-FLEET.1` (15 checks, including two firewalls with both jobs provably `running` at the same instant, and a scanner killed mid-job landing `blocked` and never `done`).

## P-VOICE.7: varied openers, active listening, spoken thinking snapshots (ADR-0269)
- **shipped:** conversation mode no longer greets every turn with the same "Got it, thinking this through" - `nextThinkingCue()` now picks the opener from TWELVE lines by a per-turn seed, restates a restatable ask via new pure `distillTopic()` ("Got it: fix the login redirect. On it now."), and once reasoning streams, new pure `thinkingSnapshot()` lifts the newest complete speakable sentence out of it so later cues report REAL progress ("Quick update: the timeout only fires on the retry path.") - continuing every 30s past the old three-cue cap while thinking moves forward, hard-capped at eight. Renderer feeds `thinkBuf` from thinking events; return type is now `{ text, snapshot }` so a stalled think is never narrated twice.
- **stubbed:** snapshots come from the reasoning stream only - a tool-heavy turn with no thinking text still gets the canned tool-flavored lines; topic distillation is first-sentence-only (a multi-sentence ask falls back to a plain opener); not exercised on-device with live TTS this session.
- **next:** listen on-device - confirm restated openers land naturally at 2.6s and snapshots do not read as robotic; consider tool-name-aware snapshots ("searching the codebase for callers") when thinking is silent but tools are running.

## 2026-08-19 - P-FLEET.1: async job handles through the Agent Firewall (ADR-0268, built; ADR-0270)
- **shipped:** the blocking single-tool firewall became a four-tool job surface over ONE gated path: pure `harness/mcp/jobs.ts` (closed state set, sticky terminals, key dedupe, queue cap 8, unknown-id-is-error, metadata-only views), `dispatch`/`job_status`/`cancel` + `prompt` re-implemented as dispatch-plus-bounded-wait (default 25s - the pinned omp bundle kills a tools/call at 30s, measured, so ADR-0268's proposed 90s was wrong), counts-only `onProgress` on the ACP client (type has no text field - anti chunk-split evasion), `AcpTimeoutError` -> `timeout` with session/cancel + force-stop, additive `jobTimeoutMs`(600s)/`maxQueue` registry fields, shutdown cancels-then-stops, `make demo-P-FLEET.1` green on all fifteen ADR checks incl. the two-connection fan-out proof and the scanner-killed-mid-job fail-closed kill test.
- **stubbed:** per-connection concurrency stays maxInFlight=1 by design (a session POOL is a later increment); fleet EventNames (`fleet_job_*`) named but deferred to P-FLEET.3's contracts increment - contracts.ts untouched; a SIGKILLed `ssh` worker can still orphan its far-side turn (P-FLEET.2's supervisor); durable job audit is P-FLEET.3.
- **next:** P-FLEET.2 - the worker supervisor + health endpoint (keep a remote `lucid acp` honest), then P-FLEET.3's fleet panel + the EventName contracts increment for durable dispatch/complete/block audit rows.

## 2026-08-19 - P-FLEET.L1: local lanes + the fleet grid dashboard (ADR-0271)
- **shipped:** N concurrent headless LUCID agents on one machine, each its own GATED omp (fleetLaneArgv() in acp_backend - one source of truth for the gate path), own cwd + own model (defaults to the master's; per-lane dropdown via session/set_config_option), streaming into editable mini windows in a movable/resizable/minimizable share-dock grid (desktop/renderer/fleet_grid.ts + styles, invariant-11 clean) with status frames - working cyan pulse, awaiting-input amber GLOW, needs-approval red GLOW (fail-closed: silence or a closed dashboard = deny, 10-min cap), done green, stopped dim - a 75% headroom guard (fleet_resources.ts over system_profile, measured-number refusals, ceiling min(6,cores/2)), /api/fleet/* + bridge API, and the master agent's fleet_status tool (harness/omp/fleet_extension.ts, token'd LUCID_FLEET_STATUS_URL, metadata only). demo-P-FLEET.L1 green (concurrent lanes, guard refusals, fail-closed approvals, cancel->awaiting-input, stopAll hygiene, no-text status); 12 new tests; root tsc + renderer project clean for touched files.
- **stubbed:** lanes skip the master's sandbox wrapper (exec is deny-unless-approved instead, and the in-omp gate still scans everything) - sandbox parity is a follow-up; no lane persistence across app restarts (lanes are ephemeral children by design); Electron-shell smoke (real dock + real lane + glow) not exercised here - no app window in this environment; master can READ lanes (fleet_status) but dispatching INTO a lane still goes through the human or P-FLEET.1 registry entries.
- **next:** on-device smoke of the dock (spawn a real lane on a second repo, watch working->needs-approval->done glow, verify the 75% refusal toast on a loaded box), then P-FLEET.2 (remote worker supervisor + health) and P-FLEET.3 (PWA fleet panel + fleet EventName contracts), folding lane dispatch-from-master into the P-FLEET.1 job surface.

## 2026-08-21 - Fleet Profiles scoped + control-panel F option (ADR-0272, SCOPE/PLAN)
- **shipped:** ADR-0272 - Fleet Profiles (project-bound full-GUI instances) scoped against the verified workspace-collision failure (shared lucid-gui.json: workspace.ts:25/42-50, memoized re-read settings_store.ts:420-462, cwd-filtered sidebar sessions.ts:72-84, port-keyed userData main.ts:25-38), incl. the lane-or-window SPAWN CHOICE the user asked for (different cwd -> headless lane in this window OR full GUI Lucid bound to that folder). Plus LucidAgentIDE.bat `F) Fleet GUI`: per-profile dirs under %LOCALAPPDATA%\LucidFleet, seeded profile-own lucid-gui.json (verified binding through real load()/currentWorkspace(), bun, match:true), saved-port reuse never 5319, busy-port duplicate warning, detached launch with LUCID_PORT + LUCID_GUI_SETTINGS_FILE + LUCID_PERSONAL_DIR (env spread confirmed at main.ts:105); menu F cancel path smoke-tested via piped stdin.
- **stubbed:** no native code yet (design only) - no FleetProfile store, no LUCID_INSTANCE_ID, no /api/instance, no bind prompt; the .bat's duplicate check is a port heuristic, not live identity; LUCID_GUI_SETTINGS_FILE/LUCID_PERSONAL_DIR are still commented as test seams (promotion is P-FLEET.P1's explicit contract change); interactive two-profile launch not exercised here (no app window in this environment).
- **next:** user runs the on-device two-profile test (F twice, distinct repos, verify workspace + session isolation and both /api/health ports), then build P-FLEET.P1: profile store + LUCID_INSTANCE_ID + /api/instance + seam promotion + demo.

## 2026-08-25 - P-FLEET.L2: unlimited lanes under sustained pressure, lanes from a repo remote, and a truthful minimized HUD (ADR-0273)
- **shipped:** the fleet's 75% instantaneous watermark and its min(6, cores/2) ceiling are GONE. fleet_resources.ts is now pure window arithmetic (pushSample / hotMs / laneAdmission): lanes are UNLIMITED and one is refused only when CPU or memory has held 90%+ unbroken for 30s, so a burst is free, a cool OR blind reading breaks the streak (a sampling gap can never read as load), a backwards clock resets rather than inventing a streak, and every refusal carries the measured percent AND duration ("system memory has been at 94% for 34s"); FleetLaneManager measures it with its own unref'd 3s sampler that retires when idle. The spawn form now opens the REAL OS folder dialog (create-new included) and takes a GitHub / GitLab / Azure DevOps https-or-ssh remote through new dependency-free desktop/git_url.ts (three clone spellings parse, a Windows path never does, an embedded password is discarded), clone-then-spawn lands the repo inside the picked folder and reuses an existing clone; tokens are stored PER HOST in the OS vault (git_pat_<slug> -> LUCID_GIT_PAT_<SLUG>), win over CI vars for that host, are never offered cross-host or to an unknown host, ride the Basic header not the URL, and ssh gets BatchMode + a key-specific hint instead of a token prompt. Minimized-pill flicker fixed at its cause (renderStatus now re-adopts the fleet pill beside share/join; mountPill guards with contains so it stops restarting its own animation) and the pill became a snapshot: one colored dot + count per lane state, needs-approval first, hover naming the lanes. New make demo-P-FLEET.L2 (7 checks) + updated demo-P-FLEET.L1; suites: fleet_resources 13, git_url 8, workspace 39, fleet_lanes 8, all green, renderer + root typecheck clean.
- **stubbed:** a saved host token reaches the server env only on the NEXT launch (the freshly typed value is passed inline for the clone happening now, the ADR-0216 pattern) - no live vault->env refresh; the clone is synchronous inside /api/fleet/spawn, so a huge repo holds that request open behind a "Cloning..." button rather than streaming progress; no ssh-key picker or known_hosts management (BatchMode fails fast and says why); Electron-shell smoke not exercised here (no app window in this environment) - the spawn form + pill layout were verified against the REAL styles.css in a headless render (note is a block paragraph, 4 clean lines; save row is flex with exactly 2 children; headroom stays one 17px nowrap line at 300px; 5-pip pill 162px).
- **next:** on-device pass - minimize with lanes in every state and watch the pill hold steady through a streaming turn (the flicker), clone a real private Azure DevOps repo into a folder created from the Explorer dialog and confirm the token survives a relaunch, then let a real box sit above 90% for 30s and read the refusal; after that P-FLEET.P1 (profile store + LUCID_INSTANCE_ID + /api/instance) per ADR-0272.

## 2026-08-25 - P-FLEET.L3/.L4/.L5 roadmap: lane fidelity, recovery spawns, reviewable timeline (ADR-0274, SCOPE/PLAN)
- **shipped:** ADR-0274 - a three-increment roadmap grounded in the code and a survey of deepseek-harness (MIT): lanes discard the diff data the master's chips already prove is on the wire (fleet_lanes #wire keeps only name+title, toolLine clamps to 120 chars); /api/fleet/prompt is text-only while /api/chat takes P-VISION.1 image blocks; the user's screenshot error IS fleet_lanes LANE_TURN_TIMEOUT_MS=600_000 - the ten-minute clock P-STALL.2 (ADR-0263) already removed from the master, and an error lane refuses prompts with no respawn path; and the headline finding: lane histories are ALREADY on disk (omp persists every ACP session as .jsonl under ~/.omp/agent/sessions/<encoded-cwd>/, sessions.ts:6-8) - invisible only because the sidebar filters to the master's cwd, so the timeline is an indexing problem, not a recording problem. Reused from dsh: result-time applied-hunk diffs on a persisted presentation channel, the unified session query topology (one corpus, derived disposable FTS), append-only-log fork semantics for recovery; explicitly NOT adopted: the Cordis plugin runtime (invariant 1) and dsh's session format (omp owns ours).
- **stubbed:** no code this session (design only). Unverified until L4's first task: whether omp's ACP tool_call_update carries the applied diff payload for lanes identically to the master path, and the exact replay shape session/new accepts for transcript-seeded context; both are the build session's first measurements. Fork-at-step from the timeline is named and deferred to after L4+L5.
- **next:** build P-FLEET.L4 (drop the lane turn clock, error-is-a-state with Retry/Respawn, manager-owned transcripts, recovery spawn that replays recorded turns and RE-ASKS any approval), keystone test: kill a child mid-turn, the respawn answers a memory-dependent follow-up; then L3 (diff chips, image thumbnails, manager-owned staged-prompt queue), then L5 (lane-session naming + the cross-chat timeline).

## 2026-08-25 - P-FLEET.L4: lanes that survive (ADR-0275, built)
- **shipped:** the lane turn clock is DELETED - a session/prompt is raced against the child's life (acp.ts die() rejects pending requests on exit), so the user's reported "acp: session/prompt timed out after 600000ms -> lane is error" cannot happen and a real mid-turn crash surfaces in 9ms (measured); error is a recoverable state with a Retry/Respawn bar (canRetry/respawns on LaneView, /api/fleet/retry streaming + /api/fleet/respawn); recovery revives IN PLACE on the same lane id with manager-owned bounded transcript memory - capability-gated session/load native resume when the agent advertises it, else a one-shot delimited TRANSCRIPT preamble on the next wire prompt (never recorded as user text, no extra turn); fail-closed survives recovery (ask open at death dies as DENY, the revived lane RE-ASKS, only a fresh allow passes); user-stopped lanes refuse prompts by design and revive only via explicit respawn, memory intact; amber `quiet Nm` chip after 90s of stream silence. fake_acp_agent gained a crash mode. 12/12 lane tests, 5/5 demo-P-FLEET.L4 checks, firewall suite unaffected (71 pass), typechecks clean, invariant-11 render verified.
- **stubbed:** transcript memory is per-PROCESS (manager memory) - app-restart recovery lands with L5's lane-session index, since omp's own .jsonl is already the durable copy and a second file would duplicate what L5 indexes; session/load native resume is untested against real omp (the fake advertises no loadSession, so tests pin the fallback path) - first on-device task; thinking text is deliberately excluded from replay memory.
- **next:** on-device: crash a real lane (kill the omp child) and watch Retry carry PELICAN-style memory, confirm real omp's loadSession capability and the native-resume path; then P-FLEET.L3 (diff chips, image thumbnails, staged prompt queue) per ADR-0274.

## 2026-08-25 - P-FLEET.L3: lane fidelity - diff chips, image thumbnails, staged prompts (ADR-0276, built)
- **shipped:** the lane wire stops discarding what omp sends - `#toolCode` mirrors the master's P-CHAT.1 rawInput extraction (write content / edit old+new pairs / hashline patch, 16K cap, paths resolved against the LANE cwd) and cards render one-line diff chips (+N/-N, filename, expandable escaped hunk) that persist in the module transcript; /api/fleet/prompt and the staged queue accept P-VISION.1 image blocks through one shared filter (cap 6) and the manager sends real ACP image blocks (fake agent PROVES receipt by echoing [images: N]) with paste-to-thumbnail in the composer and count-only replay memory ([attached N images], never base64); staged prompts live in a manager-owned FIFO per lane (cap 8, refuse loudly), Send flips to amber Stage while busy, chips are numbered one-line rows with reorder/remove, and the renderer drains the head into a visible streaming card only when the lane is idle - a busy lane refuses the drain, one-turn-per-lane stands. fleet_lanes 15/15, demo-P-FLEET.L3 green, firewall suite unaffected (71), typechecks clean, invariant-11 render verified at 300px.
- **stubbed:** the queue is engine-process-durable, not disk-durable (dies with the engine; L5's lane-session index is the durable layer); diff chips are call-time rawInput fidelity - if omp later emits result-time applied hunks the chip upgrade is additive; queued image thumbnails show as a [N img] count in the chip, not previews; drain requires an open dashboard by DESIGN (approvals need a human watching).
- **next:** on-device pass (paste a screenshot into a lane, watch an edit chip expand, stage three prompts and watch them drain in order), then P-FLEET.L5: name lane sessions at spawn, surface stopped-lane histories, and the cross-chat reviewable timeline per ADR-0274.

## 2026-08-25 - P-FLEET.L5: histories + the reviewable timeline (ADR-0277, built; the ADR-0274 roadmap is complete)
- **shipped:** every lane spawn AND recovery now NAMES its omp session in a durable JSONL ledger (~/.omp/lucid-fleet-lanes.jsonl via the fail-quiet recordLaneSession dep; LaneView exposes sessionId), turning the .jsonl histories omp already persists into attributable lane records that survive engine restarts; sessions.listAllSessions lists every workspace's sessions on the SAME P-PERF.4 mtime index; pure timeline.buildTimeline merges corpus + ledger (lane rows labeled by the LATEST record with spawn-lineage counts, kg-ingest classified by the parser, newest first, paged); GET /api/timeline + POST /api/timeline/session (reusing sessionMessages, so issue-#52 preamble stripping applies); and a Timeline dock on the rail (clock icon) - day-grouped one-line rows (time / kind badge / lane name / ellipsized title / workspace chip / turns), click-to-expand tail-limited transcripts in place, read-only by design. Torn ledger lines skip; a missing ledger degrades labels, never the surface, and never invents a lane. timeline 5/5 + fleet_lanes 16/16 + demo-P-FLEET.L5 green, firewall suite unaffected (71), typechecks clean, invariant-11 render verified.
- **stubbed:** corpus-only sourcing - the DuckDB event-log join is deferred (the omp child holds agent_obs.duckdb read-write live; acp_backend constraint), as is FTS (derived, disposable index per the dsh topology) and fork-at-step from a timeline row; the dock caps at the newest 100 with an honest count (paging UI later); lane sessions from engines that predate the ledger read as plain chats.
- **next:** on-device pass (run lanes, stop one, find it labeled on the Timeline and read its transcript; confirm the ledger accumulates across an engine restart), then the named follow-ups: DuckDB gate-event join for a security-annotated timeline, FTS search, fork-at-step.

## 2026-08-25 - release: v1.13.2 cut (the fleet fidelity arc ships)
- **shipped:** version bumped at the four release sites (desktop/package.json, version.ts APP_VERSION + the v1.13.1 and v1.13.2 changelog comment lines, both about.test.ts pins), README's what's-new chain gains the v1.13.2 section (survival / diff chips / images / staged prompts / the Timeline), the Newest paragraph rewritten to v1.13.2 with v1.13.1 demoted into the history chain, and a v1.13.2 batch row citing ADR-0274..0277. about + fleet + timeline suites green, renderer typecheck clean.
- **stubbed:** the Homebrew cask pin and the GitHub release artifacts are CI's job (the tag triggers build-desktop; the update-cask job pins after a successful cut) - not hand-edited here; on-device smoke of the packaged v1.13.2 build is the user's ritual.
- **next:** watch the v1.13.2 tag build on the three runners; after install, the on-device fleet pass (crash-revive with memory, diff chips, staged drain, the Timeline finding a stopped lane) plus the named follow-ups (DuckDB event join, FTS, fork-at-step).

## 2026-08-29 - P-FLEET.L6 + P-WSSETUP: approval scopes, full auto-mode, workspace init offer
- **shipped:** fleet lane approvals gain scopes - Allow / Allow-for-session (remembers the ask's tool kind per lane) / Deny, plus a full auto-mode (per lane or all lanes) gated behind an explicit risk-acceptance modal and persisted via fleetAutoApprove + fleetAutoRiskAcceptedAt (server refuses auto without the ack; the in-omp security gate still scans every call); auto/session grants stream as auto-approved chips; new POST /api/fleet/auto + scope on /api/fleet/answer; workspace-open now profiles the folder (new workspace_setup.ts) and offers init flows - empty folder purpose picker (app/docs/analysis/other), existing-code repo scan-and-setup - scaffolding AGENTS.md + .agents/{README,CONTEXT,PROGRESS,DECISIONS,skills/SOURCES}.md with wx-flag never-overwrite writes, asked-once per path (bounded 50); ACP children now persist stderr to ~/.omp/lucid-acp.log (rolling 512KB, collected by collect-lucid-logs.bat) and a code-1 exit quotes the last stderr line in the UI error. fleet_lanes 20 green, workspace_setup 8 green, renderer typecheck clean.
- **stubbed:** the existing-repo scan is deterministic (manifests + package.json scripts), not an LLM pass; sessionAllow surfaces only as a tooltip kind, no management UI to revoke a standing session grant short of respawning the lane; voice approvals do not yet know the session scope.
- **next:** on-device pass (enable auto on a lane, watch auto-approved chips; open a fresh empty folder and an existing repo, accept both init offers, confirm .agents lands and the offer never re-asks), then follow-ups: revoke UI for session grants, LLM-assisted repo scan into CONTEXT.md.

## 2026-08-29 - P-INTERJECT.1 + P-BROWSER.1 + P-WSSETUP fixes: mid-turn control, agent browser, preview robustness, PWA fleet
- **shipped:** duplicated delegation cards fixed (batch filtering by task ids with assignment-prefix and sole-card fallbacks, pure subagent_filter.ts); preview_open registration made resilient (typebox-or-JSON-literal schemas - the T.Optional failure that silently dropped all five preview tools) plus percent-encoded file URLs for spaces/OneDrive paths and quoted-path normalization; mid-turn interjection pipeline (interject_store + /api/interject + interject_extension appending operator notes AFTER the untrusted envelope at the next tool boundary, master and per-lane targets); stacked composer queue with hold-or-push chips, HUD Check-in card, per-lane check-in and push-now; /api/processes + status-bar Processes popover; per-lane Preview tabs (cap 8, close X, no tab stealing) with auto send-to-phone (3s gap); agent-controlled visible browser window (browser_open/screenshot/scroll/close tools, egress-gated browser_open via acp_config prompt override, main-process 500ms command loop with minted LUCID_MAIN_TOKEN, breathing-glow insertCSS + snapshot flash, renderer particle pill with Close tab / Stop agent kill switch, auto send-to-phone); PWA fleet strip with name filter + prompt/stop/approve controls, processes strip, check-in, queue-vs-push composer split. Also restored NUL-corrupted desktop/collab/host.test.ts from HEAD (OneDrive sync damage). 180 new tests green; desktop 4446 pass; typecheck unchanged from baseline.
- **stubbed:** browser window is master-session only (singleton window; lanes excluded by design); browser has no click/type tools yet (screenshot+scroll iteration only); processes list omits automations; PWA lane prompt uses a composer target chip rather than a per-lane composer.
- **next:** on-device pass (two delegation batches render separate cards; preview an OneDrive spaced path; push an interjection mid-turn and watch the next tool boundary; open the agent browser on a DOM-locked page and take snapshots; drive a lane from the phone), then follow-ups: browser click/type tools, lane browser access, automations in processes.

## 2026-08-29 - P-BROWSER.2 + mobile plan: agent browser input tools, native iOS/Android spec
- **shipped:** the agent browser gained input: browser_click (screenshot-space coordinates, mapped back onto live content bounds via the width of the last shot the agent was shown, so the model never needs the viewport size or the display ratio) and browser_type (per-character input plus Enter, newlines entered as Enter, 2000-char cap, explicit guidance to have the USER type credentials in the visible window). Both go through the window's own input path so hover menus and custom controls behave as they do for a person; both are read-tier because browser_open already carried the egress approval and the user holds the close-X kill switch; the queue, the 500ms main poll, and the kill switch carry them unchanged. browser_open's description now teaches the whole loop (open, screenshot, act, screenshot, close). New harness/omp/browser_extension.test.ts covers registration in both schema modes (a shim missing Boolean now falls back to literal instead of dropping all six tools), argument validation before any command reaches the window, and the pure helpers. Also authored docs/NATIVE-MOBILE-APPS.md (1023 lines): React Native bare workflow reusing the pure collab core with no over-the-air updates, KMP named as runner-up with four explicit triggers, and an honest edge-first notification finding (Android solves it offline with a dataSync foreground service; iOS cannot, so a background URLSession long-poll against the self-hosted relay is the only cloud-free lever and optional APNs is disclosed as a real threat-model regression, default off, excluded from the air-gap variant). 26 browser tests green; typecheck at the 31-diagnostic baseline exactly.
- **stubbed:** no drag, right-click, key-combo, or file-upload input yet; the browser stays master-session only; the mobile doc is a plan, no app code exists, and its iOS background-wake cadence needs the Phase 0 72h measurement spike before Phase 3 can be scoped honestly.
- **next:** on-device pass (open a page, screenshot, click a field, type, screenshot again, confirm the coordinate mapping lands on target at a non-1x display scale), then decide whether the iOS enterprise-distribution blocker is worth resolving with the customer before Phase 1.

## 2026-08-29 - P-BROWSER.3: drag, right-click, and key combos in the agent browser
- **shipped:** browser_drag (press, travel, release; ten interpolated moves about one frame apart so HTML5 drag-and-drop, range sliders, canvas handles and resizers actually see motion instead of reading a bare down-then-up as a click), right-click folded into browser_click as an optional button param rather than a separate tool (no native OS menu appears, so a page's own context menu is what the agent gets), and browser_keys for one combo at a time (Escape, Tab, Control+a, Shift+PageDown, F1 to F24). New pure desktop/browser_keys.ts parses combos for BOTH processes: dev.ts validates at the route so a typo returns a named error immediately, and Electron main re-parses as the authority before pressing. Fail-closed on unknown names because Chromium silently swallows an unrecognised keyCode, which would ack, change nothing, and send the agent into a retry loop. Modifier aliases fold (ctrl/control, cmd/command/super/win to meta, option to alt) so one spelling works on every host OS, order is stable, and a printable key gets a char event while a named key does not (a stray control character would otherwise land beside the keydown the page listens for). browser_open's description now names the whole action set. 47 tests green (14 parser, 12 queue, 21 extension); typecheck back at the 31-diagnostic baseline after tightening the parser's modifiers to Electron's literal union instead of string[].
- **stubbed:** no double-click, middle-click, hover-without-click, multi-key sequences in one call, or file-upload input; drag is single-pointer only (no pinch or multi-touch); the browser stays master-session only.
- **next:** on-device pass (drag a slider, right-click for a page's custom menu, Escape out of a modal, Control+a then browser_type to replace a field), and confirm the coordinate mapping at a non-1x display scale.

## 2026-08-29 - P-REMOTE.14: optional device-native speech-to-text on the phone, fail-closed against the CUI posture
- **shipped:** the phone can dictate with its OWN recognizer, but only when it can PROVE the audio stays on the device. New pure desktop/collab/device_stt_policy.ts (decideSttMode / isStrict, DOM-free and import-free) is the whole decision: no recognizer falls back to hold-to-talk; a PROVEN on-device language pack is allowed even under a strict posture; a downloadable pack becomes a one-time install offer, which under strict is the ONLY route to device transcription; strict (CUI session AND lockdown) with no proven local path returns to desktop transcription and says why; anything else is the browser vendor's cloud, offered only behind a consent sheet that names the risk in plain words and is never remembered across sessions. New tools/remote-pwa/device_stt.ts wraps the Web Speech API with locally declared types (the API is absent from the project typings), probes SpeechRecognition.available({langs, processLocally: true}), sets options.processLocally when local is proven, streams interim text into the composer, and leaves the final transcript focused for review rather than auto-sending. frames.ts gained sttSource on PromptFrame plus posture on Welcome/State (additive, version still 1); guest.ts gained a posture() accessor defaulting to the STRICTEST stance until a frame proves otherwise; host.ts advertises posture and REFUSES a device-cloud prompt outright when strict, which cannot un-leak the audio but stops cloud-derived text entering a CUI session and makes the violation loud. Platform reality is handled honestly instead of a dead button: an installed iOS PWA (LUCID's normal phone client) and every iOS WebKit browser cannot do this at all, and Safari cannot prove local, so each gets a sentence explaining what to use instead. Orchestrator additions on review: the renderer's direct-P2P host now publishes the same posture (state.sessionMode plus the asksageOnly lockdown), so a relay share upgrading to P2P mid-session no longer silently loses the dictate button, and the redundant "(voice: desktop)" marker was dropped so only the notable provenance cases label the author.
- **stubbed:** provenance rides a short marker on the from label (the only string surviving pendingGuestPrompt to guest-inbox to /api/chat to tapUserTurn to UserTurnFrame.from) plus sttSource on the inbox record; there is still no per-prompt audit EVENT, so collab_audit.ts records join/leave only. A P2P guest prompt keeps host-side enforcement but not the display marker (the renderer's P2P prompt handler ignores the extra args). Chrome's on-device pack covers 17 languages only; anything else is cloud or desktop.
- **verified:** 48 new tests (27 policy: every reason token, unknown-token jargon containment, the four availability statuses crossed with strict / open / cui-only / lockdown-only, the unprovable-claim edge, an exhaustive proof that device-cloud is NEVER reachable under strict, and a no-em-dash matrix assertion; 21 wrapper: injected fake recognizer, install true/false/rejecting/absent, interim-then-final streaming, processLocally set only when asked, iOS standalone and WebKit reasons, a rejecting probe resolving to unsupported). Collab plus PWA suites 428 pass; full desktop suite 4490 pass with only the 15 known Windows-host failures; typecheck at the 31-diagnostic baseline exactly.
- **next:** on-device pass (Android Chrome for the proven-local path and the install flow, iOS Safari in a tab for the cloud-consent sheet, iOS installed PWA to confirm the explanation appears instead of a button), then decide whether a guest turn deserves a first-class audit event carrying sttSource.

## 2026-08-29 - P-TL.2: the Timeline hides its own self-test noise and reads like an inspector
- **shipped:** the timeline was unusable at 6560 sessions because the repo's own echo and demo scripts mkdtemp an omp-echo-<rand> workspace per probe and title it "ping" or "go", burying every real conversation. New pure isSelfTestSession in timeline.ts classifies a throwaway two ways: the workspace pattern alone (omp-echo, and the lucid/omp test/selftest/smoke/demo/fixture/tmp scaffolds) is sufficient because no human names a workspace that, while a bare probe TITLE only counts when the session is 2 turns or fewer. The load-bearing safety property, tested explicitly: a real workspace past two turns is NEVER self-test whatever it is called, so a genuine chat opening with "ping" survives. buildTimeline holds them back by default and reports a selfTest count across the whole corpus; includeSelfTest opts them back in, still marked, and the route takes ?selfTest=1. The dock header shows "N sessions" plus a chip reading "N self-test hidden" that flips the view and persists, hidden entirely on a machine with none, and an all-throwaway corpus now says so instead of claiming no sessions exist. Visual pass borrowed from the trajectory inspector the user referenced: per-kind accent rail and colored badge on each row, monospace metering with zebra striping, meta pushed right and dimmed so the title column dominates, sticky day headers carrying their own session count and a composition strip whose segments are that day's real mix of chat, lane and ingest, and the expanded transcript rebuilt as a grid with a fixed uppercase monospace role gutter (you, agent, tool, system, each its own color) so a long transcript scans by source.
- **stubbed:** classification is workspace and title patterns, not provenance; a throwaway run from a NEW script naming its temp dir something else will still show until its pattern is added. The composition strip counts only the rows on the current page (newest 100), not the whole day. No bulk delete: this stays a review surface that never owns a lifecycle.
- **verified:** 10 timeline tests (every workspace pattern including an underscore variant, every probe title at 1, 2 and 3 turns, the real-session-never-swept property, hidden and shown counts, and an all-throwaway corpus); full desktop suite 4495 pass with only the 15 known Windows-host failures; typecheck at the 31-diagnostic baseline; zero em dashes.
- **next:** on-device look at the redesign, and a decision on whether the strip should summarize the whole day rather than the loaded page.

## 2026-08-29 - P-TL.3: the Timeline becomes a two-pane inspector
- **shipped:** the single scrolling column became the inspector shape the user asked for. LEFT pane is the chronology and now scrolls BOTH ways: rows carry FIXED metadata columns (time 46px, kind badge 58px, workspace or lane name 104px, turns 38px) so every title starts at the same x and the eye scans a column, then the title takes its NATURAL width and the pane scrolls sideways to it, so a long title is reachable in full instead of ellipsized out of reach. Day headers are sticky on both axes so the label survives a horizontal scroll. RIGHT pane reads the selected transcript, replacing inline expansion entirely: the chronology never reflows under the cursor and the transcript gets real room, with a header carrying the full folder path and model (finally readable) above the role-guttered messages. Below 620px of body width a container query drops the detail pane BELOW the list instead of squeezing it, so a narrow dock still works. Hover and keyboard focus are the SAME state (background lift, a 1px translate, the accent rail thickening to 3px, the model name fading in at the row end, the title brightening), selection is a distinct persistent state, and reduced-motion callers get none of the movement. A lane row shows its LANE name in the workspace slot with the cwd in the tooltip, so four columns stay aligned with nothing lost. New header filter box narrows by title, workspace, or lane name client-side. Enter and Space select (a role=button div gets neither for free) and Up/Down walk the chronology without the mouse. Also authored mockups/timeline_inspector.html, which copies the real .tl- CSS verbatim per invariant 11 so the design can be reviewed in the Preview panel without a rebuild.
- **stubbed:** the filter is client-side over the loaded page (newest 100), not a corpus query; no multi-select or compare; the detail pane has no tabs (the reference's Summary/Payload/Result/Schema/Timing split needs per-event data the timeline list does not carry yet).
- **verified:** 10 timeline tests still green, renderer typecheck clean, full typecheck at the 31-diagnostic baseline, zero em dashes in the diff or the mockup (one slipped in from a retyped comment header and was removed), and no stale references to the removed inline-expansion path remain.
- **next:** on-device look, then decide whether the detail pane earns the reference's tab split, which would mean carrying per-event records into the timeline rather than session rows.

## 2026-08-29 - vault key convergence: the vanishing Local Provider (ADR-0278)
- **shipped:** desktop/oscrypt_seed.ts + main.ts wiring - port-suffixed instances now seed their Chromium os_crypt key from the canonical userData dir (win32), with a keyless-canonical backfill, so vault credentials decrypt across restarts and ports; the [LOCAL_PROVIDERS] spawn outcome (with skip reasons) now lands in engine.log.
- **stubbed:** existing blobs encrypted under a dead per-port key are not migrated - the user re-enters the key once; a "key unreadable" indicator in the provider row remains unbuilt.
- **next:** on-device confirmation that DGX Spark's model appears in the picker after re-key + restart.

## 2026-08-29 - release: v1.14.0 cut (the control + reach arc ships) + addon site and PWA refresh
- **shipped:** version bumped at the four release sites (desktop/package.json, version.ts APP_VERSION plus a v1.14.0 changelog block, both about.test.ts pins) with the whole batch AND the other agent's ADR-0278 fix folded in: on Windows, port-keyed instances each had their OWN safeStorage os_crypt key while the credential vault stayed global, so a key written by one instance was undecryptable by the next and the Local Provider silently vanished from the model picker. README gains a v1.14.0 what's-new section (mid-turn steering, approval scopes plus risk-accepted auto mode, the watchable agent browser, phone fleet control plus provably-local dictation, the .agents scaffold, the timeline inspector, the ACP stderr log, and the safeStorage fix), a rewritten Newest paragraph, and a v1.14.0 batch row. Addon repo (separate git repo at lucidaddon_audit, gitignored from core): the phone PWA was REBUILT and redeployed into website/firebase-public/remote (app.js 37KB -> 54KB, shell 31KB -> 44KB, config.js verified byte-identical so live Firebase and relay settings were not clobbered), carrying the fleet strip, processes strip, check-in, and device STT; website/public/index.html gains four capability cards (mid-turn steering, approve-once, the watchable browser, provably-local phone dictation) with every sprite icon verified to resolve, the JSON-LD SoftwareApplication node gains softwareVersion 1.14.0 plus six featureList entries, and llms.txt gains the matching capability lines for AI answer engines.
- **also fixed while in there:** the site claimed 1,900+ tests and the README claimed 3,100+; both were stale, so the real number was MEASURED by subtracting the packaged copy (desktop 4,514 scanned minus 2,331 in desktop/release = 2,183 real, plus harness 1,066, tools 63, and 57 Python sidecar = 3,369) and both now say 3,300+ with the measured breakdown. Sitemap lastmod was still 2026-07-10 and is now current. A user-facing PWA string read "Wrong PIN - check it" with an em dash (escaped as \\u2014, which is why a literal scan missed it); that plus every em dash in the PWA and collab sources was removed, the bundle rebuilt, and both repos verified at zero.
- **stubbed / needs your call:** lucid-agents.com appears NOWHERE in the addon repo (34 references to lucid-agent.web.app, and config.js pins the OAuth authDomain to it with a comment warning that a mismatch makes Google reject sign-in), so the domain was NOT rewritten. The addon repo's last commit is a REVERT of a near-identical marketing-page change with no rationale recorded, and this cut re-applies that kind of change deliberately. Both repos are staged but NOT committed, tagged, or deployed.
- **next:** confirm whether the lucid-agents.com migration is wanted (canonical, og:url, sitemap, OAuth redirect URIs, Firebase hosting) and why the earlier site commit was reverted, then commit both repos, tag v1.14.0 to trigger build-desktop, and run firebase deploy for the site plus PWA.

## 2026-08-29 - release: v1.14.0 SHIPPED + the addon site redone on top of main
- **shipped:** v1.14.0 is tagged, pushed, and fully built: Windows NSIS plus portable, macOS arm64 plus x64, Linux AppImage/deb/rpm, the Homebrew cask auto-pinned, all three editor extensions, CI (harness plus Python scanner) and CodeQL javascript-typescript plus python all green on d4804b5. The desktop About link now points at the canonical lucid-agents.com while the PWA and sign-in origin stay on lucid-agent.web.app, where the OAuth authDomain is registered (moving it would re-break signInWithRedirect under iOS Safari storage partitioning).
- **the addon repo lesson:** the first attempt landed on feat/remote-admin-plane, which was 7 ahead of main but 15 BEHIND it, and those 15 commits touched exactly the files being edited. Deploying from that branch would have regressed main's newer phone PWA (readable Thinking, P-REMOTE.9b) and the commit that restored the hosting routes a previous deploy wiped, which is the precise failure main's own deploy-safety runbook warns about. It also explained the mystery revert seen earlier: main ALREADY carries the Fleet Mode marketing section, the corrected test counts, and the lucid-agents.com canonical (25 references), so that work was never lost, only absent from the stale branch. The work was therefore REDONE in a throwaway worktree off origin/main in main's own conventions (New in 1.14 card tags, newest-first ordering) and pushed as site/v1.14.0 for a PR into main: four capability cards at the head of the grid, the heading corrected Seventeen to Twenty-one against the real count, JSON-LD softwareVersion 1.14.0 plus six featureList entries, llms.txt capability lines, sitemap refreshed, and the PWA rebuilt (app.js 37KB to 54KB) with the rebuild VERIFIED to keep main's readable-Thinking markers. config.js was proven identical modulo line endings and then left untouched, and four files that differed only by line endings were restored so the commit carries seven real changes and never touches the auth config.
- **stubbed:** site/v1.14.0 is a branch, NOT merged and NOT deployed: firebase deploy still needs a human, and main's runbook says never drop the hosting auth/PWA routes when deploying. The stale feat/remote-admin-plane commit (e54ab63) is still on that branch and should be abandoned rather than merged, since its site edits were against the old layout.
- **next:** open the PR for site/v1.14.0 into main, deploy the site plus PWA from main after the merge, and let John Lopez know the diagnosable error path is in v1.14.0 (ask him to run tools/collect-lucid-logs.bat if it recurs, since the ACP stderr log now names the real failure).

## 2026-08-29 - deploy: the site + phone PWA are LIVE on v1.14.0
- **shipped:** merged and deployed. The addon repo's main moved again mid-task (917320a: v1.13.2 plus the Tactical GenAI Trainer, which normalized line endings and added a fleet-fidelity card), so the branch built earlier off 4541c74 could no longer fast-forward and would have fought that ship. The v1.14.0 site work was therefore RE-APPLIED directly onto current main against its live content: four New in 1.14 cards at the head of the novel grid, heading corrected Seventeen to Twenty-two (main carried 18 cards under a Seventeen heading, plus these four), JSON-LD softwareVersion 1.13.2 to 1.14.0 with six featureList entries, llms.txt capability lines, sitemap refreshed, and the phone PWA rebuilt (app.js 37KB to 54KB). The superseded site/v1.14.0 branch was deleted locally and on the remote so nobody can merge it and regress main. Deploy ran through CI (SAST gate, then keyless WIF build, image scan, Cloud Run, Firebase Hosting): all green, CodeQL green.
- **runbook observed:** firebase-public/ still carries signin/ remote/ admin/, firebase.json was never touched so the four load-bearing rewrites keep their order ahead of the Cloud Run catch-all, config.js was proven identical modulo line endings and left OUT of the commit so the registered OAuth authDomain and the commit-exact gitleaks fingerprints are unchanged, and the PWA rebuild was verified to KEEP main's readable-Thinking work. Post-deploy probe on lucid-agent.web.app: /signin, /remote, /admin, /r/** and / all 200, /signin serves "Authorize this desktop", the new PWA shell is live, and the root serves the v1.14.0 content.
- **finding worth keeping:** the two hostnames are NOT equivalent. lucid-agents.com is domain-mapped to the Cloud Run marketing service, so its root serves the site (v1.14.0 content and softwareVersion 1.14.0 confirmed live) but /signin, /remote, /admin and /r/** all 404 there, because those four live in Firebase Hosting and are only served on lucid-agent.web.app. This retroactively confirms keeping the PWA and sign-in origin on web.app was correct: those routes do not exist on the custom domain. The live marketing page was checked and has NO relative links to them (its only cross-origin reference is the absolute web.app URL, and the #remote hits are in-page anchors), so there is no broken path for a visitor today. Advertising a phone or sign-in link on lucid-agents.com would 404 until those routes are mapped.
- **next:** tell John Lopez v1.14.0 is out (the ACP stderr log now names the real failure; the Local Provider safeStorage fix may have been his actual problem), and decide whether lucid-agents.com should also serve /signin, /remote, /admin and /r before that domain is advertised anywhere.

## 2026-08-29 - P-MAINT.1: Maintainer Agent spike (public) + the licensed implementation (addon)
- **shipped (public spike, harness/maintainer/):** pure, injectable, fully tested modules for long-lived repo maintenance: os_schedule.ts emits REAL OS-native registrations (schtasks MINUTE/DAILY, a launchd plist with StartInterval or StartCalendarInterval, and both a crontab line and a systemd user timer) with correct quoting for paths containing spaces, which this repo's own path proves; manifests.ts parses package.json, requirements.txt, pyproject.toml, Cargo.toml and go.mod, reporting errors instead of throwing; advisory.ts queries OSV querybatch (the official aggregator of CVE and GHSA, no API key, so edge-first holds) with batching, real CVSS v3.1 base-score severity, and a fail-CLOSED degradation that pins a labelled coverage gap first rather than reading as clean; sbom.ts builds and diffs a valid CycloneDX 1.5 document with correct purls per ecosystem; tracker.ts renders the GitHub issue, the GitLab issue, the Azure DevOps work item (HTML in System.Description, real reference-name fields) and the PR body, each stating plainly that a machine opened it, sorted worst-severity-first, with stable timestamp-free dedupable titles; sweep.ts orchestrates with every I/O injected. harness/scripts/demo_maintainer.ts is a real dry run over THIS repo plus a make demo-maintainer target. 81 tests.
- **shipped (addon, lucidaddon_audit/maintainer/):** the licensed implementation. Entitlement is IMPORTED from the existing server/src/entitlement.ts (not replicated, so there is one signature scheme) with a new maintainer:run scope, refusing fail-closed on no key, no token, wrong key, malformed, expired, or missing scope, auditing either way and never logging the token. Three real provider clients that actually file AND deduplicate by searching for an existing open item with the same stable title first: GitHub (issues, PRs, x-github-api-version, Enterprise base), GitLab (private-token header never a query param, URL-encoded project path, issues and merge requests), Azure DevOps (Basic auth with empty username, json-patch work items with System.Title/Description/Tags, pull requests). Append-only JSONL audit of every run and write. registry.ts validates targets fail-closed. run.ts is the OS-scheduler entry point: preflight FIRST, then entitlement, then per-target sweep, and it writes NOTHING unless the operator set openPr or openTracker. A README charter and ADR-MAINTAINER-0001 record why this component is deliberately NOT metadata-only, against that repo's invariant 3, and list the compensating controls. 68 tests.
- **the OS-scheduler constraint, resolved not ignored:** acp_backend.ts carries an explicit decision against OS scheduling because it would run omp where the gate is not guaranteed armed. That objection is answered rather than overridden: the registered command is never omp and never a bare runner, it is a wrapper whose FIRST action is the fail-closed lucid check preflight, and a preflight failure aborts non-zero with an audit line, so the OS scheduler cannot start an ungated agent. The in-process 30s ticker is untouched.
- **REAL FINDING from the live dry run, needs your attention:** swept 3 manifests, 22 declarations, 21 SBOM components, and OSV returned 35 advisory matches against this repository. electron 33.0.0 carries SEVEN HIGH advisories including a context-isolation bypass (GHSA-h7rp-cf8h-j98x, CVE-2026-70601), renderer command-line switch injection (GHSA-9wfr-w7mm-pc7f), several use-after-frees, and an ASAR integrity bypass (GHSA-vmqv-hx8q-j7mg, CVE-2025-55305); dompurify 3.4.11 carries an XSS advisory (GHSA-55q2-fjhq-7xh7, CVE-2026-75838). The advisories name fixed versions. For a product whose premise is a fail-closed security boundary, a context-isolation bypass in the shipped Electron is the one to triage first.
- **stubbed:** the addon does NOT push commits, so openPr checks that the branch exists and audits write_skipped rather than filing a PR for a branch that does not exist; branch creation and the actual manifest edit are the next increment. The spike deliberately omits registry-based newer-version checking (every bump it proposes is anchored to an advisory that names a fix), which is the addon's job.
- **next:** triage the electron and dompurify advisories, then decide whether the maintainer agent may edit manifests and push a branch, which is the one remaining gap before it can open a PR unaided.

## 2026-08-30 - security: electron 33 -> 44 and dompurify 3.4.14 (all 35 advisories cleared)
- **shipped:** triaged the advisories the new Maintainer Agent found against this repo. electron ^33.0.0 -> ^44.0.0, electron-builder ^25.1.8 -> ^26.15.3, dompurify ^3.4.11 -> ^3.4.14. The declared electron 33.0.0 carried SEVEN HIGH advisories (context-isolation bypass GHSA-h7rp-cf8h-j98x/CVE-2026-70601, renderer command-line switch injection GHSA-9wfr-w7mm-pc7f, several use-after-frees, and the ASAR integrity bypass GHSA-vmqv-hx8q-j7mg/CVE-2025-55305) plus 21 MEDIUM and 7 LOW; dompurify 3.4.11 carried an XSS advisory GHSA-55q2-fjhq-7xh7 and a sanitizer-bypass GHSA-c2j3-45gr-mqc4.
- **why 44 and not the minimum:** clearing every advisory needed at least 42.0.1, but electron supports only the newest three majors (42, 43, 44), so shipping 42 would have gone end-of-life at the next release and put us straight back here. 44.0.0 clears everything and buys the longest runway.
- **why the nine-major jump was tractable, checked before doing it:** the packaged files array bundles no node_modules except monaco (pure JS), so no native module ships inside the app and the electron ABI change is a non-issue for packaging. The API surface is small and entirely long-stable: BrowserWindow, Menu, app, contextBridge, dialog, ipcMain, ipcRenderer, safeStorage, shell, webFrame, plus webContents capturePage/executeJavaScript/isLoading/on/replaceMisspelling/send/sendInputEvent/session/setWindowOpenHandler and session.addWordToSpellCheckerDictionary. None of it sits in the usual breaking-change blast radius (no remote module, no protocol registration, no permission-handler signatures). The builder went to 26 alongside so packaging knows this electron; the stale electron-builder-squirrel-windows peer warning is inert because this build targets NSIS and portable and never references squirrel.
- **verified:** desktop/tsconfig.json covers main.ts, which is where every electron API is used, and it typechecks CLEAN against electron 44's own type definitions, so no signature broke. Full typecheck at the 31-diagnostic pre-existing baseline. 95 electron-adjacent tests green (packaged_boot, engine_pf_smoke, platform, native_dialog, preview_electron, cred_vault, oscrypt_seed). Then the closing loop: the Maintainer Agent re-swept and OSV now returns ZERO advisory matches, down from 35, and correctly reported the three version changes as SBOM drift.
- **stubbed:** a real on-device launch of electron 44 has NOT happened on this host. The packaged boot gate in CI (which stages a Windows build into a write-denied Program Files tree and requires /api/health) is the next gate, and an on-device smoke of the built installer is still the user's ritual before shipping.
- **next:** watch the CI boot gate, then launch the packaged build once and exercise the electron-touching paths (safeStorage vault read, capturePage in the preview and the agent browser, sendInputEvent clicks and typing, the spellcheck context menu).

## 2026-08-30 - security follow-through: electron 44 packages clean on all three platforms
- **outcome:** electron 33 -> 44 and dompurify 3.4.11 -> 3.4.14 are on master and PROVEN through the real packaging pipeline. A dispatched test build (publish off) produced signed Windows NSIS plus portable, macOS arm64 plus x64, and Linux AppImage/deb/rpm, all reporting electron=44.0.0, and the air-gap smoke passed on ALL THREE (omp, the scanner Python, and the compiled lucid launcher resolve and run from bundled resources with no network). The Maintainer Agent re-swept afterwards: OSV returns ZERO advisory matches, down from 35.
- **the real lesson, and it was mine:** I bundled TWO upgrades, electron 33->44 and electron-builder 25->26, and every single failure came from the builder bump that nothing actually required. Builder 26 rejects the flat linux.desktop map (it models LinuxDesktopFile with additionalProperties false and wants the entries nested under entry), and far worse, builder 26 never walks into node_modules through an extraResources from, so the bundled omp shim AND @oh-my-pi/pi-coding-agent both disappeared from resources/repo and the air-gapped agent was broken. I chased that with two compensating config changes before doing the disciplined thing: restore desktop/package.json and the air-gap smoke script to the exact v1.14.0 state and move ONE variable. Electron 44 alone passed first try. Three CI runs were spent learning that; isolating the variable first would have spent one.
- **also caught:** two commits silently did not push. git push -q printed the harness's ok wrapper while git itself said Everything up-to-date, because the working tree had been switched to a local-only branch feature/creator-mode and I was pushing the stale master ref. Verifying the REMOTE hash after every push, not the command's exit, is now the rule. Worse, my git add -A on that branch swallowed another LUCID agent's in-progress creator-mode work (build_flavor.ts, creator_dev.ts, creator_monitor.ts, creator_preamble.ts, creator_registry.ts, renderer/creator_monitor.ts, plus a settings_store.ts edit) into my security commit. That commit was rebuilt with explicit paths, their 1,332 lines were left uncommitted and intact exactly as they were, and the stray branch was pointed back at master. Use explicit paths, never git add -A, in a repo another agent is live in.
- **stubbed:** a human launch of the packaged Electron 44 build still has not happened. The installers exist as CI artifacts on run 33285812028 and the air-gap smoke exercises the bundled runtimes, but nobody has clicked the app and driven safeStorage, capturePage, sendInputEvent, or the spellcheck menu on real hardware.
- **next:** install the Electron 44 artifact from that run and exercise the electron-touching paths, then cut v1.14.1 so users get the advisory fixes.
