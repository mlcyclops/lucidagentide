...
- **stubbed:** re-keying a provider is delete+re-add (no inline edit yet); zone is "internal" by default (no external toggle in the form); a provider added at runtime still takes effect on the next app restart (no in-app apply/restart trigger yet); a reachability/TLS "test connection" button is not built. Full settings-panel HYDRATION couldn't be exercised in the local preview (all cards — mcp/whitelist/personal included — sat on skeletons; a documented preview-env flake on this host, not code-specific); the card body is covered by the unit tests.
- **next:** optional P-LOCAL polish — inline edit + re-key, a "test connection" reachability/TLS probe, an external-zone toggle, and an apply-without-restart trigger. Otherwise the Local Providers epic (ADR-0135) is feature-complete for the release: declare → vault → omp models.yml → routed, all from the UI.
