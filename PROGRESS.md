...
- **verified:** `make test` 2321 pass / 0 fail + sidecar green (+24 `lucid_welcome_extension.test.ts`: welcomeEnabled matrix, renderWelcomeLines LUCID-not-omp/version/model/paint, lucidVersion parse+fallbacks, applyWelcome 4 fail-open paths; +5 `lucid_acp.test.ts`: buildTuiArgs order gate→theme→welcome + `--config`, runTui welcome on/off argv). `make demo-P-BRAND.1` green; `make demo-P-THEME.1` still green; `make demo-02` prefix-hash unchanged (invariant #6); `make test-failclosed` 15/15; root tsc clean; `lucid check` OK.
- **stubbed:** interactive PAINT is proven via the DI `applyWelcome`/`setWidget` seam + the demo preview, not a PTY screenshot (same limit as P-THEME.1). `startup.quiet` also suppresses omp's LSP/MCP startup status + tips/recent-sessions — acceptable, the LUCID banner replaces the box; re-surfacing those under the LUCID banner is a follow-up. Version read from `desktop/package.json` (mirrors APP_VERSION). Escape hatch: `LUCID_WELCOME=off` restores omp's own welcome (launcher drops both the `-e` and the overlay).
- **next:** optional — re-surface tips/LSP/recent-sessions inside the LUCID banner; a compact narrow-terminal fallback for the wordmark; a light-mode variant.
