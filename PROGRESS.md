...
- **shipped:** Windows enterprise packages (issue #345, addon #75, PI-4/ADR-A009): build.win gains msi (SCCM) + appx (Intune LOB) x64 targets alongside NSIS/portable. MSI is perMachine with a forever-pinned UpgradeCode (2568ebca-6784-4546-9463-9765440afaf3, the in-place-upgrade contract), runAfterFinish off for SYSTEM-context installs, versioned artifact names; AppX carries the enterprise identity (TechLead187.LucidAgent / CN=TechLead 187 LLC). CI attaches *.msi/*.appx to run artifacts + tag releases + rolling latest; Authenticode rides the existing WIN_CSC_LINK seam. New desktop/win_packaging.test.ts pins UpgradeCode/perMachine/identity/versioned-names so drift (the duplicate-install data-loss mode) fails CI. docs/WINDOWS-ENTERPRISE-DEPLOYMENT.md runbook: SCCM app, Intune Win32 (.intunewin) + supersedence, Intune MSIX LOB + cert trust, NSIS-per-user migration note, userData-preservation rationale, validation checklist.
- **verified:** make test 2630/2630 pass (+4 new) + sidecar suite green; desktop/package.json parses; zero em dashes in new files; air-gap smoke unchanged and still gates the same CI job before upload.
- **next:** run the runbook 6 validation checklist on a real Windows fleet (install, N->N+1 in-place upgrade, %APPDATA% preservation, signed MSIX sideload) once org Authenticode cert lands; then close addon #75.
- **shipped:** R-07 wired (issue #347, addon #36): the managed models allowed/denied policy (ADR-0068, schema-only since P-ENT.1) now ENFORCES. New pure resolveGovernedModel in desktop/checker_model.ts composes the ADR-0217 AskSage lockdown with modelAllowed (lock resolves first, then allowlist; first option satisfying BOTH wins; nothing qualifies = fail-closed block). acp_backend: enforceAsksageLock generalized to enforceModelPolicy (same two call sites - fresh-session UI sync now fires unconditionally, pre-turn clamp stays authoritative and REFUSES the turn); resolveAgentRunModel routes built-agent/scheduled runs through the same resolver; accessibleModels adds the policy as a cosmetic picker filter with the existing only-narrow-if-nonempty fail-safe. Plus the R-07 CI diff: @oh-my-pi/pi-catalog pinned 16.1.20 as a direct dep (joined the prefix_compaction OMP_PACKAGES version-pin discipline) and harness/omp/provider_catalog.test.ts pins the 57-id builtin provider universe - an omp bump that adds/removes providers fails with review-then-re-pin instructions (the pin caught real drift on day one: a newer transitive catalog carried baseten/coreweave/gitlab-duo-agent that 16.1.20 does not).
- **verified:** make test 2640/2640 pass (8 new resolveGovernedModel tests: passthrough, deny-swap, allow-restrict, deny-wins, all-denied block, lock+policy BOTH hold, lock-denies-gov block, lock-failure surfaces; 2 catalog-pin tests), desktop tsc clean, zero em dashes in new code.
- **next:** addon-side follow-on per ADR-A024: distribute the add-on's signed governance-pack allowlists into the managed models block via the ADR-0068/A010 channel; core enforcement now consumes them for real.
