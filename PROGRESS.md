...
## P-CHAT.C: settled-turn "Generate engineering report" CTA + /api/eval/report route reusing evals.ts (ADR-0181)
- **shipped:** the P-CHAT turn redesign's third increment. New PURE `harness/brief/eval_report.ts` (`buildRunRecord`/`renderTurnEvalReport`) maps a turn's OBSERVED telemetry — the tool calls the renderer saw, their per-file diffstats, the turn's tokens/cost/failures — into P-EVAL.1's `RunRecord`, then renders the reused Model-Evaluation markdown (evals.ts). Mapping rules: a tool is a file change only if it has a path AND a diffstat (reads/searches/bash are not files); repeated writes to a path merge (adds/dels summed) and the surplus over distinct files is a re-edit (rework/churn proxy feeding `wastedTokensEst`); no AC/lint/test signal is knowable at the chat seam so those metrics stay `needs_signal`, never faked (ADR-A016 honesty). `dev.ts` `/api/eval/report` (POST) coerces the payload defensively (a lossy/hostile body can't yield a negative LOC or NaN) → renders → `saveBrief(id,"evals",md)`, so it lists in the Reports panel (kind=brief, role=evals) and "Open in Reports" opens it. Renderer: `appendRunReport` appends the run-footer CTA on a SETTLED tool-using turn (snapshots `marks`/`failures`/tokens via `buildEvalTurn`; block events with `quarantined===false` now feed the failures list), POSTs `bridge.evalReport`, and swaps to the "Open in Reports" link (`openReportEntry("brief", …)`); `.runfoot`/`.report-cta`/`.reportlink` CSS mirrors the mock. Text set via `textContent` (a hostile model id / path can't break the markup).
- **verified:** `make demo-P-CHAT.C` + `harness/brief/eval_report.test.ts` (5 cases / 19 asserts) green — file classification, per-file merge, the re-edit count, hostile/lossy sanitation, and the reuse (title, provenance xychart, needs_signal tiers). All three tsconfigs clean; `license_headers --check` clean. A real report_store round-trip (renderTurnEvalReport → saveBrief → listBriefs role=evals → readBrief byte-exact, title == the `# ` heading) proves the "Open in Reports" path resolves. First-party suite 1887 pass / 6 fail (the 6 are pre-existing Windows-only path quirks in fs_browse/lucid_acp — same as baseline; +5 new tests, 0 regressions); prior demos P-EVAL.1 / P-CHAT.A / P-CHAT.B still green.
- **stubbed:** the DOM wiring (run-footer CTA render on settle, the POST, the link swap) is TYPECHECKED but UNVERIFIED — live/streaming renderer, not runnable here, needs the user's in-app QA (same honest limit as P-CHAT.A/B). `totalTokens` is a context+output proxy (no exact cumulative at the chat seam); the weekly/monthly latency rollup (`renderLatencyRollupMarkdown`) is NOT surfaced by this button (needs P-EVAL.2's persisted capture).
- **next:** user QAs C in-app; then P-EVAL.2 (the `t_sent`/`t_first_token`/`t_end` capture hook + the frozen `eval_metrics`/`api_latency` DuckDB migrations) so the report draws persisted metrics + the latency rollup, then P-EVAL.3 (report kinds + render + accordion).
...
**P-TRIV.4 - the Settings toggle + an AI re-seed ("recycle") for the Trivia Wire (ADR-0177)**
- **shipped:** the real Settings -> Trivia Wire card (on/off toggle, no more hand-editing localStorage; three opt-in re-seed source mini-checkboxes with tooltips; a "Recycle" button + status line + "use built-in" reset). `desktop/trivia_seed.ts` (PURE assemble/parse + injectable seams, mirrors intel_news): gathers opt-in context (recent session titles / KG facts / code-graph names) -> SCANS it fail-closed via scanAndDecide (a finding or dead scanner drops the whole re-seed, model NEVER called, recordBlock) -> generates on the user's SELECTED model via `backend.complete` (throwaway util session, off-transcript, permission-gated tools denied) with the context delimited (UNTRUSTED_CONTENT_*) + late -> `parseTriviaPack` gates every entry through the SAME isTriviaQuestion as the seed banks, coerces/dedupes/caps. `/api/trivia/reseed` route (gatherers wired to listSessions/personalGraph/loadCodeGraph) + `bridge.triviaReseed` + `TriviaSeedView`. app.ts: per-role generated packs persist in `lucid.trivia-pack.<role>` behind a stable-ref cache; `effectiveTriviaBank` = generated pack ?? seed bank (the permanent fail-closed floor) now feeds both game builders. demo-P-TRIV.4 + `trivia_seed.test.ts` (10 cases) + 42-test trivia core all green; typecheck/license clean; prior trivia demos still green.
- **stubbed:** generation is user-initiated only (no auto-refresh/cache like the intel wire's 20-min poll); source gathering is coarse (session TITLES, not transcripts; file-level code graph; a re-seed never triggers a heavy ingest); no difficulty adaptation; the generated pack is not re-scanned (shape-gated + escaped + never re-prompted - the load-bearing scan is on the INPUT).
- **next:** difficulty adaptation from the lifetime tally; optional periodic auto-refresh; richer source gathering (a few user messages / symbol-level code graph) once the coarse pass proves useful.

**P-EVAL.1 - Model-Evaluation metrics + API-latency rollup core (ADR-0178)**
- **shipped:** `harness/brief/evals.ts` (PURE) - `computeEvalMetrics` (efficiency / reliability / quality / product metrics, each `direct|proxy|needs_signal`, null-not-zero), `hourEt` (DST-correct 08:00-17:00 ET buckets via `Intl` America/New_York), `percentile` (nearest-rank), `rollupLatency` (per model x business-hour p50/p95, weekly/monthly) + `compareRollup` (WoW/MoM), and `renderEvalMarkdown` / `renderLatencyRollupMarkdown` (ASCII-only mermaid `xychart-beta` the P-REPORT.4 viewer bar-ifies). demo-P-EVAL.1 + `evals.test.ts` (8 cases / 59 asserts) green; typecheck + license clean; prior demos green. Public ADR-0178; private draft ADR-A016 updated (latency capture/store + per-platform + weekly dashboard rollups). 4 design mocks under `mockups/`.
- **stubbed:** PURE core only - NO DuckDB persistence (`eval_metrics` / `api_latency` frozen migrations = P-EVAL.2), NO live-stream latency capture hook (P-EVAL.2), NO report kinds / render / accordion wiring (P-EVAL.3). Spec-conformance + predicted-acceptance stay proxies pending the AC-list + lint + PR-event signals; the markdown is reused via mermaid xychart, not yet a stored report kind.
- **next:** P-EVAL.2 - the `t_sent`/`t_first_token`/`t_end` capture hook at the `acp_backend` / `/api/chat` seam + the frozen `eval_metrics` + `api_latency` DuckDB migrations + the `latency_rollup` view.

**P-CHAT.A - sectioned agent turn + collapsed subagent (ADR-0179)**
- **shipped:** `desktop/renderer/answer_sections.ts` (PURE, tested) `sectionizeAnswer` / `shouldSectionize` (fence-aware heading + rule split; a heading-less answer is never accordioned) + `renderAnswerBody` in app.ts hooked at the 3 settle points (`done` / `finally` / `renderMessage`-assistant) so a SETTLED answer becomes collapsible `.answer-sec` blocks (streaming unchanged; `_md` / copy / restore intact); the subagent card now renders collapsed by default. demo-P-CHAT.A + `answer_sections.test.ts` (6 cases / 18 asserts) green; typecheck + license clean; prior demos green.
- **stubbed:** the DOM behavior is TYPECHECKED but UNVERIFIED - not runnable here (live/streaming renderer), needs the user's in-app QA. Sections default OPEN (the collapse past ~6000px height snaps rather than animates); no per-section collapse-state persistence or deep-link yet.
- **next:** user QAs A in-app; then P-CHAT.B (inline tool-event chips interleaved into the prose + drilldowns), then P-CHAT.C (run engineering-report button + `/api/eval/report` route reusing `evals.ts`).

**P-CHAT.B - inline tool-event chips interleaved into the answer (ADR-0180)**
- **shipped:** `desktop/renderer/answer_chips.ts` (PURE, tested) `classifyTool` / `toolChip` / `interleaveChips` / `shouldInterleave` - threads each of a turn's tool calls back into the SETTLED answer as a compact expandable chip anchored where it fired (the anchor = `buf.length` when the `tool` event arrived), snapping every anchor to a fence-aware BLOCK boundary so a chip never splits a paragraph or a ``` fence; a +/- diffstat is sized from the P-CHAT.1 `code` via the shared `linediff` helpers. app.ts: the `tool` handler records a `mark` (zero visual change - the live `.thoughts` window is untouched); `renderAnswerBody(container, md, marks?)` interleaves on settle (prose runs STILL sectionize via P-CHAT.A - A nests inside B), builds `createChipRow` chips + lazy drilldowns (`renderToolCode` for code, else the step detail), and drops the now-redundant live thoughts window; `_md` / copy / restore intact, streaming + no-tool turns unchanged. `.answer-chip`/`.tchip`/`.tinline` CSS mirrors the `.answer-sec` collapse; chip text is textContent (XSS-safe). demo-P-CHAT.B + `answer_chips.test.ts` (9 cases / 38 asserts) green; typecheck clean across all 3 tsconfigs; license + prior demo-P-CHAT.A + the 249-test renderer suite green.
- **stubbed:** the DOM behavior (settle interleave, chip drilldowns, thoughts-window drop) is TYPECHECKED but UNVERIFIED - live/streaming renderer, not runnable here, needs the user's in-app QA. Failed tool calls still route to the P-TOOLFAIL.2 toolbox badge (not the inline `.fail` chip); restored/static replies have no live anchors so they sectionize without chips; chips anchor at BLOCK boundaries, not mid-paragraph inline as in the mock.
- **next:** user QAs B in-app; then P-CHAT.C (a "run engineering report" button on the settled turn + an `/api/eval/report` route reusing `evals.ts` from ADR-0178).
