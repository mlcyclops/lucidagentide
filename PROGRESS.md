...
## P-REMOTE.15: the phone composer, decluttered - menus for same-family controls, a "?" sheet for the disclaimer (ADR-0298)
- **shipped:** the PWA composer action row went from SEVEN controls plus a two-line 12.5px disclaimer to `[+] [hold-to-talk] [more] [?] ... [Stop] [Send|caret]`. Same-family controls collapsed into menus instead of buttons: Dictate + Check in under a MORE control (hidden entirely when both are unavailable, so it is never a dead "..."), Push now under a caret on the send button. Queue and Send merged into ONE submit button whose label and icon flip mid-turn (they were always the same `PromptFrame` path - the host stages a mid-turn prompt), and Stop became icon-only and mid-turn-only, so an idle composer now carries no control that cannot do anything. The long voice disclaimer moved OUT of the row into the existing sheet component as a third "voice" flavour behind a "?" button, with its body built at open time from the live `decideSttMode` decision; inline text shrank to a one-line transient status. Honesty is unchanged: the cloud consent gate is untouched and still mandatory, the "?" goes amber whenever the current path would send audio off the phone, the dictate menu row carries the caution at the point of use, and the status line goes amber while a cloud recognizer listens. The listening pulse moved from the deleted dictate button onto the MORE control, so the app still has exactly ONE recording animation.
- **verified:** measured in a real headless Chromium against the REAL `tools/remote-pwa/index.html` (not only the mockup), mid-turn - the widest state - at 320 / 344 / 360 / 375 / 390 / 414 CSS px: zero overflow, single line, and both menus inside the viewport with 12px margins on each side. Two overflow regressions and one live bug were caught by measuring rather than reading: 360px and 320px overflowed until the base sizes became the phone sizes with breakpoints at 374/344, and `element.hidden = x` is a NO-OP on an SVG element (an `HTMLElement` property, so `[hidden]` never matched) which left the send icon un-swapped - fixed with `toggleAttribute`. Root tsc clean, PWA bundle builds (app.js 56.7 KiB), stt + collab suites 53/0, no em dashes. `mockups/pwa_session_preview.html` rewritten to copy the composer/menu/sheet CSS VERBATIM from the real file (invariant #11) and sized to `min(390px, 100vw)` so it renders identically in the Preview panel's phone-portrait mode.
- **next:** DEPLOYED to lucid-agent.web.app/remote together with P-PWA-FLEET.2 (see below). Remaining on-device check: the MORE menu's tap-outside dismissal on iOS (the blur-before-tap seam the actions row already guards with a 200ms delay, now also with the open-menu check) is the one thing only a real phone can confirm.

## P-PWA-FLEET.2: the phone fleet collapses to 33px, and a lane is driven IN its own lane (ADR-0299)
- **shipped:** three things spotted on the phone's Fleet strip. (1) SPACE - the permanently-expanded head (a Fleet title + filter input over a horizontal scroller of 220px cards) became an AUTO-COLLAPSED double-decker like `#procs`/`#botstrip`: collapsed it is 33px and still carries the DESKTOP dock pill's own count pips plus a roll-up phrase ("1 need approval, 1 waiting on you, ..."), tinted red when a lane is blocked on you; the filter moved into the panel. The bar sits ABOVE its panel because `#fleet` is top-anchored - cloning botstrip's panel-then-bar order put the panel above its own label, caught by screenshotting it. (2) A FULLER LANE VIEW - the panel is now vertical full-width cards, each with its OWN composer (input + Check in + Stop + one Send/Queue button, with Push beside it while the lane is busy), so `#lane-target`, `setLaneTarget`, the `data-fleet-act="prompt"` button and BOTH master-composer lane branches are deleted and the master input has exactly one destination. The lane composer offers exactly what `CollabGuest` can do for a lane (`fleetPrompt` text-only, `interject` for Push/Check in, `fleetStop`, the three `fleetAnswer` scopes) and nothing else - no spawn, no model picker, no queue reorder, because the guest protocol has none of those. (3) COLOUR PARITY - the phone had invented its own palette and disagreed with the desktop on four of seven states (working+starting both green, needs-approval+awaiting-input both amber, error+stopped both red, `done` unstyled); it now uses the desktop's exact hexes and rgba glows keyed off the SAME `lane-<status>` class, carries the desktop's rule that ONLY the two action-needed states animate, and honours `prefers-reduced-motion`. New pure `desktop/collab/fleet_status.ts` owns the order, the wording, and `laneRollup`; `fleet_grid.ts` imports it and its local `PILL_ORDER`/`STATUS_WORDS`/`lanesByStatus` are deleted, so the two surfaces cannot drift.
- **verified:** +8 `fleet_status` tests (attention-first order, the exact attention/busy matrix across all seven states, an unknown status COUNTED but never promoted to attention, name/id/status fallbacks, empty roll-up) and +3 rewritten lane-card tests (the four live acts present and the five dead ones absent, the Send/Queue + Push flip, the `lane-<status>` class on every state). Measured in headless Chromium at 320/360/390: collapsed fleet 33px leaving the transcript 660 of 844; expanded 447/246; lane action rows zero overflow and no wrap; computed `--lane` matches the desktop hex on all seven states; only needs-approval + awaiting-input animate, and `prefers-reduced-motion: reduce` returns `animationName: none` for every card. Drove a lane end to end in the mockup (typed into the `web-tests` card, hit ITS Send, got `[lane: web-tests] ...`). Root tsc clean, PWA bundle 59.3 KiB, collab fleet/view/guest green and renderer 1098/0; the 2 `relay_client` failures are a pre-existing flake that passes on rerun and touches nothing here. Swept the pre-existing em dashes out of the two files I edited.
- **deployed:** shipped WITH P-REMOTE.15 in one `firebase deploy --only hosting` from `lucidaddon_audit/website` (project lucid-agent, 19 files). The live `firebase-public/remote/config.js` was deliberately NOT overwritten - the staging copy names the seven build outputs explicitly and omits it, and a sha256 check confirmed it unchanged (the source copy differs from the live one only by CRLF, so this was a real footgun avoided rather than a theoretical one). Live-probed over HTTPS afterwards: index.html 55.9 KiB + app.js 59.2 KiB served, config.js still carries the real relay host + apiKey, all five NEW markers present in both (help-btn / more-menu / send-more / voice-status / send-icon-queue and fleet-bar / fleet-pips / fleet-panel / lane-drive), and all five RETIRED ids absent from both (lane-target, stt-note, checkin-btn, queue-btn, push-btn). `lane-needs-approval` and `row-anchor` are correctly absent from app.js: the first is built as a `lane-${status}` template so the literal never appears, the second is static markup.
- **next:** on-device, confirm a lane input keeps its draft across a live 2.5s snapshot repaint (drafts are lifted by lane id and written back, but only a real poll cadence proves it) and the iOS tap-outside dismissal of the composer's MORE menu. RESOLVED by ADR-0302: no auto-expand, and the rule generalizes to every phone collapsible.

## P-PWA-FOCUS.1: a fleet lane's CONVERSATION reaches the phone, and tapping it takes the composer (ADR-0300)
- **shipped:** the user could see Stella's status and prompt her but not READ her, and the reason was structural: `LaneEvent` and `ChatEvent` are disjoint unions, `Lane.transcript` had no accessor, and `lane.sinks` were added/removed per turn, so no lane byte had a path to a guest. Now: `EventFrame` gains ONE optional `lane` field (absent = master, which is what every pre-focus peer already meant, so `foldEvent` is untouched); a new guest frame `watch { target }` makes the phone SUBSCRIBE to exactly one conversation and the host unicasts lane events only to peers that asked, so idle lanes stream nothing at a phone on cellular; `lane-sync` replays the lane's existing bounded respawn transcript on switch so a lane that has worked ten minutes does not open empty; new pure `laneEventToChatEvent` translates token/thinking/tool/done/error and DROPS permission/auto-approved/status because the lane card already reports all three (double-reporting one fact in two places that can disagree, and an approval rendered as a transcript line looks actionable on a surface that cannot answer it); `ChatEvent` gains a broadcast-only `lane-error` rendered as a dashed-ruled `lane failed` chip, deliberately NOT the `block` chip, because `block` means the SECURITY GATE refused something and dressing a crash in the gate's clothing teaches the user to misread the one signal that must stay unambiguous; `FleetLanes.observe()` attaches to all lanes present AND future and survives a turn ending, with the dev tap checking `laneWatched()` BEFORE translating so a fleet with no phone attached does zero per-event work. Tapping a Process row or a lane header sets focus: that target owns the transcript, the composer, Stop, Push now and Check in, with a focus bar naming the lane and a way back. `#lane-target` is deleted, so the master composer has exactly one destination again.
- **verified:** `make demo-P-PWA-FOCUS.1` green, 7 checks over a real CollabHost + two real CollabGuests, asserting the bandwidth guarantee FIRST at the wire level (frames sent, not just callbacks fired): an unsubscribed guest receives zero frames from a whole lane turn; a VIEW-only guest may watch (read-only, not edit-gated) and gets the replay unicast; lane events never touch the master transcript, gauge or `onEvent`, not even a lane `done` carrying text, while a real master turn still lands; `watch("master")` unsubscribes and the lane goes silent; two guests on two lanes get no crosstalk (per-peer); and a `watch` from a peer that never sent `hello` is ignored fail-closed so a lane transcript never replays to an unauthenticated peer. Plus 9 adapter tests (every mapping, `done` carries NO text so the phone cannot be handed an authoritative empty reply that erases streamed tokens, unknown types drop) and 4 lane-engine tests (an observer follows lanes spawned LATER, survives a second turn, a throwing observer never starves the sinks behind it, `laneTranscript` hands out a copy). Root tsc clean, PWA bundle 62.6 KiB, collab+fleet 452/0, renderer 1103/0. Measured the focus bar on the REAL index.html at 390x844: 47px, zero overflow, four element children (invariant #11), and the `lane failed` chip computes `border-left: dashed` so it is provably not the `block` chip. Swept pre-existing em dashes from the four collab files I edited.
- **NOT done, and it is the user's second ask:** the cross-screen-lock SYNC. The existing P-REMOTE.11 catch-up is still MASTER-only and still fires only on a reconnect welcome-replay, so there is no sync button, no auto-sync inside 60s, and no scroll-to-first-unseen, per lane or otherwise. Per-target transcripts + the focus model landed here are its prerequisite, which is exactly why it is a separate increment rather than a rushed tail on this one.
- **next:** DEPLOYED together with P-PWA-FOCUS.2 (below).

## P-PWA-FOCUS.2: the sync - what you missed, per conversation, and where you left off (ADR-0301)
- **shipped:** the second half of the user's ask. `seenTurns`/`awayAt` (global singletons that could not describe more than one conversation) are replaced by `seen: Map<target, number>`, updated in render() ONLY while the page is visible and ONLY for the focused target - that single rule is the mechanism, because a lane nobody is looking at then accumulates a real backlog instead of being forgiven by a repaint the user never saw. New pure `desktop/collab/sync_state.ts` `planSync(targets, focus, awayMs)` owns the ordering (focused first, then descending count, then label), the counts, the auto decision and the summary wording, DOM-free so it is unit-tested rather than inferred from a phone. 60s or less auto-syncs (a glance is not a departure); a longer absence shows a `Sync` action, because silently yanking the scroll position ten minutes later is as disorienting as losing the place was. `renderTranscript` gained an optional `newFrom` index that draws ONE `[data-sync-mark]` rule, which sync scrolls to with `block: "center"` so the last thing you HAD seen stays visible as context; render() no longer jumps to the newest message while a boundary is showing, since that is precisely what loses the place. Sync NEVER repoints the composer: a backlog in a different conversation becomes a tappable card row, because silently moving which agent the input is aimed at is how a prompt lands on the wrong agent. The old master-only catch-up card body (a re-print of missed turn text) is deleted; the transcript carries the boundary now and the card answers WHERE instead.
- **verified:** +36 pure tests across `sync_state.test.ts` (focused-first ordering over a 5x louder target, descending-count and label tie-breaks, a SHRUNKEN target from a fresh lane-sync replay yielding zero rather than negative, `firstUnseen === seen`, the 60000ms boundary inclusive with 60001 not auto, negative/NaN/Infinity awayMs auto, every singular/plural summary form) and the `renderTranscript` boundary suite (marker inside prior, inside items, exactly at the seam, none for 0/negative/total/total+5/1.5/NaN/Infinity, exactly one marker even with byte-identical duplicate entries, and two-arg output byte-identical to before the change). Root tsc clean, PWA bundle 64.7 KiB, collab+fleet 488/0, renderer 1107/0, no em dashes. Measured on the REAL index.html at 390x844: card bar 35px single row with zero overflow, jump rows 362px no overflow, the marker renders and `scrollIntoView` centres it in the scroller. A real bug was caught by measuring rather than reading: `<button id="sync-go">` was nested INSIDE `<button id="catchup-bar">`, which is invalid HTML the parser hoists out, silently breaking the row onto two lines; it is now a sibling under a `.cu-head` flex row.
- **deployed:** FOCUS.1 + FOCUS.2 shipped together in one `firebase deploy --only hosting` (project lucid-agent, 19 files), live `config.js` preserved again by naming the seven build outputs explicitly (sha256 confirmed unchanged). Live-probed: index.html 61.8 KiB + app.js 64.6 KiB served, real relay host intact, and every marker present on the right surface - `focusbar`/`focus-back`/`lane-fail`/`sync-go`/`cu-head`/`cu-jump`/`sync-mark` in the html, `data-proc-id`/`data-focus-lane`/"new since you looked away" in app.js where the bundled renderer emits them, and all four retired identifiers (`seenTurns`, `awayAt`, `cu-turn`, `lane-target`) absent from both.
- **next:** on-device: lock the phone mid-turn and confirm the auto-sync boundary lands where you stopped reading, then leave it locked past 60s and confirm the Sync button path; also confirm a lane draft survives a live 2.5s snapshot repaint. RESOLVED (ADR-0302): the fleet strip must NOT auto-expand on a needs-approval lane, and no phone panel ever opens itself - attention is colour and count in the always-visible bar. Guard comment sits at the decision site in renderFleetStrip.
