...
## P-NVIM.7: view blocked tool calls from Neovim, via `lucid blocks` + `:LucidBlocks`
- **shipped:** the GUI Security panel's blocked-tool-call list, now in the terminal and Neovim. A new read-only `lucid blocks` CLI (`tools/blocks_cli.ts`, wired into the launcher's `main()` like `lucid kb`/`lucid stats`) lists the tool calls the security gate quarantined: `--all` includes reviewed (approved/dismissed) rows, `--json` for editors. It merges the two sources the GUI already uses at `/api/security`: the lock-free JSONL block log (`~/.omp/lucid-blocks.jsonl`, or `$LUCID_BLOCK_LOG`) plus the DuckDB quarantines (`securitySnapshot`, opened READ_ONLY, skipped when a live gate holds the DB). Pure read: no agent, no gate spawn, no mutation. Because a bare-lucid TUI/ACP session holds `agent_obs.duckdb` read-write (a cross-process READ_ONLY open fails), the in-process gate now mirrors each block to the JSONL when the launcher opts in via `LUCID_BLOCK_LOG`: a new scanner-free `harness/omp/block_log.ts`, called from `security_extension.ts` AFTER the block decision, env-gated and fail-safe so it can never perturb the fail-closed gate (the desktop GUI never sets the env, so it never double-writes). The plugin (`extensions/neovim/lua/lucid/init.lua`) adds `M.blocks()` + `:LucidBlocks` + `<leader>lb`: a read-only float listing each block (tool, severity, findings, reason, status tag), the terminal-native mirror of the GUI panel.
- **verified:** `make test` 2331 pass / 0 fail + sidecar green. New `harness/omp/block_log.test.ts` (9: `buildBlockRecord` severity floored at high, a critical finding raising it, the `type×n` findings summary, fail-closed to `scanner-unavailable`, the empty-findings `quarantined` placeholder, distinct ids; `mirrorBlock` env-gated no-op, JSONL append with recursive mkdir, and a swallowed write failure) and `tools/blocks_cli.test.ts` (8: `readBlockLog` missing-file, skip-corrupt/blank/id-less, approve+dismiss marker replay, defaults, last-write-wins; `blockList` active-vs-`--all`; `runBlocks --json` and human), both filtered to `source === "log"` so the repo's real DuckDB rows never make them flaky. `lucid_acp.test.ts` gains `main blocks --json` routing (blocks never falls through to the TUI/ACP). The Neovim pure helper `_blocks_lines` is added to the headless spec (`helpers_spec.lua`, `luac -p` clean; run by CI where nvim exists). `make demo-P-NVIM.7` green; `make test-failclosed` 15/15 (the fail-closed keystone stayed green after the gate edit); root tsc clean; license-header check green.
- **stubbed:** `lucid blocks` is read-only: releasing or approving a quarantine stays a GUI action (the audited fail-closed override), and the CLI never mutates. The mirror is metadata-only (tool, a severity summary, the reason) and never carries raw scanned content. The headless nvim spec and the interactive `:LucidBlocks` flow weren't run locally (nvim not installed on this box); `_blocks_lines` is `luac`-checked and covered by the CI headless run, and the `vim.system`/float glue mirrors the proven `:LucidKb`/`:LucidStats` paths.
- **next:** optional: a `:LucidBlocks` review action (approve/dismiss) backed by a future audited `lucid blocks approve <id>`, and surfacing a block's artifact link so it cross-references with `:LucidKb`.
