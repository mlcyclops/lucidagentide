...
- **next:** cut a test installer build (workflow_dispatch, publish OFF) to see the strict gate green on the real runner; then relax the ADR-0250 installer clamp (re-enable per-machine installs, its own increment); then drop the DO-NOT-MERGE tip commit, PR this branch, and cut v1.12.2 with the compiled engine. Increment D (bat) rides with the parked wip branch.
