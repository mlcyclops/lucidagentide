...
- **stubbed:** the MAIN-process wiring is NOT built yet: (a) a safe writer/merger for `~/.omp/agent/models.yml` (preserve any non-LUCID providers; don't clobber hand-authored YAML; track LUCID-managed ids in a sidecar so removed providers drop out); (b) the `omp acp` spawn integration (`acp_backend.ts`/`dev.ts`: write models.yml before spawn + merge `{envVar→vault secret}` into the CHILD env + restart on change); (c) auto-registering the endpoint in the whitelist (`egressProposal` → `WhitelistEntry` + `AuthRef→vault`). A call-level live proof needs a running Ollama (else `omp models` listing is the proof). No Settings UI (P-LOCAL.3).
- **next:** finish P-LOCAL.2 wiring (safe models.yml writer + spawn env injection + egress), then P-LOCAL.3 (the collapsed Settings → Local Providers card: add/edit/delete servers+models, store key to vault, reachability/TLS "test connection").
