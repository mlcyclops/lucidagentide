...
- **next:** Increment C (CI smoke that installs to a `Program Files`-ACL location + boots `bin/lucid-engine` to `/api/health` - the regression that would have caught the brick), then relax the ADR-0250 installer clamp to re-enable per-machine installs, then Increment D (`LucidAgentIDE.bat` diagnostics).

-----

## P-WINBOOT.1: Windows installed-app startup hardening (ADR-0250) - the Program Files blank-box brick
...
## P-KG-INGEST.5: the chat-history ingest cannot hang, and Stop always stops (ADR-0252)
...
## P-VOICE.7: varied openers, active listening, spoken thinking snapshots (ADR-0257)
...
## 2026-08-19 - P-FLEET.1: async job handles through the Agent Firewall (ADR-0256, built; ADR-0258)
- **shipped:** the blocking single-tool firewall became a four-tool job surface over ONE gated path: pure `harness/mcp/jobs.ts` (closed state set, sticky terminals, key dedupe, queue cap 8, unknown-id-is-error, metadata-only views), `dispatch`/`job_status`/`cancel` + `prompt` re-implemented as dispatch-plus-bounded-wait (default 25s - the pinned omp bundle kills a tools/call at 30s, measured, so ADR-0256's proposed 90s was wrong), counts-only `onProgress` on the ACP client (type has no text field - anti chunk-split evasion), `AcpTimeoutError` -> `timeout` with session/cancel + force-stop, additive `jobTimeoutMs`(600s)/`maxQueue` registry fields, shutdown cancels-then-stops, `make demo-P-FLEET.1` green on all fifteen ADR checks incl. the two-connection fan-out proof and the scanner-killed-mid-job fail-closed kill test.
...
## 2026-08-19 - P-FLEET.L1: local lanes + the fleet grid dashboard (ADR-0259)
