...
## ADR-0187 - P-EVAL.1: per-run Model-Evaluation metrics + API-latency rollup core (pure), BUILT
...
**Status:** Accepted. PURE core BUILT + tested; the settle-time DOM wiring (renderMessage/done/finally ->
`renderAnswerBody`) is WIRED against master and typecheck-clean, the `.answer-sec` CSS + collapse verified
in the browser preview. Only the real streaming settle path (an actual assistant turn rendering sections)
is unverified here - it needs a live model turn. Increment A of the P-CHAT turn redesign (B: inline tool
chips; C: run engineering-report button). Follows the mock (`mockups/agent_turn_redesign.html`).

**Subagent-card correction (supersedes the original draft):** this increment does NOT collapse the
delegation card during a run. Master's P-TASK.5 (ADR-0180) added live subagent activity to that card and
already collapses it on finish; starting it collapsed would hide the live activity, so master's behavior
(open-while-running, collapse-on-settle) wins. No change to `createSubagentCard`.
...
(run engineering-report button + `/api/eval/report` route reusing `evals.ts` from ADR-0187).

## ADR-0189 - P-CHAT.B: inline tool-event chips interleaved into the answer (pure core BUILT; WIRED onto master; live-streaming QA pending)
...
surfaces) and closes the P-CHAT.A/B "next" by wiring P-EVAL.1's evals.ts (ADR-0187) to a real button.
...
P-EVAL.1/ADR-0187); `report_store.ts` `saveBrief`/`listBriefs`/`readBrief` (P-REPORT.1/2); the P-REPORT.4
